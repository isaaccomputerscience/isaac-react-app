...
export const AggregateQuestionStats = ({
  userProgress,
}: {
  ...
  const fullCorrectThisYear =
    userProgress?.totalQuestionsCorrectThisAcademicYear;
  const fullAttemptThisYear =
    userProgress?.totalQuestionsAttemptedThisAcademicYear;
  const partCorrect = userProgress?.totalQuestionPartsCorrect;
  const partAttempt = userProgress?.totalQuestionPartsAttempted;
  const partCorrectThisYear =
    userProgress?.totalQuestionPartsCorrectThisAcademicYear;
  const partAttemptThisYear =
    userProgress?.totalQuestionPartsAttemptedThisAcademicYear;
  const fullPercentage = safePercentage(fullCorrect, fullAttempt);
  const fullPercentageThisYear = safePercentage(
    fullCorrectThisYear,
    fullAttemptThisYear,
  );
  const partPercentage = safePercentage(partCorrect, partAttempt);
  const partPercentageThisYear = safePercentage(
    partCorrectThisYear,
    partAttemptThisYear,
  );
      ...
      <RS.Row className="mb-3">
        <RS.Col md={6}>
          ...
          <ProgressBar percentage={fullPercentageThisYear || 0}>
            {fullPercentageThisYear == null
              ? "No data"
              : `${fullCorrectThisYear} of ${fullAttemptThisYear}`}
          </ProgressBar>
        </RS.Col>
        <RS.Col md={6}>
      ...
      <RS.Row className="mb-3">
        <RS.Col md={6}>
          ...
          <ProgressBar percentage={partPercentageThisYear || 0}>
            {partPercentageThisYear == null
              ? "No data"
              : `${partCorrectThisYear} of ${partAttemptThisYear}`}
          </ProgressBar>
        </RS.Col>
        <RS.Col md={6}>
          <p className="mb-0">Since account creation:</p>
