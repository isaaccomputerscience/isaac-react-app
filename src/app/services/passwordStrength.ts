...
const zxcvbnSrc =
  "https://cdn.isaaccomputerscience.org/vendor/dropbox/zxcvbn-isaac.js";
...
function calculatePasswordStrength(
  ...
  const passwordToCheck = password
    .substring(0, maxPasswordCheckChars)
    .replace(/\s/g, "");
  const zxcvbnResult: ZxcvbnResult = (window as any)["zxcvbn"](
    passwordToCheck,
    isaacTerms,
  );
  const feedback: PasswordFeedback = {
    zxcvbn: zxcvbnResult,
    feedbackText: passwordStrengthText[zxcvbnResult.score],
  };
  return feedback;
}
...
export async function checkPwnedPasswords(
  password: string,
  callback: (feedback: PasswordFeedback | null) => void,
) {
  try {
    const sha1Hex = await getSHA1Hash(password);
    const sha1Prefix = sha1Hex.substr(0, 5);
    const sha1Suffix = sha1Hex.substring(5);
    const pwnedPasswordResponse = await pwnedPasswordsAPI.get(
      `/range/${sha1Prefix}`,
    );
    const useCount = breachedPasswordUseCount(
      pwnedPasswordResponse.data,
      sha1Suffix,
    );
    ...
    if (useCount > 10000) {
      ...
      feedback = {
        feedbackText: passwordStrengthText[0],
        pwnedPasswordCount: useCount,
      };
    } else if (useCount > 0) {
      feedback = {
        feedbackText: passwordStrengthText[1],
        pwnedPasswordCount: useCount,
      };
    }
    if (feedback) {
...
export function passwordDebounce(
  password: string,
  callback: (feedback: PasswordFeedback | null) => void,
) {
  if (timer !== null) {
