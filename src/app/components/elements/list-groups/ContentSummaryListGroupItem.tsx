...
export const ContentSummaryListGroupItem = ({
  ...
  const isContentsIntendedAudience = isIntendedAudience(
    item.audience,
  ...
  itemClasses += isContentsIntendedAudience
    ? "bg-transparent "
    : "de-emphasised ";

  let title = item.title;
  let titleClasses = "content-summary-link-title flex-grow-1 ";
  const itemSubject = tags.getSpecifiedTag(
    TAG_LEVEL.subject,
    item.tags as TAG_ID[],
  );
  if (itemSubject) {
  ...
  const hierarchyTags = tags
    .getByIdsAsHierarchy((item.tags || []) as TAG_ID[])
    .filter((t, i) => i !== 0); // CS always has Computer Science at the top level

  const questionIcon = item.correct ? (
  ...
  switch (item.type) {
    ...
    case DOCUMENT_TYPE.FAST_TRACK_QUESTION:
      ...
      linkDestination = `/${documentTypePathPrefix[DOCUMENT_TYPE.QUESTION]}/${
        item.id
      }`;
      icon = questionIcon;
      iconLabel = item.correct ? "Completed question icon" : "Question icon";
      audienceViews = filterAudienceViewsByProperties(
        determineAudienceViews(item.audience),
        AUDIENCE_DISPLAY_FIELDS,
      );
      typeLabel = "Question";
      break;
    case DOCUMENT_TYPE.CONCEPT:
      linkDestination = `/${documentTypePathPrefix[DOCUMENT_TYPE.CONCEPT]}/${
        item.id
      }`;
    ...
    case DOCUMENT_TYPE.EVENT:
      linkDestination = `/${documentTypePathPrefix[DOCUMENT_TYPE.EVENT]}/${
        item.id
      }`;
    ...
    case DOCUMENT_TYPE.TOPIC_SUMMARY:
      linkDestination = `/${
        documentTypePathPrefix[DOCUMENT_TYPE.TOPIC_SUMMARY]
      }/${item.id?.slice("topic_summary_".length)}`;
      icon = <img src="/assets/work-md.svg" alt="Topic summary" />;
      iconLabel = "Topic summary page icon";
      typeLabel = "Topic";
      break;
    case DOCUMENT_TYPE.GENERIC:
      linkDestination = `/${documentTypePathPrefix[DOCUMENT_TYPE.GENERIC]}/${
        item.id
      }`;
    ...
    default:
      ...
      console.error(
        "Not able to display item as a ContentSummaryListGroupItem: ",
        item,
      );
      return null;
  }
  ...
  return (
    ...
    >
      <Link to={{ pathname: linkDestination, search: search, hash: hash }}>
        <span
          className="content-summary-link-title align-self-center"
          role="img"
        ...
        <div className="d-md-flex flex-fill">
          <div className={"align-self-center " + titleClasses}>
            <div className="d-flex">
              ...
              {typeLabel && (
                <span
                  className={
                    "small text-muted align-self-end d-none d-md-inline ml-2 mb-1"
                  }
                >
                  ({typeLabel})
                </span>
            ...
            {displayTopicTitle && hierarchyTags && (
              <div className={"hierarchy-tags"}>
                {hierarchyTags.map((tag) => (
          ...
          {audienceViews && displayStage && (
            <StageAndDifficultySummaryIcons audienceViews={audienceViews} />
          )}
        </div>
      </Link>
    </RS.ListGroupItem>
...
export const linkToContent = (
  search: string | undefined,
  item: ContentSummaryDTO,
  displayTopicTitle: boolean | undefined,
) => {
  return (
    <ContentSummaryListGroupItem
      item={item}
      search={search}
      key={item.type + "/" + item.id}
      displayTopicTitle={displayTopicTitle}
    />
  );
};

export const LinkToContentSummaryList = ({
  items,
  search,
  displayTopicTitle,
  ...rest
