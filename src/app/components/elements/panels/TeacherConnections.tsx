...
export const TeacherConnections = ({
  user,
  authToken,
  editingOtherUser,
  userToEdit,
}: TeacherConnectionsProps) => {
  const dispatch = useAppDispatch();
  const activeAuthorisations = useAppSelector(
    selectors.connections.activeAuthorisations,
  );
  const studentAuthorisations = useAppSelector(
    selectors.connections.otherUserAuthorisations,
  );
  const [getGroupMemberships, { data: groupMemberships }] =
    isaacApi.endpoints.getGroupMemberships.useLazyQuery();
  const [changeMyMembershipStatus] =
    isaacApi.endpoints.changeMyMembershipStatus.useMutation();

  useEffect(() => {
    if (user.loggedIn && user.id) {
      dispatch(
        getActiveAuthorisations(
          (editingOtherUser && userToEdit?.id) || undefined,
        ),
      );
      dispatch(
        getStudentAuthorisations(
      ...
      <RS.Container>
        <h3>
          ...
          <RS.UncontrolledTooltip
            placement="bottom"
            target="teacher-connections-title"
          >
            The teachers that you are connected to can view your Isaac
            assignment progress.
          </RS.UncontrolledTooltip>
        </h3>
        ...
        <RS.Row>
          <RS.Col lg={7}>
            <p>
              Enter the code given by your teacher to create a teacher
              connection and join a group.
            </p>
            ...
            <div className="connect-list">
              ...
              <div className="connect-list-inner">
                    ...
                    activeAuthorisations.map((teacherAuthorisation) => (
                      <React.Fragment key={teacherAuthorisation.id}>
                        <li>
                          <span className="icon-person-active" />
                          <span
                            id={`teacher-authorisation-${teacherAuthorisation.id}`}
                          >
                          ...
                          >
                            This user ({teacherAuthorisation.email}) has access
                            to your data. To remove this access, click
                            &apos;Revoke&apos;.
                          </RS.UncontrolledTooltip>
                          <RS.Button
                            color="link"
                ...
                {activeAuthorisations && activeAuthorisations.length === 0 && (
                  <p className="teachers-connected">
              ...
              <RS.Col lg={7}>
                <h3>
                  <span>
                    ...
                    <span
                      id="student-connections-title"
                      className="icon-help"
                    />
                  </span>
                  <RS.UncontrolledTooltip
                    placement="bottom"
                    target="student-connections-title"
                  >
                    These are the students who have shared their Isaac data with
                    you. These students are also able to view your name and
                    email address on their Teacher connections page.
                  </RS.UncontrolledTooltip>
                </h3>
                <p>
                  You can invite students to share their Isaac data with you
                  through the <Link to="/groups">group management page</Link>.
                </p>
              </RS.Col>
              <RS.Col lg={5}>
                <div className="connect-list">
                  ...
                  <div className="connect-list-inner">
                      ...
                      {studentAuthorisations &&
                        studentAuthorisations.map((student) => (
                          <li key={student.id}>
                            ...
                            <RS.UncontrolledTooltip
                              placement="bottom"
                              target={`student-authorisation-${student.id}`}
                            >
                              You have access to this user&apos;s data and they
                              can see your name and email address. To remove
                              this access, click &apos;Remove&apos;.
                            </RS.UncontrolledTooltip>
                            <RS.Button
                              color="link"
                              className="revoke-teacher"
                    ...
                    {studentAuthorisations &&
                      studentAuthorisations.length === 0 && (
                        <p className="teachers-connected">
                      ...
                      <p className="remove-link">
                        <RS.Button
                          ...
                          onClick={() =>
                            ...
                            user.id &&
                            dispatch(
                              releaseAllAuthorisationsAfterPrompt(user.id),
                            )
                          }
                          disabled={editingOtherUser}
                        >
        ...
        <RS.Row>
          <RS.Col>
            <h3>
              ...
              <RS.UncontrolledTooltip
                placement="bottom"
                target="group-memberships-title"
              >
                These are the groups you are currently a member of. Groups on
                Isaac let teachers set assignments to multiple students in one
                go.
              </RS.UncontrolledTooltip>
            </h3>
            <p>
              You can manage who is able to set you assignments by temporarily
              leaving a group. While you are inactive in a group you won&apos;t
              receive any assignments from that group. If you want to
                          ...
                          {membership.group.ownerSummary && (
                            <ul className="list-unstyled">
                              ...
                              {membership.group.additionalManagers &&
                                membership.group.additionalManagers.map(
                                  (manager) => (
                                    <li key={manager.id}>
                                      {extractTeacherName(manager)}
                                    </li>
                                  ),
                                )}
                            </ul>
                        ...
                        <td>
                          {membership.membershipStatus ===
                            ...
                            <React.Fragment>
                              ...
                              <span
                                id={`leave-group-action-${membership.group.id}`}
                                className="icon-help"
                              />
                              <RS.UncontrolledTooltip
                                placement="bottom"
                              ...
                              >
                                If you leave a group you will no longer receive
                          ...
                          {membership.membershipStatus ===
                            ...
                            <React.Fragment>
                              ...
                              <span
                                id={`rejoin-group-action-${membership.group.id}`}
                                className="icon-help"
                              />
                              <RS.UncontrolledTooltip
                                placement="bottom"
                              ...
                              >
                                If you rejoin a group you will see all the
                                assignments set since the group was created.
                              </RS.UncontrolledTooltip>
                            </React.Fragment>
                          )}
                        </td>
            ...
            {groupMemberships && groupMemberships.length === 0 && (
              <p className="teachers-connected text-center">
