...
["Student", "Tutor"].forEach((role) =>
  describe(`${role}s can make event bookings when conditions are met`, () => {
    const user = role === "Student" ? studentUser : tutorUser;

    it(
      "Returns true for an open, pre-deadline, student event with spaces available and logged-in " +
...
describe("Students can join event waiting list when conditions are met", () => {
  it(
    "Returns true for an open, pre-deadline, student event with no spaces available and logged-in " +
    ...
    () => {
      ...
      // Act
      const canJoinWaitingList = userCanBeAddedToEventWaitingList(
        studentUser,
        event,
      );
...
describe("Teachers can join event waiting list when conditions are met", () => {
  it(
    "Returns true for an open, pre-deadline, teacher event with no spaces available and logged-in " +
      "teacher user with no prior booking or reservation",
    () => {
      // Arrange
      event.placesAvailable = 0;
