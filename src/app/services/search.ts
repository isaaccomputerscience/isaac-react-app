...
export const pushSearchToHistory = function (
  history: History,
  searchQuery: string,
  typesFilter: DOCUMENT_TYPE[],
) {
  ...
  history.push({
    ...
    search: queryString.stringify(
      { ...previousQuery, ...newQueryOptions },
      { encode: false },
    ),
  });
};

export const searchResultIsPublic = function (
...
export function parseLocationSearch(
  ...
  const possibleFilters = (
    Array.isArray(parsedFilters) ? parsedFilters[0] || "" : parsedFilters || ""
  ).split(",");
