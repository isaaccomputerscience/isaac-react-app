...
export function generateSingleLetterMenuItem(
  letter: string,
  label?: string,
): MenuItemProps {
  return {
...
export function generateLetterMenuItem(l: string): MenuItemProps | undefined {
  ...
  const item = generateSingleLetterMenuItem(
    GREEK_LETTERS_MAP[parts[0]] || parts[0],
    GREEK_LETTERS_MAP[parts[0]] ? `\\${parts[0]}` : parts[0],
  );
    ...
    if (!modifiers.includes(parts[parts.length - 1])) {
      ...
      if (isNaN(parseInt(subscriptLetter))) {
        subscriptSymbol = {
          type: "Symbol",
          properties: {
            letter: GREEK_LETTERS_MAP[subscriptLetter] || subscriptLetter,
        ...
        subscriptSymbol = {
          type: "Num",
          properties: { significand: subscriptLetter },
        };
      }
      item.children = { subscript: subscriptSymbol };
      item.menu.label += `_{${
        GREEK_LETTERS_MAP[subscriptLetter]
          ? `\\${subscriptLetter}`
          : subscriptLetter
      }}`;
...
export function generateLogicFunctionsItems(
  syntax: LogicSyntax = "logic",
): MenuItemProps[] {
  const labels: any = {
    logic: {
      and: "\\land",
  ...
  return [
    {
      ...
      menu: { label: labels[syntax]["and"], texLabel: true, className: "and" },
    },
    {
      type: "LogicBinaryOperation",
      properties: { operation: "or" },
      menu: { label: labels[syntax]["or"], texLabel: true, className: "or" },
    },
    {
      type: "LogicBinaryOperation",
      properties: { operation: "xor" },
      menu: { label: labels[syntax]["xor"], texLabel: true, className: "xor" },
    },
    {
      type: "LogicNot",
      properties: {},
      menu: { label: labels[syntax]["not"], texLabel: true, className: "not" },
    },
    {
      type: "Relation",
      properties: { relation: "equiv" },
      menu: {
        label: labels[syntax]["equiv"],
        texLabel: true,
        className: "equiv",
      },
    },
    ...
    },
    {
      ...
      menu: { label: "\\small{(x)}", texLabel: true, className: "brackets" },
    },
  ];
}
  ...
  return [
    {
      ...
      menu: { label: "+", texLabel: true, className: "plus" },
    },
    {
      type: "BinaryOperation",
      properties: { operation: "-" },
      menu: { label: "-", texLabel: true, className: "minus" },
    },
    {
      type: "BinaryOperation",
      properties: { operation: "±" },
      menu: { label: "\\pm", texLabel: true, className: "plusminus" },
    },
    {
      type: "Fraction",
      properties: {},
      menu: { label: "\\frac{a}{b}", texLabel: true, className: "fraction" },
    },
    {
      type: "Brackets",
      properties: { type: "round" },
    ...
    },
    {
      type: "AbsoluteValue",
      properties: {},
      menu: { label: "\\small{|x|}", texLabel: true, className: "abs" },
    },
    {
      type: "Radix",
      properties: {},
      menu: {
        label: "\\small{\\sqrt{x}}",
        texLabel: true,
        className: "radix sqrt",
      },
    },
    {
      type: "Relation",
      properties: { relation: "=" },
      menu: { label: "=", texLabel: true, className: "relation equal" },
    },
    {
      type: "Relation",
      properties: { relation: "<" },
      menu: { label: "<", texLabel: true, className: "relation less" },
    },
    {
      type: "Relation",
      properties: { relation: ">" },
      menu: { label: ">", texLabel: true, className: "relation greater" },
    },
    {
      type: "Relation",
      properties: { relation: "<=" },
      menu: {
        label: "\\leq",
        texLabel: true,
        className: "relation less-or-equal",
      },
    },
    {
      type: "Relation",
      ...
      menu: {
        label: "\\geq",
        texLabel: true,
        className: "relation greater-or-equal",
      },
    },
  ];
}
...
export function generateMathsLogFunctionItem(name: string): MenuItemProps {
  if (name === "ln") {
    return {
      ...
      properties: {
        name: "ln",
        innerSuperscript: false,
        allowSubscript: false,
      },
      menu: { label: "\\text{ln}", texLabel: true, className: "function ln" },
    };
  } else {
    // assume log, defaults to base 10, can have other bases
    return {
      type: "Fn",
      properties: {
        name: "log",
      ...
      menu: { label: "\\text{log}", texLabel: true, className: "function log" },
    };
  }
}
...
export function generateMathsDifferentialItem(
  parsedDifferential: string[],
): MenuItemProps {
  const nameToLetterMap: { [name: string]: string } = {
    delta: "δ",
    Delta: "∆",
    d: "d",
  };
  const nameToLatexMap: { [name: string]: string } = {
    delta: "\\delta",
    Delta: "\\Delta",
    d: "d",
  };
  ...
  const differentialLatex =
    "\\mathrm{" + (nameToLatexMap[differentialType] || "?") + "}";

  const differentialSymbol: MenuItemProps = {
    type: "Differential",
    properties: { letter: differentialLetter },
    menu: { label: differentialLatex, texLabel: true, className: "" },
  };
  ...
  if (differentialOrder > 1) {
    ...
    differentialSymbol.menu.label =
      differentialSymbol.menu.label + "^{" + differentialOrder + "}";
  }

  if (differentialArgument) {
    differentialSymbol.children = {
      ...differentialSymbol.children,
      argument: generateLetterMenuItem(differentialArgument),
    };
    differentialSymbol.menu.label =
      differentialSymbol.menu.label +
      differentialSymbol.children.argument.menu.label;
  }

  return differentialSymbol;
}

export function generateMathsDefaultDerivativesItems(
  availableSymbols: string[] | undefined,
): MenuItemProps[] {
  // Do we need to generate special derivatives?
  if (!availableSymbols || availableSymbols.length === 0) {
    const derivativeItem: MenuItemProps = {
      type: "Derivative",
      properties: {},
      menu: {
    ...
    return [
      ...
      },
      {
        ...
        menu: {
          ...
          texLabel: true,
          className: "differential-upper-delta",
        },
      },
      {
        type: "Differential",
        ...
        menu: {
          label: "\\mathrm{\\delta}^{\\circ}\\circ",
          texLabel: true,
          className: "differential-lower-delta",
        },
      },
      derivativeItem,
    ];
...
export function generateMathsDerivativeAndLetters(symbol: string): {
  derivative: MenuItemProps[];
  ...
  if (isDefined(GREEK_LETTERS_MAP[top]) || /^[a-zA-Z]$/.test(top)) {
    ...
    letters.push(
      generateSingleLetterMenuItem(
        GREEK_LETTERS_MAP[top] || top,
        GREEK_LETTERS_MAP[top] ? "\\" + top : top,
      ),
    );
  }
  ...
  for (const p of Object.entries(orders)) {
    ...
    letters.push(
      generateSingleLetterMenuItem(
        GREEK_LETTERS_MAP[letter] || letter,
        GREEK_LETTERS_MAP[letter] ? "\\" + letter : letter,
      ),
    );
    const order = p[1];
    const o = {
  ...
  const texLabel =
    "\\frac{\\mathrm{d}" +
    (derivativeOrder > 1 ? `^{${derivativeOrder}}` : "") +
    `}{${texBottom}}`;
  const derivativeObject: MenuItemProps = {
    type: "Derivative",
    properties: {},
    menu: {
      label: texLabel,
    ...
    children:
      derivativeOrder > 1
        ? {
            order: {
              type: "Num",
              properties: { significand: `${derivativeOrder}` },
            },
          }
        : {},
  };
  ...
  if (isDefined(GREEK_LETTERS_MAP[top]) || /^[a-zA-Z]$/.test(top)) {
    ...
    const compoundTexLabel =
      "\\frac{\\mathrm{d}" +
      (derivativeOrder > 1 ? `^{${derivativeOrder}}` : "") +
      `${GREEK_LETTERS_MAP[top] || top}}{${texBottom}}`;
    const compoundDerivativeObject: MenuItemProps = {
      type: "Derivative",
      properties: {},
      menu: {
        label: compoundTexLabel,
        texLabel: true,
        fontSize: "1.5em",
        className: "derivative",
      },
    };
    compoundDerivativeObject.children = {
      numerator: compoundNumerator,
      denominator,
    };
...
export function generateMathsDifferentialAndLetters(symbol: string): {
  differential?: MenuItemProps | null;
  letters?: MenuItemProps[] | null;
} {
  ...
  if (
    differentialType === "d" &&
    differentialOrder === 0 &&
    differentialArgument == null
  ) {
    return { letters: [generateSingleLetterMenuItem("d")] };
  } else {
    return {
      differential: generateMathsDifferentialItem(
        parsedDifferential as string[],
      ),
      letters: differentialArgument
        ? [
            generateSingleLetterMenuItem(
              GREEK_LETTERS_MAP[differentialArgument] || differentialArgument,
              GREEK_LETTERS_MAP[differentialArgument]
                ? "\\" + differentialArgument
                : differentialArgument,
            ),
          ]
        : null,
  ...
  return [
    ...
    },
    {
      ...
      menu: {
        label: "\\text{(l)}",
        texLabel: true,
        className: "chemical-state liquid",
      },
    },
    {
      type: "StateSymbol",
      ...
      menu: {
        label: "\\text{(aq)}",
        texLabel: true,
        className: "chemical-state aqueous",
      },
    },
    {
      type: "StateSymbol",
      ...
      menu: {
        label: "\\text{(s)}",
        texLabel: true,
        className: "chemical-state solid",
      },
    },
    {
      type: "StateSymbol",
      ...
      menu: {
        label: "\\text{(m)}",
        texLabel: true,
        className: "chemical-state metal",
      },
    },
  ];
}
  ...
  return [
    {
      ...
      menu: {
        label: "+",
        texLabel: true,
        className: "chemical-operations plus",
      },
    },
    {
      type: "BinaryOperation",
      ...
      menu: {
        label: "-",
        texLabel: true,
        className: "chemical-operations minus",
      },
    },
    {
      type: "Fraction",
      ...
      menu: {
        label: "\\frac{a}{b}",
        texLabel: true,
        className: "chemical-operations fraction",
      },
    },
    {
      type: "Relation",
      ...
      menu: {
        label: "\\rightarrow",
        texLabel: true,
        className: "chemical-operations rightarrow",
      },
    },
    {
      type: "Relation",
      ...
      menu: {
        ...
        texLabel: true,
        className: "chemical-operations equilibrium",
      },
    },
    {
      type: "Brackets",
      ...
      menu: {
        label: "(x)",
        texLabel: true,
        className: "chemical-operations brackets round",
      },
    },
    {
      type: "Brackets",
      ...
      menu: {
        label: "[x]",
        texLabel: true,
        className: "chemical-operations brackets square",
      },
    },
    {
      type: "Relation",
      ...
      menu: {
        label: "\\cdot",
        texLabel: true,
        className: "chemical-operations dot",
      },
    },
  ];
}

export function generateChemicalElementMenuItem(
  symbol: string,
  ...
  if (CHEMICAL_ELEMENTS.includes(symbol)) {
    return {
      ...
      menu: {
        label: `\\text{${symbol}}`,
        texLabel: true,
        className: `chemical-element ${symbol}`,
      },
    };
    ...
    return {
      ...
      menu: {
        ...CHEMICAL_PARTICLES[symbol].menu,
        className: `chemical-particle ${symbol}`,
      },
    };
  }
  return undefined;
...
export const generateDefaultMenuItems = (
  parsedAvailableSymbols: string[],
  ...
  mathsDerivatives: generateMathsDefaultDerivativesItems(
...
export function setupAndTeardownDocStyleAndListeners({
  onCursorMoveEnd,
  onMouseMove,
  onTouchMove,
  handleKeyPress,
  ...
  return () => {
    ...
    if (inequalityModalRef.current) {
      ...
      inequalityModalRef.current.removeEventListener(
        "touchstart",
        onTouchStart,
      );
    ...
    const canvas =
      inequalityModalRef.current?.getElementsByTagName("canvas")[0];
    if (canvas) {
...
export function generateMenuItems({
  ...
  const baseItems = generateDefaultMenuItems(
    parsedAvailableSymbols,
    logicSyntax,
  );
  ...
  if (parsedAvailableSymbols.length > 0) {
    ...
    parsedAvailableSymbols.forEach((l) => {
      ...
      if (availableSymbol.endsWith("()")) {
        ...
        if (TRIG_FUNCTION_NAMES.includes(functionName)) {
          customMenuItems.otherFunctions.push(
            generateMathsTrigFunctionItem(functionName),
          );
          ...
          customMenuItems.otherFunctions.push(
            generateMathsLogFunctionItem(functionName),
          );
        } else {
          // What
          // eslint-disable-next-line no-console
          console.warn(
            `Could not parse available symbol "${availableSymbol} as a function"`,
          );
        }
        ...
        DIFFERENTIAL_REGEX.test(availableSymbol) &&
        !/^(?:Delta|delta|d)$/.test(availableSymbol)
      ) {
    ...
    return [
      {
        ...baseItems,
        mathsDerivatives: [
          ...baseItems.mathsDerivatives,
          ...customMenuItems.mathsDerivatives,
        ],
        letters: uniqWith(
          [...baseItems.letters, ...customMenuItems.letters],
          (a, b) => isEqual(a, b),
        ...
        otherFunctions: [
          ...baseItems.otherFunctions,
          ...customMenuItems.otherFunctions,
        ],
        chemicalElements: [
          ...baseItems.chemicalElements,
          ...customMenuItems.chemicalElements,
        ],
      },
      false,
    ...
    if (editorMode === "logic") {
      ...
      return [
        {
          ...baseItems,
          upperCaseLetters: "ABCDEGHIJKLMNOPQRSUVWXYZ"
            .split("")
            .map((letter) => generateSingleLetterMenuItem(letter)),
          lowerCaseLetters: "abcdeghijklmnopqrsuvwxyz"
            .split("")
            .map((letter) => generateSingleLetterMenuItem(letter)),
        },
        true,
      ...
      return [
        {
          ...baseItems,
          chemicalElements: CHEMICAL_ELEMENTS.map(
            generateChemicalElementMenuItem,
          ),
          chemicalParticles: Object.keys(CHEMICAL_PARTICLES).map(
            generateChemicalElementMenuItem,
          ),
        },
        true,
      ...
      return [
        {
          ...
          lowerCaseLetters: "abcdefghijklmnopqrstuvwxyz"
            .split("")
            .map((letter) => generateSingleLetterMenuItem(letter)),
          upperCaseGreekLetters: UPPER_CASE_GREEK_LETTERS.map((letter) =>
            generateSingleLetterMenuItem(
              GREEK_LETTERS_MAP[letter] || letter,
              GREEK_LETTERS_MAP[letter] ? "\\" + letter : letter,
            ),
          ),
          lowerCaseGreekLetters: LOWER_CASE_GREEK_LETTERS.map((letter) =>
            generateSingleLetterMenuItem(
              GREEK_LETTERS_MAP[letter] || letter,
              GREEK_LETTERS_MAP[letter] ? "\\" + letter : letter,
            ),
...
export function handleMoveCallback(
  {
    previousCursor,
    movingMenuBar,
    movingMenuItem,
  ...
  if (previousCursor.current) {
    ...
    if (movingMenuBar.current) {
      ...
      const menuItems =
        movingMenuBar.current.getElementsByClassName("menu-item");
  ...
  if (potentialSymbolSpec.current && sketch.current) {
    sketch.current.updatePotentialSymbol(
      potentialSymbolSpec.current as WidgetSpec,
      x,
      y,
    );
  }
...
export function onCursorMoveEndCallback({
  movingMenuItem,
  previousCursor,
  sketch,
...
export function prepareInequality({
  editorMode,
  inequalityModalRef,
