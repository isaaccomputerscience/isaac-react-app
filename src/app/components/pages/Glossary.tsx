...
export function formatGlossaryTermId(rawTermId: string) {
  const idRegExp = new RegExp(
    "([a-z0-9-_]+)\\|?(?:(aqa|ocr)\\|?)?([a-z0-9-_~]+)?",
  );
  const simplifiedTermId = idRegExp.exec(
    rawTermId.split("|").slice(1).join("|"),
  );
  return simplifiedTermId
    ?.slice(1, 3)
...
export const Glossary = () => {
  ...
  const topics = tags.allTopicTags.sort((a, b) =>
    a.title.localeCompare(b.title),
  );
  const [filterTopic, setFilterTopic] = useState<Tag>();
  const rawGlossaryTerms = useAppSelector(
    (state: AppState) => state && state.glossaryTerms,
  );

  const glossaryTerms = useMemo(() => {
    function groupTerms(
      sortedTerms: GlossaryTermDTO[] | undefined,
    ): { [key: string]: GlossaryTermDTO[] } | undefined {
      if (sortedTerms) {
        const groupedTerms: { [key: string]: GlossaryTermDTO[] } = {};
        for (const term of sortedTerms) {
          if (isDefined(filterTopic) && !term.tags?.includes(filterTopic.id))
            continue;
  ...
  const glossaryTermRefs = useRef<Map<string, HTMLElement>>(
   ...
   */
  const alphabetHeaderRefs = useRef<Map<string, HTMLElement>>(
    new Map<string, HTMLElement>(),
  );
  ...
  const alphabetScrollerCallback = (
  ...
  useEffect(() => {
    if (
      alphabetScrollerSentinel.current &&
      !alphabetScrollerObserver.current &&
      !alphabetScrollerFlag.current
    ) {
      const options = {
        root: null,
        rootMargin: "0px",
      ...
      alphabetScrollerObserver.current = new IntersectionObserver(
        alphabetScrollerCallback,
        options,
      );
      alphabetScrollerObserver.current.observe(
        alphabetScrollerSentinel.current,
      );
  ...
  const alphabetList =
    ...
    "#ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("").map((k) => {
      if (glossaryTerms.hasOwnProperty(k)) {
        return (
          <div
            className={`key alphascroller-key-${k}`}
            data-key={k}
            key={k}
            role="button"
            tabIndex={0}
            onKeyUp={onKeyUpScrollTo}
            onClick={scrollToKey}
          >
            {k}
          </div>
        ...
        return (
          <div className="key unavailable" data-key={k} key={k}>
            {k}
          </div>
        );
      }
  ...
  const metaDescriptionCS =
    "Confused about an A level or GCSE Computer Science term? Look it up in our glossary.";
  ...
  const thenRender = (
    <div className="glossary-page">
      <Container>
        ...
        <Row>
          <Col md={{ size: 9 }} className="py-4">
            <Row className="no-print">
              <Col md={{ size: 4 }}>
                <Label for="terms-search" className="sr-only">
                  Search by term
                </Label>
                <Input
                  id="terms-search"
              ...
              <Col className="mt-3 mt-md-0">
                <Label for="topic-select" className="sr-only">
                  Topic
                </Label>
                <Select
                  ...
                  onChange={(e) =>
                    setFilterTopic(
                      topics.find(
                        (v) => v.id === (e as Item<TAG_ID> | undefined)?.value,
                      ),
                    )
                  }
                  isClearable
            ...
            <Row className="only-print">
              <Col>
                ...
                {isDefined(filterTopic) && (
                  <span className="pr-4">
                    Topic: <strong>{filterTopic.title}</strong>
                  </span>
                )}
              </Col>
            </Row>
        ...
        {(!glossaryTerms || Object.entries(glossaryTerms).length === 0) && (
          <Row>
            <Col md={{ size: 8, offset: 2 }} className="py-4">
              {searchText === "" && (
                <p>
                  There are no glossary terms in the glossary yet! Please try
                  again later.
                </p>
              )}
              {searchText !== "" && (
                <p>
                  We could not find glossary terms to match your search
                  criteria.
                </p>
              )}
            </Col>
        ...
        {glossaryTerms && Object.keys(glossaryTerms).length > 0 && (
          <Col className="pt-2 pb-4">
            <div className="no-print">
              <div id="sentinel" ref={alphabetScrollerSentinel}>
                &nbsp;
              </div>
              <div
                ref={stickyAlphabetListContainer}
                id="stickyalphabetlist"
                className="alphabetlist pb-4"
              >
                {alphabetList}
              </div>
              <div className="alphabetlist pb-4">{alphabetList}</div>
            </div>
              ...
              >
                <Col md={{ size: 1, offset: 1 }}>
                  <h2
                    style={{
                      position: "sticky",
                      top: `calc(0px - ${ALPHABET_HEADER_OFFSET}px)`,
                    }}
                  >
                    {letter}
                  </h2>
                ...
                <Col>
                  {terms.map((term) => (
                    <IsaacGlossaryTerm
                      key={term.id}
                      ref={(el: HTMLElement) => {
                        glossaryTermRefs.current.set(
                          (term.id && formatGlossaryTermId(term.id)) ?? "",
                          el,
                        );
                      }}
                      doc={term}
