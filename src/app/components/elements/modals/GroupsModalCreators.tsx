...
const AdditionalManagerSelfRemovalModalBody = ({
  group,
}: {
  group: AppGroup;
}) => (
  <p>
    You are about to remove yourself as a manager from &apos;{group.groupName}
    &apos;. This group will no longer appear on your Assignment Progress page or
    on the Manage Groups page. You will still have student connections with the
    students who agreed to share data with you. The group owner will{" "}
    <strong>not</strong> be notified.
  </p>
);
export const additionalManagerSelfRemovalModal = (
  group: AppGroup,
      ...
      <Col>
        <Button
          ...
          onClick={() => {
            if (group.id && user.id) {
              store.dispatch(
                isaacApi.endpoints.deleteGroupManager.initiate({
                  groupId: group.id,
...
const CurrentGroupInviteModal = ({
  firstTime,
  group,
}: CurrentGroupInviteModalProps) => {
  const tokenQuery = isaacApi.endpoints.getGroupToken.useQuery(
    group.id as number,
  );
  return (
    <>
...
const CurrentGroupManagersModal = ({
  ...
  const [deleteGroupManager] =
    isaacApi.endpoints.deleteGroupManager.useMutation();
  const [promoteGroupManager] =
    isaacApi.endpoints.promoteGroupManager.useMutation();
  ...
  const additionalManagers =
    (group &&
      sortBy(
        ...
        (manager) => manager.familyName && manager.familyName.toLowerCase(),
      )) ||
    [];
  ...
  function promoteManager(manager: UserSummaryWithEmailAddressDTO) {
    if (group?.id) {
      if (
        ...
        promoteGroupManager({
          groupId: group.id,
          managerUserId: manager.id as number,
        }).then((response) => {
  ...
  function setAdditionalManagerPrivileges(
  ...
  return !group ? (
    ...
    <div className={"mb-4"}>
      ...
      <p>
        Sharing this group lets other teachers add and remove students, set new
        assignments and view assignment progress. It will not automatically let
        additional teachers see detailed mark data unless students give access
        to the new teacher.
      </p>
            ...
            <tbody>
              <tr key={group.ownerSummary.email} data-testid={"group-owner"}>
                <td>
                  ...
                  {group.ownerSummary.givenName} {group.ownerSummary.familyName}{" "}
                  ({group.ownerSummary.email})
                </td>
              </tr>
            </tbody>
          </Table>
        ...
        >
          ...
          {group.additionalManagerPrivileges ? (
            <>
              ...
              Un-tick the above box if you would like to remove these additional
              privileges.
            </>
          ) : (
            <>
              ...
              including the owner), remove group members, and archive and rename
              the group.
            </>
          )}
        </Alert>
