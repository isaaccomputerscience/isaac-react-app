...
const feedbackOptionsList: QuizFeedbackOption[] = Object.keys(
  feedbackOptions,
).map((key) => {
...
const feedbackOptionsMap = feedbackOptionsList.reduce(
  (obj, option) => {
...
export function QuizSettingModal({
  quiz,
  ...
  const [feedbackMode, setFeedbackMode] = useState<QuizFeedbackMode | null>(
    initialFeedbackMode ?? null,
  );
  ...
  const dueDateInvalid =
    isDefined(dueDate) && dueDate.getTime() < now.getTime();
  const feedbackModeInvalid =
    validated.has("feedbackMode") && feedbackMode === null;
    ...
    <div className="mb-4">
      <RS.Label className="w-100 mb-4">
        ...
        <ShowLoadingQuery
          ...
          thenRender={(groups) => {
            const groupOptions: Item<number>[] = groups.map((g) => ({
              label: g.groupName as string,
            ...
            return (
              <Select
                ...
                styles={{
                  control: (styles) => ({
                    ...styles,
                    ...(groupInvalid ? { borderColor: "#dc3545" } : {}),
                  }),
      ...
      <RS.Label className="w-100 mb-4">
        ...
        <Select
          ...
          styles={{
            control: (styles) => ({
              ...styles,
              ...(feedbackModeInvalid ? { borderColor: "#dc3545" } : {}),
            }),
            menuPortal: (base) => ({ ...base, zIndex: 9999 }),
          }}
        />
        ...
        <RS.Button
          ...
          onClick={async () => {
            ...
            try {
              ...
              toastId = await dispatch(
                showToast({
                  color: "success",
                  title: "Test set",
                  body:
                    "Test set to " + selectedGroups[0].label + " successfully",
                  timeout: 7000,
                  buttons: [
