...
const defaultPageNavigation = (currentGameboard?: GameboardDTO) => ({
  breadcrumbHistory: [],
  currentGameboard,
});
...
export const useNavigation = (
  ...
  const { data: currentGameboard } =
    isaacApi.endpoints.getGameboardById.useQuery(gameboardId || skipToken);
  ...
  if (
    ...
    const gameboardHistory =
      currentGameboard && gameboardId === currentGameboard.id
    ...
    const modifiedQuestionHistory = questionHistoryList.length
      ? questionHistoryList.join(",")
      : undefined;
    ...
    return {
      ...
      backToCollection: currentGameboard
        ? {
            ...
            to: `/gameboards#${currentGameboard.id}`,
          }
        : undefined,
      nextItem: !previousQuestion
        ? determineNextGameboardItem(currentGameboard, currentDocId)
        : undefined,
      ...
      search: queryString.stringify(
        previousQuestion ? { board, modifiedQuestionHistory } : { board },
      ),
      currentGameboard,
    };
  }
  ...
  if (gameboardId) {
    ...
    return {
      ...
      previousItem: determinePreviousGameboardItem(
        currentGameboard,
        currentDocId,
      ),
      search,
      creationContext: determineCurrentCreationContext(
        currentGameboard,
        currentDocId,
      ),
      currentGameboard,
  ...
  if (topic) {
    ...
    return {
      ...
      nextItem: determineNextTopicContentLink(
        currentTopic,
        currentDocId,
        userContext,
        user,
