...
export const ActivityGraph = ({
  ...
  const foundDates = answeredQuestionsByDate
    ? Object.keys(answeredQuestionsByDate)
    : [];
  if (foundDates && foundDates.length > 0) {
    const nonZeroDates = foundDates.filter(
      (date) => answeredQuestionsByDate && answeredQuestionsByDate[date] > 0,
    );
  ...
  useEffect(() => {
    ...
    if (selectedDates.length === 1) {
      ...
      minDate = formatISODateOnly(
        new Date(firstDate.getFullYear(), firstDate.getMonth() - 1, 1),
      );
      maxDate = formatISODateOnly(
        new Date(firstDate.getFullYear(), firstDate.getMonth() + 1, 1),
      );
        ...
        columns: [
          ...
          [
            ...
            ...selectedDates.map((date) =>
              answeredQuestionsByDate ? answeredQuestionsByDate[date] || 0 : 0,
            ),
          ],
        ],
