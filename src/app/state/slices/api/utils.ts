...
export const anonymiseIfNeededWith =
  <T>(anonymisationCallback: (nonanonymousData: T) => T) =>
  (nonanonymousData: T) =>
    persistence.load(KEY.ANONYMISE_USERS) === "YES"
      ? anonymisationCallback(nonanonymousData)
      : nonanonymousData;

export const anonymiseListIfNeededWith =
  <T>(anonymisationCallback: (nonanonymousData: T) => T) =>
  (nonanonymousData: T[]) =>
    persistence.load(KEY.ANONYMISE_USERS) === "YES"
      ? nonanonymousData.map(anonymisationCallback)
      : nonanonymousData;
...
export const anonymisationFunctions = {
  ...
  userSummary: (overrideGivenName?: string, overrideFamilyName?: string) =>
    function userSummary<T extends UserSummaryWithEmailAddressDTO>(
      userSummary: T,
      index?: number,
    ): T {
      return {
        ...
        givenName:
          overrideGivenName ?? "Test Student" + (index ? ` ${index + 1}` : ""),
  ...
  appGroup: (appGroup: AppGroup): AppGroup => ({
    ...appGroup,
    ownerSummary:
      appGroup?.ownerSummary &&
      anonymisationFunctions.userSummary(
        "Group",
        "Manager 1",
      )(appGroup.ownerSummary),
    additionalManagers: appGroup?.additionalManagers?.map((us, i) =>
      anonymisationFunctions.userSummary("Group", `Manager ${i + 2}`)(us),
    ),
    groupName: `Demo Group ${appGroup?.id}`,
    members: appGroup?.members?.map(anonymisationFunctions.userSummary()),
  }),
  assignments: (
    quizAssignments: QuizAssignmentDTO[] | NOT_FOUND_TYPE | null,
  ) => {
    ...
    return quizAssignments.map((assignment) => {
      ...
      return {
        ...
        ...anonymisationFunctions.assignment({ assignment: assignment })
          .assignment,
        groupName,
  ...
  assignment: (
    assignmentState: { assignment: QuizAssignmentDTO } | { error: string },
  ): { assignment: QuizAssignmentDTO } | { error: string } => {
  ...
  quizAttempt: produce<{ studentAttempt: QuizAttemptFeedbackDTO }>(
    (quizAttempt) => {
  ...
  userProgress: (userProgress: UserProgress): UserProgress =>
    userProgress && {
      ...userProgress,
      userDetails:
        userProgress?.userDetails &&
        anonymisationFunctions.userSummary()(userProgress?.userDetails),
    },
  activeAuthorisations: (
    activeAuthorisations: UserSummaryWithEmailAddressDTO[],
  ): UserSummaryWithEmailAddressDTO[] =>
    activeAuthorisations?.map((a, i) =>
      anonymisationFunctions.userSummary("Demo", `Teacher ${i + 1}`)(a),
    ),
  otherUserAuthorisations: (
    otherUserAuthorisations: UserSummaryDTO[],
  ): UserSummaryDTO[] =>
    otherUserAuthorisations?.map(anonymisationFunctions.userSummary()),
  groupMembershipDetail: (
    groupMembership: GroupMembershipDetailDTO,
  ): GroupMembershipDetailDTO => ({
    ...groupMembership,
