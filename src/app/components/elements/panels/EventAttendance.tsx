  ...
  function filterOnSurname(booking: EventBookingDTO) {
    return (
      ...
      booking.userBooked.familyName
        .toLocaleLowerCase()
        .includes(familyNameFilter.toLocaleLowerCase())
    );
  }
      ...
      {canRecordAttendance && atLeastOne(bookings.length) && (
        <Accordion
          ...
          disabled={
            ...
            "You cannot record attendance for a cancelled event"
          }
        >
          {isEventLeader(user) && (
            <div className="bg-grey p-2 mb-3 text-center">
              As an event leader, you are only able to see the bookings of users
              who have granted you access to their data.
            </div>
          )}
              ...
              <tbody>
                {bookings
                  ...
                  .map((booking) => {
                    ...
                    const userSchool =
                      booking.userBooked &&
                      userIdToSchoolMapping[booking.userBooked.id as number];
                    ...
                    return (
                      <tr key={booking.bookingId}>
                        ...
                        <td className="align-middle text-center">
                          {displayAttendanceAsSymbol(booking.bookingStatus)}
                        </td>
                        <td className="align-middle">
                          {userBooked.familyName}, {userBooked.givenName}
                        </td>
                        <td className="align-middle">
                          {additionalInformation?.jobTitle ||
                            additionalInformation?.yearGroup ||
                            ""}
                        </td>
                        ...
                        {userSchool && userSchool.urn && (
                          <td className="align-middle">{userSchool.name}</td>
                        )}{" "}
                        ...
                        <td className="align-middle">
                          {additionalInformation?.accessibilityRequirements ||
                            ""}
                        </td>
                        <td className="align-middle">
                          {additionalInformation?.medicalRequirements || ""}
                        </td>
                        <td className="align-middle">
                          {additionalInformation?.emergencyName || ""}
                        </td>
                        <td className="align-middle">
                          {additionalInformation?.emergencyNumber || ""}
                        </td>
                      </tr>
                    );
                  })}
