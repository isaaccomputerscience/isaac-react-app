...
export const AnvilApp = ({ doc }: AnvilAppProps) => {
  ...
  if (questionPart !== undefined) {
    ...
    if (questionPart.bestAttempt && questionPart.bestAttempt.correct) {
      appParams["problem_previously_correct"] =
        questionPart.bestAttempt.correct.toString();
    }
  }
  ...
  const queryParams = Object.keys(appParams)
    .map((key) => {
  ...
  const onMessage = function (e: any) {
    if (
      iframeRef.current &&
      e.source !== (iframeRef.current as HTMLIFrameElement).contentWindow
    ) {
      return;
    }
