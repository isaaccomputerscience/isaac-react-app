...
export function useIFrameMessages(
  ...
  sendMessage: (obj: Record<string, unknown>) => void;
} {
  const uidRef = useRef(uid);

  const [receivedData, setReceivedData] = useState(undefined);

  const [targetDomainSource, setTargetDomainSource] =
    useState<MessageEventSource>();
  ...
  const sendMessage = useCallback(
    (obj: Record<string, unknown>) => {
      ...
      if (typeof iFrameRef === "object" && iFrameRef?.current) {
        ...
        undefined !== targetDomainSource &&
        undefined !== targetDomainOrigin
      ) {
        // @ts-ignore
        ...
        console.log(
          "If foreignDomain is undefined, useIFrameMessages can only reply to messages (i.e. can send only after the first message has been received)",
        );
      }
    },
  ...
  const handleReceive = useCallback(
    ...
    [
      ...
      iFrameRef,
      targetDomainSource,
      setTargetDomainSource,
      setTargetDomainOrigin,
    ],
  );
...
export const isFound = <T>(
  resource: undefined | null | NOT_FOUND_TYPE | T,
): resource is NonNullable<T> => {
...
export function useOutsideCallback(
  ref: RefObject<any>,
  callback: () => void,
