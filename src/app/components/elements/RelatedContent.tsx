...
function getURLForContent(content: ContentSummaryDTO) {
  return `/${documentTypePathPrefix[content.type as DOCUMENT_TYPE]}/${
    content.id
  }`;
}
        ...
        <div className="flex-fill simple-card my-3 p-3 text-wrap">
          ...
          <div className="d-none d-lg-flex">
            <ListGroup className="w-50">
              {firstColQuestions.map((contentSummary) =>
                renderItem(contentSummary),
              )}
            </ListGroup>
            <ListGroup className="w-50">
              {secondColQuestions.map((contentSummary) =>
                renderItem(contentSummary),
              )}
            </ListGroup>
          </div>
...
export function RelatedContent({
  ...
  const audienceFilteredContent = content.filter((c) =>
    isIntendedAudience(c.audience, userContext, user),
  );
  const showConceptGameboardButton = isTutorOrAbove(
    useAppSelector(selectors.user.orNull),
  );
  ...
  const questions = sortedContent.filter(
    (contentSummary) =>
      contentSummary.type === DOCUMENT_TYPE.QUESTION ||
      contentSummary.type === DOCUMENT_TYPE.FAST_TRACK_QUESTION,
  );

  const makeListGroupItem: RenderItemFunction = (
    contentSummary: ContentSummaryDTO,
  ) => {
    ...
    return (
      <ListGroupItem
        key={getURLForContent(contentSummary)}
        className="w-100 mr-lg-3"
      >
        <Link
