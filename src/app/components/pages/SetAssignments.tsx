...
const AssignGroup = ({ groups, board, allowScheduling }: AssignGroupProps) => {
  ...
  const dueDateInvalid =
    dueDate && scheduledStartDate ? nthHourOf(0, scheduledStartDate).valueOf() > dueDate.valueOf() : false;
...
const HexagonGroupsButton = ({ toggleAssignModal, assignees, id }: HexagonGroupsButtonProps) => (
  <button onClick={toggleAssignModal} id={id} className="board-subject-hexagon-container">
    ...
    <span className="groups-assigned" title={"Groups assigned"}>
      <strong>{isDefined(assignees) ? assignees.length : <Spinner size="sm" />}</strong> group
      {(!assignees || assignees.length != 1) && "s"}
      {isDefined(assignees) && (
...
const SetAssignmentsModal = (props: SetAssignmentsModalProps) => {
  ...
  function confirmUnassignBoard(groupId: number, groupName?: string) {
    if (
      board?.id &&
      confirm(
        `Are you sure you want to unassign this gameboard from ${groupName ? `group ${groupName}` : "this group"}?`,
      )
    ) {
    ...
    <Modal isOpen={isOpen} data-testid={"set-assignment-modal"} toggle={toggle}>
      ...
      <ModalBody>
        ...
        {allowScheduling && (
          <div className="py-2">
            <Label>
              Pending assignments: <span className="icon-help mx-1" id={`pending-assignments-help-${board?.id}`} />
            </Label>
            <UncontrolledTooltip placement="left" autohide={false} target={`pending-assignments-help-${board?.id}`}>
              Assignments that are scheduled to begin at a future date. Once the start date passes, students will be
              able to see the assignment, and will receive a notification email.
            </UncontrolledTooltip>
              ...
              <Container className="mb-4">
                {scheduledAssignees.map((assignee) => (
                  <Row data-testid={"pending-assignment"} key={assignee.groupId} className="px-1">
                    ...
                    {assignee.startDate && (
                      <>
                        <span id={`start-date-${assignee.groupId}`} className="ml-auto mr-2">
                          🕑{" "}
      ...
      {boardView == BoardViews.table ? (
        ...
        <tr className="board-card" data-testid={"assignment-gameboard-table-row"}>
          <td>
            <div className="board-subject-hexagon-container table-view">
              <HexagonGroupsButton toggleAssignModal={toggleAssignModal} id={hexagonId} assignees={assignees} />
            </div>
          </td>
              ...
              <table className="w-100">
                <tbody>
                  {boardStagesAndDifficulties.map(([stage, difficulties]) => {
                    return (
                      <tr key={stage}>
                        <td className="text-center align-middle border-top-0 p-1 w-50">{stageLabelMap[stage]}</td>
        ...
        >
          <CardBody className="pb-4 pt-4">
            <button className="close" onClick={confirmDeleteBoard} aria-label="Delete gameboard">
              ×
            </button>
            <HexagonGroupsButton toggleAssignModal={toggleAssignModal} id={hexagonId} assignees={assignees} />
            <aside>
  ...
  return (
    <Container>
      ...
      {groups && groups.length === 0 && (
        <Alert color="warning">
          You have not created any groups to assign work to. Please <Link to="/groups">create a group here first.</Link>
        </Alert>
      )}
      {boards && boards.totalResults === 0 ? (
        ...
        <>
          {boards && boards.totalResults > 0 && (
            <h4>
              You have <strong>{boards.totalResults}</strong> gameboard{boards.totalResults > 1 && "s"} ready to
              assign...
            </h4>
          )}
          {!boards && (
                ...
                {boardView == BoardViews.card ? (
                  ...
                  <>
                    ...
                    <div className="text-center mt-3 mb-4" style={{ clear: "both" }}>
                      <p>
                        Showing <strong>{boards.boards.length}</strong> of <strong>{boards.totalResults}</strong>
                      </p>
                          ...
                          <tbody>
                            {boards.boards
                              .filter(
                                (board) =>
                                  board.title &&
                                  board.title.toLowerCase().includes(boardTitleFilter.toLowerCase()) &&
