...
const IsaacSymbolicLogicQuestion = ({
  doc,
  questionId,
  readonly,
}: IsaacQuestionProps<IsaacSymbolicLogicQuestionDTO>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<LogicFormulaDTO>(questionId);

  const [modalVisible, setModalVisible] = useState(false);
  const editorSeed = useMemo(
    () => jsonHelper.parseOrDefault(doc.formulaSeed, undefined),
    [],
  );
  ...
  function currentAttemptPythonExpression(): string {
    return (
      (currentAttemptValue &&
        ...
        currentAttemptValue.result.python) ||
      ""
    );
  }
  ...
  useEffect(() => {
    if (!currentAttempt || !currentAttemptValue || !currentAttemptValue.symbols)
      return;
        ...
        if (openBracketsCount !== closeBracketsCount) {
          _errors.push(
            "You are missing some " +
              (closeBracketsCount > openBracketsCount ? "opening" : "closing") +
              " brackets.",
          );
        }
        setErrors(_errors);
  ...
  const symbolList = doc.availableSymbols
      ...
      <div
        ...
        dangerouslySetInnerHTML={{
          __html: previewText
            ...
            : "Click to enter your expression",
        }}
      />
      {modalVisible && (
        <InequalityModal
