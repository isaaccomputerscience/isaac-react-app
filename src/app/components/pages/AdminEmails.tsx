...
export const AdminEmails = (props: AdminEmailsProps) => {
  ...
  const [csvIDs, setCSVIDs] = useState(
    props.location.state?.csvIDs || ([] as number[]),
  );
  ...
  const emailTemplateSelector = useAppSelector(
    (state: AppState) =>
      state && state.adminEmailTemplate && state.adminEmailTemplate,
  );

  const numberOfUsers = csvIDs.length;
  const canSubmit =
    emailTemplateSelector && emailType != "null" && numberOfUsers > 0;
    ...
    <RS.Container id="admin-emails-page">
      ...
      <RS.Card className="p-3 my-3">
        ...
        <RS.CardBody>
          ...
          <p>
            Users who have opted out of this type of email will not receive
            anything. Administrative emails cannot be opted out of and should be
            avoided.
          </p>
          <RS.Input
            id="email-type-input"
          ...
          <RS.Row>
            ...
            <RS.Col>
              <RS.Input
                ...
                value="Load template"
                className={
                  "btn btn-block btn-secondary border-0 " +
                  classnames({ disabled: contentObjectID.length == 0 })
                }
      ...
      {emailTemplateSelector && (
        <>
          <RS.Card className="p-3 my-3">
            ...
            <RS.CardBody>
              <ul>
                <li>
                  <b>Subject:</b>{" "}
                  {emailTemplateSelector.subject || "no subject"}
                </li>
                {emailTemplateSelector.from && (
                  <li>
                    <b>From:</b> {emailTemplateSelector.fromName || ""} &lt;
                    {emailTemplateSelector.from}&gt;
                  </li>
                )}
                {emailTemplateSelector.replyTo && (
                  <li>
                    <b>Reply-To:</b> {emailTemplateSelector.replyToName || ""}{" "}
                    &lt;{emailTemplateSelector.replyTo}&gt;
                  </li>
                )}
                <li>
                  <b>Sent via:</b>&nbsp;
                  {emailTemplateSelector.sender?.includes("@mail.isaac")
                    ? "MailGun"
                    : "University"}
                </li>
              </ul>
          ...
          <RS.Card className="p-3 my-3">
            ...
            <RS.Label>
              The preview below uses fields taken from your account (e.g.
        ...
        <RS.CardBody>
          <div className="text-center">
            {!emailSent ? (
              <React.Fragment>
                {numberOfUsers >= RECIPIENT_NUMBER_WARNING_VALUE && (
                  <div className="alert alert-warning">
                    ...
                    <strong>{numberOfUsers}</strong> selected recipients.
                  </div>
                )}
                <RS.Input
                  type="button"
                  value="Send emails"
                  className={
                    "btn btn-xl btn-secondary border-0 " +
                    classnames({ disabled: !canSubmit })
                  }
                  disabled={!canSubmit}
                  onClick={() => {
                    if (
              ...
              <React.Fragment>
                Request made, to send another refresh.
              </React.Fragment>
            )}
          </div>
        </RS.CardBody>
