...
export function TopicSummaryLinks({
  items,
  search,
}: {
  ...
  return (
    <RS.ListGroup className="mt-3 link-list list-group-links">
      {items
        ...
        .sort((itemA, itemB) => {
          return makeIntendedAudienceComparator(user, userContext)(
            itemA,
            itemB,
          );
        })
        ...
        .map((item) => {
          const intendedAudience = isIntendedAudience(
            item.audience,
            userContext,
            user,
          );
          ...
          return {
            ...item,
            deEmphasised: !intendedAudience,
            hidden: !intendedAudience && !showOtherContent,
          };
        })
        ...
        .map((item, index) => (
          <RS.ListGroupItem key={item.id} className="topic-summary-link">
            <RS.Button
              tag={Link}
              to={{
                pathname: `/${documentTypePathPrefix[DOCUMENT_TYPE.CONCEPT]}/${
                  item.id
                }`,
                search,
              }}
              ...
              className={
                ...
                classNames({ "de-emphasised": item.deEmphasised })
              }
            >
              <div
                className={
              ...
              <div className="title pl-3 d-flex">
                ...
                {item.deEmphasised && (
                  <div className="ml-auto mr-3 d-flex align-items-center">
                    <span
                      id={`audience-help-${index}`}
                      className="icon-help mx-1"
                    />
                    <RS.UncontrolledTooltip
                      placement="bottom"
                      target={`audience-help-${index}`}
                    >
