...
import {
  ...
  FRIENDLY_DATE,
  TIME_ONLY,
} from "../components/elements/DateString";
...
export const studentOnlyEventMessage = (eventId?: string) => (
  <React.Fragment>
    ...
    <Link
      to={`/contact?subject=${encodeURI(
        "Non-student attendance at " + eventId,
      )}`}
    >
      contact us
    </Link>
...
export const augmentEvent = (event: IsaacEventPageDTO): AugmentedEvent => {
  ...
  if (event.date != null) {
    ...
    augmentedEvent.isWithinBookingDeadline =
      ...
      (event.bookingDeadline
        ? now <= new Date(event.bookingDeadline).getTime()
        : true);
  }
  ...
  augmentedEvent.isNotClosed = !["CLOSED", "CANCELLED"].includes(
    event.eventStatus as string,
  );
  ...
  if (augmentedEvent.eventThumbnail && augmentedEvent.eventThumbnail.src) {
    augmentedEvent.eventThumbnail.src = apiHelper.determineImageUrl(
      augmentedEvent.eventThumbnail.src,
    );
  } else {
...
export const formatWaitingListBookingStatusMessage = (
  event: AugmentedEvent,
) => {
  if (event.isWaitingListOnly) {
...
export const formatCancelBookingButtonMessage = (event: AugmentedEvent) => {
  if (event.userBookingStatus == "CONFIRMED") {
    ...
    return event.isWaitingListOnly
      ? "Cancel booking request"
      : "Leave waiting list";
  }
};

export const formatManageBookingActionButtonMessage = (
  event: AugmentedEvent,
) => {
  if (event.userBookingStatus === "RESERVED") {
...
export const formatBookingModalConfirmMessage = (
  event: AugmentedEvent,
  userCanMakeEventBooking?: boolean,
) => {
...
export const userIsTeacherAtAStudentEvent = (
  user: Immutable<PotentialUser> | null,
  event: AugmentedEvent,
) => {
...
export const userCanMakeEventBooking = (
  user: Immutable<PotentialUser> | null,
  event: AugmentedEvent,
) => {
  return (
...
export const userCanBeAddedToEventWaitingList = (
  user: Immutable<PotentialUser> | null,
  event: AugmentedEvent,
) => {
  return (
    ...
    (event.userBookingStatus === undefined ||
      !["WAITING_LIST", "CONFIRMED", "RESERVED"].includes(
...
export const userCanReserveEventSpaces = (
  user: Immutable<PotentialUser> | null,
  event: AugmentedEvent,
) => {
  return (
