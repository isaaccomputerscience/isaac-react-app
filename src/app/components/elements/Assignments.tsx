...
export const AssignmentCard = ({
  ...
  const boardStagesAndDifficulties = useMemo(
    () => determineGameboardStagesAndDifficulties(assignment.gameboard),
    [assignment.gameboard],
  );
  const topics = tags
    .getTopicTags(
      Array.from(
        (assignment.gameboard?.contents || []).reduce((a, c) => {
          if (isDefined(c.tags) && c.tags.length > 0) {
            return new Set([
              ...Array.from(a),
              ...c.tags.map((id) => id as TAG_ID),
            ]);
          }
          return a;
        }, new Set<TAG_ID>()),
      ).filter((tag) => isDefined(tag)),
    )
    .map((tag) => tag.title)
    .sort();
      ...
      <Row className="board-card" data-testid={"my-assignment"}>
        <Col xs={4} md={2} lg={1}>
          <div className="board-subject-hexagon-container">
            {isDefined(assignment.gameboard) &&
        ...
        <Col xs={8} md={3} lg={4} className="pl-lg-5">
          ...
          {isDefined(assignmentStartDate) && (
            <p className="mb-0" data-testid={"gameboard-assigned"}>
              <strong>Assigned:</strong> {formatDate(assignmentStartDate)}
            </p>
          )}
          ...
          {isDefined(assignment.groupName) && (
            <p className="mb-0">
              <strong>Group:</strong> {assignment.groupName}
            </p>
          )}
        ...
        <Col xs={7} md={5} className="mt-sm-2">
          <p className="mb-0">
            ...
            {assignment.gameboard?.contents?.length || "0"}
          </p>
        ...
        <Col xs={5} md={2} className="mt-sm-2 text-right">
          ...
          {isDefined(assignment.dueDate) &&
            isDefined(assignment.gameboard) &&
            now > midnightOf(assignment.dueDate) &&
            assignment.gameboard.percentageCompleted !== 100 && (
              <p>
                <strong className="overdue">Overdue:</strong>{" "}
    ...
    <>
      ...
      {assignments.length === 0 &&
        (showOld ? (
          ...
          <p className="text-center py-4">
            <strong>There are no assignments to display.</strong>
          </p>
        ))}
    </>
  );
