...
const determineUrlQueryPresets = (user?: Immutable<PotentialUser> | null) => {
  ...
  if (
    urlQuery?.preset == "teacherRequest" &&
    user?.loggedIn &&
...
export const Contact = () => {
  ...
  const [presetSubject, presetMessage, presetPlaceholder] =
    determineUrlQueryPresets(user);
  const [firstName, setFirstName] = useState(
    (user && user.loggedIn && user.givenName) || "",
  );
  const [lastName, setLastName] = useState(
    (user && user.loggedIn && user.familyName) || "",
  );
  const [email, setEmail] = useState(
    (user && user.loggedIn && user.email) || "",
  );
      ...
      <div className="pt-4">
        <Row>
          ...
          >
            ...
            <p>
              If you&apos;d like to find out more about our upcoming events,
              visit our <a href="/events">Events Page</a>
            </p>
