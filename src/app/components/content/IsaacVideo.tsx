...
function rewrite(src: string) {
  ...
  if (possibleVideoId) {
    ...
    const optionalStart = possibleStartTime
      ? `&start=${possibleStartTime[2]}`
      : "";
...
function onPlayerStateChange(
  event: any,
  wrappedLogAction: (eventDetails: object) => void,
  pageId?: string,
) {
...
export function IsaacVideo(props: IsaacVideoProps) {
      ...
      if (node !== null && $window.YT) {
        try {
            ...
            new $window.YT.Player(node, {
              events: {
                onStateChange: (event: any) => {
                  onPlayerStateChange(
                    event,
                    (eventDetails) => dispatch(logAction(eventDetails)),
                    pageId,
                  );
                },
              },
          ...
          ReactGA.gtag("event", "exception", {
            description: `youtube_error: ${
              error?.message || "problem with YT library"
            }`,
            fatal: false,
          });
        }
  ...
  const detailsForPrintOut = (
    <div className="only-print py-2 mb-4">{altTextToUse}</div>
  );
