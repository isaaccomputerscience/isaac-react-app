...
import {
  ...
  ParsingError,
} from "inequality-grammar";
import { selectors, useAppSelector } from "../../state";
import {
  EditorMode,
  LogicSyntax,
} from "../elements/modals/inequality/constants";

const InequalityModal = lazy(
  () => import("../elements/modals/inequality/InequalityModal"),
);
...
const Equality = withRouter(
  ({
    ...
    {},
    {},
    { board?: string; mode?: string; symbols?: string }
  >) => {
    ...
    const [editorMode, setEditorMode] = useState<EditorMode>(
      (queryParams.mode as EditorMode) || "logic",
    );
    const segueEnvironment = useAppSelector(
      selectors.segue.environmentOrUnknown,
    );
    ...
    function updateState(state: any) {
      ...
      const previousPythonExpression =
        currentAttempt.value?.result?.python || "";
      if (
        !previousPythonExpression ||
        previousPythonExpression !== pythonExpression
      ) {
        setCurrentAttempt({
          type: "formula",
    ...
    const _updateEquation = (pycode: string) => {
      ...
      debounceTimer.current = window.setTimeout(() => {
        ...
        if (
          isDefined(parsedExpression) &&
          (isError(parsedExpression) ||
            (parsedExpression.length === 0 && pycode !== ""))
        ) {
          ...
          if (isError(parsedExpression) && parsedExpression.error) {
            _errors.push(
              `Syntax error: unexpected token "${
                parsedExpression.error.token.value || ""
              }"`,
            );
          }
          ...
          if (/\|.+?\|/.test(pycode)) {
            _errors.push(
              "Vertical bar syntax for absolute value is not supported; use abs() instead.",
            );
          }
          if (badCharacters.test(pycode)) {
            ...
            _errors.push(
              "Some of the characters you are using are not allowed: " +
                usedBadChars.join(" "),
            );
          }
          ...
          if (pycode === "") {
            ...
            if (sketchRef.current) {
              sketchRef.current.parseSubtreeObject(
                parsedExpression[0],
                true,
                true,
                pycode,
    ...
    useLayoutEffect(() => {
      const { sketch } = makeInequality(hiddenEditorRef.current, 100, 0, [], {
        textEntry: true,
    ...
    const availableSymbols =
      queryParams.symbols &&
      (queryParams.symbols as string).split(",").map((s) => s.trim());
        ...
        <Container>
          ...
          <Row>
            <Col md={{ size: 2 }} className="py-4 syntax-picker mode-picker">
              <div>
                ...
                <Input
                  type="select"
                  name="mode"
                  id="inequality-mode-select"
                  value={editorMode as string}
              ...
              {editorMode === "logic" && (
                <div className="mt-4">
                  <Label for="inequality-syntax-select">
                    Boolean Logic Syntax
                  </Label>
                  <Input
                    type="select"
                    name="syntax"
                    id="inequality-syntax-select"
                    value={editorSyntax}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            <Col md={{ size: 8 }} className="py-4 question-panel">
              {(editorMode === "maths" ||
                (isStaff(user) && editorMode === "logic")) && (
                <div className="eqn-editor-input mt-4">
                  <div
                    ref={hiddenEditorRef}
              ...
              <div className="equality-page">
                <div
                  ...
                  className={`eqn-editor-preview rounded ${
                    !previewText ? "empty" : ""
                  } ${editorMode !== "maths" ? "mt-4" : ""}`}
                  tabIndex={0}
                  onClick={() => setModalVisible(true)}
                  onKeyDown={ifKeyIsEnter(() => setModalVisible(true))}
                  dangerouslySetInnerHTML={{
                    __html: previewText
                      ? katex.renderToString(previewText)
                      : `<small>${
                          editorMode === "maths" ? "or c" : "C"
                        }lick here to enter a formula</small>`,
                  }}
                />
                {modalVisible && (
                  <InequalityModal
                    ...
                    onEditorStateChange={(state: any) => {
                      setCurrentAttempt({
                        ...
                        pythonExpression:
                          (state && state.result && state.result.python) || "",
                        symbols: [],
                      });
          ...
          {currentAttemptValue &&
            ...
            currentAttemptValue.result.tex && (
              <Row>
                <Col
                  md={{ size: 8, offset: 2 }}
                  className="py-4 inequality-results"
                >
                  <h4>LaTeX</h4>
                  ...
                  {(segueEnvironment === "DEV" || isStaff(user)) && (
                    <>
                      ...
                      <pre>
                        {currentAttemptValue &&
                          currentAttemptValue.symbols &&
                          JSON.stringify(currentAttemptValue.symbols)}
                      </pre>
                    </>
                  )}
                </Col>
