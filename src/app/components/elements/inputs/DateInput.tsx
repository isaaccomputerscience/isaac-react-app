...
export const DateInput = (props: DateInputProps) => {
  ...
  const valueAsDate =
    value != undefined ? new Date(value as unknown as string) : undefined;

  const defaultValue = props.defaultValue;
  const defaultDate =
    defaultValue != undefined
      ? new Date(defaultValue as unknown as string)
      : undefined;

  const values: {
    [what: string]: {
      get: () => number | undefined;
      set: (to: number | undefined) => void;
      reset: () => void;
    };
  } = {
    day: useWrappedState<number>(
      extractDay(valueAsDate) || extractDay(defaultDate),
    ),
    month: useWrappedState<number>(
      extractMonth(valueAsDate) || extractMonth(defaultDate),
    ),
    year: useWrappedState<number>(
      extractYear(valueAsDate) || extractYear(defaultDate),
    ),
  };
  ...
  const change = (what: string) => (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!props.disableDefaults && (what == "day" || what == "month")) {
      ...
      if (values["month"].get() == undefined) {
        const defaultMonth =
          day && day <= currentDay ? currentMonth + 1 : currentMonth;
        values["month"].set(defaultMonth > 12 ? 1 : defaultMonth);
        month = defaultMonth;
      }
      if (values["year"].get() == undefined) {
        const defaultYear =
          month > 12 || month < currentMonth ? currentYear + 1 : currentYear;
        const scaledYear =
          props.yearRange && props.yearRange.length > 0
  ...
  const controlPropsWithValidationStripped = {
    ...controlProps,
    valid: undefined,
    invalid: undefined,
  };
  ...
  return (
    <React.Fragment>
      <InputGroup
        id={props.id}
        {...controlPropsWithValidationStripped}
        className={inputGroupClasses}
      >
        <Input
          className="date-input-day mr-1"
          type="select"
          {...controlProps}
          aria-label={`Day${props.labelSuffix ? props.labelSuffix : ""}`}
          onChange={change("day")}
          value={values.day.get() || ""}
        >
          {values.day.get() === undefined && (
            <option value="" disabled hidden>
              Day
            </option>
          )}
          {range(1, Math.max(lastInMonth(), values.day.get() || 0) + 1).map(
            (day) => (
        ...
        <Input
          className="date-input-month mr-1"
          type="select"
          {...controlProps}
          aria-label={`Month${props.labelSuffix ? props.labelSuffix : ""}`}
          onChange={change("month")}
          value={values.month.get() || ""}
        >
          {values.month.get() === undefined && (
            <option value="" disabled hidden>
              Month
            </option>
          )}
        ...
        <Input
          className="date-input-year mr-1"
          type="select"
          {...controlProps}
          aria-label={`Year${props.labelSuffix ? props.labelSuffix : ""}`}
          onChange={change("year")}
          value={values.year.get() || ""}
        >
          {values.year.get() === undefined && (
            <option value="" disabled hidden>
              Year
            </option>
          )}
        ...
        {(props.noClear === undefined || !props.noClear) && (
          <Button
            close
            {...controlPropsWithValidationStripped}
            className="mx-1"
