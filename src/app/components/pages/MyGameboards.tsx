...
const Board = (props: BoardTableProps) => {
  ...
  const updateBoardSelection = (board: GameboardDTO, checked: boolean) => {
    if (checked) {
      ...
      setSelectedBoards(
        selectedBoards.filter((thisBoard) => thisBoard.id !== board.id),
      );
    }
  };
  ...
  const boardStagesAndDifficulties =
...
export const MyGameboards = () => {
  ...
  const [boardCompletion, setBoardCompletion] = useState<BoardCompletions>(
    BoardCompletions.any,
  );
    ...
    if (
      confirm(
        `Are you sure you want to remove ${
          selectedBoards && selectedBoards.length > 1
            ? selectedBoards.length + " boards"
            : selectedBoards[0].title
        } from your account?`,
      )
    ) {
      selectedBoards &&
  ...
  const switchViewAndClearSelected = useCallback(
  ...
  const pageHelp = <span>A summary of your gameboards</span>;
      ...
      {boards && boards.totalResults == 0 ? (
        ...
        <>
          <div className="mt-4 mb-2">
            {boards && boards.totalResults && boards.totalResults > 0 && (
              <h4>
                You have completed <strong>{completed}</strong> of{" "}
                <strong>{boards.totalResults}</strong> gameboard
                {boards.totalResults > 1 && "s"}, with{" "}
                <strong>{inProgress}</strong> on the go and{" "}
                <strong>{notStarted}</strong> not started
              </h4>
            )}
            {!boards && (
              <h4>
                You have <IsaacSpinner size="sm" inline /> saved gameboards...
              </h4>
            )}
          </div>
          <div>
                ...
                <Col sm={6} lg={3} xl={2}>
                  <Label className="w-100">
                    ...
                    <Input
                      type="select"
                      value={boardView}
                      onChange={switchViewAndClearSelected}
                    >
                      ...
                      <Row>
                        ...
                        <Col sm={6} lg={2}>
                          <Label className="w-100">
                            ...
                            <Input
                              type="select"
                              value={boardCompletion}
                              onChange={(e) =>
                                setBoardCompletion(
                                  e.target.value as BoardCompletions,
                                )
                              }
                            >
                              {Object.values(BoardCompletions).map(
                                (completion) => (
                            ...
                            <tr>
                              <th className="align-middle pointer-cursor">
                                <button
                                  ...
                                  onClick={() =>
                                    boardOrder == BoardOrder.completion
                                      ? setBoardOrder(BoardOrder["-completion"])
                                      : setBoardOrder(BoardOrder.completion)
                                  }
                                >
                                  Completion{" "}
                              ...
                              >
                                Difficulties{" "}
                                <span
                                  id={`difficulties-help`}
                                  className="icon-help mx-1"
                                />
                                <RS.UncontrolledTooltip
                                  placement="bottom"
                                  target={`difficulties-help`}
                                >
                                  Practice:{" "}
                                  {difficultiesOrdered
                                    .slice(0, 2)
                              ...
                              <th colSpan={2}>
                                <div className="text-right align-middle">
                                  <Button
                                    ...
                                    className="btn-sm"
                                    onClick={confirmDeleteMultipleBoards}
                                  >
