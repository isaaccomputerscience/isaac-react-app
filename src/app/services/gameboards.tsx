...
export function boardCompletionSelection(
  board: GameboardDTO,
  boardCompletion: BoardCompletions,
) {
  if (
    boardCompletion == BoardCompletions.notStarted &&
    (board.percentageCompleted == 0 || !board.percentageCompleted)
  ) {
    return true;
  } else if (
    boardCompletion == BoardCompletions.completed &&
    board.percentageCompleted &&
    board.percentageCompleted == 100
  ) {
    return true;
  } else if (
    boardCompletion == BoardCompletions.inProgress &&
    board.percentageCompleted &&
    board.percentageCompleted != 100 &&
    board.percentageCompleted != 0
  ) {
    return true;
  } else return boardCompletion == BoardCompletions.any;
}

const createGameboardLink = (gameboardId: string) =>
  `/gameboards#${gameboardId}`;
...
export const determineGameboardHistory = (currentGameboard: GameboardDTO) => {
  return createGameboardHistory(
    currentGameboard.title as string,
    currentGameboard.id as string,
  );
};

export const determineNextGameboardItem = (
  currentGameboard: GameboardDTO | NOT_FOUND_TYPE | undefined,
  currentDocId: string,
) => {
  const boardQuestions: (string | undefined)[] = [];
  if (isFound(currentGameboard) && currentGameboard.contents) {
    currentGameboard.contents.map((question) =>
      boardQuestions.push(question.id),
    );
...
export const determinePreviousGameboardItem = (
  currentGameboard: GameboardDTO | NOT_FOUND_TYPE | undefined,
  currentDocId: string,
) => {
  const boardQuestions: (string | undefined)[] = [];
      ...
      if (gameboardContentIds.includes(currentDocId)) {
        ...
        if (previousIndex > -1) {
          ...
          return {
            ...
            to: `/questions/${previousContent.id}`,
          };
        }
      }
    }
...
export const determineCurrentCreationContext = (
  currentGameboard: GameboardDTO | NOT_FOUND_TYPE | undefined,
  currentDocId: string,
) => {
  if (isFound(currentGameboard) && currentGameboard.contents) {
    return currentGameboard.contents.filter(
      (gameboardItem) => gameboardItem.id === currentDocId,
    )[0]?.creationContext;
  }
};
...
export function comparatorFromOrderedValues<T>(orderedPropertyValues: T[]) {
  return function comparator(a?: T, b?: T) {
    ...
    return (
      orderedPropertyValues.indexOf(a!) - orderedPropertyValues.indexOf(b!)
    );
  };
}

// A function that returns ordered (stage, difficulties) tuples for a gameboard
export function determineGameboardStagesAndDifficulties(
  gameboard: GameboardDTO | undefined,
): [Stage, Difficulty[]][] {
  ...
  stagesOrdered.forEach((stage) => {
    if (stageDifficultiesMap[stage]) {
      const orderedAndDeduplicatedDifficulties = Array.from(
        new Set(stageDifficultiesMap[stage]),
      ).sort(comparatorFromOrderedValues(difficultiesOrdered));
      orderedStageDifficulties.push([
        stage,
        orderedAndDeduplicatedDifficulties,
      ]);
    }
  });
...
const parseBoardLimitAsNumber: (limit: BoardLimit) => NumberOfBoards = (
  limit: BoardLimit,
) => (limit === BoardLimit.All ? BoardLimit.All : parseInt(limit, 10));

export const useGameboards = (
  initialView: BoardViews,
  initialLimit: BoardLimit,
) => {
  ...
  const [numberOfBoards, setNumberOfBoards] = useState<NumberOfBoards>(
    parseBoardLimitAsNumber(boardLimit),
  );

  // Fetch gameboards from server, no aggregation since we want a fresh list
  const loadInitial = useCallback(
    (limit: NumberOfBoards) => {
  ...
  const orderedBoards = useMemo<Boards | null>(() => {
    ...
    const boardOrderKind = (
      boardOrderNegative ? boardOrder.slice(1) : boardOrder
    ) as "created" | "visited" | "completion" | "title";
    const orderedBoards = sortBy(
      boards?.boards,
      BOARD_SORT_FUNCTIONS[boardOrderKind],
    );
    if (["visited", "created", "-completion", "-title"].includes(boardOrder))
      orderedBoards.reverse();
    return {
