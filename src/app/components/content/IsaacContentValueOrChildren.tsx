...
type ContentOrAccordionChunk = ContentDTO[] & {
  isAccordion?: boolean;
...
export const IsaacContentValueOrChildren = ({
  value,
  encoding,
  children,
}: ContentValueOrChildrenProps) => {
  ...
  children?.forEach((child) => {
    if (
      (breakOnTypeChange && child.type != lastType) ||
      (!breakOnTypeChange && child.type == "isaacFeaturedProfile")
    ) {
  ...
  if (value && value != "" && children && children.length > 0) {
    throw new Error(
      "Cannot render both value and children:\n" +
        "\tVALUE:\n" +
    ...
    <React.Fragment>
      {value && (
        <div className="content-value">
          <Markup trusted-markup-encoding={encoding}>{value}</Markup>
        </div>
      )}
      {contentChunks.map((contentChunk, chunkIndex) => {
        if (contentChunk.isAccordion) {
          ...
          return (
            <div className="clearfix content-chunk" key={chunkIndex}>
              {contentChunk.map((content, contentIndex) => (
                <IsaacContent
                  doc={content}
                  key={contentIndex}
