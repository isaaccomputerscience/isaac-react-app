...
const composeCollisionAlgorithms =
  (first: CollisionDetection, second: CollisionDetection): CollisionDetection =>
  (args) => {
    const collisions = first(args);
    return collisions.length > 0 ? collisions : second(args);
  };

const isDropZone = (item: { id: UniqueIdentifier } | null) =>
  item?.id === CLOZE_ITEM_SECTION_ID ||
  String(item?.id).slice(0, 10) === CLOZE_DROP_ZONE_ID_PREFIX;

const augmentInlineItemWithUniqueReplacementID = (
  idv: Immutable<ClozeItemDTO> | undefined,
) =>
  isDefined(idv)
    ? { ...idv, replacementId: `${idv?.id}|${uuid_v4()}` }
    : undefined;
const augmentNonSelectedItemWithReplacementID = (
  item: Immutable<ClozeItemDTO>,
) => ({ ...item, replacementId: item.id });
const itemNotNullAndNotInAttempt =
  (currentAttempt: { items?: (Immutable<ItemDTO> | undefined)[] }) =>
  (i: Immutable<ClozeItemDTO> | undefined) =>
    i ? !currentAttempt.items?.map((si) => si?.id).includes(i.id) : false;
...
const replaceNullItems = (items: readonly Immutable<ItemDTO>[] | undefined) =>
  items?.map((i) => (i.id === NULL_CLOZE_ITEM_ID ? undefined : i));
...
const ItemSection = ({
  ...
  const isOverContainer =
    isOver || (over ? isDefined(items.find((i) => i.id === over.id)) : false);

  return (
    <div className={"mb-3"}>
      <Label className="mt-3">Items: </Label>
      <Label className={"sr-only"} id={"item-section-info"}>
        To pick up an item, press space or enter. Use the up and down arrow keys
        to navigate between drop zones and items in the question. Press space or
        enter again to drop the item into a new position, or to swap it with
        another item being hovered over.
      </Label>
      <SortableContext items={itemIds} strategy={() => null}>
        <div
          aria-labelledby={"item-section-info"}
          ref={setNodeRef}
          aria-label={"Non-selected items"}
          className={`item-section rounded p-2 bg-grey ${
            isOverContainer ? "border border-dark" : "border-light"
          }`}
        >
          {items.map((item, i) => (
            <Item
              key={i}
              item={item}
              id={item.replacementId as string}
              type={"item-section"}
            />
          ))}
        </div>
...
const isTouchEvent = (event: MouseEvent | TouchEvent): event is TouchEvent => {
  return ["touchstart", "touchmove", "touchend", "touchcancel"].includes(
    event.type,
  );
};
...
const useAutoScroll = (
  {
    ...
    acceleration,
    interval,
  }: { active: boolean; acceleration?: number; interval?: number } = {
    active: false,
  },
) => {
  ...
  const autoScrollListener = useCallback((event: MouseEvent | TouchEvent) => {
    ...
    if (y <= thresholdInPixels) {
      setScrollAmount(
        (Math.max(baseline, y) - thresholdInPixels) / thresholdInPixels,
      );
    } else if (y >= referenceHeight - thresholdInPixels) {
      setScrollAmount(
        (Math.min(referenceHeight, y) - (referenceHeight - thresholdInPixels)) /
          thresholdInPixels,
      );
    } else {
      setScrollAmount(0);
  ...
  const updateScrollAmount = useCallback(
    (scrollAmount: number, acceleration = 10, interval = 5) => {
      if (scrollAmount !== 0) {
        const scaledScrollAmount =
          Math.abs(scrollAmount) * scrollAmount * acceleration;
...
const IsaacClozeQuestion = ({
  doc,
  questionId,
  readonly,
  validationResponse,
}: IsaacQuestionProps<IsaacClozeQuestionDTO, ItemValidationResponseDTO>) => {
  const { currentAttempt: rawCurrentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<ItemChoiceDTO>(questionId);
  const currentAttempt = useMemo(
    () =>
      rawCurrentAttempt
        ? {
            ...rawCurrentAttempt,
            items: replaceNullItems(rawCurrentAttempt.items),
          }
        : undefined,
    [rawCurrentAttempt],
  );
  ...
  const [nonSelectedItems, setNonSelectedItems] = useState<
    Immutable<ClozeItemDTO>[]
  >(
    doc.items
      ? [...doc.items].map(augmentNonSelectedItemWithReplacementID)
      : [],
  );

  const registeredDropRegionIDs = useRef<Map<string, number>>(
    new Map(),
  ).current;

  const [inlineDropValues, setInlineDropValues] = useState<
    (Immutable<ClozeItemDTO> | undefined)[]
  ...
  const inlineDropValueMap = useMemo<{ [p: string]: ClozeItemDTO }>(
    () =>
      Array.from(registeredDropRegionIDs.entries()).reduce(
        (dict, [dropId, i]) =>
          Object.assign(dict, { [dropId]: inlineDropValues[i] }),
        {},
      ),
    [inlineDropValues],
  );
  ...
  const [dropZoneValidationMap, setDropZoneValidationMap] = useState<{
  ...
  useEffect(() => {
    if (isDefined(itemsCorrect)) {
      ...
      // drop-zone.
      setDropZoneValidationMap(
        Array.from(registeredDropRegionIDs.entries()).reduce(
          (dict, [dropId, i]) =>
            Object.assign(dict, {
              [dropId]: {
                correct: itemsCorrect.at(i),
                itemId: inlineDropValueMap[dropId]?.id,
              },
            }),
          {},
        ),
  ...
  const shouldGetFocus = useCallback(
    (id: string) => {
      if (id === focusId) {
  ...
  useEffect(
    function updateStateOnCurrentAttemptChange() {
      if (currentAttempt?.items) {
        setInlineDropValues(
          currentAttempt.items.map(augmentInlineItemWithUniqueReplacementID),
        );
        ...
        if (!withReplacement) {
          setNonSelectedItems(
            (nsis) =>
              nsis
                .filter(itemNotNullAndNotInAttempt(currentAttempt))
                .map(augmentNonSelectedItemWithReplacementID) || [],
          );
        }
      }
    },
  ...
  useEffect(
    function updateStateOnDocChange() {
      setInlineDropValues(
        (currentAttempt?.items ?? []).map(
          augmentInlineItemWithUniqueReplacementID,
        ),
      );
  ...
  const findItemById = (id: string) =>
    nonSelectedItems.find((i) => i.replacementId === id) ??
    inlineDropValues.find((i) => i && i.replacementId === id);
  ...
  useAutoScroll({
    active: isDefined(activeItem) && !usingKeyboard,
    acceleration: 5,
    interval: 5,
  });

  const registerInlineDropRegion = useCallback(
    (dropRegionId: string, index: number) => {
  ...
  const onDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const isFromItemSection = isDefined(
        nonSelectedItems.find((i) => i.replacementId === active?.id),
      );
      const isToItemSection =
        isDefined(nonSelectedItems.find((i) => i.replacementId === over?.id)) ||
        over?.id === CLOZE_ITEM_SECTION_ID;
      ...
      const updateAttempt = (idvs: (Immutable<ClozeItemDTO> | undefined)[]) => {
        ...
        const itemChoice: ItemChoiceDTO = {
          type: "itemChoice",
          items: Array(registeredDropRegionIDs.size)
            .fill(null)
            .map((_, i) => {
              const item = idvs[i];
      ...
      if (isFromItemSection) {
        ...
        if (isToItemSection) {
          ...
          const toIndex =
            inlineDropIndex(over.id as string) ??
            idvs.findIndex((i) => i?.replacementId === over.id);
          // Cancel if error
          if (toIndex === -1) return;
        ...
        if (isToItemSection) {
          ...
          if (over?.id === CLOZE_ITEM_SECTION_ID) {
            ...
            if (!withReplacement)
              nsis.push({ ...item, replacementId: item.id });
          } else {
            const toIndex = nsis.findIndex((i) => i.replacementId === over.id);
            ...
            if (item.id === over.id && withReplacement) {
              ...
              idvs[fromIndex] = augmentInlineItemWithUniqueReplacementID(
                nsis[toIndex],
              );
              if (!withReplacement)
                nsis.splice(toIndex, 1, { ...item, replacementId: item.id });
            }
          }
          newFocusId = item.id;
          updateAttempt(idvs);
  ...
  const customCollision: CollisionDetection = useCallback(
    (args) => {
      ...
      const itemSection = initialCollisions.find(
        (c) => c.id === CLOZE_ITEM_SECTION_ID,
      );
      if (itemSection) {
        const justNonSelectedItems = args.droppableContainers.filter(
          (dc) =>
            nonSelectedItems.findIndex((i) => i.replacementId === dc.id) !== -1,
        );
        const rectNSICollisions = initialCollisions.filter(
          (c) => !isDropZone(c) && c.id !== CLOZE_ITEM_SECTION_ID,
        );
        if (rectNSICollisions.length > 0) {
          return closestCenter({
            ...args,
            droppableContainers: justNonSelectedItems,
          });
        }
        return initialCollisions;
      }
      const justDropZonesAndItemSection = args.droppableContainers.filter(
        (c) => isDropZone(c) || c.id === CLOZE_ITEM_SECTION_ID,
      );
      return closestCenter({
        ...args,
        droppableContainers: justDropZonesAndItemSection,
      });
    },
    ...
    () => ({
      ...
      announcements: {
        ...
        onDragOver({ active, over }) {
          if (!over)
            return usingKeyboard
              ? undefined
              : `Draggable item ${active.data.current?.text} is no longer over a droppable area.`;

          if (active.id === over.id)
            return `Draggable item ${active.data.current?.text} is over it's previous position.`;
          ...
          if (!isDropZone(over)) {
            ...
            const dropZoneIndex = (over.id as string).replace(
              CLOZE_DROP_ZONE_ID_PREFIX,
              "",
            );
        ...
        onDragEnd({ active, over }) {
          if (!over)
            return `Draggable item ${active.data.current?.text} was dropped.`;

          if (active.id === over.id)
            return `Draggable item ${active.data.current?.text} was returned to it's previous position.`;
  ...
  return (
    <div
      className="question-content cloze-question"
      id={cssFriendlyQuestionPartId}
    >
      ...
      >
        ...
        >
          ...
          <DragOverlay>
            {activeItem && (
              <Badge className="p-2 cloze-item is-dragging">
                <IsaacContentValueOrChildren
                  value={activeItem.value}
                  encoding={activeItem.encoding || "html"}
                >
