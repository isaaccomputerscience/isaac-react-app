...
): user is Immutable<LoggedInUser> {
...
export function isTutorOrAbove(
  user?: { readonly role?: UserRole; readonly loggedIn?: boolean } | null,
): boolean {
  return isDefined(user) && user.role !== "STUDENT" && (user.loggedIn ?? true);
}
...
export function isTeacherPending(
  user?: {
    ...
    readonly teacherPending?: boolean;
  } | null,
): boolean {
  return (
    isDefined(user) && user.teacherPending === true && (user.loggedIn ?? true)
  );
}

export function isTeacherOrAbove(
  user?: { readonly role?: UserRole; readonly loggedIn?: boolean } | null,
): boolean {
  return (
    isDefined(user) &&
...
export function isEventManager(
  ...
  return (
    isDefined(user) && user.role === "EVENT_MANAGER" && (user.loggedIn ?? true)
  );
}
...
export function isEventLeader(
  user?: { readonly role?: UserRole; readonly loggedIn?: boolean } | null,
): boolean {
  return (
    isDefined(user) && user.role === "EVENT_LEADER" && (user.loggedIn ?? true)
  );
}

export function isEventLeaderOrStaff(
  user?: { readonly role?: UserRole; readonly loggedIn?: boolean } | null,
): boolean {
...
export const roleRequirements: Record<
  UserRole,
  (
    u: { readonly role?: UserRole; readonly loggedIn?: boolean } | null,
  ) => boolean
> = {
  STUDENT: isStudent,
...
export function extractTeacherName(
  ...
  return (
    (teacher.givenName ? teacher.givenName.charAt(0) + ". " : "") +
    teacher.familyName
  );
}

export function schoolNameWithPostcode(
  schoolResult: School,
): string | undefined {
