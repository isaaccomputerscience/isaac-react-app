...
import {
  ...
  useTableCompatiblePortalsInHtml,
} from "./utils";
...
const Table = ({
  ...
  classes,
  rootElement,
}: TableData & { rootElement: HTMLElement }) => {
  ...
  const [modifiedHtml, renderPortalElements] =
    useTableCompatiblePortalsInHtml(tableHtml);
  ...
  if (modifiedHtml && parentElement) {
    return ReactDOM.createPortal(
      <div
        className={classNames(outerClasses, "isaac-table")}
        ref={updateExpandRef}
      >
...
export const useExpandContent = (
  ...
  const show =
    expandable && !isMobile() && above["md"](deviceSize) && !expandableParent;
    ...
    (show && (
      <div className={"expand-button position-relative"}>
        ...
        >
          <div>
            <span>
              <img aria-hidden src={"/assets/expand-arrow.svg"} alt="expand" />{" "}
  ...
  const outerClasses = show
    ? expanded
      ? "isaac-expand-bg expand-outer"
      : "expand-outer"
    : "";
...
export const useAccessibleTablesInHtml: PortalInHtmlHook = (html) => {
  ...
  for (let i = tableElements.length - 1; i >= 0; i--) {
    ...
    const tableCurrentClasses = (table.getAttribute("class") || "").split(
      /\s+/,
    );
    ...
    if (table.parentElement?.closest("table") === null) {
      ...
      tableInnerHTMLs.push({
        id: i,
        html: table.innerHTML,
        classes: tableCurrentClasses,
      });
