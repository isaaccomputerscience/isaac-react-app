...
export const BooleanNotationInput = ({
  booleanNotation,
  setBooleanNotation,
  isRequired = false,
}: BooleanNotationInputProps) => {
  return (
    <FormGroup>
      <Label
        className={`d-inline-block pr-2 ${isRequired ? "form-required" : ""}`}
        htmlFor="boolean-notation-preference"
      >
      ...
      <Input
        ...
        value={
          ...
          Object.keys(BOOLEAN_NOTATION).reduce(
            (val: string, key) =>
              booleanNotation?.[key as keyof BooleanNotation] ? key : val,
            BOOLEAN_NOTATION.MATH,
          )
        }
        onChange={(event: ChangeEvent<HTMLInputElement>) => {
          ...
          const newBooleanNotation: BooleanNotation = {
            ...EMPTY_BOOLEAN_NOTATION_RECORD,
          };
          newBooleanNotation[event.target.value as keyof BooleanNotation] =
            true;
          setBooleanNotation(newBooleanNotation);
        }}
        required={isRequired}
      >
        <option value={BOOLEAN_NOTATION.MATH}>
          And (&and;) Or (&or;) Not (&not;)
        </option>
        <option value={BOOLEAN_NOTATION.ENG}>
          And (&middot;) Or (+) Not (bar)
        </option>
      </Input>
    </FormGroup>
  );
