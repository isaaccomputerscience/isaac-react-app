...
import {
  IsaacNumericQuestionDTO,
  QuantityDTO,
  QuantityValidationResponseDTO,
} from "../../../IsaacApiTypes";
import {
  Button,
  Col,
...
>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<QuantityDTO>(questionId);

  const currentAttemptValue = currentAttempt?.value;
  const currentAttemptUnits = currentAttempt?.units;
  const currentAttemptValueWrong =
    validationResponse && validationResponse.correctValue === false;
  const currentAttemptUnitsWrong =
    validationResponse && validationResponse.correctUnits === false;

  const dispatch = useAppDispatch();
  const userId = useAppSelector(
    (state: AppState) => (state?.user?.loggedIn && state.user.id) || undefined,
  );
  const units = useAppSelector(
    (state: AppState) => state?.constants?.units || undefined,
  );

  useEffect(() => {
    dispatch(requestConstantsUnits());
  }, [dispatch]);
    ...
    <div className="numeric-question">
      ...
      <Row className="no-print">
        <Col className="d-flex flex-column flex-sm-row flex-md-column flex-lg-row">
            ...
            <Label className="w-100">
              ...
              <InputGroup className={"feedback-zone nq-feedback"}>
                <Input
                  type="text"
                  value={currentAttemptValue || ""}
                  invalid={currentAttemptValueWrong}
                  onChange={updateValue}
                  readOnly={readonly}
                />
                {currentAttemptValueWrong && (
                ...
                {!readonly && (
                  <InputGroupAddon addonType="append">
                    ...
                    >
                      Here are some examples of numbers you can write:
                      <br />
                      <br />
                      3.7
                      <br />
          ...
          {(doc.requireUnits || doc.displayUnit) && (
            <div className="unit-selection w-100 w-sm-50 w-md-100 w-lg-25">
              <Label className="w-100 ml-sm-2 ml-md-0 ml-lg-5">
                ...
                >
                  <DropdownToggle
                    ...
                    className={classNames("feedback-zone px-2 py-1", {
                      "border-dark display-unit": !noDisplayUnit,
                      "feedback-showing":
                        currentAttemptUnitsWrong && noDisplayUnit,
                    })}
                    color={noDisplayUnit ? undefined : "white"}
                  >
                    <Markup encoding={"latex"}>
                      {wrapUnitForSelect(
                        noDisplayUnit ? currentAttemptUnits : doc.displayUnit,
                      )}
                    </Markup>
                    {currentAttemptUnitsWrong && noDisplayUnit && (
                      <div className={"feedback-box"}>
                  ...
                  <DropdownMenu right>
                    {selectedUnits.map((unit) => (
                      <DropdownItem
                        key={wrapUnitForSelect(unit)}
                        data-unit={unit || "None"}
                        className={
                          unit === currentAttemptUnits
                            ? "btn bg-grey selected"
                            : ""
                        }
                        onClick={(e: FormEvent) => {
                          updateUnits(unit);
