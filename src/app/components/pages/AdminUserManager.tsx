...
export const AdminUserManager = () => {
  ...
  const userIdToSchoolMapping = useAppSelector(
    selectors.admin.userSchoolLookup,
  );
  const currentUser = useAppSelector(
    (state: AppState) => (state?.user?.loggedIn && state.user) || null,
  );
  let promotableRoles: UserRole[] = [
    "STUDENT",
    "TUTOR",
    "TEACHER",
    "EVENT_LEADER",
  ...
  const verificationStatuses: EmailVerificationStatus[] = [
    "NOT_VERIFIED",
    "DELIVERY_FAILED",
  ];
      ...
      dispatch(
        getUserIdSchoolLookup(
          searchResults
            ...
            .filter((result) => result != undefined) as number[],
        ),
      );
    }
  ...
  const updateQuery = (update: { [key: string]: string | null }) => {
    ...
    Object.entries(update).forEach(
      ([key, value]) => (nulledUpdate[key] = value || null),
    );
  ...
  const selectAllToggle = () => {
    if (
      ...
      searchResults.length === selectedUserIds.length
    ) {
      setSelectedUserIds([]);
  ...
  const confirmUnverifiedUserPromotions = function () {
    if (isDefined(searchResults)) {
      const unverifiedSelectedUsers = selectedUserIds
        .map(
          (selectedId) =>
            searchResults.filter((result) => result.id === selectedId)[0],
        )
  ...
  const modifyUserEmailVerificationStatusesAndUpdateResults = async (
    ...
    const selectedEmails =
      searchResults
        ...
        .map((user) => user.email || "") || [];
    await dispatch(
      adminModifyUserEmailVerificationStatuses(status, selectedEmails),
    );
    dispatch(adminUserSearchRequest(searchQuery));
    setSelectedUserIds([]);
    setUserUpdating(false);
    ...
    <RS.Container>
        ...
        <RS.Form name="register" onSubmit={search}>
          <RS.CardBody>
            <RS.Row>
              <RS.Col md={6}>
                <RS.FormGroup>
                  <RS.Label htmlFor="family-name-search">
                    Find a user by family name:
                  </RS.Label>
                  <RS.Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      updateQuery({ familyName: e.target.value })
                    }
                  />
                </RS.FormGroup>
                <RS.FormGroup>
                  <RS.Label htmlFor="email-search">
                  ...
                  <RS.Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      updateQuery({ email: e.target.value })
                    }
                  />
                </RS.FormGroup>
                <RS.FormGroup>
                  <RS.Label htmlFor="school-other-search">
                    Find by manually entered school:
                  </RS.Label>
                  <RS.Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      updateQuery({ schoolOther: e.target.value })
                    }
                  />
                </RS.FormGroup>
              </RS.Col>
              ...
              <RS.Col md={6}>
                ...
                <RS.FormGroup>
                  <RS.Label htmlFor="postcode-search">
                    Find users with school within a given distance of postcode:
                  </RS.Label>
                  <RS.Row>
                    <RS.Col md={7}>
                      <RS.Input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updateQuery({ postcode: e.target.value })
                        }
                      />
                    </RS.Col>
                ...
                <RS.FormGroup>
                  <RS.Label htmlFor="school-urn-search">
                    Find a user with school URN:
                  </RS.Label>
                  <RS.Input
                    ...
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      updateQuery({ schoolURN: e.target.value })
                    }
                  />
                </RS.FormGroup>
              </RS.Col>
      ...
      <RS.Card className="my-4">
        <RS.CardTitle tag="h4" className="pl-4 pt-3 mb-0">
          Manage users (
          {(isDefined(searchResults) && searchResults.length) || 0})<br />
          ...
          <RS.Row className="pb-4">
            <RS.Col>
              <RS.UncontrolledButtonDropdown>
                ...
                <RS.DropdownMenu>
                  <RS.DropdownItem header>
                    Promote or demote selected users to:
                  </RS.DropdownItem>
              ...
              {isDefined(currentUser) && currentUser.role === "ADMIN" && (
                <RS.UncontrolledButtonDropdown>
                  ...
                  <RS.DropdownMenu>
                    <RS.DropdownItem header>
                      Change email verification status for users to:
                    </RS.DropdownItem>
                    {verificationStatuses.map((status) => (
                      <RS.DropdownItem
                        ...
                        disabled={selectedUserIds.length === 0}
                        onClick={() =>
                          modifyUserEmailVerificationStatusesAndUpdateResults(
                            status,
                          )
                        }
                      >
