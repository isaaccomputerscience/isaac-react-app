...
import {
  ...
  passMark,
  ResultsTable,
} from "../elements/quiz/QuizProgressCommon";
...
export const ProgressDetails = ({
  ...
  const selectedQuestion: GameboardItem | undefined =
    assignment.gameboard.contents[selectedQuestionNumber];

  type SortOrder =
    | number
    | "name"
    | "totalQuestionPartPercentage"
    | "totalQuestionPercentage";
  ...
  const progressData = useMemo<[AppAssignmentProgress, boolean][]>(
    () =>
      assignment.progress.map((p) => {
        ...
        const ret = p.results.reduce<AppAssignmentProgress>(
          (oldP, results, i) => {
            const tickCount = ["PASSED", "PERFECT"].includes(results)
              ? oldP.tickCount + 1
              : oldP.tickCount;
            const questions = assignment.gameboard.contents;
            return {
              ...oldP,
              tickCount,
              correctQuestionPartsCount:
                oldP.correctQuestionPartsCount + p.correctPartResults[i],
              incorrectQuestionPartsCount:
                oldP.incorrectQuestionPartsCount + p.incorrectPartResults[i],
              notAttemptedPartResults: [
                ...oldP.notAttemptedPartResults,
                questions[i].questionPartsTotal -
                  p.correctPartResults[i] -
                  p.incorrectPartResults[i],
              ],
            };
          },
          initialState,
  ...
  const studentsCorrect = progressData.reduce(
    (sc, pd) => sc + (pd[1] ? 1 : 0),
    0,
  );

  // Calculate 'class average', which isn't an average at all, it's the percentage of ticks per question.
  const [assignmentAverages, assignmentTotalQuestionParts] = useMemo<
    [number[], number]
  >(() => {
    return (
      questions?.reduce(
        ([aAvg, aTQP], q, i) => {
          const tickCount = progress.reduce(
            (tc, p) =>
              ["PASSED", "PERFECT"].includes(p.results[i]) ? tc + 1 : tc,
            0,
          );
  ...
  const semiSortedProgress = useMemo(
    () =>
      orderBy(
        progress,
        (item) => {
          return (
            item.user.authorisedFullAccess &&
            item.notAttemptedPartResults.reduce(function (sum, increment) {
  ...
  function sortItem(props: ComponentProps<"th"> & { itemOrder: SortOrder }) {
    ...
    const clickToSelect =
      typeof itemOrder === "number"
        ? () => setSelectedQuestion(itemOrder)
        : undefined;
    const sortArrows =
      typeof itemOrder !== "number" || itemOrder === selectedQuestionNumber ? (
        <button
          className="sort"
          onClick={() => {
            toggleSort(itemOrder);
          }}
        >
  ...
  const tableHeaderFooter = (
    <tr className="progress-table-header-footer">
      {sortItem({ key: "name", itemOrder: "name" })}
      {questions.map((q, index) =>
        sortItem({
          key: q.id,
          itemOrder: index,
          className: isSelected(q),
          children: `${assignmentAverages[index]}%`,
        }),
      )}
      {sortItem({
        key: "totalQuestionPartPercentage",
        itemOrder: "totalQuestionPartPercentage",
        className: "total-column left",
        children: "Total Parts",
      })}
      {sortItem({
        key: "totalQuestionPercentage",
        itemOrder: "totalQuestionPercentage",
  ...
  function markClassesInternal(
    studentProgress: AppAssignmentProgress,
    status: GameboardItemState | null,
    correctParts: number,
    ...
    if (!studentProgress.user.authorisedFullAccess) {
      ...
      status === "FAILED" ||
      incorrectParts / totalParts > 1 - passMark
    ) {
      return "failed";
  ...
  useLayoutEffect(() => {
    ...
    if (table) {
      ...
      const firstRow = (table.firstChild as HTMLTableSectionElement)
        .firstChild as HTMLTableRowElement;
      const questionTH = firstRow.children[
      ...
      if (offsetLeft < parentLeft) {
        ...
        if (offsetRight > parentRight) {
          newScrollLeft =
            parentScrollLeft + offsetRight - parentRight + width / 2;
        }
      }
  ...
  return (
    <div className="assignment-progress-progress">
      <div className="progress-header">
        <strong>{studentsCorrect}</strong> of <strong>{progress.length}</strong>{" "}
        students have completed the gameboard{" "}
        ...
        <>
          <div className="progress-questions">
            <Button
              ...
              onClick={() => setSelectedQuestion(selectedQuestionNumber - 1)}
            >
              ◄
            </Button>
            <div>
              <Link
                to={`/questions/${selectedQuestion.id}?board=${assignment.gameboardId}`}
              >
                <strong>
                  Q<span className="d-none d-md-inline">uestion</span>:{" "}
                </strong>
            ...
            <Button
              color="tertiary"
              disabled={selectedQuestionNumber === questions.length - 1}
              onClick={() => setSelectedQuestion(selectedQuestionNumber + 1)}
            >
              ►
            </Button>
                ...
                {sortedProgress.map((studentProgress) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <th
                        className="total-column left"
                        title={fullAccess ? undefined : "Not Sharing"}
                      >
                        {fullAccess
                          ? formatMark(
...
const ProgressLoader = ({ assignment }: { assignment: EnhancedAssignment }) => {
  const assignmentProgressQuery =
    isaacApi.endpoints.getAssignmentProgress.useQuery(assignment.id);
  return (
    <ShowLoadingQuery
...
const AssignmentDetails = ({
  ...
  function openAssignmentDownloadLink(
  ...
  return (
    <div className="assignment-progress-gameboard" key={assignment.gameboardId}>
      <div
        className={classNames("gameboard-header", {
          "text-muted": assignmentHasNotStarted,
        })}
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <Button
          color="link"
          className="gameboard-title align-items-center"
          onClick={() => setIsExpanded(!isExpanded)}
        >
          <span
            className={classNames({ "text-muted": assignmentHasNotStarted })}
          >
            {assignment.gameboard?.title}
            {assignmentHasNotStarted && (
              <span className="gameboard-due-date">
                (Scheduled:&nbsp;
                {formatDate(getAssignmentStartDate(assignment))})
              </span>
            )}
        ...
        <div className="gameboard-links align-items-center">
          <Button color="link" tag="a" className="mr-md-0">
            ...
            <span className="d-none d-lg-inline">mark sheet</span>
          </Button>
          ...
          <Button
            ...
            color="link"
            tag="a"
            href={getAssignmentCSVDownloadLink(assignment.id as number)}
            onClick={openAssignmentDownloadLink}
          >
            Download CSV
          </Button>
...
export const AssignmentProgressLegend = ({
  showQuestionKey,
}: {
  ...
  return (
    <div className="p-4">
      <div className="assignment-progress-legend">
        {showQuestionKey && (
          <>
        ...
        {showQuestionKey && (
          <Label htmlFor="key" className="mt-2">
            Section key:
          </Label>
        )}
        <ul id="key" className="block-grid-xs-5">
          ...
          <li className="d-flex flex-wrap px-2">
            ...
            <div className="key-description">
              <span className="d-none d-md-inline">Not attempted</span>
              <span className="d-inline d-md-none">No attempt</span>
            </div>
          </li>
          <li className="d-flex flex-wrap px-2">
            <div className="key-cell">
        ...
        <div className="assignment-progress-options">
          <label>
            ...
            <input
              type="checkbox"
              checked={pageSettings.colourBlind}
              onChange={(e) => pageSettings.setColourBlind(e.target.checked)}
            />
          </label>
          <label>
            Percent view&nbsp;
            <input
              type="checkbox"
              checked={pageSettings.formatAsPercentage}
              onChange={(e) =>
                pageSettings.setFormatAsPercentage(e.target.checked)
              }
            />
          </label>
        </div>
...
const QuizProgressLoader = ({
  quizAssignment,
}: {
  quizAssignment: QuizAssignmentDTO;
}) => {
  const dispatch = useAppDispatch();
  const quizAssignmentId = isDefined(quizAssignment.id)
    ? quizAssignment.id
    : null;
  ...
  useEffect(() => {
    if (isFound(quizAssignments)) {
      userFeedback = quizAssignments.find((qa) => qa.id === quizAssignmentId)
        ?.userFeedback;
    }
  }, [quizAssignments]);
  ...
  return isDefined(quizAssignment.id) && quizAssignment.id > 0 ? (
    <div className="assignment-progress-gameboard" key={quizAssignment.id}>
      ...
      >
        ...
        <div className="gameboard-links align-items-center">
          <Button color="link" className="mr-md-0">
            {isExpanded ? "Hide " : "View "}{" "}
            <span className="d-none d-lg-inline">mark sheet</span>
          </Button>
          <span className="d-none d-md-inline">,</span>
          <Button
            ...
            color="link"
            tag="a"
            href={getQuizAssignmentCSVDownloadLink(quizAssignment.id)}
            onClick={openAssignmentDownloadLink}
          >
            Download CSV
          </Button>
          ...
          <Button
            ...
            color="link"
            tag="a"
            href={`/test/assignment/${quizAssignment.id}/feedback`}
            onClick={openSingleAssignment}
          >
            View individual assignment
          </Button>
...
const GroupDetails = ({ group }: { group: AppGroup }) => {
  ...
  const { groupBoardAssignments, groupQuizAssignments } = useGroupAssignments(
    group.id,
  );
  const assignments = groupBoardAssignments ?? [];
  const quizAssignments = groupQuizAssignments ?? [];

  const assignmentTabComponents =
    assignments.length > 0 ? (
      assignments.map((assignment) => (
        <AssignmentDetails
          key={assignment.gameboardId}
          assignment={assignment}
        />
      ))
    ) : (
      <div className="p-4 text-center">
        There are no assignments for this group.
      </div>
    );
  const quizTabComponents =
    quizAssignments.length > 0 ? (
      quizAssignments.map((quizAssignment) => (
        <QuizDetails key={quizAssignment.id} quizAssignment={quizAssignment} />
      ))
    ) : (
      <div className="p-4 text-center">
        There are no tests assigned to this group.
      </div>
    );
  ...
  return (
    ...
    >
      <AssignmentProgressLegend
        showQuestionKey={activeTab === MARKBOOK_TYPE_TAB.tests}
      />
...
function getGroupProgressCSVDownloadLink(groupId: number) {
  return (
    API_PATH + "/assignments/assign/group/" + groupId + "/progress/download"
  );
}
  ...
  return (
    <>
      <div
        ...
        className={
          isExpanded
            ? "assignment-progress-group active align-items-center"
            : "assignment-progress-group align-items-center"
        }
      >
        ...
        <div className="py-2">
          <strong>{assignmentCount}</strong> Assignment
          {assignmentCount != 1 && "s"}
          <span className="d-none d-md-inline"> set</span>
        </div>
            ...
            <UncontrolledButtonDropdown size="sm">
              ...
              <DropdownMenu>
                {Object.values(SortOrder).map((item) => (
                  <DropdownItem key={item} onClick={() => setSortOrder(item)}>
                    {item}
                  </DropdownItem>
                ))}
              </DropdownMenu>
            ...
            <div className="assignment-progress-container mb-5">
              ...
              >
                {sortedGroups.map((group) => (
                  <GroupAssignmentProgress key={group.id} group={group} />
                ))}
              </AssignmentProgressPageSettingsContext.Provider>
              {sortedGroups.length === 0 && (
                <Container className="py-5">
                  <h3 className="text-center">
                    You&apos;ll need to create a group using{" "}
                    <Link to="/groups">Manage groups</Link> to set an
                    assignment.
                  </h3>
                </Container>
              )}
