...
export const handlers = [
  rest.get(API_PATH + "/gameboards/user_gameboards", (req, res, ctx) => {
    ...
    const limit =
      (limitStr && parseInt(limitStr)) || mockGameboards.totalResults;

    const limitedGameboards = produce(mockGameboards, (g) => {
      if (startIndex === 0 && limitStr === "ALL") return g;
      g.results = g.results.slice(
        startIndex,
        Math.min(startIndex + limit, mockGameboards.totalResults),
      );
      g.totalNotStarted = g.results.length;
      g.totalResults = g.results.length;
    });

    return res(ctx.status(200), ctx.json(limitedGameboards));
  }),
  ...
  rest.get(API_PATH + "/assignments", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockMyAssignments));
  }),
  rest.get(API_PATH + "/quiz/assignments", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockQuizAssignments));
  }),
  ...
  rest.get(API_PATH + "/assignments/assign", (req, res, ctx) => {
    ...
    return res(ctx.status(200), ctx.json(setAssignments));
  }),
  ...
  rest.get(API_PATH + "/users/user_preferences", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockUserPreferences));
  }),
    ...
    return res(
      ...
      ctx.json({
        ...
        weeklyStreakRecord: {
          currentStreak: 0,
          currentActivity: 0,
          largestWeeklyStreak: 0,
        },
      }),
    );
  }),
  rest.get(API_PATH + "/auth/user_authentication_settings", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockUserAuthSettings));
  }),
  rest.get(API_PATH + "/info/segue_environment", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json({ segueEnvironment: "DEV" }));
  }),
  ...
  rest.get(API_PATH + "/pages/pods/:tag", (req, res, ctx) => {
    ...
    const mockPods = tag === "promo" ? mockPromoPods : mockNewsPods;
    ...
    return res(ctx.status(200), ctx.json(podsFilteredByTag));
  }),
  rest.get(API_PATH + "/pages/fragments/:fragmentId", (req, res, ctx) => {
    const { fragmentId } = req.params;
    return res(ctx.status(200), ctx.json(mockFragment(fragmentId as string)));
  }),
  rest.get(API_PATH + "/pages/:pageId", (req, res, ctx) => {
    const { pageId } = req.params;
    return res(ctx.status(200), ctx.json(mockPage(pageId as string)));
  }),
  rest.get(API_PATH + "/quiz/assigned", (req, res, ctx) => {
    return res(ctx.status(200), ctx.json(mockAssignedQuiz));
  }),
...
export const buildNewGroupHandler = (newGroup: any) =>
  jest.fn((req, res, ctx) => {
    return res(ctx.status(200), ctx.json(newGroup));
  });
export const buildAuthTokenHandler = (newGroup: any, token: string) =>
  jest.fn((req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
...
export const buildNewManagerHandler = (
  groupToAddManagerTo: any,
  newManager: any,
) =>
  jest.fn((req, res, ctx) => {
    return res(
...
export const buildGroupHandler = (groups?: any[]) =>
  jest.fn((req, res, ctx) => {
    const archived =
      req.url.searchParams.get("archived_groups_only") === "true";
    const filteredGroups = (groups ?? mockGroups).filter(
      (g) => g.archived === archived,
    );
    return res(ctx.status(200), ctx.json(filteredGroups));
  });
export const buildGroupMembershipsHandler = (
  members?: UserSummaryWithGroupMembershipDTO[],
) =>
  jest.fn((req, res, ctx) => {
