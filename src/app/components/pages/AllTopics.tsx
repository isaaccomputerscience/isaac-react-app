...
export function AllTopicsWithoutAStage() {
  ...
  const mostRecentAllTopicsPath = persistence.load(
    KEY.MOST_RECENT_ALL_TOPICS_PATH,
  );
  ...
  if (mostRecentAllTopicsPath) {
    ...
    queryParams &&
    queryParams.stage &&
    !STAGE_NULL_OPTIONS.has(queryParams.stage as STAGE)
  ) {
...
export const AllTopics = ({ stage }: { stage: STAGE.A_LEVEL | STAGE.GCSE }) => {
  ...
  const stageExamBoards = Array.from(
    {
      [STAGE.GCSE]: EXAM_BOARDS_CS_GCSE,
      [STAGE.A_LEVEL]: EXAM_BOARDS_CS_A_LEVEL,
    }[stage],
  );

  useEffect(
    function recordMostRecentAllTopicsStage() {
      ...
      return (
        <React.Fragment>
          ...
          {topic.comingSoonDate && !topic.new && (
            <Badge color="light" className="border bg-white">
              Coming {topic.comingSoonDate}
            </Badge>
          )}
          {topic.new && !topic.comingSoonDate && (
  ...
  const charToCutAt = "H";
  const firstColTags = subcategoryTags.filter(function (subcategory) {
    return subcategory.title.charAt(0) <= charToCutAt;
  });
  const secondColTags = subcategoryTags.filter(function (subcategory) {
    return subcategory.title.charAt(0) > charToCutAt;
  });

  const topicColumn = (subTags: Tag[]) => {
    return (
      <Col key={TAG_ID.computerScience + "_" + subTags[0].id} md={6}>
        {subTags
          .sort((a, b) => (a.title > b.title ? 1 : -1))
          // Overwrite subcategory with stage properties
          .map((subcategory) => ({
            ...subcategory,
            ...subcategory.stageOverride?.[stage],
          }))
          .map((subcategory) => {
            const subcategoryDescendentIds = tags
              .getDescendents(subcategory.id)
              .map((t) => t.id);
  ...
  return (
    <div className="pattern-02">
      <Container>
        <TitleAndBreadcrumb
          currentPageTitle={
            stage === STAGE.A_LEVEL ? "A level topics" : "GCSE topics"
          }
        />
              ...
              All: (
                <>
                  <Row>
                    <Col
                      lg={{ size: 8, offset: 2 }}
                      className="bg-light-grey pt-md-4"
                    >
                      <PageFragment
                        fragmentId={`${stage}_all_topics`}
                  ...
                  <Row>
                    <Col
                      lg={{ size: 8, offset: 2 }}
                      className="bg-light-grey py-md-4 d-md-flex"
                    >
            ...
            ...stageExamBoards.map((examBoard) => ({
              [examBoard.toUpperCase()]: (
                <Row>
                  <Col
                    lg={{ size: 8, offset: 2 }}
                    className="bg-light-grey py-md-4"
                  >
                    <PageFragment
                      fragmentId={`${stage}_specification_${examBoard}`}
                    />
                  </Col>
                </Row>
              ),
