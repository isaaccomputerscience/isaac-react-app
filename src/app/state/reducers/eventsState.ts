...
export const events = (events: EventsState = null, action: Action) => {
  ...
  switch (action.type) {
    case ACTION_TYPE.EVENTS_RESPONSE_SUCCESS:
      return {
        events: Array.from(
          new Set([...currentEvents, ...action.augmentedEvents]),
        ),
        total: action.total,
      };
...
export const eventBookings = (
  eventBookings: EventBookingsState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const eventBookingsForGroup = (
  eventBookingsForGroup: EventBookingsState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const eventBookingsForAllGroups = (
  eventBookingsForAllGroups: EventBookingsState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const eventOverviews = (
  eventOverviews: EventOverviewsState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const eventMapData = (
  eventMapData: EventMapDataState = null,
  action: Action,
) => {
  switch (action.type) {
