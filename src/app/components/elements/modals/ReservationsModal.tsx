...
const ReservationsModal = () => {
  ...
  const user = useAppSelector((state: AppState) =>
    isLoggedIn(state?.user) ? (state?.user as RegisteredUserDTO) : undefined,
  );
  ...
  const sortedActiveGroups = useMemo<AppGroup[]>(
    () => sortBy(activeGroups ?? [], (g) => g.groupName),
    [activeGroups],
  );
  ...
  const selectedGroup =
    sortedActiveGroups &&
    sortedActiveGroups.find((g) => g.id === selectedGroupId);
  ...
  const eventBookingsForGroup = useAppSelector(
    (state: AppState) => (state && state.eventBookingsForGroup) || [],
  );
  const eventBookingsForAllGroups = useAppSelector(
    (state: AppState) => (state && state.eventBookingsForAllGroups) || [],
  );
  ...
  const [cancelReservationCheckboxes, setCancelReservationCheckboxes] =
    useState<{ [key: number]: boolean }>({});
  const [
    checkAllCancelReservationsCheckbox,
    setCheckAllCancelReservationsCheckbox,
  ] = useState<boolean>();
  const [groupDropdownOpen, setGroupDropdownOpen] = useState(false);
  const [unbookedUsersById, setUnbookedUsersById] = useState<{
    [id: number]: AppGroupMembership;
  }>({});

  useEffect(() => {
    setUnbookedUsersById(
      unbookedUsers.reduce(
        (acc: { [id: number]: AppGroupMembership }, u) => ({
          ...acc,
          [u.id as number]: u,
        }),
        {},
  ...
  const [modifiedBookingsForAllGroups, setModifiedBookingsForAllGroups] =
    useState(eventBookingsForAllGroups);
  ...
  useEffect(() => {
    ...
    const newUnbookedUsers = _orderBy(
      modifiedBookingsForAllGroups
        .filter(
          (booking) =>
            !bookedUserIds.includes(booking.userBooked?.id as number),
        )
        // do not allow the reservation of teachers on a student only event
        .filter(
          (booking) =>
            !(
              selectedEvent?.isStudentOnly &&
              booking.userBooked?.role !== "STUDENT"
            ),
        ),
    ...
    for (const user of newUnbookedUsers) {
      if (!user.userBooked?.id || !user.userBooked?.authorisedFullAccess)
        continue;
    ...
    if (selectedGroup && selectedGroup.members) {
      ...
      const newUnbookedUsers = _orderBy(
        selectedGroup.members
          ...
          .filter(
            (member) =>
              !(selectedEvent?.isStudentOnly && member.role !== "STUDENT"),
          ),
        ["authorisedFullAccess", "familyName", "givenName"],
        ["desc", "asc", "asc"],
      );
  ...
  const requestReservations = () => {
    if (
      ...
      const reservableIds = Object.entries(userCheckboxes)
        .filter((c) => c[1])
        .map((c) => parseInt(c[0]));
      dispatch(
        reserveUsersOnEvent(selectedEvent.id, reservableIds, selectedGroup.id),
      );
    }
    setCheckAllCheckbox(false);
  };
  ...
  const cancelReservations = () => {
    if (selectedEvent && selectedEvent.id) {
      // do we need this group id
      const cancellableIds = Object.entries(cancelReservationCheckboxes)
        .filter((c) => c[1])
        .map((c) => parseInt(c[0]));
      selectedGroup?.id
        ? dispatch(
    ...
    if (selectedEvent && selectedEvent.groupReservationLimit) {
      ...
      const candidateBookings = Object.entries(userCheckboxes)
        ...
        .filter(
          ([id]) =>
            !selectedEvent.isAStudentEvent ||
            unbookedUsersById[id as any]?.role === "STUDENT",
        );

      return (
        candidateBookings.length + bookings.length >
        selectedEvent.groupReservationLimit
      );
    }
  ...
  return (
    <React.Fragment>
      <div id="reservation-modal">
        {!selectedEvent?.allowGroupReservations && (
          <p>This event does not allow group reservations.</p>
        )}
        {selectedEvent?.allowGroupReservations && (
          <Col>
            <Row className="mb-5">
              <Col md={3}>
                <ShowLoading until={sortedActiveGroups}>
                  <React.Fragment>
                    {sortedActiveGroups && sortedActiveGroups.length > 0 && (
                      <Dropdown
              ...
              {sortedActiveGroups && sortedActiveGroups.length === 0 && (
                <p>
                  ...
                  page to book your students onto an event
                </p>
              )}
              <Col
                cols={12}
              ...
              >
                {sortedActiveGroups &&
                  ...
                  (!selectedGroup || !selectedGroup.members) && (
                    <p>
                      Select one of your groups from the dropdown menu to see
                      its members.
                    </p>
                  )}
                {selectedGroup &&
                  ...
                  selectedGroup.members.length == 0 && (
                    <p>
                      This group has no members. Please select another group.
                    </p>
                  )}
                <React.Fragment>
                  <Table bordered responsive className="bg-white reserved">
                      ...
                      <tr>
                        <th className="align-middle checkbox">
                          <CustomInput
                            ...
                            label="All"
                            checked={
                              checkAllCancelReservationsCheckbox || false
                            }
                            onChange={() =>
                              toggleAllCancelReservationCheckboxes()
                            }
                          />
                        </th>
                        <th className="align-middle student-name">Student</th>
                    ...
                    <tbody>
                          ...
                          booking.userBooked.id && (
                            <tr
                              ...
                              className={classNames({
                                "bg-light text-muted": bookingCancelled,
                              })}
                            >
                              <td className="align-middle text-center">
                                <CustomInput
                                  ...
                                  disabled={
                                    bookingCancelled ||
                                    (!booking.userBooked.authorisedFullAccess &&
                                      booking.userBooked
                                        .emailVerificationStatus !== "VERIFIED")
                                  }
                                  onChange={() =>
                                    toggleCancelReservationCheckboxForUser(
                                      booking.userBooked?.id,
                                    )
                                  }
                                />
                              ...
                              <td className="align-middle">
                                {booking.userBooked.givenName +
                                  " " +
                                  booking.userBooked.familyName}
                                {booking.userBooked.emailVerificationStatus !==
                                  "VERIFIED" && (
                      ...
                      {eventBookingsForAllGroups.length == 0 && (
                        <tr>
                          <td colSpan={4}>
                            None of the members of this group are booked in for
                            this event.
                          </td>
                        </tr>
                      )}
                  ...
                  <div className="text-center mb-3">
                    <Button
                      ...
                      outline
                      disabled={
                        !Object.values(cancelReservationCheckboxes).some(
                          (v) => v,
                        )
                      }
                      onClick={cancelReservations}
                    >
                      Cancel reservations
                    </Button>
                  </div>
                  ...
                  selectedGroup.members.length > 0 && (
                    <React.Fragment>
                      <Table
                        bordered
                        responsive
                        className="mt-3 bg-white unreserved"
                      >
                        <thead>
                          <tr>
                          ...
                          <tr>
                            <th className="w-auto text-nowrap align-middle checkbox">
                              <CustomInput
                                ...
                                disabled={
                                  unbookedUsers.filter(
                                    (user) => user.authorisedFullAccess,
                                  ).length === 0
                                }
                              />
                            </th>
                                  ...
                                  <tr key={user.id}>
                                    <td className="w-auto align-middle text-center">
                                      <CustomInput
                                        ...
                                        checked={
                                          userCheckboxes[user.id] || false
                                        }
                                        disabled={
                                          !user.authorisedFullAccess ||
                                          user.emailVerificationStatus !==
                                            "VERIFIED"
                                        }
                                        onChange={() =>
                                          toggleCheckboxForUser(user.id)
                                        }
                                      />
                                    </td>
                      ...
                      <Row className="toolbar">
                        <Col>
                          {isReservationLimitReached() && (
                            <p className="text-danger">
                              You can only reserve a maximum of{" "}
                              {selectedEvent &&
                                selectedEvent.groupReservationLimit}{" "}
                              group members onto this event.
                            </p>
                          )}
                          <div className="text-center">
                            <Button
                              color="primary"
                              disabled={
                                !Object.values(userCheckboxes).some((v) => v) ||
                                isReservationLimitReached()
                              }
                              onClick={requestReservations}
                            >
                              Reserve places
                            </Button>
                          </div>
