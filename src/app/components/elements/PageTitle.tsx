...
function AudienceViewer({
  ...
  const viewsWithMyStage = audienceViews.filter(
    (vc) => vc.stage === userContext.stage,
  );
  // If there is a possible audience view that is correct for our user context, show that specific one
  const viewsToUse =
    viewsWithMyStage.length > 0 ? viewsWithMyStage.slice(0, 1) : audienceViews;
  const filteredViews = filterAudienceViewsByProperties(
    viewsToUse,
    AUDIENCE_DISPLAY_FIELDS,
  );
  ...
  return (
    <div className="h-subtitle pt-sm-0 mb-sm-0 d-sm-flex">
      {filteredViews.map((view, i) => (
        <div
          key={`${view.stage} ${view.difficulty} ${view.examBoard}`}
          className={"d-flex d-sm-block"}
        >
...
export const PageTitle = ({
  ...
  const openModal = useAppSelector((state: AppState) =>
    Boolean(state?.activeModals?.length),
  );
  ...
  return (
    ...
    >
      <div className="mr-auto" data-testid={"main-heading"}>
        ...
        {subTitle && (
          <span className="h-subtitle d-none d-sm-block">{subTitle}</span>
        )}
      </div>
      <Helmet>
...
export const formatPageTitle = (
  ...
  <Markup encoding={disallowLaTeX ? "plaintext" : "latex"}>
    {currentPageTitle}
  </Markup>
);
