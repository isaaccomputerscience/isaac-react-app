...
export const QuestionProgressCharts = (props: QuestionProgressChartsProps) => {
  ...
  const defaultSearchChoiceTag = tags.getSpecifiedTags(
    searchTagLevel,
    tags.allTagIds,
  )[0];
    ...
    charts.push(
      bb.generate({
        data: {
          ...
          colors: colourPicker(
            topicColumns.map((column) => column[0]) as string[],
          ),
          type: "donut",
        },
        donut: {
      ...
      <RS.Col xl={6} md={4} className="mt-4 d-flex flex-column">
        <div className="height-40px text-flex-align mb-2">
          <Select
            ...
            defaultValue={{
              value: defaultSearchChoiceTag.id,
              label: defaultSearchChoiceTag.title,
            }}
            options={tags
              .getSpecifiedTags(searchTagLevel, tags.allTagIds)
              .map((tag) => {
        ...
        <div className="d-flex flex-grow-1">
          <div
            id={`${subId}-topicChart`}
            className="text-center-width doughnut-binding  align-self-center"
          >
