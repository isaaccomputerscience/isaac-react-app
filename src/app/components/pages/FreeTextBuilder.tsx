...
function choiceHash(choice: FreeTextRule) {
  ...
  const definingProperties =
    ...
    value +
    correct +
    stringHash(JSON.stringify(explanation)) +
    caseInsensitive +
    allowsAnyOrder +
    allowsExtraWords +
    allowsMisspelling;
...
function testCaseHash(testCaseInput: TestCaseDTO) {
  return (
    ((testCaseInput.answer && testCaseInput.answer.value) || "") +
    testCaseInput.expected
  );
}
...
function convertQuestionChoicesToJson(
  questionChoices: (FreeTextRule & { choiceNumber: number })[],
) {
...
function convertJsonToQuestionChoices(jsonString: string) {
  ...
  if (Array.isArray(parsedJson)) {
    ...
    parsedJson.hasOwnProperty("type") &&
    parsedJson.type == "isaacFreeTextQuestion"
  ) {
    choicesArray = parsedJson.choices;
  } else {
    throw TypeError("Neither a Choices array nor a FreeTextQuestion");
  }
  return choicesArray.map((choice: FreeTextRule, i: number) =>
    Object.assign(choice, { choiceNumber: i }),
  );
}
...
function generateDefaultTestCase() {
  ...
  return {
    ...
    expected: true,
    answer: { type: "stringChoice", value: "" },
  };
}
const defaultTestCaseExample = generateDefaultTestCase();
function removeTestCaseNumber(
  testCase: TestCaseDTO & { testCaseNumber: number },
) {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
function convertTestCasesToCsv(testCases: TestCaseDTO[]) {
  return testCases
    .map((tc) => `${tc.expected},${tc.answer?.value?.split("\n").join("\\n")}`)
    .join("\n");
}
function convertCsvToTestCases(
  testCasesCsv: string,
): (TestCaseDTO & { testCaseNumber: number })[] {
  return testCasesCsv.split("\n").map((testCaseString, index) => {
    const [expected, ...value] = testCaseString.split(",");
    return {
      testCaseNumber: index,
      expected: expected === "true",
      answer: { type: "stringChoice", value: value.join(",") },
    };
  });
}
...
function isEditableExplanation(explanation?: any) {
  return (
    ...
    explanation.children[0].value !== undefined
  );
}

export const FreeTextBuilder = () => {
  const dispatch = useAppDispatch();
  const testCaseResponses = useAppSelector(
    (state: AppState) => (state && state.testQuestions) || [],
  );

  const [questionChoices, setQuestionChoices] = useState<
    (FreeTextRule & { choiceNumber: number })[]
  >([JSON.parse(JSON.stringify(defaultChoiceExample))]);
  const [questionChoicesJson, setQuestionChoicesJson] = useState(
    convertQuestionChoicesToJson(questionChoices),
  );
  const [jsonParseError, setJsonParseError] = useState(false);
  useEffect(() => {
    setQuestionChoicesJson(convertQuestionChoicesToJson(questionChoices));
  }, [questionChoices]);

  const [testCases, setTestCases] = useState<
    (TestCaseDTO & { testCaseNumber: number })[]
  >([JSON.parse(JSON.stringify(defaultTestCaseExample))]);
  const [testCasesCsv, setTestCasesCsv] = useState(
    convertTestCasesToCsv(testCases),
  );
  const [csvParseError, setCsvParseError] = useState(false);
  useEffect(() => {
    setTestCasesCsv(convertTestCasesToCsv(testCases));
  }, [testCases]);

  const [choicesHashAtPreviousRequest, setChoicesHashAtPreviousRequest] =
        ...
        Object.assign(response, {
          match:
            response.expected !== undefined
              ? response.expected === response.correct
              : undefined,
        }),
      )
      .forEach(
        (testCaseResponse) =>
          (testCaseResponseMap[testCaseHash(testCaseResponse)] =
            testCaseResponse),
      );
  }
  const numberOfResponseMatches = Object.values(testCaseResponseMap).filter(
    (testCase) => testCase.match,
  ).length;
  const cleanQuestionChoices = questionChoices
    .map(removeChoiceNumber)
    .filter(notEqualToDefaultChoice);
  const cleanTestCases = testCases
    .map(removeTestCaseNumber)
    .filter(notEqualToDefaultTestCase);
  const atLeastOneQuestionChoiceAndTestCase =
    atLeastOne(cleanQuestionChoices.length) &&
    atLeastOne(cleanTestCases.length);

  return (
    <RS.Container>
      <TitleAndBreadcrumb
        className="mb-4"
        currentPageTitle="Free-text question builder"
      />
      <RS.Form
            ...
            <Tabs className="d-flex flex-column-reverse" tabTitleClass="px-3">
              {{
                      ...
                      {questionChoices.map((choice) => (
                        <tr key={choice.choiceNumber}>
                          <td>
                            <RS.Label className="mb-3 w-100">
                              ...
                              <div className="d-flex align-items-center">
                                ...
                                />
                                <span
                                  id={`choice-help-${choice.choiceNumber}`}
                                  className="icon-help mr-2"
                                />
                              </div>
                            </RS.Label>
                                ...
                                <RS.Table>
                                  ...
                                  <tr>
                                    ...
                                    <td className="text-light">
                                      Separate an OR list of word choices.
                                    </td>
                                  </tr>
                                  <tr>
                                    <td>
                            ...
                            <RS.Row>
                              <RS.Col xs={3} className="text-center">
                                ...
                                >
                                  <RS.Label>
                                    Ignore case{" "}
                                    {checkMark(choice.caseInsensitive)}
                                  </RS.Label>
                                </RS.Button>
                              </RS.Col>
                              <RS.Col xs={3} className="text-center">
                                    ...
                                    setQuestionChoices(
                                      questionChoices.map((c) =>
                                        choice == c
                                          ? {
                                              ...c,
                                              allowsAnyOrder: !c.allowsAnyOrder,
                                            }
                                          : c,
                                      ),
                                    )
                                ...
                                >
                                  <RS.Label>
                                    Any order {checkMark(choice.allowsAnyOrder)}
                                  </RS.Label>
                                </RS.Button>
                              </RS.Col>
                              <RS.Col xs={3} className="text-center">
                                ...
                                >
                                  <RS.Label>
                                    Extra words{" "}
                                    {checkMark(choice.allowsExtraWords)}
                                  </RS.Label>
                                </RS.Button>
                              </RS.Col>
                              <RS.Col xs={3} className="text-center">
                                ...
                                >
                                  <RS.Label>
                                    Misspelling{" "}
                                    {checkMark(choice.allowsMisspelling)}
                                  </RS.Label>
                                </RS.Button>
                              </RS.Col>
                            </RS.Row>
                          ...
                          <td className="align-middle">
                            ...
                            >
                              <div className="h4 px-4">
                                {checkMark(choice.correct)}
                              </div>
                            </RS.Button>
                          </td>
                          <td>
                            <RS.Label>
                              ...
                              {isEditableExplanation(choice.explanation) ? (
                                <RS.Input
                                  type="textarea"
                                  value={
                                    (choice.explanation as any).children[0]
                                      .value
                                  }
                                  onChange={(event) => {
                                    const explanation =
                                      choice.explanation as any;
                                    explanation.children[0].value =
                                      event.target.value;
                                    setQuestionChoices(
                                      questionChoices.map((c) =>
                                        choice == c ? { ...c, explanation } : c,
                                      ),
                                    );
                                  }}
                                />
                                ...
                                <IsaacContent
                                  doc={choice.explanation as ContentBase}
                                />
                              )}
                            </RS.Label>
                          </td>
                              ...
                              onClick={() =>
                                setQuestionChoices(
                                  questionChoices.filter(
                                    (choiceInState) => choice !== choiceInState,
                                  ),
                                )
                              }
                            >
                        ...
                        <td colSpan={4} className="text-center pb-3">
                          <RS.Button
                            ...
                            onClick={() =>
                              setQuestionChoices([
                                ...questionChoices,
                                generateDefaultChoice(),
                              ])
                            }
                          >
                            <img
                              src="/assets/add_circle_outline.svg"
                              alt="Add matching rule"
                            />
                          </RS.Button>
                        </td>
                      </tr>
                ...
                JSON: (
                  <div className="mb-3">
                    <p>
                      JSON for the <strong>choices</strong> part of your
                    ...
                    <div className="text-center">
                      <RS.Button
                        ...
                        onClick={() => {
                          try {
                            setQuestionChoices(
                              convertJsonToQuestionChoices(questionChoicesJson),
                            );
                          } catch (e) {
            ...
            <Tabs className="d-flex flex-column-reverse" tabTitleClass="px-3">
              {{
                    ...
                    <tbody>
                      {testCases.map((testCase) => {
                        const testCaseResponse =
                          testCaseResponseMap[testCaseHash(testCase)];
                        const matchFailure =
                          !!testCaseResponse && testCaseResponse.match == false;
                        return (
                          <tr key={testCase.testCaseNumber}>
                            ...
                            <td
                              className={`w-10 text-center align-middle ${
                                matchFailure ? "alert-danger" : "bg-light"
                              }`}
                            >
                              {testCaseResponse &&
                                checkMark(testCaseResponse.correct)}
                            </td>
                            <td
                            ...
                            >
                              {testCaseResponse?.explanation && (
                                <IsaacContent
                                  doc={testCaseResponse.explanation}
                                />
                              )}
                            </td>
                            <td
                            ...
                            >
                              {testCaseResponse &&
                                checkMark(testCaseResponse.match)}
                            </td>
                            <td
                              className={`${
                                matchFailure ? "alert-danger" : "bg-light"
                ...
                CSV: (
                  <div className="mb-3">
                    <p>
                      Enter test cases as CSV with the headers:
