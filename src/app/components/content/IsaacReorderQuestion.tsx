...
const ReorderDraggableItem = ({
  item,
  index,
  inAvailableItems,
  readonly,
}: {
  item: Immutable<ItemDTO>;
  index: number;
  ...
  return (
    ...
    >
      {(provided) => {
        return (
          <div
            id={`${item.id || index}|reorder-item-${
              inAvailableItems ? "available" : "choice"
            }`}
...
const IsaacReorderQuestion = ({
  doc,
  questionId,
  readonly,
}: IsaacQuestionProps<IsaacReorderQuestionDTO>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<ItemChoiceDTO>(questionId);

  const [availableItems, setAvailableItems] = useState<Immutable<ItemDTO>[]>([
    ...(doc.items ?? []),
  ]);

  const moveItem = (
    src: Immutable<ItemDTO>[] | undefined,
    fromIndex: number,
    dst: Immutable<ItemDTO>[] | undefined,
    toIndex: number,
  ) => {
    if (!src || !dst) return;
  ...
  const onCurrentAttemptUpdate = (
    newCurrentAttempt?: Immutable<ItemChoiceDTO>,
    newAvailableItems?: Immutable<ItemDTO>[],
  ) => {
          ...
          <Col md={{ size: 6 }} className="parsons-available-items">
            ...
            <Droppable droppableId="availableItems">
              {(provided, snapshot) => (
                <div
                  ref={provided.innerRef}
                  className={classNames("parsons-items", {
                    empty: !(availableItems && availableItems.length > 0),
