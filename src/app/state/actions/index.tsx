...
export const logInUser =
  (provider: AuthenticationProvider, credentials: CredentialsAuthDTO) => async (dispatch: Dispatch<Action>) => {
...
export const handlePasswordReset =
  (params: { token: string; password: string }) => async (dispatch: Dispatch<Action>) => {
    try {
      dispatch({ type: ACTION_TYPE.USER_PASSWORD_RESET_REQUEST });
...
export const handleProviderCallback =
  (provider: AuthenticationProvider, parameters: string) => async (dispatch: Dispatch<Action>) => {
...
export const handleEmailAlter =
  (params: { userid: string | null; token: string | null }) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const submitMessage =
  (params: { firstName: string; lastName: string; emailAddress: string; subject: string; message: string }) =>
  async (dispatch: Dispatch<Action>) => {
...
export const authenticateWithTokenAfterPrompt =
  (userId: number, userSubmittedAuthenticationToken: string | null) => async (dispatch: Dispatch<Action>) => {
...
export const revokeAuthorisationAfterPrompt =
  (userId: number, otherUser: UserSummaryWithEmailAddressDTO) => async (dispatch: Dispatch<Action>) => {
    dispatch(openActiveModal(revocationConfirmationModal(userId, otherUser)) as any);
  };
export const revokeAuthorisation =
  (userId: number, userToRevoke: UserSummaryWithEmailAddressDTO) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const releaseAuthorisationAfterPrompt =
  (userId: number, student: UserSummaryDTO) => async (dispatch: Dispatch<Action>) => {
...
export const registerQuestions =
  (questions: QuestionDTO[], accordionClientId?: string, isQuiz?: boolean) => (dispatch: Dispatch<Action>) => {
...
export const getMyAnsweredQuestionsByDate =
  (userId: number | string, fromDate: number, toDate: number, perDay: boolean) =>
  async (dispatch: Dispatch<Action>) => {
...
export const testQuestion =
  (questionChoices: FreeTextRule[], testCases: TestCaseDTO[]) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const adminUserGetRequest =
  (userid: number | undefined) => async (dispatch: Dispatch<Action | ((d: Dispatch<Action>) => void)>) => {
...
export const adminModifyUserRoles =
  (role: UserRole, userIds: number[]) => async (dispatch: Dispatch<Action | ((d: Dispatch<Action>) => void)>) => {
...
export const sendAdminEmail =
  (contentid: string, emailType: string, roles: EmailUserRoles) => async (dispatch: Dispatch<Action>) => {
    ...
    try {
      ...
      dispatch(
        showToast({ color: "success", title: "Email sent", body: "Email sent successfully", timeout: 3000 }) as any,
      );
    } catch (e) {
...
export const sendAdminEmailWithIds =
  (contentid: string, emailType: string, ids: number[]) => async (dispatch: Dispatch<Action>) => {
...
export const sendProvidedEmailWithUserIds =
  (emailTemplate: EmailTemplateDTO, emailType: string, ids: number[]) => async (dispatch: Dispatch<Action>) => {
...
export const bookMyselfOnEvent =
  (eventId: string, additionalInformation: AdditionalInformation) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const reserveUsersOnEvent =
  (eventId: string, userIds: number[], groupId: number) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const cancelReservationsOnEvent =
  (eventId: string, userIds: number[], groupId: number | undefined) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const addMyselfToWaitingList =
  (eventId: string, additionalInformation: AdditionalInformation, waitingListOnly?: boolean) =>
  async (dispatch: Dispatch<Action>) => {
    try {
...
export const bookUserOnEvent =
  (eventBookingId: string, userId: number, additionalInformation: AdditionalInformation) =>
  async (dispatch: Dispatch<Action>) => {
    try {
...
export const resendUserConfirmationEmail =
  (eventBookingId: string, userId?: number) => async (dispatch: Dispatch<Action>) => {
