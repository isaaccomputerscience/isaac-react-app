...
function inSection(section: IsaacQuizSectionDTO, questions: QuestionDTO[]) {
  return questions.filter(
    (q) => q.id?.startsWith((section.id as string) + "|"),
  );
}
...
function QuizContents({
  ...
  if (isDefined(attempt.completedDate)) {
    return attempt.feedbackMode === "NONE" ? (
      ...
      <h4>
        Your mark is {attempt.quiz?.individualFeedback?.overallMark?.correct} /{" "}
        {attempt.quiz?.total}
      </h4>
    ) : (
      <table className="quiz-marks-table">
        <tbody>
          <tr>
            <th>Overall mark</th>
            <td>
              {attempt.quiz?.individualFeedback?.overallMark?.correct} /{" "}
              {attempt.quiz?.total}
            </td>
          </tr>
          <tr>
            ...
            return (
              <tr key={k}>
                {attempt.feedbackMode === "DETAILED_FEEDBACK" ? (
                  <td>
                    <Link replace to={pageLink(index + 1)}>
                      {section.title}
                    </Link>
                  </td>
                ) : (
                ...
                <td>
                  {
                    attempt.quiz?.individualFeedback?.sectionMarks?.[
      ...
      <div>
        ...
        <ul>
          {Object.keys(sections).map((k, index) => {
            ...
            const answerCount = questionsInSection.filter(
              (q) => q.bestAttempt !== undefined,
            ).length;
              ...
              <li key={k}>
                ...
                <small className="text-muted">
                  {completed
                    ? "Completed"
                    : anyStarted
                    ? `${answerCount} / ${questionsInSection.length}`
                    : ""}
                </small>
              </li>
            );
...
function QuizHeader({ attempt, preview, user }: QuizAttemptProps) {
  ...
  if (preview) {
      ...
      <>
        ...
        <div className="d-flex">
          ...
          {isTeacherOrAbove(user) && (
            <Button
              onClick={() =>
                dispatch(showQuizSettingModal(attempt.quiz as IsaacQuizDTO))
              }
            >
              Set Test
            </Button>
    ...
    return (
      <p className="d-flex">
        <span>
          ...
          {isDefined(attempt.completedDate) && (
            <>
              <br />
        ...
        {isDefined(assignment.dueDate) && (
          <>
            <Spacer />
            {isDefined(attempt.completedDate) ? "Was due:" : "Due:"}&nbsp;
    ...
    return (
      <p>
        You{" "}
        {attempt.completedDate ? "freely attempted" : "are freely attempting"}{" "}
        this test.
      </p>
    );
  }
  ...
  const openQuestionModal = (attempt: QuizAttemptDTO) => {
    dispatch(
      openActiveModal({
        ...
        body: <QuizRubric attempt={attempt} />,
      }),
    );
  };
      ...
      <Col md={{ size: 8, offset: 2 }} className="py-4 question-panel">
          ...
          {rubric && renderRubric && (
            <Col className="text-right">
              <Button
                ...
                title="Show instructions in a modal"
                onClick={() => {
                  rubric && openQuestionModal(attempt);
                }}
              >
                Show instructions
              </Button>
        ...
        {attribution && (
          <p className="text-muted">
            <Markup trusted-markup-encoding={"markdown"}>{attribution}</Markup>
          </p>
        )}
      </Col>
    </Row>
...
const QuizTitle = ({
  ...
  const crumbs =
    preview && isTeacherOrAbove(user) ? teacherQuizzesCrumbs : myQuizzesCrumbs;
  if (page === null) {
    return (
      <TitleAndBreadcrumb
    ...
    return (
      <TitleAndBreadcrumb
        ...
        intermediateCrumbs={[
          ...crumbs,
          { title: quizTitle, replace: true, to: pageLink() },
        ]}
      />
    );
  }
...
export function QuizPagination({
  ...
  pageLink,
  finalLabel,
}: QuizAttemptProps & QuizPaginationProps) {
...
export function QuizAttemptComponent(props: QuizAttemptProps) {
  ...
  const questionNumbers = Object.assign(
    {},
    ...questions.map((q, i) => ({ [q.id as string]: i + 1 })),
  );
  return (
    <QuizAttemptContext.Provider
      value={{ quizAttempt: props.attempt, questionNumbers }}
    >
      <QuizTitle {...props} />
      {page === null ? (
        <div className="mt-4">
