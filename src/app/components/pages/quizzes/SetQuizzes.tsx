...
function formatAssignmentOwner(
  user: RegisteredUserDTO,
...
function QuizAssignment({ user, assignment }: QuizAssignmentProps) {
  ...
  const isCancelling =
    "cancelling" in assignment &&
    (assignment as { cancelling: boolean }).cancelling;
  return (
    <div className="p-2">
      <RS.Card className="card-neat">
        <RS.CardBody>
          <h4 className="border-bottom pb-3 mb-3">
            {assignment.quizSummary?.title || assignment.quizId}
          </h4>
          ...
          <p>
            Set on:{" "}
            <strong>
              {formatDate(assignment.creationDate)} by{" "}
              {formatAssignmentOwner(user, assignment)}
            </strong>
          </p>
...
const SetQuizzesPageComponent = ({ user, location }: SetQuizzesPageProps) => {
  ...
  useEffect(() => {
    ...
    const tab: MANAGE_QUIZ_TAB =
      (hashAnchor && MANAGE_QUIZ_TAB[hashAnchor as any]) || MANAGE_QUIZ_TAB.set;
    setActiveTab(tab);
  }, [hashAnchor]);
  ...
  const { titleFilter, setTitleFilter, filteredQuizzes } =
    useFilteredQuizzes(user);

  const pageHelp = (
    <span>
      Use this page to manage and set tests to your groups. You can assign any
      test the Isaac team have built.
      <br />
    ...
    <>
      {isEventLeaderOrStaff(user) &&
        ...
        quiz.hiddenFromRoles?.includes("TEACHER") && (
          <div className="small text-muted d-none d-md-block ml-2">
            hidden from teachers
          </div>
        )}
      {((quiz.hiddenFromRoles && !quiz.hiddenFromRoles?.includes("STUDENT")) ||
        quiz.visibleToStudents) && (
        <div className="small text-muted d-none d-md-block ml-2">
          visible to students
        </div>
      )}
              ...
              {filteredQuizzes && (
                <>
                  ...
                  {filteredQuizzes.length === 0 && (
                    <p>
                      <em>
                        There are no tests you can set which match your search
                        term.
                      </em>
                    </p>
                  )}
            ...
            <ShowLoading
              ...
              ifNotFound={
                <RS.Alert color="warning">
                  Tests you have assigned have failed to load, please try
                  refreshing the page.
                </RS.Alert>
              }
            >
              {quizAssignments && quizAssignments !== NOT_FOUND && (
                <>
                  {quizAssignments.length === 0 && (
                    <p>You have not set any tests to your groups yet.</p>
                  )}
                  {quizAssignments.length > 0 && (
