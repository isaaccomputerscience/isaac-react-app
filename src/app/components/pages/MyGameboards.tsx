  ...
  return boardView == BoardViews.table ? (
    ...
    <Card className="board-card card-neat" data-testid={"my-gameboard-card"}>
      <CardBody className="pb-4 pt-4">
        <button className="close" onClick={confirmCardDeleteBoard} aria-label="Delete gameboard">
          ×
        </button>
...
export const MyGameboards = () => {
  ...
  const {
    ...
    boardTitleFilter,
    setBoardTitleFilter,
  } = useGameboards(isMobile() ? BoardViews.card : BoardViews.table, isMobile() ? BoardLimit.six : BoardLimit.All);

  function confirmDeleteMultipleBoards() {
    if (
      confirm(
        `Are you sure you want to remove ${
          selectedBoards && selectedBoards.length > 1 ? selectedBoards.length + " boards" : selectedBoards[0].title
        } from your account?`,
      )
    ) {
      selectedBoards &&
        ...
        <>
          <div className="mt-4 mb-2">
            {boards && boards.totalResults && boards.totalResults > 0 && (
              <h4>
                You have completed <strong>{completed}</strong> of <strong>{boards.totalResults}</strong> gameboard
                {boards.totalResults > 1 && "s"}, with <strong>{inProgress}</strong> on the go and{" "}
              ...
              <Row>
                <Col sm={6} lg={3} xl={2}>
                  <Label className="w-100">
                    Display in{" "}
                    <Input type="select" value={boardView} onChange={switchViewAndClearSelected}>
                ...
                <>
                  <Row>
                    <Col sm={6} lg={3} xl={2}>
                      <Label className="w-100">
                        Display in{" "}
                        <Input type="select" value={boardView} onChange={switchViewAndClearSelected} className="p-2">
                          {Object.values(BoardViews).map((view) => (
                            <option key={view} value={view}>
                              {view}
                            </option>
                        ...
                        <Col sm={6} lg={2}>
                          <Label className="w-100">
                            ...
                            <Input
                              ...
                              onChange={(e) => setBoardCompletion(e.target.value as BoardCompletions)}
                            >
                              {Object.values(BoardCompletions).map((completion) => (
