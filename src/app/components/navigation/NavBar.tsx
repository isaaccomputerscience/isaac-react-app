...
export const LinkItem = ({
  ...
  DropdownItemProps & { muted?: boolean; badgeTitle?: string }
>) => (
  <DropdownItem
    tag={Link}
    className={classNames("pl-4 py-3 p-md-3", { "text-muted": muted })}
    {...props}
  >
    {children}
    {badgeTitle && (
      <Badge
        color="light"
        className="border-secondary border bg-white ml-2 mr-1"
      >
        {badgeTitle}
      </Badge>
    )}
...
export const LinkItemComingSoon = ({
  children,
}: {
  ...
  <LinkItem to="/coming_soon" aria-disabled="true">
    ...
    <Badge
      color="light"
      className="border-secondary border bg-white ml-auto mr-1"
    >
      Coming soon
    </Badge>
  </LinkItem>
  ...
  <MenuOpenContext.Consumer>
    {({ setMenuOpen }) => (
      <UncontrolledDropdown nav inNavbar>
        ...
        <DropdownMenu
          className="p-3 pt-0 m-0 mx-lg-4"
          onClick={() => setMenuOpen(false)}
        >
          {children}
        </DropdownMenu>
...
export function MenuBadge({
  ...
  message,
  ...rest
}: { count: number; message: string } & HTMLProps<HTMLDivElement>) {
  if (count == 0) {
...
export function useAssignmentsCount() {
  ...
  const { data: assignments } = isaacApi.endpoints.getMyAssignments.useQuery(
    user?.loggedIn ? undefined : skipToken,
    ...
    <MenuOpenContext.Provider value={{ menuOpen, setMenuOpen }}>
      <Navbar className="main-nav p-0" color="light" light expand="md">
        ...
        <Collapse
          isOpen={menuOpen}
          navbar
          className="px-0 mx-0 mx-xl-5 px-xl-5"
        >
