  ...
  if (questionId && queryParams.board && currentGameboard && currentGameboard.id === queryParams.board) {
    ...
    if (gameboardItem) {
      ...
      if (gameboardDeterminedViews.length > 0) {
        ...
        if (
          !gameboardDeterminedViews.map((v) => v.examBoard).includes(examBoard) &&
...
const _EXAM_BOARD_ITEM_OPTIONS = [
  /* best not to export - use getFiltered */ { label: "AQA", value: EXAM_BOARD.AQA },
...
const _STAGE_ITEM_OPTIONS = [
  /* best not to export - use getFiltered */ { label: "GCSE", value: STAGE.GCSE },
          ...
          !(
            ...
            (
              filter.byUserContexts.some(
                (uc) => uc.stage === i.value && EXAM_BOARD_NULL_OPTIONS.has(uc.examBoard as EXAM_BOARD),
              ) ||
...
export function determineAudienceViews(
  ...
  return viewsToDisplay.sort((a, b) => comparatorFromOrderedValues(stagesOrdered)(a.stage, b.stage));
}
  ...
  return (
    audience?.filter((audienceRecord) => {
      return Object.entries(partialViewingContext).every(
        ([key, value]) => audienceRecord[key as keyof AudienceContext]?.[0] === value,
      );
    }) ?? []
  );
}
