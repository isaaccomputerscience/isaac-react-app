...
const EventMisuseTable = ({
  topMisuses,
}: {
  topMisuses: MisuseStatisticDTO[];
}) => {
  const [resetMisuseMonitor] =
    isaacApi.endpoints.resetMisuseMonitor.useMutation();
  return topMisuses.length === 0 ? (
    <span className={"font-italic"}>
      No misuse statistics for this event type! 🎉
    </span>
  ) : (
    <Table>
      <thead>
          ...
          <tr>
            ...
            <td>
              ...
              {m.currentCounter > 0 && (
                <Button
                  ...
                  onClick={() =>
                    resetMisuseMonitor({
                      eventLabel: m.eventType,
                      agentIdentifier: m.agentIdentifier,
                    })
                  }
                >
                  Reset
            ...
            <td>
              {m.lastEventTimestamp
                ? new Date(m.lastEventTimestamp).toString()
                : "None"}
            </td>
            <td>
              {m.isMisused ? (
...
export const MisuseStats = () => {
  const { data: misuseStats } =
    isaacApi.endpoints.getMisuseStatistics.useQuery(10);
  ...
  return (
    <>
      ...
      <Select
        options={eventLabels.map((el) => ({
          value: el,
          label: el.replace("Handler", ""),
        }))}
        onChange={selectOnChange(setOpenEventTable, true)}
      />
      <br />
      {misuseStats &&
        openEventTable.length > 0 &&
        openEventTable[0] in misuseStats && (
          <EventMisuseTable topMisuses={misuseStats[openEventTable[0]]} />
        )}
    </>
  );
};
