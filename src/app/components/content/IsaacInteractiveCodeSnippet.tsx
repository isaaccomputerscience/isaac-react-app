...
interface IsaacInteractiveCodeProps {
  doc: InteractiveCodeSnippetDTO;
}

export const IsaacInteractiveCodeSnippet = ({
  doc,
}: IsaacInteractiveCodeProps) => {
  const dispatch = useAppDispatch();
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const uid = useRef((doc?.id || "") + uuid_v4().slice(0, 8));
  const { receivedData, sendMessage } = useIFrameMessages(
    uid.current,
    iframeRef,
  );
    ...
    switch (receivedData.type) {
      ...
      case "setupFail":
        ...
        if (segueEnvironment !== "PROD") {
          console.error(
            `IsaacInteractiveCodeSnippet ${
              receivedData.type === "setupFail" ? "setup code" : "checker"
            } error: ${receivedData.message}`,
          );
        } else {
          dispatch(
            logAction({
              ...
              questionId: doc.id,
              error: `IsaacInteractiveCodeSnippet ${
                receivedData.type === "setupFail" ? "setup code" : "checker"
              } error: ${receivedData.message}`,
            }),
          );
      ...
      case "checker":
        if (
          doc?.expectedResult &&
          doc?.testCode &&
          receivedData.result !== "CHECKER_RESULT_UNDEFINED"
        ) {
      ...
      default:
        if (segueEnvironment !== "PROD") {
          console.error(
            "IsaacInteractiveCodeSnippet received a malformed message from the editor iframe!",
          );
        }
    }
  ...
  return (
    <iframe
      title={"Code Sandbox"}
      src={CODE_EDITOR_BASE_URL + "/#" + uid.current}
      ref={iframeRef}
      onLoad={sendQuestion}
      className={"isaac-code-iframe w-100 mb-1"}
      style={{
        resize: "none",
        height: iFrameHeight,
        border: "none",
