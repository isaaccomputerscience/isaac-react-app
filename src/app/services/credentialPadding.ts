...
function paddingLengthRequired(bodyLength: number) {
  ...
  return (
    Math.pow(2, Math.max(Math.ceil(Math.log(bodyLength) / Math.log(2)), 8)) -
    bodyLength
  );
}
...
export function securePadCredentials(credentials: CredentialsAuthDTO) {
  let paddedCredentials: PaddedCredentialsAuthDTO = Object.assign(
    { _randomPadding: "" },
    credentials,
  );
...
export function securePadPasswordReset(passwordResetParams: {
  password: string;
}) {
  ...
  let paddingLength = paddingLengthRequired(requestBodyByteLength);
  return Object.assign(passwordResetParams, {
    _randomPadding: generatePadding(paddingLength),
  });
}
