...
export function useUserContext(): UseUserContextReturnType {
  ...
  const transientUserContext =
    useAppSelector((state: AppState) => state?.transientUserContext) || {};
  ...
  if (programmingLanguage) {
    preferredProgrammingLanguage = Object.values(PROGRAMMING_LANGUAGE).find(
      (key) => programmingLanguage[key] === true,
    );
  }
  ...
  if (
    stageQueryParam &&
    Object.values(STAGE).includes(stageQueryParam) &&
    !STAGE_NULL_OPTIONS.has(stageQueryParam)
  ) {
    ...
    isLoggedIn(user) &&
    user.registeredContexts?.length &&
    user.registeredContexts[0].stage
  ) {
  ...
  if (
    examBoardQueryParam &&
    Object.values(EXAM_BOARD).includes(examBoardQueryParam) &&
    !EXAM_BOARD_NULL_OPTIONS.has(examBoardQueryParam)
  ) {
    ...
    isLoggedIn(user) &&
    user.registeredContexts?.length &&
    user.registeredContexts[0].examBoard
  ) {
  ...
  if (booleanNotation) {
    preferredBooleanNotation = Object.values(BOOLEAN_NOTATION).find(
      (key) => booleanNotation[key] === true,
    );
  }
  ...
  if (
    ...
    queryParams.board &&
    currentGameboard &&
    currentGameboard.id === queryParams.board
  ) {
    const gameboardItem = currentGameboard.contents?.filter(
      (c) => c.id === questionId,
    )[0];
    if (gameboardItem) {
      const gameboardDeterminedViews = determineAudienceViews(
        gameboardItem.audience,
        gameboardItem.creationContext,
      );
      // If user's stage selection is not one specified by the gameboard change it
      if (gameboardDeterminedViews.length > 0) {
        if (
          !gameboardDeterminedViews.map((v) => v.stage).includes(stage) &&
          !STAGE_NULL_OPTIONS.has(stage)
        ) {
        ...
        if (
          !gameboardDeterminedViews
            .map((v) => v.examBoard)
            .includes(examBoard) &&
          !EXAM_BOARD_NULL_OPTIONS.has(examBoard)
        ) {
...
const _EXAM_BOARD_ITEM_OPTIONS = [
  /* best not to export - use getFiltered */
    ...
    _EXAM_BOARD_ITEM_OPTIONS
      ...
      .filter(
        (i) =>
          ...
          (filter?.byStages.includes(STAGE.GCSE) &&
            EXAM_BOARDS_CS_GCSE.has(i.value)) || // if there is gcse in stages allow GCSE boards
          (filter?.byStages.includes(STAGE.A_LEVEL) &&
            EXAM_BOARDS_CS_A_LEVEL.has(i.value)), // if there is a_level in stage allow A Level boards
      )
      // includeNullOptions flag
      .filter(
        (i) =>
          filter?.includeNullOptions || !EXAM_BOARD_NULL_OPTIONS.has(i.value),
      )
        ...
        (i) =>
          ...
          filter?.byUser.registeredContexts
            ?.filter(
              (rc) =>
                !filter.byStages ||
                filter.byStages.length === 0 ||
                filter.byStages.includes(rc.stage as STAGE) ||
                STAGE_NULL_OPTIONS.has(rc.stage as STAGE),
            )
            .some((rc) =>
              EXAM_BOARD_NULL_OPTIONS.has(rc.examBoard as EXAM_BOARD),
            ) ||
      ...
      .filter(
        (i) =>
          ...
          !filter?.byUserContexts
            .filter(
              (uc) =>
                !filter.byStages || filter.byStages.includes(uc.stage as STAGE),
            )
            .map((uc) => uc.examBoard)
            .includes(i.value),
      )
...
export function getFilteredStageOptions(filter?: StageFilterOptions) {
  return (
    _STAGE_ITEM_OPTIONS
      ...
      .filter(
        (i) => filter?.includeNullOptions || !STAGE_NULL_OPTIONS.has(i.value),
      )
      ...
      .filter(
        (i) =>
          ...
          !isLoggedIn(filter?.byUser) ||
          // user has a null option selected
          filter?.byUser.registeredContexts?.some((rc) =>
            STAGE_NULL_OPTIONS.has(rc.stage as STAGE),
          ) ||
          // stage is one of registered context selections
          filter?.byUser.registeredContexts
      ...
      .filter(
        (i) =>
          ...
          !(
            ...
            (
              filter.byUserContexts.some(
                (uc) =>
                  uc.stage === i.value &&
                  EXAM_BOARD_NULL_OPTIONS.has(uc.examBoard as EXAM_BOARD),
              ) ||
              // every exam board has been recorded for the stage
              getFilteredExamBoardOptions({
                byUser: filter.byUser,
                byStages: [i.value],
                byUserContexts: filter.byUserContexts,
              }).length === 0
            )
          ),
      )
...
function produceAudienceViewingCombinations(
...
export function determineAudienceViews(
  audience?: AudienceContext[],
  ...
  if (creationContext) {
    viewsFilteredByCreationContext = allViews.filter((viewingContext) => {
      ...
      if (creationContext.examBoard && viewingContext.examBoard) {
        viableView =
          viableView &&
          creationContext.examBoard.includes(viewingContext.examBoard);
      }
      if (creationContext.difficulty && viewingContext.difficulty) {
        viableView =
          viableView &&
          creationContext.difficulty.includes(viewingContext.difficulty);
      }
      return viableView;
    });
  }
  ...
  const viewsToDisplay =
    viewsFilteredByCreationContext.length > 0
      ? viewsFilteredByCreationContext
      : allViews;
...
export function filterAudienceViewsByProperties(
  views: ViewingContext[],
  properties: (keyof ViewingContext)[],
): ViewingContext[] {
...
export function findAudienceRecordsMatchingPartial(
  audience: ContentBaseDTO["audience"],
  partialViewingContext: Partial<ViewingContext>,
) {
  return (
    audience?.filter((audienceRecord) => {
      return Object.entries(partialViewingContext).every(
        ([key, value]) =>
...
export function isIntendedAudience(
  intendedAudience: ContentBaseDTO["audience"],
  ...
  return intendedAudience.some((audienceClause) => {
    ...
    if (audienceClause.stage) {
      ...
      const satisfiesStageCriteria =
        userStage === STAGE.ALL || audienceClause.stage.includes(userStage);
    ...
    if (audienceClause.examBoard) {
      ...
      const satisfiesExamBoardCriteria =
        userExamBoard === EXAM_BOARD.ALL ||
        audienceClause.examBoard.includes(userExamBoard);
    ...
    if (audienceClause.role) {
      const satisfiesRoleCriteria = audienceClause.role.some(
        (role) =>
...
export function mergeDisplayOptions(
  source: ContentBaseDTO["display"],
  update: ContentBaseDTO["display"],
): ContentBaseDTO["display"] {
...
export function notRelevantMessage(
  userContext: UseUserContextReturnType,
): string {
  const message = [];
...
export function stringifyAudience(
  audience: ContentDTO["audience"],
  userContext: UseUserContextReturnType,
): string {
  ...
  if (!audience) {
    ...
    audience.forEach(
      (audienceRecord) =>
        audienceRecord.stage?.forEach((stage) => stagesSet.add(stage)),
    );
  }
  // order stages
  const audienceStages = Array.from(stagesSet).sort(
    comparatorFromOrderedValues(stagesOrdered),
  );
  // if you are one of the options - only show that option
  const stagesFilteredByUserContext = audienceStages.filter(
    (s) => userContext.stage === s,
  );
  let stagesToView =
  ...
  if (
    stagesToView.includes(STAGE.GCSE) &&
    stagesToView.includes(STAGE.A_LEVEL)
  ) {
...
export function makeIntendedAudienceComparator(
  user: Immutable<PotentialUser> | null,
  userContext: UseUserContextReturnType,
) {
  // Make "relevant" sections appear first
  return function intendedAudienceComparator(
    sectionA: { audience?: ContentBaseDTO["audience"] },
    sectionB: { audience?: ContentBaseDTO["audience"] },
  ) {
    const isAudienceA = isIntendedAudience(
      sectionA.audience,
      userContext,
      user,
    );
    const isAudienceB = isIntendedAudience(
      sectionB.audience,
      userContext,
      user,
    );
