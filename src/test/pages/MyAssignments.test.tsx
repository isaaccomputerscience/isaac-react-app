...
describe("MyAssignments", () => {
  ...
  it("should allow users to filter assignments on gameboard title", async () => {
    ...
    expect(await screen.findAllByTestId("my-assignment")).toHaveLength(
      mockMyAssignments.length,
    );
  });
  ...
  it('should open "Older" tab when tab is clicked, and tab should contain no assignments', async () => {
    ...
    await waitFor(
      ...
      {
        onTimeout: augmentErrorMessage(
          "Tabs did not change to the correct state fast enough",
        ),
      },
    );
  ...
  it('should contain assignments with undefined due date and older than a month in the "Older Assignments" tab', async () => {
    renderMyAssignments([
      rest.get(API_PATH + "/assignments", (req, res, ctx) => {
        ...
        const assignmentsWithOneOld = produce<any[]>(
          mockMyAssignments,
          (as) => {
        ...
        return res(ctx.status(200), ctx.json(assignmentsWithOneOld));
      }),
    ]);
  ...
  it('should show the scheduled start date as the "Assigned" date if it exists', async () => {
    ...
    const assignedDateEl =
      within(myAssignment).getByTestId("gameboard-assigned");
    const assignedDate = assignedDateEl?.textContent?.replace(
      /Assigned:\s?/,
      "",
    );
    expect(assignedDate).toMatch(DDMMYYYY_REGEX);
    expect(dayMonthYearStringToDate(assignedDate)?.valueOf()).toEqual(
      DAYS_AGO(-1, true),
    );
  });
