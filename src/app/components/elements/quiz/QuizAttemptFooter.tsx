...
export function QuizAttemptFooter(props: QuizAttemptProps) {
  ...
  if (page === null) {
    ...
    const completedSections = Object.keys(sections).reduce(
      (map, sectionId) => {
        map[sectionId] = true;
        return map;
      },
    ...
    questions.forEach((question) => {
      const sectionId = extractSectionIdFromQuizQuestionId(
    ...
    if (allCompleted) {
      controls = (
        <>
          <Button color="tertiary" tag={Link} replace to={pageLink(1)}>
            Review answers
          </Button>
          <Spacer />
          All sections complete
          <Spacer />
          <Button color="primary" onClick={submitQuiz}>
            {submitButton}
          </Button>
        </>
      );
      ...
      prequel = (
        <p>
          Click &lsquo;{primaryButton}&rsquo; when you are ready to{" "}
          {primaryDescription} the test.
        </p>
      );
      if (anyAnswered) {
        controls = (
          <>
            <div className="text-center">
              {totalCompleted} / {sectionCount} sections complete
              <br />
              <Button
                onClick={() =>
                  window.confirm(
                    "Are you sure? You haven't answered all of the questions",
                  ) && submitQuiz()
                }
              >
                {submitButton}
        ...
        controls = (
          <>
            <Spacer />
            <Button color="primary" tag={Link} replace to={pageLink(1)}>
              {primaryButton}
            </Button>
          </>
        );
