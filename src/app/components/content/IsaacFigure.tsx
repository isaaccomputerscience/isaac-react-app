...
export const IsaacFigure = ({ doc }: IsaacFigureProps) => {
  ...
  return (
    <div className="figure_panel">
      <FigureNumberingContext.Consumer>
        {(figureNumbers) => {
          const figureString =
            figId && Object.keys(figureNumbers).includes(figId)
              ? `Figure\u00A0${figureNumbers[figId]}`
              : "Figure";
          return (
            <figure>
              ...
              <div className="text-center figure-caption">
                {doc.children && doc.children.length > 0 && figId && (
                  <div>
                    <strong className="text-secondary figure-reference">
                      {figureString}
                    </strong>
                  </div>
                )}
                <IsaacContentValueOrChildren
                  ...
                  value={
                    doc.value &&
                    figId &&
                    `<strong class="text-secondary figure-reference">${figureString}:</strong> ${doc.value}`
                  }
                >
                  {doc.children}
                </IsaacContentValueOrChildren>
