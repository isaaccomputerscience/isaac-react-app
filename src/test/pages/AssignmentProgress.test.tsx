...
describe("AssignmentProgress", () => {
  ...
  it('shows both "Assignments" and "Tests" tabs to teachers', async () => {
    ...
    const mockTestAssignments = mockQuizAssignments.filter(
      (q) => q.groupId === mockGroup.id,
    );
    renderTestEnvironment({
      role: "TEACHER",
      extraEndpoints: [
    ...
    const assignmentsTab = await screen.findByRole("button", {
      name: `Assignments (${mockAssignments.length})`,
    });
    const testsTab = await screen.findByRole("button", {
      name: `Tests (${mockTestAssignments.length})`,
    });
    await userEvent.click(assignmentsTab);
    for (const assignmentTitle of mockAssignments.map(
      (a) => a.gameboard?.title,
    )) {
      await screen.findByText(assignmentTitle, { exact: false });
    }
    await userEvent.click(testsTab);
    for (const testTitle of mockTestAssignments.map(
      (q) => q.quizSummary?.title,
    )) {
  ...
  it("only shows assignments to tutors, with no tabs shown", async () => {
    ...
    const assignmentsTab = screen.queryByRole("button", {
      name: `Assignments (${mockAssignments.length})`,
    });
    const testsTab = screen.queryByRole("button", {
      name: `Tests (${mockTestAssignments.length})`,
    });
