...
function QuizFooter(props: QuizAttemptProps) {
  ...
  if (page === null) {
    prequel = (
      <p className="mt-3">
        Click on a section title or click &lsquo;Next&rsquo; to look at{" "}
        {isDefined(studentUser) ? "their" : "your"} detailed feedback.
      </p>
    );
    controls = (
      <>
        <Spacer />
        <Button tag={Link} replace to={pageLink(1)}>
          Next
        </Button>
      </>
    );
    ...
    controls = (
      <QuizPagination {...props} page={page} finalLabel="Back to Overview" />
    );
  }
...
const pageHelp = <span>See the feedback for this test attempt.</span>;

export const QuizAttemptFeedback = ({ user }: { user: RegisteredUserDTO }) => {
  const { quizAttemptId, page, studentId, quizAssignmentId } = useParams<{
  ...
  const { attempt, studentUser, questions, sections, error } =
    useCurrentQuizAttempt(numericStudentId);

  const dispatch = useAppDispatch();

  useEffect(() => {
    isDefined(quizAttemptId) &&
      dispatch(loadQuizAttemptFeedback(parseInt(quizAttemptId, 10)));

    if (isDefined(numericStudentId) && isDefined(quizAssignmentId)) {
      dispatch(
        loadStudentQuizAttemptFeedback(
          parseInt(quizAssignmentId, 10),
          numericStudentId,
        ),
      );
    }
  ...
  const pageLink = useCallback(
    (page?: number) => {
      ...
      if (isDefined(studentId) && isDefined(quizAssignmentId)) {
        return (
          `/test/attempt/feedback/${quizAssignmentId}/${studentId}` + pagePath
        );
      } else {
  ...
  const subProps: QuizAttemptProps = {
  ...
  return (
    <Container className={`mb-5 ${attempt?.quiz?.subjectId}`}>
      <ShowLoading until={attempt || error}>
        ...
        {isDefined(error) && (
          <>
            <TitleAndBreadcrumb
              currentPageTitle="Test Feedback"
