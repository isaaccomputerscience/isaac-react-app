...
describe("SetAssignments", () => {
  ...
  it("should start in card view, with 6 gameboards shown", async () => {
    ...
    const viewDropdown: HTMLInputElement =
      await screen.findByLabelText("Display in");
    expect(viewDropdown.value).toEqual("Card View");
    expect(screen.queryAllByTestId("assignment-gameboard-card")).toHaveLength(
      6,
    );
  });
  ...
  it("should show all gameboards in table view", async () => {
    ...
    const gameboardRows = await screen.findAllByTestId(
      "assignment-gameboard-table-row",
    );
  ...
  it("should show all the correct information for a gameboard in card view", async () => {
    renderSetAssignments();
    const gameboards = await screen.findAllByTestId(
      "assignment-gameboard-card",
    );
    ...
    const assignmentsToGameboard = mockSetAssignments.filter(
      (a) => a.gameboardId === mockGameboard.id,
    );
    const groupsAssignedHexagon =
      await within(gameboard).findByTitle("Groups assigned");
    expect(groupsAssignedHexagon.textContent).toEqual(
      `${assignmentsToGameboard.length} group${
        assignmentsToGameboard.length === 1 ? "" : "s"
      }`,
    );
    ...
    expect(
      mockGameboard.creationDate -
        (dayMonthYearStringToDate(dateText)?.valueOf() ?? 0),
    ).toBeLessThanOrEqual(ONE_DAY_IN_MS);
    ...
    const title = within(gameboard).getByRole("link", {
      name: mockGameboard.title,
    });
  ...
  it("should let you assign a gameboard in card view (using the modal)", async () => {
    ...
    renderTestEnvironment({
      ...
      extraEndpoints: [
        rest.post(
          API_PATH + "/assignments/assign_bulk",
          async (req, res, ctx) => {
    ...
    const modalOpenButton = within(gameboards[0]).getByRole("button", {
      name: /Assign\s?\/\s?Unassign/,
    });
    ...
    const group1Choice = within(selectContainer).getByText(
      mockActiveGroups[1].groupName,
    );
    await userEvent.click(group1Choice);

    // Check scheduled start date and due date are there
    within(modal).getByLabelText("Schedule an assignment start date", {
      exact: false,
    });
    ...
    const currentAssignments =
      within(modal).queryAllByTestId("current-assignment");
    const pendingAssignments =
      within(modal).queryAllByTestId("pending-assignment");
    const allAssignments = currentAssignments.concat(pendingAssignments);
    expect(allAssignments).toHaveLength(1);
    expect(allAssignments[0].textContent).toContain(
      mockActiveGroups[0].groupName,
    );
    ...
    await waitFor(() => {
      const newCurrentAssignments =
        within(modal).queryAllByTestId("current-assignment");
      expect(
        newCurrentAssignments.map((a) => a.textContent).join(","),
      ).toContain(mockActiveGroups[1].groupName);
    });
    ...
    const groupsAssignedHexagon = await within(gameboards[0]).findByTitle(
      "Groups assigned",
    );
