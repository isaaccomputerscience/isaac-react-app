...
export function TrueFalseRadioInput({
  ...
  const invalid =
    submissionAttempted && typeof stateObject?.[propertyName] !== "boolean";
  ...
  return (
    <RS.FormGroup>
      <div className="d-flex flex-nowrap">
        <RS.Label
          htmlFor={`${id}-t`}
          className="w-50 text-right text-nowrap pr-2"
        >
          {trueLabel}
      ...
      {invalid && (
        <div
          id={`${id}-feedback`}
          className="required-before d-flex flex-nowrap text-center"
        >
          required
        </div>
      )}
