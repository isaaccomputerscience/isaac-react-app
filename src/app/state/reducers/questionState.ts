  ...
  switch (action.type) {
    ...
    case ACTION_TYPE.QUESTION_ATTEMPT_RESPONSE_SUCCESS:
      return !question.bestAttempt || !question.bestAttempt.correct
        ? {
            ...question,
            validationResponse: action.response,
            bestAttempt: action.response,
          }
...
function augmentQuestions(questions: AppQuestionDTO[]): QuestionsState {
  return {
    questions,
    pageCompleted: questions.every(
      (q) => q.validationResponse && q.validationResponse.correct,
    ),
  };
}
  ...
  switch (action.type) {
    ...
    case ACTION_TYPE.QUESTION_DEREGISTRATION: {
      const filteredQuestions =
        qs &&
        qs.questions.filter((q) => q.id && !action.questionIds.includes(q.id));
      return filteredQuestions && filteredQuestions.length
        ? augmentQuestions(filteredQuestions)
        : null;
    }
    ...
    case ACTION_TYPE.QUESTION_ATTEMPT_RESPONSE_SUCCESS: {
      return (
        ...
        augmentQuestions(
          qs.questions.map((q) =>
            q.id === action.questionId ? question(q, action) : q,
          ),
        )
      );
    }
    ...
    case ACTION_TYPE.USER_PREFERENCES_RESPONSE_SUCCESS:
      if (
        qs &&
        action.userPreferences.DISPLAY_SETTING?.HIDE_QUESTION_ATTEMPTS
      ) {
        return {
          ...qs,
