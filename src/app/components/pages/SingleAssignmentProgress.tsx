...
const SingleProgressDetails = ({
  ...
  return (
    <div
      className={
        "assignment-progress-details single-assignment" +
    ...
    >
      ...
      <div className="single-download mb-2 mx-4">
        <Button
          ...
          color="link"
          tag="a"
          href={getAssignmentCSVDownloadLink(assignment.id)}
...
export const SingleAssignmentProgress = ({
  ...
  const assignmentQuery = isaacApi.endpoints.getSingleSetAssignment.useQuery(
    assignmentId || skipToken,
  );
  const { data: assignment } = assignmentQuery;
  const assignmentProgressQuery =
    isaacApi.endpoints.getAssignmentProgress.useQuery(
      assignmentId || skipToken,
    );

  const augmentAssignmentWithProgress = (
    assignment: AssignmentDTO,
    assignmentProgress: AppAssignmentProgress[],
  ): EnhancedAssignmentWithProgress =>
    ({
      ...assignment,
      progress: assignmentProgress,
    }) as EnhancedAssignmentWithProgress;
    ...
    <>
      <Container>
        <TitleAndBreadcrumb
          ...
          currentPageTitle={`Assignment Progress${
            assignment?.gameboard?.title && ": " + assignment?.gameboard?.title
          }`}
          className="mb-4"
        />
      </Container>
      <ShowLoadingQuery
        query={combineQueries(
          assignmentQuery,
          assignmentProgressQuery,
          augmentAssignmentWithProgress,
        )}
