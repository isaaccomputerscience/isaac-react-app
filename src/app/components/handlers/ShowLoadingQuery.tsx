...
export const DefaultQueryError = ({
  error,
  title,
}: {
  ...
  return (
    <Alert color={"warning"} className={"my-2"}>
      ...
      <br />
      You may want to refresh the page, or{" "}
      <a href={`mailto:${WEBMASTER_EMAIL}`}>email</a> us if this continues to
      happen. Please include in your email the name and email associated with
      this Isaac {SITE_SUBJECT_TITLE} account, alongside the details of the
      error given above.
    </Alert>
  );
};
...
export function combineQueries<T, R, S>(
  firstQuery: ShowLoadingQueryInfo<T>,
  secondQuery: ShowLoadingQueryInfo<R>,
  combineResult: (
    firstQueryResult: NonNullable<T>,
    secondQueryResult: NonNullable<R>,
  ) => NonNullable<S>,
): ShowLoadingQueryInfo<S> {
  return {
    data:
      isFound<T>(firstQuery.data) && isFound<R>(secondQuery.data)
        ? combineResult(firstQuery.data, secondQuery.data)
        : undefined,
...
export const pairResults = <T, R>(first: T, second: R): [T, R] => [
  first,
  second,
];
...
export function ShowLoadingQuery<T>({
  query,
  thenRender,
  children,
  placeholder,
  ...
  ifNotFound,
  defaultErrorTitle,
}: ShowLoadingQueryProps<T>) {
  const { data, isLoading, isError, error } = query;
  const renderError = () =>
    ifError ? (
      <>{ifError(error)}</>
    ) : (
      <DefaultQueryError error={error} title={defaultErrorTitle} />
    );
  if (isError && error) {
    return "status" in error &&
      typeof error.status === "number" &&
      [NOT_FOUND, NO_CONTENT].includes(error.status) &&
      ifNotFound ? (
      <>{ifNotFound}</>
    ) : (
      renderError()
  ...
  return isDefined(data) ? (
    isFound<T>(data) ? (
      <>{thenRender ? thenRender(data) : children}</>
    ) : ifNotFound ? (
      <>{ifNotFound}</>
    ) : (
      renderError()
