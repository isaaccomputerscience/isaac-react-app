...
} from "../elements/views/QuestionProgressCharts";
...
interface MyProgressProps
  extends RouteComponentProps<{ userIdOfInterest: string }> {
  user: PotentialUser;
}
const MyProgress = withRouter((props: MyProgressProps) => {
  ...
  const viewingOwnData =
    userIdOfInterest === undefined ||
    (user.loggedIn && parseInt(userIdOfInterest) === user.id);
  ...
  const myAnsweredQuestionsByDate = useAppSelector(
    selectors.user.answeredQuestionsByDate,
  );
  const userAnsweredQuestionsByDate = useAppSelector(
    selectors.teacher.userAnsweredQuestionsByDate,
  );
  ...
  useEffect(() => {
    if (viewingOwnData && user.loggedIn) {
      ...
      dispatch(
        getMyAnsweredQuestionsByDate(user.id as number, 0, Date.now(), false),
      );
    } else if (isTeacherOrAbove(user)) {
      dispatch(getUserProgress(userIdOfInterest));
      dispatch(
        getUserAnsweredQuestionsByDate(userIdOfInterest, 0, Date.now(), false),
      );
    }
  ...
  const progress =
    !viewingOwnData && isTeacherOrAbove(user) ? userProgress : myProgress;
  const answeredQuestionsByDate =
    !viewingOwnData && isTeacherOrAbove(user)
      ? userAnsweredQuestionsByDate
      : myAnsweredQuestionsByDate;

  const userName = `${progress?.userDetails?.givenName || ""}${
    progress?.userDetails?.givenName ? " " : ""
  }${progress?.userDetails?.familyName || ""}`;
  const pageTitle = viewingOwnData
    ? "My progress"
    : `Progress for ${userName || "user"}`;
              ...
              "Question activity": (
                <div>
                        ...
                        {{
                          "Correct questions": (
                            <QuestionProgressCharts
                              ...
                              questionsByStageAndDifficulty={
                                progress?.correctByStageAndDifficulty || {}
                              }
                              flushRef={tabRefs[0]}
                            />
                          ),
                          "Attempted questions": (
                            <QuestionProgressCharts
                              ...
                              questionsByStageAndDifficulty={
                                progress?.attemptsByStageAndDifficulty || {}
                              }
                              flushRef={tabRefs[1]}
                            />
                          ),
                  ...
                  <div className="mt-4">
                    ...
                    <Row>
                      {statistics.questionTypeStatsList.map((qType: string) => {
                        const correct =
                          progress?.correctByType?.[qType] || null;
                        const attempts =
                          progress?.attemptsByType?.[qType] || null;
                        const percentage = safePercentage(correct, attempts);
                        return (
                          <Col
                            key={qType}
                            className={`${statistics.typeColWidth} mt-2 type-progress-bar`}
                          >
                            <div className={"px-2"}>
                  ...
                  <Row id="progress-questions">
                    {progress?.mostRecentQuestions &&
                      progress?.mostRecentQuestions.length > 0 && (
                    ...
                    {progress?.oldestIncompleteQuestions &&
                      progress?.oldestIncompleteQuestions.length > 0 && (
                        <Col md={12} lg={6} className="mt-4">
