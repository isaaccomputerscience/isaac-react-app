...
export const customKeyboardCoordinates: KeyboardCoordinateGetter = (
  ...
  const dropZoneContainers = sortBy(
    droppableContainers
      .toArray()
      .filter((dc) => dc.id.toString().startsWith(CLOZE_DROP_ZONE_ID_PREFIX)),
    (dc: DroppableContainer) => dc.id,
  );

  // Get non-selected item droppables ordered left-to-right
  const itemSectionDroppableContainer = droppableContainers.get(
    CLOZE_ITEM_SECTION_ID,
  );
  if (!itemSectionDroppableContainer) return;
  const nsiIds: string[] | undefined =
    itemSectionDroppableContainer?.data.current?.itemIds;
  ...
  const orderedDroppableContainers = dropZoneContainers.concat(
    nsiDroppableContainers?.length > 0
      ? nsiDroppableContainers
      : [itemSectionDroppableContainer],
  );
    ...
    case KeyboardCode.Up:
      nextDroppable =
        orderedDroppableContainers[
          Math.max(
            0,
            orderedDroppableContainers.findIndex(
              (dc) =>
                dc.data.current?.itemId === currentId || dc.id === currentId,
            ) - 1,
          )
        ];
      break;
