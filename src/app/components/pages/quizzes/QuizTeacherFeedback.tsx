...
const pageHelp = (
  <span>See the feedback for your students for this test assignment.</span>
);
...
export const QuizTeacherFeedback = ({ user }: { user: RegisteredUserDTO }) => {
  ...
  const setFeedbackMode = async (mode: QuizFeedbackMode) => {
    ...
    try {
      ...
      await dispatch(
        updateQuizAssignmentFeedbackMode(numericQuizAssignmentId, mode),
      );
    } finally {
  ...
  const assignment =
    assignmentState && "assignment" in assignmentState
      ? assignmentState.assignment
      : null;
  const error =
    assignmentState && "error" in assignmentState
      ? assignmentState.error
      : null;
  const quizTitle =
    (assignment?.quiz?.title || assignment?.quiz?.id || "Test") + " results";
    ...
    if (assignment?.dueDate && newDate > assignment.dueDate) {
      try {
        ...
        if (
          ...
          dispatch(
            updateQuizAssignmentDueDate(numericQuizAssignmentId, newDate),
          ).then((succeeded) => {
            if (succeeded) {
              dispatch(
                showToast({
          ...
          <>
            ...
            <p className="d-flex">
              <span>
                ...
                on {formatDate(assignment.creationDate)}
              </span>
            ...
            <Row>
                ...
                <Col xs={12} sm={6} md={4}>
                  <Label for="dueDate" className="pr-1">
                    ...
                    <DateInput
                      ...
                      invalid={
                        (dueDate && dueDate < assignment.dueDate) ?? undefined
                      }
                      yearRange={yearRange}
                      noClear
                  ...
                  <div className={"mt-2 w-100 text-center mb-2"}>
                    {dueDate && dueDate > assignment.dueDate && (
                      <Button
                        ...
                        outline
                        className={"btn-md"}
                        onClick={() => setValidDueDate(dueDate)}
                      >
                        {settingDueDate ? (
                          <>
                            Saving{" "}
                            <IsaacSpinner
                              size="sm"
                              className="quizFeedbackModeSpinner"
                            />
                          </>
                        ) : (
                          "Extend due date"
                ...
                <UncontrolledDropdown className="d-inline-block">
                  ...
                  >
                    {settingFeedbackMode ? (
                      ...
                      feedbackNames[
                        assignment.quizFeedbackMode as QuizFeedbackMode
                      ]
                    )}
                  </DropdownToggle>
                  <DropdownMenu>
              ...
              <Col sm={12} md={"auto"} className={"text-right mt-2 mt-md-0"}>
                <Button
                  ...
                  href={getQuizAssignmentCSVDownloadLink(
