...
function rightAngledConnectionLine(
  ...
  // First stroke
  result += svgMoveTo(
    sourceHexagonX + hexagon.x.right + hexagon.padding,
    hexagon.y.top + hexagon.quarterHeight,
  );
  result += svgLine(
    sourceHexagonX + hexagon.x.right + hexagon.padding,
    hexagon.y.center,
  ...
  result += svgLine(
    targetHexagonX + hexagon.x.right + hexagon.padding,
    hexagon.y.center,
  );
  // Last stroke
  result += svgLine(
    targetHexagonX + hexagon.x.right + hexagon.padding,
    hexagon.y.bottom - hexagon.quarterHeight,
  );

  return result;
}

function fastTrackConnectionLine(
  hexagonProperties: HexagonProportions,
  sourceIndex: number,
  targetIndex: number,
) {
  ...
  const sourceHexagonX =
    sourceIndex <= targetIndex
      ? sourceIndex * hexagonWidth
      : Math.max(sourceIndex - 1, 0) * hexagonWidth;
  const targetHexagonX =
    targetIndex <= sourceIndex
      ? targetIndex * hexagonWidth
      : Math.max(targetIndex - 1, 0) * hexagonWidth;
  ...
  if (sourceIndex <= targetIndex) {
    ...
    result += svgMoveTo(
      sourceHexagonX + hexagon.x.rightDiag,
      hexagon.y.topDiag,
    );
  }
  ...
  if (targetIndex <= sourceIndex) {
    result += svgLine(
      targetHexagonX + hexagon.x.leftDiag,
      hexagon.y.bottomDiag,
    );
  } else {
    result += svgLine(
      targetHexagonX + hexagon.x.rightDiag,
      hexagon.y.bottomDiag,
    );
  }
...
function mobileConnectionLine(
  ...
  if (rowIndex === 0) {
    ...
    const targetHexagonY =
      hexagon.y.bottom +
      2 * hexagon.padding +
      targetIndex * (hexagon.y.bottom + hexagon.padding);
    result += svgMoveTo(sourceHexagonX + hexagon.x.center, hexagon.y.bottom);

    // First stroke
    result += svgLine(
      sourceHexagonX + hexagon.x.center,
      hexagon.y.bottom + hexagon.padding,
    );
    // Connections
    ...
    // Final stroke
    result += svgLine(
      hexagon.x.center + hexagon.padding,
      targetHexagonY + hexagon.y.center,
    );
  } else {
    const sourceHexagonX = hexagon.x.center + hexagon.padding;
    const sourceHexagonY =
      hexagon.y.bottom +
      2 * hexagon.padding +
      sourceIndex * (hexagon.y.bottom + hexagon.padding);
    ...
    // First stroke
    result += svgMoveTo(
      sourceHexagonX + hexagon.x.right,
      sourceHexagonY + hexagon.y.center,
    );
...
export function HexagonConnection({
  ...
  const filteredTargetIndices = targetIndices.filter(
    (i) => ![sourceIndex, i].includes(-1),
  ); // Filter "not found" selections
