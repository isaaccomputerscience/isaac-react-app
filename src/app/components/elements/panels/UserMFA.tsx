...
const UserMFA = ({
  ...
  const [mfaVerificationCode, setMFAVerificationCode] = useState<
    string | undefined
  >(undefined);
  const [qrCodeStringBase64SVG, setQrCodeStringBase64SVG] = useState<
    string | undefined
  >(undefined);
  ...
  const [disableAccountMFA] =
    isaacApi.endpoints.disableAccountMFA.useMutation();
  ...
  function setupMFA(
    ...
    if (totpSharedSecret && mfaVerificationCode) {
      setupAccountMFA({
        sharedSecret: totpSharedSecret.sharedSecret,
        mfaVerificationCode,
      }).then(resetMFASecret);
    }
  }
  ...
  return (
    <CardBody className="pt-0">
      ...
      {!editingOtherUser &&
      userAuthSettings &&
      userAuthSettings.hasSegueAccount ? (
        <Row>
          <Col>
            <Row>
              <Col md={{ size: 6, offset: 3 }}>
                <p>
                  ...
                  {userAuthSettings.mfaStatus || successfulMFASetup
                    ? "Enabled"
                    : "Disabled"}
                </p>
              </Col>
            </Row>
            {isDefined(totpSharedSecret) &&
            isDefined(totpSharedSecret.sharedSecret) ? (
              <Form onSubmit={setupMFA}>
                <Row>
                  <Col md={{ size: 6, offset: 3 }}>
                    <h5>Configure Two-factor Authentication (2FA)</h5>
                    <p>
                      <strong>Step 1:</strong> Scan the QRcode below on your
                      phone
                    </p>
                    <div className="qrcode-mfa vertical-center">
                      {qrCodeStringBase64SVG && (
                        <img
                          src={
                            "data:image/svg+xml;base64," + qrCodeStringBase64SVG
                          }
                          alt={
                            "Follow this URL to setup 2FA: " + authenticatorURL
                          }
                        />
                      )}
                    </div>
                    ...
                    <FormGroup>
                      <p>
                        <strong>Step 2:</strong> Enter the code provided by your
                        app
                      </p>
                      <Label htmlFor="setup-verification-code">
                        Verification Code
                      </Label>
                      <Input
        ...
        <Row className="pt-4">
          <Col className="text-center">
            {!editingOtherUser &&
              userAuthSettings &&
              userAuthSettings.linkedAccounts && (
                <p>
              ...
              <p>
                <FormGroup>
                  <Button
                    className="btn-secondary"
                    onClick={() =>
                      userToUpdate.id && disableAccountMFA(userToUpdate.id)
                    }
                  >
