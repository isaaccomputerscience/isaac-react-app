...
export const Accordion = withRouter(
  ({
    id,
    trustedTitle,
    index,
    ...
    const page = useAppSelector(
      (state: AppState) => (state && state.doc) || null,
    );

    // Toggle
    const isFirst = index === 0;
    const [open, setOpen] = useState(
      disabled ? false : startOpen === undefined ? isFirst : startOpen,
    );
    ...
    const questionsOnPage =
      useAppSelector(selectors.questions.getQuestions) || [];
    const questionsInsideAccordionSection = questionsOnPage?.filter(
      (q) => q.accordionClientId === clientId.current,
    );
    ...
    const isConceptPage =
      page && page != NOT_FOUND && page.type === DOCUMENT_TYPE.CONCEPT;
