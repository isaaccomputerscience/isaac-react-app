...
export const sortQuestions =
  (sortState: { [s: string]: string }, creationContext?: AudienceContext) =>
  (questions: ContentSummaryDTO[]) => {
    if (sortState["title"] && sortState["title"] != SortOrder.NONE) {
      const sortedQuestions = questions.sort((a, b) =>
        bookSort(a.title || "", b.title || ""),
      );
      return sortState["title"] == SortOrder.ASC
        ? sortedQuestions
    ...
    if (sortState.difficulty && sortState.difficulty != SortOrder.NONE) {
      questions.sort(function compareFirstDifficulty(a, b) {
        const firstDifficultyA = determineAudienceViews(
          a.audience,
          creationContext,
        )[0]?.difficulty;
        const firstDifficultyB = determineAudienceViews(
          b.audience,
          creationContext,
        )[0]?.difficulty;
        if (firstDifficultyA === firstDifficultyB) return 0;
        const aIndex = difficultiesOrdered.indexOf(
          firstDifficultyA as Difficulty,
        );
        const bIndex = difficultiesOrdered.indexOf(
          firstDifficultyB as Difficulty,
        );
        return sortState.difficulty === SortOrder.ASC && aIndex > bIndex
          ? 1
          : -1;
      });
      return questions;
...
export const convertContentSummaryToGameboardItem = (
...
export const convertGameboardItemToContentSummary = (
...
export const convertTagToSelectionOption = (tag: Tag) => {
  return {
    value: tag.id,
    label: `${tag.title}${
      tag.comingSoonDate ? ` (Coming ${tag.comingSoonDate})` : ""
    }`,
...
export const groupTagSelectionsByParent = (parent: Tag) => {
  return {
    ...
    options: tags
      ...
      .map(convertTagToSelectionOption)
...
export const loadGameboardSelectedQuestions = (gameboard: GameboardDTO) => {
  return (
    gameboard.contents &&
    gameboard.contents
      .map(convertGameboardItemToContentSummary)
      .reduce((map, question) => {
