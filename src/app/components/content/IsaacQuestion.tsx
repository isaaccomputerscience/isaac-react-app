...
} from "../elements/inputs/ConfidenceQuestions";
import { Loading } from "../handlers/IsaacSpinner";
import classNames from "classnames";

export const IsaacQuestion = withRouter(
  ({ doc, location }: { doc: ApiTypes.QuestionDTO } & RouteComponentProps) => {
    ...
    const invalidFormatError = validationResponseTags?.includes(
      "unrecognised_format",
    );
    const invalidFormatErrorStdForm =
      validationResponseTags?.includes("invalid_std_form");
    const fastTrackInfo = useFastTrackInformation(
      doc,
      location,
      canSubmit,
      correct,
    ...
    const invalidFormatFeedback = (
      <p>
        Your answer is not in a format we recognise, please enter your answer as
        a decimal number.
        <br />
        {invalidFormatErrorStdForm && (
          <>
            When writing standard form, you must include <code>^</code> or{" "}
            <code>**</code> between the 10 and the exponent.
            <br />
          </>
        )}
      </p>
          ...
          onSubmit={function submitCurrentAttempt(event) {
            ...
            if (questionPart?.currentAttempt) {
              ...
              if (
                ...
                currentGameboard?.id &&
                !currentGameboard.savedToCurrentUser
              ) {
                dispatch(
                  saveGameboard({
        ...
        >
          <div
            className={classNames("question-component p-md-5", doc.type, {
              ...
              ].includes(doc.type as string),
            })}
          >
            <Suspense fallback={<Loading />}>
              <QuestionComponent
                questionId={doc.id as string}
                doc={doc}
            ...
            {/* CS Hints */}
            <IsaacLinkHints
              questionPartId={doc.id as string}
              hints={doc.hints}
            />
            ...
            {showQuestionFeedback && validationResponse && !canSubmit && (
              <div
                className={`validation-response-panel p-3 mt-3 ${
                  correct ? "correct" : ""
                }`}
              >
                <div className="pb-1">
                ...
                {validationResponse.explanation && (
                  <div className="mb-2">
                    {invalidFormatError ? (
                      ...
                      <IsaacContent
                        doc={validationResponse.explanation as ContentDTO}
                      />
                    )}
                  </div>
                )}
            ...
            {locked && (
              <RS.Alert color="danger" className={"no-print"}>
                This question is locked until at least{" "}
                {<DateString formatter={TIME_ONLY}>{locked}</DateString>} to
              ...
              !locked && (
                ...
                >
                  {secondaryAction && (
                    <div
                      className={classNames(
                        "m-auto pt-3 pb-1 w-100 w-sm-50 w-md-100 w-lg-50",
                        { "pr-sm-2 pr-md-0 pr-lg-3": primaryAction },
                      )}
                    >
                      <input
                        {...secondaryAction}
                        className="h-100 btn btn-outline-primary btn-block"
                      />
                    </div>
                  )}
                  {primaryAction && (
                ...
                <RS.Col xl={{ size: 10, offset: 1 }}>
                  {doc.hints && (
                    <p className="no-print text-center pt-2 mb-0">
                      <small>
                        {
                          "Don't forget to use the hints above if you need help."
                        }
                      </small>
                    </p>
                  )}
