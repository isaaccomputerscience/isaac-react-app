...
const isaacBaseQuery: BaseQueryFn<
  string | FetchArgs,
  unknown,
  FetchBaseQueryError
> = async (args, api, extraOptions) => {
  ...
  if (
    ...
    !(result.error.data as { bypassGenericSiteErrorPage?: boolean })
      ?.bypassGenericSiteErrorPage
  ) {
...
export const resultOrNotFound = <T>(
  result: T,
  error: FetchBaseQueryError | SerializedError | undefined,
) => {
  return error && "status" in error && error.status === NOT_FOUND
    ? NOT_FOUND
    : result;
};
...
interface QueryLifecycleSpec<T, R> {
  onQueryStart?: (
    args: T,
    api: { dispatch: Dispatch<any>; getState: () => any },
  ) => void | { resetOptimisticUpdates: () => void };
  successTitle?: string;
...
export const mutationSucceeded = <T>(
  response: { data: T } | { error: FetchBaseQueryError | SerializedError },
): response is { data: T } => {
  return response.hasOwnProperty("data");
};

export const extractDataFromQueryResponse = <T>(
  response: { data?: T } | { error: FetchBaseQueryError | SerializedError },
): T | NOT_FOUND_TYPE | undefined => {
  if ("data" in response) {
...
export const getRTKQueryErrorMessage = (
  ...
  if (e?.hasOwnProperty("data")) {
    ...
    return {
      status: e.status,
      message: e?.data?.errorMessage ?? API_REQUEST_FAILURE_MESSAGE,
    };
  }
  if (e?.hasOwnProperty("message")) {
    ...
    return {
      status: se.code,
      message: se?.message ?? API_REQUEST_FAILURE_MESSAGE,
    };
  }
...
const isaacApi = createApi({
  ...
  endpoints: (build) => ({
    ...
    getNewsPodList: build.query<
      IsaacPodDTO[],
      { subject: string; orderDescending?: boolean }
    >({
      query: ({ subject }) => ({
        url: `/pages/pods/${subject}`,
      }),
      transformResponse: (
        response: { results: IsaacPodDTO[]; totalResults: number },
        meta,
        arg,
      ) => {
    ...
    getGameboards: build.query<
      Boards,
      { startIndex: number; limit: NumberOfBoards; sort: BoardOrder }
    >({
    ...
    getGameboardById: build.query<GameboardDTO, string | null>({
      ...
      providesTags: (result) =>
        result && result.id ? [{ type: "Gameboard", id: result.id }] : [],
    }),
    ...
    createGameboard: build.mutation<
      GameboardDTO,
      { gameboard: GameboardDTO; previousId?: string }
    >({
      query: ({ gameboard }) => ({
        url: "gameboards",
        method: "POST",
    ...
    generateTemporaryGameboard: build.mutation<
      GameboardDTO,
      { [key: string]: string }
    >({
      query: (params) => {
    ...
    renameAndLinkUserToGameboard: build.mutation<
      void,
    ...
    unlinkUserFromGameboard: build.mutation<void, string>({
      ...
      invalidatesTags: (_, error, boardId) =>
        !error ? [{ type: "Gameboard", id: boardId }] : [],
      onQueryStarted: onQueryLifecycleEvents({
        successTitle: "Gameboard deleted",
        successMessage:
          "You have successfully unlinked your account from this gameboard.",
    ...
    getMySetAssignments: build.query<AssignmentDTO[], number | undefined>({
      ...
      providesTags: (result, _, groupId) =>
        result
          ? groupId
            ? [{ type: "GroupAssignments", id: groupId }]
            : ["AllSetAssignments"]
          : [],
    }),
    ...
    getSingleSetAssignment: build.query<EnhancedAssignment, number>({
      ...
      providesTags: (result, _, assignmentId) =>
        result ? [{ type: "SetAssignment", id: assignmentId }] : [],
    }),
    ...
    getAssignmentProgress: build.query<AppAssignmentProgress[], number>({
      ...
      transformResponse: anonymiseIfNeededWith<AppAssignmentProgress[]>(
        anonymisationFunctions.progressState,
      ),
    }),
    ...
    assignGameboard: build.mutation<AssignmentFeedbackDTO[], AssignmentDTO[]>({
      ...
      invalidatesTags: (result) =>
        result ? ["AssignmentProgress", "AllMyAssignments"] : [],
    }),

    unassignGameboard: build.mutation<
      void,
      { boardId: string; groupId: number }
    >({
      ...
      invalidatesTags: (_, error) =>
        !error ? ["AssignmentProgress", "AllMyAssignments"] : [],
      onQueryStarted: onQueryLifecycleEvents({
        onQueryStart: ({ boardId, groupId }, { dispatch }) => {
          ...
          const allAssignmentsPromise = dispatch(
            isaacApi.util.updateQueryData(
              "getMySetAssignments",
              undefined,
              (assignments) => {
                return (assignments ?? []).filter(
                  (a) => a.groupId !== groupId || a.gameboardId !== boardId,
                );
              },
            ),
          );
          const groupAssignmentsPromise = dispatch(
            isaacApi.util.updateQueryData(
              "getMySetAssignments",
              groupId,
              (assignments) => {
                return (assignments ?? []).filter(
                  (a) => a.gameboardId !== boardId,
                );
              },
            ),
          );
    ...
    getGroups: build.query<AppGroup[], boolean>({
      ...
      transformResponse: anonymiseListIfNeededWith<AppGroup>(
        anonymisationFunctions.appGroup,
      ),
      keepUnusedDataFor: 60,
    }),
    ...
    createGroup: build.mutation<AppGroup, string>({
      ...
      onQueryStarted: onQueryLifecycleEvents({
        onQuerySuccess: (_, newGroup, { dispatch }) => {
          ...
          dispatch(
            isaacApi.util.updateQueryData("getGroups", false, (groups) => [
              ...groups,
              newGroup,
            ]),
          );
      ...
      transformResponse: anonymiseIfNeededWith<AppGroup>(
        anonymisationFunctions.appGroup,
      ),
    }),
    ...
    updateGroup: build.mutation<
      void,
      { updatedGroup: AppGroup; message?: string }
    >({
      query: ({ updatedGroup }) => ({
        method: "POST",
      ...
      invalidatesTags: (_, error, { updatedGroup }) =>
        !isDefined(error)
          ? [{ type: "GroupAssignments", id: updatedGroup.id }]
          : [],
      onQueryStarted: onQueryLifecycleEvents({
    ...
    getGroupMemberships: build.query<
      GroupMembershipDetailDTO[],
      number | undefined
    >({
      query: (userId) => ({
      ...
      providesTags: (_, __, userId) =>
        userId
          ? [{ type: "GroupMemberships", id: userId }]
          : ["MyGroupMemberships"],
      onQueryStarted: onQueryLifecycleEvents({
        errorTitle: "Loading group memberships failed",
      }),
      transformResponse: anonymiseListIfNeededWith<GroupMembershipDetailDTO>(
        anonymisationFunctions.groupMembershipDetail,
      ),
    }),

    changeMyMembershipStatus: build.mutation<
      void,
      { groupId: number; newStatus: MEMBERSHIP_STATUS }
    >({
    ...
    getGroupMembers: build.query<UserSummaryWithGroupMembershipDTO[], number>({
      ...
      keepUnusedDataFor: 0,
      transformResponse:
        anonymiseListIfNeededWith<UserSummaryWithGroupMembershipDTO>(
          anonymisationFunctions.userSummary(),
        ),
    }),

    deleteGroupMember: build.mutation<
      void,
    ...
    addGroupManager: build.mutation<
      AppGroup,
      { groupId: number; managerEmail: string }
    >({
          ...
          [true, false].forEach((archivedGroupsOnly) => {
            dispatch(
              isaacApi.util.updateQueryData(
                ...
                (groups) =>
                  groups.map((g) =>
                    g.id === groupWithNewManager.id
                      ? {
                          ...g,
                          additionalManagers:
                            groupWithNewManager.additionalManagers,
                        }
                      : g,
                  ),
              ),
    ...
    deleteGroupManager: build.mutation<
      void,
      { groupId: number; managerUserId: number }
    >({
                  ...
                  (groups) =>
                    groups.map((g) =>
                      g.id === groupId
                        ? {
                            ...g,
                            additionalManagers: g.additionalManagers?.filter(
                              (m) => m.id !== managerUserId,
                            ),
                          }
                        : g,
                    ),
    ...
    promoteGroupManager: build.mutation<
      void,
      { groupId: number; managerUserId: number }
    >({
      query: ({ groupId, managerUserId }) => ({
      ...
      onQueryStarted: onQueryLifecycleEvents({
        ...
        successMessage:
          "You have been demoted to an additional manager of this group",
          ...
          [true, false].forEach((archivedGroupsOnly) => {
            dispatch(
              isaacApi.util.updateQueryData(
                ...
                (groups) =>
                  groups.map((g) =>
                    g.id === groupId ? { ...g, token: tokenDTO.token } : g,
                  ),
              ),
            );
          });
    ...
    setupAccountMFA: build.mutation<
      void,
      { sharedSecret: string; mfaVerificationCode: string }
    >({
    ...
    getMisuseStatistics: build.query<
      { [eventLabel: string]: MisuseStatisticDTO[] },
      number
    >({
      query: (n) => ({
    ...
    resetMisuseMonitor: build.mutation<
      void,
      { eventLabel: string; agentIdentifier: string }
    >({
          ...
          dispatch(
            showSuccessToast(
              ...
              `${eventLabel.replace(
                "MisuseHandler",
                "",
              )} misuse event count reset for user`,
            ),
          );
        },
      }),
