...
import {
  ...
  UserEmailPreference,
} from "../panels/UserEmailPreferences";
import {
  BooleanNotation,
  DisplaySettings,
  ValidationUser,
...
const RequiredAccountInfoBody = () => {
  ...
  const [userToUpdate, setUserToUpdate] =
    useState<Immutable<ValidationUser>>(initialUserValue);
  ...
  const [emailPreferences, setEmailPreferences] = useEmailPreferenceState(
    initialEmailPreferencesValue,
  );

  const initialUserContexts =
  ...
  return (
    <RS.Form onSubmit={formSubmission}>
        ...
        <RS.CardBody className="p-0">
          {!isTutorOrAbove(user) && (
            <div className="text-left mb-4">
              Account type:{" "}
              <b>{user?.loggedIn && user.role && UserFacingRole[user.role]}</b>{" "}
              <span>
                <small>
      ...
      <div className="text-muted small pb-2">
        Providing this information helps us understand the usage of Isaac{" "}
        {SITE_SUBJECT_TITLE}. Full details on how we use your personal data can
        be found in our{" "}
