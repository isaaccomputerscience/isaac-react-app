...
const MainSearchComponent = ({ history }: MainSearchProps) => {
  ...
  return (
    <Navbar className="search" expand="md">
      <NavbarToggler
        className={showSearchBox ? "open" : ""}
        aria-label={showSearchBox ? "Close search menu" : "Open search menu"}
            ...
            <Form inline onSubmit={doSearch}>
              <FormGroup className="search--main-group">
                ...
                <input
                  type="hidden"
                  name="types"
                  value="isaacQuestionPage,isaacConceptPage"
                />
              </FormGroup>
            </Form>
          </NavItem>
