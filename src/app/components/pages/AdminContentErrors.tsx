...
const contentErrorDetailsListItem = (
  errorDetailsListItem: string,
  index: number,
) => {
  ...
  return (
    <tr key={index}>
      <td className="text-break">
        ...
        <a
          href={EDITOR_URL + errorRecord.partialContent.canonicalSourceFile}
          target="_blank"
          rel="noopener"
        >
      ...
      <td>{(!!errorRecord.partialContent.published).toString()}</td>
      {/*The published key may be missing, also meaning false*/}
      <td>{(!errorRecord.successfulIngest).toString()}</td>
      <td>
        <ul>{errorRecord.listOfErrors.map(contentErrorDetailsListItem)}</ul>
      </td>
    </tr>
  );
};
...
export const AdminContentErrors = () => {
  ...
  useEffect(() => {
    dispatch(getAdminContentErrors());
  }, [dispatch]);
  const errors = useAppSelector(
    (state: AppState) => state?.adminContentErrors || null,
  );

  const [errorFilter, setErrorFilter] = useState<string>("");
  const errorReducer = (show: boolean, errorStr: string) =>
    show || errorStr.toLowerCase().includes(errorFilter.toLowerCase());

  const [publishedFilter, setPublishedFilter] = useState<PUBLISHED_FILTER[]>([
            ...
            <Row>
              <Col lg={4} className="mb-2">
                ...
                <Input
                  ...
                  placeholder="Filter errors by error message"
                />
              </Col>
              <Col lg={6} className="mb-2">
                <Label htmlFor="published-filter-select">
                ...
                <Select
                  ...
                  options={[
                    {
                      value: PUBLISHED_FILTER.PUBLISHED,
                      label: PUBLISHED_FILTER.PUBLISHED,
                    },
                    {
                      value: PUBLISHED_FILTER.UNPUBLISHED,
                      label: PUBLISHED_FILTER.UNPUBLISHED,
                    },
                  ]}
                ...
                <Table responsive bordered>
                  <tbody>
                    <tr>
                      ...
                      <th title="Files with critical errors will not be available on Isaac!">
                        Critical Error
                      </th>
                    ...
                    {errors.errorsList
                      .filter((error) =>
                        error.listOfErrors.reduce(errorReducer, false),
                      )
                      .filter(
                        (error) =>
                          (error.partialContent.published &&
                            publishedFilter.includes(
                              PUBLISHED_FILTER.PUBLISHED,
                            )) ||
                          (!error.partialContent.published &&
                            publishedFilter.includes(
