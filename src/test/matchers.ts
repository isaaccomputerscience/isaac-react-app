...
expect.extend({
  async toHaveBeenRequestedWith(
    ...
    for (const call of received.mock.calls) {
      ...
      if (await predicate(req)) {
        return {
          message: () =>
            `Request ${this.utils.printReceived(req)} passed given predicate.`,
          pass: true,
        };
      }
    }
  ...
  toHaveModalTitle(recieved: HTMLElement, title: string) {
    ...
    return {
      pass,
      message: () =>
        `Expected title to contain: ${this.utils.printExpected(
          title,
        )}\nReceived: ${this.utils.printReceived(modalHeader?.textContent)}${
