  ...
  notificationWebSocket.onmessage = function (event) {
    ...
    if (websocketMessage.heartbeat) {
      ...
      if (
        ...
        new Date(lastKnownServerTime).getDate() !==
          new Date(newServerTime).getDate()
      ) {
    ...
    if (websocketMessage.userSnapshot) {
      store.dispatch(
        partiallyUpdateUserSnapshot(websocketMessage.userSnapshot),
      );
      ...
      websocketMessage.notifications.forEach(function (entry: any) {
        ...
        if (
          notificationMessage.dailyStreakRecord &&
          notificationMessage.weeklyStreakRecord
        ) {
          store.dispatch(
      ...
      case 1013: // 'Try Again Later': should attempt to reopen, but in at least a minute!
        ...
        if (event.reason === "TRY_AGAIN_LATER") {
          ...
          if (webSocketErrorCount >= 3) {
            console.error(
              "WebSocket reconnect failed multiple times. Aborting retry!",
            );
            break;
          }
          ...
          const randomRetryIntervalSeconds =
            30 + Math.floor(Math.random() * 60);
          console.log(
            "WebSocket connection lost. Reconnect attempt in " +
              randomRetryIntervalSeconds +
              "s.",
          );
          webSocketCheckTimeout = window.setTimeout(
            checkForWebSocket,
            randomRetryIntervalSeconds * 1000,
          );
        }
        break;
      default: // Unexpected closure code: log and abort retrying!
        console.error(
          "WebSocket closed by server error (Code " +
            event.code +
            "). Aborting retry!",
        );
        if (webSocketCheckTimeout !== null) {
