...
import { highlightJsService } from "../../services";
...
const IsaacCodeSnippet = ({ doc }: IsaacCodeProps) => {
  ...
  const logViewOnGitHub = () => {
    dispatch(
      logAction({
        type: "VIEW_GITHUB_CODE",
        pageId: (rootDoc as ContentDTO).id,
        githubUrl: doc.url,
      }),
    );
  };

  const [scrollPromptRef, updateScrollPromptRef] =
    useStatefulElementRef<HTMLDivElement>();
  ...
  return (
    <div
      ref={updateExpandRef}
      className={classNames("position-relative code-snippet", outerClasses)}
    >
      {expandButton}
      <div className={innerClasses}>
        {/* ScrollShadows uses ResizeObserver, which doesn't exist on Safari <= 13 */}
        {window.ResizeObserver && <ScrollShadows element={scrollPromptRef} />}
        <Row>
          <Col>
            <pre ref={updateScrollPromptRef} className="line-numbers">
              <code
                ref={codeSnippetRef}
                className={doc.disableHighlighting ? "plaintext" : doc.language}
              >
                {doc.code}
              </code>
            </pre>
