...
type StringNot<X extends string, Y> = X extends Y
  ? never
  : string extends X
  ? never
  : X;
...
export function Markup<T extends string>({
  encoding,
  "trusted-markup-encoding": trustedMarkupEncoding,
  className,
  children,
}: MarkupProps<T> | TrustedMarkupProps) {
  const renderKaTeX = useRenderKatex();
  ...
  switch (encoding ?? trustedMarkupEncoding) {
    ...
    case "latex":
      ...
      return (
        <span
          dangerouslySetInnerHTML={{ __html: renderKaTeX(escapedMarkup) }}
          className={className}
        />
      );
    case "plaintext":
    ...
    default:
      return (
        <div>
          ...
          <i>
            {encoding} | {utils.escapeHtml(children)}{" "}
          </i>
          ]
        </div>
      );
