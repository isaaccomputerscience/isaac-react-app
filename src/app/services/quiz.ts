...
export function useFilteredQuizzes(user: RegisteredUserDTO) {
  ...
  const [filteredQuizzes, setFilteredQuizzes] = useState<
    Array<QuizSummaryDTO> | undefined
  >();
  ...
  useEffect(() => {
    if (isDefined(titleFilter) && isDefined(quizzes)) {
      const results = quizzes
        .filter(
          (quiz) =>
            quiz.title?.toLowerCase().match(titleFilter.toLowerCase()) ||
            quiz.id?.toLowerCase().match(titleFilter.toLowerCase()),
        )
        .filter(
          (quiz) =>
            isEventLeaderOrStaff(user) ||
            (quiz.hiddenFromRoles
              ? !quiz.hiddenFromRoles?.includes("TEACHER")
              : true),
        );
...
export function useCurrentQuizAttempt(studentId?: number) {
  const currentUserAttemptState = useAppSelector(
    selectors.quizzes.currentQuizAttempt,
  );
  const [currentUserAttempt, currentUserError] = useMemo(() => {
    if (!isDefined(currentUserAttemptState)) return [];
    return [
      "attempt" in currentUserAttemptState
        ? currentUserAttemptState.attempt
        : undefined,
      "error" in currentUserAttemptState
        ? currentUserAttemptState.error
        : undefined,
    ];
  }, [currentUserAttemptState]);

  const studentAttemptState = useAppSelector(
    selectors.quizzes.currentStudentQuizAttempt,
  );
  const [studentAttempt, studentError, studentUser] = useMemo(() => {
    if (!isDefined(studentAttemptState)) return [];
    return [
      "studentAttempt" in studentAttemptState
        ? studentAttemptState.studentAttempt.attempt
        : undefined,
      "error" in studentAttemptState ? studentAttemptState.error : undefined,
      "studentAttempt" in studentAttemptState
        ? studentAttemptState.studentAttempt.user
        : undefined,
    ];
  }, [studentAttemptState]);
  ...
  const attemptWithQuizSubject = attempt
    ? {
        ...attempt,
        quiz: attempt?.quiz && tags.augmentDocWithSubject(attempt.quiz),
      }
    : undefined;
  ...
  useEffect(() => {
    ...
    return () =>
      dispatch(deregisterQuestions(questions.map((q) => q.id as string)));
  }, [dispatch, questions]);

  return {
    attempt: attemptWithQuizSubject,
    error,
    studentUser,
    questions,
    sections,
...
export function partitionCompleteAndIncompleteQuizzes(
  assignmentsAndAttempts: QuizAttemptOrAssignment[],
): [QuizAttemptOrAssignment[], QuizAttemptOrAssignment[]] {
  return partition(assignmentsAndAttempts, (a) =>
    isDefined(isAttempt(a) ? a.completedDate : a.attempt?.completedDate),
  );
}
