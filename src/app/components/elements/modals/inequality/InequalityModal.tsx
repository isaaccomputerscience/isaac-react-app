...
const InequalityMenuContext = React.createContext<
  | {
      ...
      openNewMenuTab: (newMenu: [InequalityMenuTabType, InequalityMenuSubMenuTabType]) => void;
    }
  | undefined
>(undefined);
    ...
    return (
      <div className="top-menu maths other-functions">
        ...
        {activeSubMenu === "trigFunctions" && (
          <ul className="sub-menu trig-functions">{menuItems.mathsTrigFunctions.map(buildIndexedMenuItem)}</ul>
        )}
        {activeSubMenu === "hypFunctions" && (
          <ul className="sub-menu hyp-functions">{menuItems.mathsHypFunctions.map(buildIndexedMenuItem)}</ul>
        )}
        {activeSubMenu === "logFunctions" && (
          <ul className="sub-menu log-functions">{menuItems.mathsLogFunctions.map(buildIndexedMenuItem)}</ul>
        )}
        {activeSubMenu === "derivatives" && (
          <ul className="sub-menu derivatives">{menuItems.mathsDerivatives.map(buildIndexedMenuItem)}</ul>
        )}
      </div>
    );
  } else {
    ...
    return (
      <div className="top-menu letters">
        ...
        {activeSubMenu === "upperCaseLetters" && (
          <ul className="sub-menu uppercaseletters">{menuItems.upperCaseLetters.map(buildIndexedMenuItem)}</ul>
        )}
        {activeSubMenu === "lowerCaseLetters" && (
          <ul className="sub-menu lowercaseletters">{menuItems.lowerCaseLetters.map(buildIndexedMenuItem)}</ul>
        )}
        {editorMode === "maths" && (
          <>
...
const InequalityMenu = React.forwardRef<HTMLDivElement, InequalityMenuProps>(
  ({ open, setOpen, editorMode, logicSyntax, defaultMenu, disableLetters, menuItems }, menuRef) => {
    ...
    const openNewMenuTab: (newMenu: [InequalityMenuTabType, InequalityMenuSubMenuTabType]) => void = ([
      newMenu,
      newSubMenu,
    ]) => {
          ...
          <div className={classNames("inequality-ui menu-bar", open ? "open" : "closed")}>
              ...
              <div className="top-menu numbers">
                <div className="keypad-box">
                  <div className="top-row">
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                      <InequalityMenuNumber key={n} n={n} update={() => updateNumberInputValue(n)} />
                    ))}
                  </div>
                ...
                <div className="input-box">
                  <div
                    className={classNames("menu-item", isDefined(numberInputValue) ? "active" : "inactive")}
                    data-item={
                      isDefined(numberInputValue)
                        ? JSON.stringify({ type: "Num", properties: { significand: `${numberInputValue}` } })
                        : null
                    }
                  >
                    ...
                    <VHexagon />
                    {isDefined(numberInputValue) ? (
                      <Markup encoding={"latex"} className={"d-block"}>{`$${numberInputValue}$`}</Markup>
                    ) : (
            ...
            {activeMenu === "basicFunctions" && (
              <div className="top-menu basic-functions">
                {editorMode === "logic" && (
                  <ul className="sub-menu">{menuItems.logicFunctionsItems.map(buildIndexedMenuItem)}</ul>
                )}
                {editorMode === "maths" && (
                  <ul className="sub-menu">{menuItems.mathsBasicFunctionsItems.map(buildIndexedMenuItem)}</ul>
                )}
              </div>
            )}
...
const InequalityModal = ({
  ...
  const parsedAvailableSymbols = useMemo(
    () => Array.from(new Set(parsePseudoSymbolicAvailableSymbols(availableSymbols))).filter((s) => s.trim() !== ""),
    [availableSymbols],
  );
  ...
  return (
    <div id="inequality-modal" ref={inequalityModalRef}>
      <div className="inequality-ui confirm button" role="button" tabIndex={-1} onClick={close} onKeyUp={close}>
        OK
      </div>
      <Markup
        encoding={"latex"}
        className={classNames("d-block inequality-ui katex-preview", previewTexString === "" ? "empty" : "")}
      >
      ...
      <div
        id="inequality-trash"
        className={classNames("inequality-ui trash button", { "trash-with-reset": showReset })}
      >
        Trash
      </div>

      {showQuestionReminder && (questionDoc?.value || (questionDoc?.children && questionDoc?.children?.length > 0)) && (
      ...
      <div className="orientation-warning">
        The Isaac Equation Editor may only be used in landscape mode. Please rotate your device.
      </div>
