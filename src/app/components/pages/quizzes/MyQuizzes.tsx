            ...
            {assignment ? (
              <>
                {status === Status.Unstarted && (
                  <RS.Button
                    tag={Link}
                    to={`/test/assignment/${assignment.id}`}
                  >
                    Start test
                  </RS.Button>
                )}
                ...
                {status === Status.Complete &&
                  (assignment.quizFeedbackMode !== "NONE" ? (
                    <RS.Button
                      tag={Link}
                      to={`/test/attempt/${assignment.attempt?.id}/feedback`}
                    >
                      View feedback
                    </RS.Button>
                  ) : (
              ...
              attempt && (
                <>
                  {status === Status.Started && (
                    <RS.Button
                      tag={Link}
                      to={`/test/attempt/${attempt.quizId}`}
                    >
                      Continue test
                    </RS.Button>
                  )}
                  {status === Status.Complete &&
                    (attempt.feedbackMode !== "NONE" ? (
                      <RS.Button
                        tag={Link}
                        to={`/test/attempt/${attempt.id}/feedback`}
                      >
                        View feedback
                      </RS.Button>
                    ) : (
    ...
    <>
      ...
      {quizzes.length > 0 && (
        <div className="block-grid-xs-1 block-grid-md-2 block-grid-lg-3 my-2">
          {quizzes.map((item) => (
            <QuizItem
              key={(isAttempt(item) ? "at" : "as") + item.id}
              item={item}
            />
          ))}
        </div>
...
const MyQuizzesPageComponent = ({ user }: MyQuizzesPageProps) => {
  ...
  const pageHelp = (
    <span>
      ...
      <br />
      You can also take some tests freely whenever you want to test your
      knowledge.
    </span>
  );
  ...
  const [completedQuizzes, incompleteQuizzes] =
    partitionCompleteAndIncompleteQuizzes(assignmentsAndAttempts);
      ...
      case "TUTOR":
        return (
          (quiz.hiddenFromRoles &&
            !quiz.hiddenFromRoles?.includes("STUDENT")) ||
            ...
            <ShowLoading
              ...
              ifNotFound={
                <RS.Alert color="warning">
                  Your test assignments failed to load, please try refreshing
                  the page.
                </RS.Alert>
              }
            >
              <QuizGrid
                ...
                empty="You don't have any incomplete or assigned tests."
              />
            </ShowLoading>
          ),
          ...
          ["Completed tests"]: (
            ...
            >
              <QuizGrid
                ...
                empty="You haven't completed any tests."
              />
            </ShowLoading>
          ),
                    ...
                    {quizzes.filter(showQuiz).map((quiz) => (
                      ...
                      >
                        <div className="d-flex flex-grow-1 flex-column flex-sm-row align-items-center p-3">
                          ...
                          <RS.Button
                            tag={Link}
                            to={{ pathname: `/test/attempt/${quiz.id}` }}
                          >
                            Take test
                          </RS.Button>
                        </div>
