...
const canSendPasswordResetRequest = function (
  user: AppGroupMembership,
  passwordRequestSent: boolean,
) {
  return (
    !passwordRequestSent &&
    user.authorisedFullAccess &&
    user.emailVerificationStatus != "DELIVERY_FAILED"
  );
};

const passwordResetInformation = function (
  ...
  if (canSendPasswordResetRequest(member, passwordRequestSent)) {
    ...
    message =
      "Password reset request cannot be sent because this user's account email address is either invalid or not accepting email.";
  }
  return message;
};
...
const MemberInfo = ({ group, member, user }: MemberInfoProps) => {
  ...
  function confirmDeleteMember() {
    if (
      confirm(
        `Are you sure you want to remove this user from the group '${group.groupName}'?`,
      )
    ) {
      deleteMember({
  ...
  const userHasAdditionalGroupPrivileges =
    (isDefined(user.id) && isDefined(group.ownerId) && user.id === group.ownerId
      ? true
      : group.additionalManagerPrivileges) ?? false;

  return (
    <div
      className="p-2 member-info-item d-flex justify-content-between"
        ...
        <div>
          {member.emailVerificationStatus == "DELIVERY_FAILED" && (
            <Tooltip
              tipText={
                <>
                  This user&apos;s account email address is invalid or not
                  accepting email.
                  <br />
                  They will not be able to reset their password or receive
                  update emails. Ask them to login and update it, or contact us
          ...
          {member.groupMembershipInformation &&
            member.groupMembershipInformation.status == "INACTIVE" && (
          ...
          <>
            ...
            >
              <Button
                ...
                disabled={
                  !canSendPasswordResetRequest(member, passwordRequestSent)
                }
              >
...
const GroupEditor = ({
  ...
  createNewGroup,
  groupNameInputRef,
}: GroupCreatorProps & GroupEditorProps) => {
  const dispatch = useAppDispatch();
  ...
  function saveUpdatedGroup(event: React.FormEvent) {
    ...
    if (!newGroupName || newGroupName.length === 0) {
      dispatch(
        showErrorToast(
          `Cannot ${group ? "rename" : "create"} group`,
    ...
    if (group) {
      ...
      updateGroup({
        ...
        message:
          "Group " +
          group.groupName +
          (updatedGroup.archived ? " archived" : " unarchived"),
      });
    }
  }
  ...
  function groupUserIds(group?: AppGroup) {
    ...
    group &&
      ...
      group.members.map(
        (member: AppGroupMembership) =>
          member.groupMembershipInformation.userId &&
          member.authorisedFullAccess &&
          ...
          {group && (
            <Col xs={7} sm={6} md={9} lg={9} className="text-right">
              ...
              {isTeacherOrAbove(user) && (
                <>
                  ...
                  >
                    ...
                    <span className="d-none d-xl-inline"> group</span> managers
                  </Button>
              ...
              <Button
                ...
                onClick={() =>
                  dispatch(
                    showGroupInvitationModal({ group, user, firstTime: false }),
                  )
                }
              >
                Invite users
...
const MobileGroupCreatorComponent = ({
  className,
  createNewGroup,
}: GroupCreatorProps & { className: string }) => {
  const [newGroupName, setNewGroupName] = useState("");
  ...
  return (
    <Row className={className}>
      ...
      <Col size={12} className="mb-2">
        <Input
          length={50}
          placeholder="Enter the name of your group here"
          value={newGroupName}
          onChange={(e) => setNewGroupName(e.target.value)}
          aria-label="Group Name"
        />
...
export const Groups = ({ user }: { user: RegisteredUserDTO }) => {
  ...
  const createNewGroup: (newGroupName: string) => Promise<boolean> = async (
    newGroupName: string,
  ) => {
  ...
  const confirmDeleteGroup = (groupToDelete: AppGroup) => {
    if (user.id === groupToDelete.ownerId) {
      ...
      if (
        confirm(
          "You cannot delete this group, because you are not the group owner.  Do you want to remove yourself as a manager of '" +
            groupToDelete.groupName +
            "'?",
        )
      ) {
        dispatch(
          showAdditionalManagerSelfRemovalModal({ group: groupToDelete, user }),
        );
      }
    }
  };
  ...
  const pageHelp = (
    <span>
      Use this page to manage groups. You can add users to a group by giving
      them the group code and asking them paste it into their account settings
      page.
      <br />
      You can find the code for an existing group by selecting the group and
      clicking <i>Invite Users</i>.
    </span>
  );
  ...
  return (
    <Container>
      <TitleAndBreadcrumb
        currentPageTitle="Manage groups"
        className="mb-4"
        help={pageHelp}
        modalId="groups_help"
      />
                    ...
                    return (
                      <NavItem key={index} className="mx-2">
                        <NavLink
                          ...
                          onKeyDown={ifKeyIsEnter(tab.activate)}
                        >
                          {tab.name}
                        </NavLink>
                      </NavItem>
                  ...
                  <Col>
                    {sortedGroups &&
                      sortedGroups.map((g: AppGroup) => (
                        <div
                          key={g.id}
                          className="group-item p-2"
                          data-testid={"group-item"}
                        >
                          <div className="d-flex justify-content-between align-items-center">
                            <Button
                              color="link text-left"
