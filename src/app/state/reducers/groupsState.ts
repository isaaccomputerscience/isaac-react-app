...
export const activeAuthorisations = (
  activeAuthorisations: ActiveAuthorisationsState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const otherUserAuthorisations = (
  otherUserAuthorisations: OtherUserAuthorisationsState = null,
  action: Action,
) => {
  switch (action.type) {
...
const groupMembership = (
  ...
  switch (action.type) {
    case ACTION_TYPE.GROUP_CHANGE_MEMBERSHIP_STATUS_RESPONSE_SUCCESS:
      return {
        membershipStatus: action.newStatus,
...
export const groupMemberships = (
  groupMemberships: GroupMembershipsState = null,
  action: Action,
) => {
  switch (action.type) {
    ...
    case ACTION_TYPE.GROUP_CHANGE_MEMBERSHIP_STATUS_RESPONSE_SUCCESS:
      return (
        ...
        groupMemberships.map((m) =>
          m.group.id === action.groupId ? groupMembership(m, action) : m,
        )
      );
    default:
