...
export const TeacherRequest = () => {
  ...
  const { data: warningFragment } =
    isaacApi.endpoints.getPageFragment.useQuery(warningFragmentId);

  const [firstName, setFirstName] = useState(
    (user?.loggedIn && user.givenName) || "",
  );
  const [lastName, setLastName] = useState(
    (user?.loggedIn && user.familyName) || "",
  );
  ...
  const [emailVerified, setEmailVerified] = useState(
    user?.loggedIn && user.emailVerificationStatus === "VERIFIED",
  );
              ...
              {isTeacherOrAbove(user) && (
                <Row>
                  <Col className="text-center pt-3">
                    ...
                    <p className="mt-3">
                      Go to the <Link to="/teachers">Teacher tools page</Link>{" "}
                      to start using your new account features.
                    </p>
                  </Col>
                </Row>
              )}
                ...
                (messageSent && !errorMessage ? (
                  <Row>
                    <Col className="text-center">
                      ...
                      <p>
                        We will be in touch shortly. Please note that account
                        verification is a manual process and may take a few
                        days.
                      </p>
                    </Col>
                  </Row>
                    ...
                    <CardBody>
                      ...
                      <Row>
                        <Col size={12} md={6}>
                          <FormGroup>
                            <Label
                              htmlFor="user-verification-input"
                              className="form-required"
                            >
                              URL of a page on your school website which shows
                              your name and email address, or your school phone
                              number
                            </Label>
                            <Input
                        ...
                        <Row>
                          <Col>
                            <small className="text-danger text-left">
                              ...
                              on the link in the verification email to confirm
                      ...
                      {typeof school == "undefined" && (
                        <Row>
                          <Col>
                            <small className="text-danger text-left">
                              You have not provided your school — please add
                              your school on your{" "}
                              <Link to="/account">My Account</Link> page. If you
                              are a private tutor or parent, you can{" "}
                              <Link to="/tutor_account_request">
                                request an Isaac {SITE_SUBJECT_TITLE} Tutor
                                account
                              </Link>
                              .
                            </small>
                      ...
                      {allowedDomain == false && (
                        <Row>
                          <Col>
                            <small className="text-danger text-left">
                              You have not used your school email address —
                              please change your email address on your{" "}
