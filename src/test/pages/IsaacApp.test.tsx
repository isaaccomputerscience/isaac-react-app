...
describe("IsaacApp", () => {
  ...
  it("should show featured news pods before non-featured ones, and order pods correctly based on id (CS only)", async () => {
    ...
    const featuredNewsPod =
      await within(featuredNewsSection).findByTestId("news-pod");
    const newsCarouselPods =
      await within(newsCarousel).findAllByTestId("news-pod");
    const allNewsPodsInOrder = [featuredNewsPod].concat(newsCarouselPods);
    const newsPodLinks = allNewsPodsInOrder.map(
      (p) => within(p).queryAllByRole("link")[0]?.getAttribute("href"),
    );
