...
export const ChunkOrClientError = ({
  resetErrorBoundary,
  error,
}: FallbackProps) => {
  ...
  return isChunkError ? (
    ...
    <ClientError error={error} resetErrorBoundary={resetErrorBoundary} />
  );
};
...
export const ClientError = ({ resetErrorBoundary, error }: FallbackProps) => {
  ...
  const usefulInformationLabels: {
    [k in keyof typeof usefulInformation]: string;
  } = {
    userId: "User ID",
    location: "Location",
  ...
  const plainTextUsefulInformation =
    ...
    Object.entries(usefulInformation)
      .map(
        ([key, value]) =>
          `${
            usefulInformationLabels[key as keyof typeof usefulInformation]
          }: ${value}`,
      )
      .join("\n\n");
  ...
  return (
    <Container>
      <div>
        ...
        <h3 className="my-4">
          <small>
            {"We're sorry, but an error has occurred in the Isaac app!"}
          </small>
        </h3>
        <h3>
          <small>
            ...
            {", or "}
            <Link
              to={`/contact?subject=App Error&message=${encodeURI(
                plainTextUsefulInformation,
              )}`}
              onKeyPress={() => resetErrorBoundary()}
              onClick={() => resetErrorBoundary()}
            >
                ...
                <small>
                  {Object.entries(usefulInformation).map(([key, value]) => (
                    <p key={key}>
                      <strong>
                        {
                          usefulInformationLabels[
                            key as keyof typeof usefulInformation
                          ]
                        }
                        :{" "}
                      </strong>
