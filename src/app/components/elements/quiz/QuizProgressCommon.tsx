...
export function markQuestionClasses(
  row: QuizUserFeedbackDTO,
  mark: Mark | undefined,
  totalOrUndefined: number | undefined,
) {
...
export function ResultRow({ row, assignment }: ResultRowProps) {
  ...
  const _returnToStudent = async () => {
    try {
      setWorking(true);
      await dispatch(
        returnQuizToStudent(assignment.id as number, row.user?.id as number),
      );
    } finally {
      setWorking(false);
      dispatch(closeActiveModal());
  ...
  return (
    <tr
      className={`${row.user?.authorisedFullAccess ? "" : " not-authorised"}`}
      title={`${row.user?.givenName + " " + row.user?.familyName}`}
    >
      <td className="student-name">
        {valid ? (
          <>
            ...
            {!working && dropdownOpen && (
              <div className="py-2 px-3">
                <Button size="sm" onClick={returnToStudent}>
      ...
      {!valid && (
        <td colSpan={sections.map(questionsInSection).flat().length + 1}>
          {message}
        </td>
      )}
      {valid && (
        <>
          {sections.map((section) => {
            ...
            return questionsInSection(section)
              .map((question) => {
                const questionMark =
                  row.feedback?.questionMarks?.[question.id as string] ||
                  ({} as Mark);
                const icon =
                ...
                return (
                  <td
                    key={question.id}
                    className={markQuestionClasses(row, mark, outOf)}
                  >
                    {icon}
                  </td>
                );
          ...
          <th className="total-column">
            {formatMark(
              row.feedback?.overallMark?.correct as number,
      ...
      <table className="progress-table border">
        <tbody>
          <tr className="bg-white">
            <th rowSpan={2} className="bg-white border-bottom student-name">
              &nbsp;
            </th>
            {sections.map((section) => (
              <th
                key={section.id}
                colSpan={questionsInSection(section).length}
                className="border font-weight-bold"
              >
                {section.title}
              </th>
            ))}
            <th rowSpan={2} className="border-bottom total-column">
              Overall
            </th>
          </tr>
          <tr className="bg-white">
            {sections
              .map((section) =>
                questionsInSection(section).map((question, index) => (
                  <th key={question.id} className="border">
                    {`Q${index + 1}`}
                  </th>
                )),
              )
