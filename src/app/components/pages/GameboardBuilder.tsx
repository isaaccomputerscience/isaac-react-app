...
const GameboardBuilderRow = lazy(
  () => import("../elements/GameboardBuilderRow"),
);
...
const GameboardBuilder = ({ user }: { user: RegisteredUserDTO }) => {
  ...
  const { data: baseGameboard } = isaacApi.endpoints.getGameboardById.useQuery(
    baseGameboardId || skipToken,
  );
  const [generateTemporaryGameboard] =
    isaacApi.endpoints.generateTemporaryGameboard.useMutation();
  const [createGameboard, { isLoading: isWaitingForCreateGameboard }] =
    isaacApi.endpoints.createGameboard.useMutation();
  ...
  const cloneGameboard = useCallback(
    (gameboard: GameboardDTO) => {
      ...
      setSelectedQuestions(
        loadGameboardSelectedQuestions(gameboard) ||
          new Map<string, ContentSummary>(),
      );
      setWildcardId(
        (isStaff(user) && gameboard.wildCard && gameboard.wildCard.id) ||
          undefined,
      );
  ...
  const canSubmit =
    ...
    selectedQuestions.size <= 10 &&
    gameboardTitle != "" &&
    isValidGameboardId(gameboardURL);
  ...
  useEffect(() => {
    return history.block(() => {
      ...
      dispatch(
        logAction({ type: "LEAVE_GAMEBOARD_BUILDER", events: eventLog }),
      );
    });
  }, []);
  ...
  const pageHelp = (
    <span>
      You can create custom question sets to assign to your groups. Search by
      question title or topic and add up to ten questions to a gameboard.
      <br />
      You cannot modify a gameboard after it has been created. You&apos;ll find
      a link underneath any existing gameboard to duplicate and edit it.
    </span>
  );
  ...
  return (
    <Container id="gameboard-builder">
      <div ref={sentinel} />
      <TitleAndBreadcrumb
        currentPageTitle="Gameboard builder"
        help={pageHelp}
        modalId="gameboard_builder_help"
      />
        ...
        <CardBody>
          ...
          {isStaff(user) && (
            <Row className="mt-2">
              <Col>
                ...
                <Select
                  inputId="question-search-level"
                  isMulti
                  options={[
                    { value: "ISAAC_BOARD", label: "Created by Isaac" },
                    {
                      value: "CONFIDENCE_RESEARCH_BOARD",
                      label: "Confidence research board",
                    },
                  ]}
                  name="colors"
                  value={gameboardTags}
                ...
                >
                  ...
                  {isDefined(wildcards) &&
                    wildcards.map((wildcard) => {
                      return (
                        <option key={wildcard.id} value={wildcard.id}>
                          {wildcard.title}
                        </option>
                      );
                    })}
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (
                  ...
                  wildcard = wildcards.filter(
                    (wildcard) => wildcard.id == wildcardId,
                  )[0];
                }
                ...
                createGameboard({
                  gameboard: {
                    ...
                    contents: questionOrder
                      .map((questionId) => {
                        const question = selectedQuestions.get(questionId);
                        return (
                          question &&
                          convertContentSummaryToGameboardItem(question)
                        );
                      })
                      .filter(
                  ...
                  const error =
                    "error" in gameboardOrError
                      ? gameboardOrError.error
                      : undefined;
                  const gameboardId =
                    "data" in gameboardOrError
                      ? gameboardOrError.data.id
                      : undefined;
                  dispatch(
                    openActiveModal({
                ...
                dispatch(
                  logAction({ type: "SAVE_GAMEBOARD", events: eventLog }),
                );
              }}
            >
          ...
          {!canSubmit && (
            <div
              id="gameboard-help"
              color="light"
              className={`text-center mb-0 pt-3 pb-0 ${
                selectedQuestions.size > 10 ? "text-danger" : ""
              }`}
            >
              Gameboards require both a title and between 1 and 10 questions.
              {!isValidGameboardId(gameboardURL) && (
                <div className="text-danger">
                  The gameboard ID should contain numbers, lowercase letters,
