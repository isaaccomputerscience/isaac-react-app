...
const MenuItem = (props: MenuItemProps) => {
  return (
    ...
    >
      <VHexagon />
      <Markup encoding={"latex"}>{`$${props.menu.label}$`}</Markup>
    </li>
  );
};
const buildIndexedMenuItem = (props: MenuItemProps, i: number) => (
...
export const VHexagon = () => (
  <svg
    ...
    x="0px"
    y="0px"
    viewBox="0 0 173.5 200"
    className="v-hexagon"
    xmlSpace="preserve"
    enableBackground={"new 0 0 173.5 200"}
  >
...
export const TabTriangle = () => (
  <svg
    ...
    xmlSpace="preserve"
    enableBackground={"new 0 0 76 23"}
  >
    <polygon points="0,0 136,0 68,23" />
  </svg>
);

type InequalityMenuTabType =
  | "numbers"
  | "letters"
  | "basicFunctions"
  | "mathsOtherFunctions"
  ...
  | "operations"
  | null;
type InequalityMenuSubMenuTabType =
  | "upperCaseLetters"
  | "lowerCaseLetters"
  | "trigFunctions"
  | "hypFunctions"
  ...
  | null;
const InequalityMenuContext = React.createContext<
  | {
      activeMenu: [InequalityMenuTabType, InequalityMenuSubMenuTabType];
      openNewMenuTab: (
        newMenu: [InequalityMenuTabType, InequalityMenuSubMenuTabType],
      ) => void;
    }
  | undefined
>(undefined);

const InequalityMenuTab = ({
  menu,
  latexTitle,
  subMenu,
  className,
  ...
  menu: InequalityMenuTabType;
  subMenu?: InequalityMenuSubMenuTabType;
  latexTitle: string;
  className?: string;
  isSubMenu?: boolean;
}) => {
  ...
  const {
    activeMenu: [activeMenu, activeSubMenu],
    openNewMenuTab,
  } = menuContext;

  const navigate = () => openNewMenuTab([menu, subMenu ?? null]);
  const active =
    activeMenu === menu && (isSubMenu ? activeSubMenu === subMenu : true);

  return (
    <li
      className={classNames(active ? "active" : "inactive", className)}
      onClick={navigate}
      onKeyUp={navigate}
    >
  ...
  return (
    <div
      ...
      data-item={JSON.stringify({
        type: "Num",
        properties: { significand: n.toString() },
      })}
      onClick={update}
      onTouchEnd={update}
      onKeyUp={update}
    >
      <VHexagon />
      <Markup encoding={"latex"}>{`$${n.toString()}$`}</Markup>
    </div>
  );
};

const MathOtherFunctionsMenu = ({
  defaultMenu,
  menuItems,
  activeSubMenu,
}: {
  defaultMenu: boolean;
  menuItems: MenuItems;
  activeSubMenu: InequalityMenuSubMenuTabType;
}) => {
  if (
    defaultMenu ||
    (menuItems.otherFunctions.length === 0 &&
      menuItems.mathsDerivatives.length === 0)
  ) {
    return (
      <div className="top-menu maths other-functions">
        <ul className="sub-menu-tabs">
          <InequalityMenuTab
            isSubMenu
            menu={"mathsOtherFunctions"}
            subMenu={"trigFunctions"}
            className={"trig-functions"}
            latexTitle={"\\sin"}
          />
          <InequalityMenuTab
            isSubMenu
            menu={"mathsOtherFunctions"}
            subMenu={"hypFunctions"}
            className={"hyp-functions"}
            latexTitle={"\\text{hyp}"}
          />
          <InequalityMenuTab
            isSubMenu
            menu={"mathsOtherFunctions"}
            subMenu={"logFunctions"}
            className={"log-functions"}
            latexTitle={"\\text{log}"}
          />
          {menuItems.mathsDerivatives.length > 0 && (
            <InequalityMenuTab
              ...
              latexTitle={"\\frac{\\mathrm{d}y}{\\mathrm{d}x}"}
            />
          )}
        </ul>
        {activeSubMenu === "trigFunctions" && (
          <ul className="sub-menu trig-functions">
            {menuItems.mathsTrigFunctions.map(buildIndexedMenuItem)}
          </ul>
        )}
        {activeSubMenu === "hypFunctions" && (
          <ul className="sub-menu hyp-functions">
            {menuItems.mathsHypFunctions.map(buildIndexedMenuItem)}
          </ul>
        )}
        {activeSubMenu === "logFunctions" && (
          <ul className="sub-menu log-functions">
            {menuItems.mathsLogFunctions.map(buildIndexedMenuItem)}
          </ul>
        )}
        {activeSubMenu === "derivatives" && (
          <ul className="sub-menu derivatives">
      ...
      <div className="top-menu letters">
        <ul className="sub-menu-tabs">
          <InequalityMenuTab
            isSubMenu
            menu={"letters"}
            subMenu={"lowerCaseLetters"}
            latexTitle={"ab"}
          />
          <InequalityMenuTab
            isSubMenu
            menu={"letters"}
            subMenu={"upperCaseLetters"}
            latexTitle={"AB"}
          />
          {editorMode === "maths" && (
            <>
              <InequalityMenuTab
                isSubMenu
                menu={"letters"}
                subMenu={"lowerCaseGreekLetters"}
                latexTitle={"αβ"}
              />
              <InequalityMenuTab
                isSubMenu
                menu={"letters"}
                subMenu={"upperCaseGreekLetters"}
                latexTitle={"ΓΔ"}
              />
            </>
          )}
        ...
        {activeSubMenu === "upperCaseLetters" && (
          <ul className="sub-menu uppercaseletters">
            {menuItems.upperCaseLetters.map(buildIndexedMenuItem)}
          </ul>
        )}
        {activeSubMenu === "lowerCaseLetters" && (
          <ul className="sub-menu lowercaseletters">
            {menuItems.lowerCaseLetters.map(buildIndexedMenuItem)}
          </ul>
        )}
        {editorMode === "maths" && (
          <>
            {activeSubMenu === "upperCaseGreekLetters" && (
              <ul className="sub-menu uppercasegreekletters">
                {menuItems.upperCaseGreekLetters.map(buildIndexedMenuItem)}
              </ul>
            )}
            {activeSubMenu === "lowerCaseGreekLetters" && (
              <ul className="sub-menu lowercasegreekletters">
                {menuItems.lowerCaseGreekLetters.map(buildIndexedMenuItem)}
              </ul>
            )}
          </>
        )}
    ...
    return (
      <div className="top-menu letters">
        <ul className="sub-menu letters">
          {menuItems.letters.map(buildIndexedMenuItem)}
        </ul>
      </div>
    );
  }
...
const InequalityMenu = React.forwardRef<HTMLDivElement, InequalityMenuProps>(
  (
    ...
    >([null, editorMode === "logic" ? "upperCaseLetters" : "lowerCaseLetters"]);
    const openNewMenuTab: (
      newMenu: [InequalityMenuTabType, InequalityMenuSubMenuTabType],
    ) => void = ([newMenu, newSubMenu]) => {
    ...
    const updateNumberInputValue = (n: number) => {
      if (!isDefined(numberInputValue) || numberInputValue === 0)
        return setNumberInputValue(n);
      // Past this point, we know that `numberInputValue` is defined and non-zero
      setNumberInputValue(
        numberInputValue * 10 + Math.sign(numberInputValue) * n,
      );
    };
    const flipNumberInputValueSign = () =>
      setNumberInputValue(-(numberInputValue ?? 0));
    ...
    const [unparsedChemicalElements, setUnparsedChemicalElements] =
      useState<string>();
    const onUnparsedChemicalElementsChange = (
      event: FormEvent<HTMLInputElement>,
    ) => setUnparsedChemicalElements(event.currentTarget.value);
    const [parsedChemicalElements, upperCaseWarning] = useMemo<
      [MenuItemProps[] | undefined, boolean]
    >(() => {
      if (isDefined(unparsedChemicalElements)) {
        const splitUnparsed = unparsedChemicalElements
          ...
          .split(",")
          .filter((s) => s !== "");
        const splitChemicalElements = splitUnparsed.filter((s) =>
          CHEMICAL_ELEMENTS.includes(s),
        );
        const upperCaseWarning = splitUnparsed.some(
          (e) => e[0] !== e[0].toUpperCase(),
        );
        return [
          uniq(splitChemicalElements)
            .map(generateChemicalElementMenuItem)
            .filter(isDefined),
          upperCaseWarning,
        ];
      }
    ...
    return (
      <nav className="inequality-ui" ref={menuRef}>
        <InequalityMenuContext.Provider
          value={{ activeMenu: [activeMenu, activeSubMenu], openNewMenuTab }}
        >
          <div
            className={classNames(
              "inequality-ui menu-bar",
              open ? "open" : "closed",
            )}
          >
            {activeMenu === "numbers" && (
              <div className="top-menu numbers">
                <div className="keypad-box">
                  <div className="top-row">
                    {[1, 2, 3, 4, 5, 6].map((n) => (
                      <InequalityMenuNumber
                        key={n}
                        n={n}
                        update={() => updateNumberInputValue(n)}
                      />
                    ))}
                  </div>
                  <div className="bottom-row">
                    ...
                    >
                      <VHexagon />
                      <Markup encoding={"latex"}>{"$\\pm$"}</Markup>
                    </div>
                  </div>
                </div>
                <div className="input-box">
                  <div
                    className={classNames(
                      "menu-item",
                      isDefined(numberInputValue) ? "active" : "inactive",
                    )}
                    data-item={
                      isDefined(numberInputValue)
                        ? JSON.stringify({
                            type: "Num",
                            properties: { significand: `${numberInputValue}` },
                          })
                        : null
                    }
                  >
                  ...
                  {isDefined(numberInputValue) && (
                    <div
                      ...
                      role="button"
                      tabIndex={0}
                      onClick={clearNumberInputValue}
                      onKeyUp={clearNumberInputValue}
                    />
                  )}
                </div>
              </div>
            )}
            {activeMenu === "letters" && !disableLetters && (
              <LettersMenu
            ...
            {activeMenu === "basicFunctions" && (
              <div className="top-menu basic-functions">
                {editorMode === "logic" && (
                  <ul className="sub-menu">
                    {menuItems.logicFunctionsItems.map(buildIndexedMenuItem)}
                  </ul>
                )}
                {editorMode === "maths" && (
                  <ul className="sub-menu">
                    {menuItems.mathsBasicFunctionsItems.map(
                      buildIndexedMenuItem,
                    )}
                  </ul>
                )}
              </div>
                      ...
                      <div className="input-box">
                        ...
                        {upperCaseWarning && (
                          <p className="uppercase-warning">
                            Careful: chemical element names start with an
                            upper-case letter.
                          </p>
                        )}
                      </div>
            ...
            <ul>
              ...
              {editorMode === "chemistry" && (
                <>
                  ...
                  <InequalityMenuTab
                    menu={"operations"}
                    latexTitle={"\\rightarrow\\, \\rightleftharpoons\\, +"}
                  />
                </>
              )}
            </ul>
...
const InequalityModal = ({
  ...
  editorSeed,
}: InequalityModalProps) => {
  const parsedAvailableSymbols = useMemo(
    () =>
      Array.from(
        new Set(parsePseudoSymbolicAvailableSymbols(availableSymbols)),
      ).filter((s) => s.trim() !== ""),
    [availableSymbols],
  );
  ...
  const [showQuestionReminder, setShowQuestionReminder] =
    useState<boolean>(true);
  const onQuestionReminderClick = () =>
    setShowQuestionReminder((prev) => !prev);
  ...
  const showHelpModal = () =>
    dispatch(
      openActiveModal({
        ...
        title: "Quick Help",
        body: (
          <PageFragment fragmentId={`eqn_editor_help_modal_${editorMode}`} />
        ),
      }),
    );
  ...
  const prepareAbsoluteElement = useCallback((element?: Element | null) => {
    if (element) {
      ...
      if (menuItem) {
        potentialSymbolSpec.current = JSON.parse(
          menuItem.getAttribute("data-item") || "",
        );
  ...
  const onMouseDown = useCallback((e: MouseEvent) => {
    ...
    const element = document.elementFromPoint(
      previousCursor.current.x,
      previousCursor.current.y,
    );
  ...
  return (
    <div id="inequality-modal" ref={inequalityModalRef}>
      <div
      ...
      <Markup
        encoding={"latex"}
        className={classNames(
          "d-block inequality-ui katex-preview",
          previewTexString === "" ? "empty" : "",
        )}
      >
      ...
      {showReset && (
        <div
          id="inequality-reset"
          className="inequality-ui reset button"
          role="button"
          tabIndex={-1}
          onClick={resetToInitialState}
          onKeyUp={resetToInitialState}
        >
          Reset
        </div>
      )}
      <div
        id="inequality-trash"
        className={classNames("inequality-ui trash button", {
          "trash-with-reset": showReset,
        })}
      >
        Trash
      </div>
      ...
      {showQuestionReminder &&
        (questionDoc?.value ||
          (questionDoc?.children && questionDoc?.children?.length > 0)) && (
          <div className="question-reminder">
            <IsaacContentValueOrChildren
              value={questionDoc.value}
              encoding={questionDoc.encoding}
            >
      ...
      <div className="orientation-warning">
        The Isaac Equation Editor may only be used in landscape mode. Please
        rotate your device.
      </div>
