...
function itemiseByValue<R extends { value: string }>(
  values: string[],
  options: R[],
) {
...
function itemiseConcepts(concepts: string[]): Item<string>[] {
  return concepts
    ...
    .map((concept) => ({
      label: QUESTION_FINDER_CONCEPT_LABEL_PLACEHOLDER,
      value: concept,
    }));
}
...
function processQueryString(query: string): QueryStringResponse {
  ...
  const examBoardItems = itemiseByValue(
    ...
    getFilteredExamBoardOptions({
      byStages: stageItems.map((item) => item.value as STAGE),
    }),
  );
  ...
  const conceptItems = itemiseConcepts(
    arrayFromPossibleCsv(concepts as Nullable<string[] | string>),
  );
  ...
  const topicTags = topics
    ? tags.getSpecifiedTags(
        tagHierarchy[2],
        (topics instanceof Array ? topics : topics.split(",")) as TAG_ID[],
      )
    : null;
  const fieldTags =
    topicTags &&
    Array.from(
      new Set(topicTags.map((tag) => tag.parent).filter(isDefined)),
    ).map((tagId) => itemiseTag(tags.getById(tagId)));
...
function generateCSBoardName(selections: Item<TAG_ID>[][]) {
  ...
  while (selectionIndex-- > 0) {
    if (
      selections[selectionIndex].length > 0 &&
      selections[selectionIndex].length <= 2
    ) {
...
const itemiseAndGroupConceptsByTag =
  (conceptDTOs: ContentSummaryDTO[]) => (tag: Item<TAG_ID>) => {
    return {
      label: tag.label,
...
const CSFilter = ({
    ...
    function updateConceptChoices() {
      ...
      if (availableConcepts.length > 0) {
        const conceptsFilteredByAvailable = concepts.filter(
          (c) =>
            availableConcepts.find((ac) => isItemEqual(Object.is, ac, c)) !==
            undefined,
        );
        if (
          concepts.filter(
            (c) => c.label === QUESTION_FINDER_CONCEPT_LABEL_PLACEHOLDER,
          ).length > 0
        ) {
          setConcepts(
            concepts.reduce((acc: Item<string>[], c) => {
              const newLabel = availableConcepts.find(
                (ac) => ac.value === c.value,
              )?.label;
              return newLabel
                ? acc.concat([{ value: c.value, label: newLabel }])
                : acc;
            }, []),
          );
  ...
  function setTierSelection(topics: Item<TAG_ID>[]) {
    ...
    setSelections([
      [itemiseTag(tags.getById(TAG_ID.computerScience))],
      Array.from(strands).map(itemiseTag),
      topics,
    ]);
  }
      ...
      <RS.Row>
        ...
        <RS.Col md={6}>
          ...
          <Select
            ...
            placeholder="Any"
            value={examBoards}
            options={getFilteredExamBoardOptions({
              byStages: stages.map((item) => item.value as STAGE),
            })}
      ...
      <RS.Row className="mt-lg-3 mb-sm-3">
        <RS.Col md={6}>
          <RS.Label className={`mt-2 mt-lg-0`} htmlFor="difficulty-selector">
            ...
            <RS.UncontrolledTooltip
              target={`difficulty-help-tooltip`}
              placement="bottom"
            >
              ...
              <br />
              C2 questions cover more than one concept which must be selected
              and combined with skill.
            </RS.UncontrolledTooltip>
          </RS.Label>
          <Select
...
export const GameboardFilter = withRouter(
  ({ location }: RouteComponentProps) => {
    ...
    const [gameboard, setGameboard] = useState<
      GameboardDTO | NOT_FOUND_TYPE | null | undefined
    >();
    const gameboardIdAnchor = location.hash ? location.hash.slice(1) : null;
    const [generateTemporaryGameboard] =
      isaacApi.endpoints.generateTemporaryGameboard.useMutation();
    ...
    const [selections, setSelections] =
      useState<Item<TAG_ID>[][]>(querySelections);
    ...
    const [stages, setStages] = useState<Item<string>[]>(
      queryStages.length > 0
        ? queryStages
        : itemiseByValue([userContext.stage], getFilteredStageOptions()),
    );
    useEffect(
    ...
    const [difficulties, setDifficulties] =
      useState<Item<string>[]>(queryDifficulties);
    ...
    const [pendingCustomBoardTitle, setPendingCustomBoardTitle] =
      useState<string>();
    ...
    function loadNewGameboard(
      ...
      generateTemporaryGameboard(params)
        ...
        .then((gameboard) => {
          ...
          history.replace({
            search: queryString.stringify(params, { encode: false }),
          });
        });
    }
    ...
    useEffect(() => {
      if (
        gameboardIdAnchor &&
        (!isFound(gameboard) || gameboardIdAnchor !== gameboard.id)
      ) {
    ...
    const pageHelp = (
      <span>
        You can build a gameboard by selecting the areas of interest, stage and
        difficulties.
        <br />
    ...
    return (
      <RS.Container id="gameboard-generator" className="mb-5">
        <TitleAndBreadcrumb
          currentPageTitle="Question Finder"
          help={pageHelp}
          modalId="gameboard_filter_help"
        />
        ...
        <RS.Card id="filter-panel" className="mt-4 px-2 py-3 p-sm-4 pb-5">
          ...
          {filterExpanded ? (
            <RS.Row className={"mb-3"}>
              <RS.Col>
                <span>
                  Specify your search criteria and we will generate a random
                  selection of up to 10 questions for your chosen filter(s).
                  Shuffle the questions to get a new random selection.
                </span>
              </RS.Col>
            </RS.Row>
          ) : (
          ...
          <RS.Button
            color="link"
            className="filter-go-to-questions"
            ...
            <RS.Col xs={8} lg={"auto"} className="ml-auto text-right">
              <RS.Button
                ...
                to={`/add_gameboard/${gameboard.id}/${
                  customBoardTitle ?? gameboard.title
                }`}
