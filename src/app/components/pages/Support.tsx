...
const support: {
  student: SupportCategories;
  teacher: SupportCategories;
  tutor?: SupportCategories;
} = {
  student: {
...
export const SupportPageComponent = ({
  ...
  function tabTitleClass(tabName: string, tabIndex: number) {
    return (
      "support-tab-" + section?.categories[categoryNames[tabIndex - 1]].icon
    );
  }
    ...
    <Container>
      <Row>
        <Col>
          ...
          {isDefined(type) && type !== "tutor" && (
            <MetaDescription description={metaDescriptionMap[type]} />
          )}
        </Col>
      </Row>
      <Row>
            ...
            {fromPairs(
              Object.values(section.categories).map((category) => {
                ...
                return [
                  category.title,
                  <PageFragment
                    fragmentId={`support_${type}_${category.category}`}
                  />,
                ];
              }),
            )}
