...
const middlewareRegistrationActions = [
  {
    type: "isaacApi/config/middlewareRegistered",
    payload: "some unique string",
  },
];

function expectActionsToStartWithMiddlewareRegistration(
  actualActions: Action[],
): Action[] {
  expect(
    actualActions
      .slice(0, middlewareRegistrationActions.length)
      .map((a) => a.type),
  ).toEqual(["isaacApi/config/middlewareRegistered"]);
...
describe("requestCurrentUser action", () => {
  ...
  it("dispatches USER_LOG_IN_RESPONSE_SUCCESS after a successful request", async () => {
    ...
    const userAuthSettings =
      userAuthenticationSettings[dameShirley.id as number];
    ...
    axiosMock
      .onGet(`/auth/user_authentication_settings`)
      .replyOnce(200, userAuthSettings);
    ...
    const expectedFinalActions = [
      { type: ACTION_TYPE.CURRENT_USER_RESPONSE_SUCCESS, user: dameShirley },
    ];

    const actualActions = store.getActions();

    const actualIsaacActions =
      expectActionsToStartWithMiddlewareRegistration(actualActions);
    expect(actualIsaacActions.length).toEqual(
      expectedFirstActions.length +
        expectedAsyncActions.length +
        expectedFinalActions.length,
    );
    expect(actualIsaacActions.slice(0, expectedFirstActions.length)).toEqual(
      expectedFirstActions,
    );
    expectedAsyncActions.forEach((expectedAsyncAction) => {
      expect(
        actualIsaacActions.slice(
          expectedFirstActions.length,
          -expectedFinalActions.length,
        ),
      ).toContainEqual(expectedAsyncAction);
    });
    expect(actualIsaacActions.slice(-expectedFinalActions.length)).toEqual(
      expectedFinalActions,
    );
...
describe("registerQuestion action", () => {
  it("dispatches a question registration action", () => {
    ...
    const expectedActions = [
      { type: ACTION_TYPE.QUESTION_REGISTRATION, questions: [manVsHorse] },
    ];
    const store = mockStore();
...
describe("fetchSearch action", () => {
  ...
  it("dispatches SEARCH_RESPONSE_SUCCESS after a successful request", async () => {
    axiosMock
      .onGet(`/search`, { params: { types: "bar", query: "foo" } })
      .replyOnce(200, searchResultsList);
    const store = mockStore();
...
describe("toasts actions", () => {
  ...
  it("registers an action to hide and remove the toast if a timeout is set", async () => {
    const store = mockStore();
    const toastId: string = await store.dispatch(
      showToast(someTimedToast) as any,
    );
    const expectedActions: Action[] = [
      { type: ACTION_TYPE.TOASTS_SHOW, toast: someTimedToast },
    ];
    let actualActions = store.getActions();
    let actualIsaacActions =
      expectActionsToStartWithMiddlewareRegistration(actualActions);
    expect(actualIsaacActions).toEqual(expectedActions);

    jest.runOnlyPendingTimers();
    expectedActions.push({ type: ACTION_TYPE.TOASTS_HIDE, toastId: toastId });
    ...
    jest.runOnlyPendingTimers();
    expectedActions.push({ type: ACTION_TYPE.TOASTS_REMOVE, toastId });
    actualActions = store.getActions();
    actualIsaacActions =
      expectActionsToStartWithMiddlewareRegistration(actualActions);
    expect(actualIsaacActions).toEqual(expectedActions);
  ...
  it("does not register any actions related to the toast if a timeout is not set", async () => {
    const store = mockStore();
    const toastId: string = await store.dispatch(
      showToast(someUnTimedToast) as any,
    );
    const expectedActions: Action[] = [
...
describe("requestEmailVerification action", () => {
  ...
  it("failure dispatches request, failure, and a toast if logged in", async () => {
    axiosMock
      .onPost(`/users/verifyemail`)
      .replyOnce(500, { bypassGenericSiteErrorPage: true });
    const store = mockStore({ user: { ...profWheeler, loggedIn: true } });
    await store.dispatch(requestEmailVerification() as any);
    const expectedActions = [
      { type: ACTION_TYPE.USER_REQUEST_EMAIL_VERIFICATION_REQUEST },
