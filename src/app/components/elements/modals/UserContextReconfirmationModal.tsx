...
const buildModalText = (
  buildConnectionsLink: (text: string) => React.ReactNode,
) => ({
  teacher: {
    intro: (
      <span>
        To ensure you see the correct content, please make sure the details
        below are correct. This also helps us understand the usage of Isaac
        Computer Science.
      </span>
    ),
    connections: (
      <span>
        If you have changed school or have a different class group, you might
        also want to{" "}
        {buildConnectionsLink("review your student and group connections.")}
      </span>
    ),
  },
  tutor: {
    ...
    connections: (
      <span>
        If you have recently changed which students you tutor, might also want
        to {buildConnectionsLink("review your student and group connections.")}
      </span>
    ),
  },
  student: {
    ...
    connections: (
      <span>
        If you have changed school or have a different teacher, you might also
        want to {buildConnectionsLink("review your teacher connections.")}
      </span>
    ),
  },
});
...
const UserContextReconfirmationModalBody = () => {
  ...
  return (
    <Form onSubmit={formSubmission} className={"mb-2"}>
      <p>{modalText.intro}</p>
      ...
      <div className="text-center pb-3 pt-1">
        <Button
          type={"submit"}
          color={"secondary"}
          className={"border-0 my-1 w-lg-50 w-100"}
        >
