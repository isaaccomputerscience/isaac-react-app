...
const MacrosWithMathsBoolean = Object.assign(
  {},
  BaseMacros,
  BooleanLogicMathsMacros,
);
const MacrosWithEngineeringBoolean = Object.assign(
  {},
  BaseMacros,
  BooleanLogicEngineeringMacros,
);
const KatexBaseMacros = mathjaxToKatex(BaseMacros);
const KatexMacrosWithMathsBool = mathjaxToKatex(MacrosWithMathsBoolean);
const KatexMacrosWithEngineeringBool = mathjaxToKatex(
  MacrosWithEngineeringBoolean,
);
...
function startMatch(match: RegExpMatchArray): Search {
  ...
  if (delim != null) {
    ...
    return {
      ...
      endPattern: new RegExp(endPattern(delim.end), "g"),
      mode: delim.mode,
      pcount: 0,
      olen: match[0].length,
    };
...
function endMatch(match: RegExpExecArray, search: Search) {
  ...
  } else if (match[0] === "{" && search.pcount !== undefined) {
    search.pcount++;
  } else if (match[0] === "}" && search.pcount !== undefined) {
    search.pcount--;
  }
...
function munge(latex: string) {
  return latex.replace(/eqnarray/g, "aligned").replace(/\\newline/g, "\\\\");
}

const REF = "==REF==yzskvUeunVc==";
const ENDREF = "==ENDREF==";
const REF_REGEXP = new RegExp(REF + "(.*?)" + ENDREF, "g");
const SR_REF_REGEXP = new RegExp(
  "start text, " + REF_REGEXP.source + ", end text,",
  "g",
);
...
export function katexify(
  ...
  function createReference(
    ...
    if (ref) {
      ...
      if (number) {
        ...
        return format
          ? `<strong class="text-secondary figure-reference">${figure}</strong>`
          : figure;
      }
    }
    return ifMissing;
  ...
  while ((match = start.exec(html)) !== null) {
    ...
    if (search.just) {
      ...
      while (
        !search.matched &&
        (match = search.endPattern.exec(html)) !== null
      ) {
        endMatch(match, search);
      }
      if (search.matched && match) {
        const latex = html.substring(
          index + (search.olen || 0),
          match.index + match[0].length - (search.clen || 0),
        );
        const latexUnEntitied = he.decode(latex);
        const latexMunged = munge(latexUnEntitied);
        let macrosToUse =
          booleanNotation === BOOLEAN_NOTATION.ENG
            ? KatexMacrosWithEngineeringBool
            : KatexMacrosWithMathsBool;
        macrosToUse = {
          ...macrosToUse,
          "\\ref": (context: {
            consumeArgs: (n: number) => { text: string }[][];
          }) => {
        ...
        try {
          ...
          screenReaderText = `${renderA11yString(
            latexMunged,
            katexOptions,
          )}${pauseChars}`;
        ...
        if (screenReaderText) {
          ...
          const katexMathML = katex
            .renderToString(
              latexMunged.replace(dropZoneRegex, "clickable drop zone"),
        ...
        if (showScreenReaderHoverText) {
          ...
          katexRenderResult = katexRenderResult.replace(
            ...
            `<span class="katex-html" title="${
              screenReaderText
                ? "Screenreader text: " +
                  screenReaderText.replace(/,/g, "").replace(/\s\s+/g, " ")
...
export const useRenderKatex = () => {
  ...
  return (markup: string) =>
    katexify(
      markup,
      user,
      preferredBooleanNotation && BOOLEAN_NOTATION[preferredBooleanNotation],
