...
export function Item({
  ...
  useEffect(() => {
    if (
      ...
      dropRegionContext?.shouldGetFocus(id)
    ) {
  ...
  return (
    <Badge
      ...
      className={classNames(
        type === "item-section" && "m-2",
        "p-2 cloze-item feedback-zone",
        isDefined(isCorrect) && "feedback-showing",
      )}
      style={style}
      innerRef={setNodeRef}
      {...listeners}
    ...
    >
      <span className={"sr-only"}>
        {item.altText ?? item.value ?? "cloze item without a description"}
      </span>
      <span aria-hidden={true}>
        <IsaacContentValueOrChildren
          value={item.value}
          encoding={item.encoding || "html"}
        >
...
function InlineDropRegion({
  ...
  const item = dropRegionContext
    ? dropRegionContext.inlineDropValueMap[droppableId]
    : undefined;
  ...
  const isCorrect =
    item &&
    dropRegionContext &&
    dropRegionContext.dropZoneValidationMap[droppableId]?.itemId === item.id
  ...
  if (dropRegionContext && droppableTarget) {
    return ReactDOM.createPortal(
      <span
        ...
        className={classNames(
          "d-inline-block cloze-drop-zone",
          !item &&
            `rounded bg-grey border ${isOver ? "border-dark" : "border-light"}`,
        )}
        ref={setNodeRef}
      >
        {item ? (
