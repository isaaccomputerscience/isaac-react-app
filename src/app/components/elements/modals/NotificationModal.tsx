...
const NotificationModalBody = (notification: { notification: any }) => {
  ...
  const user = useAppSelector(
    (state: AppState) => (state && state.user) || null,
  );
  ...
  function respond(response: string) {
    ...
    if (
      ...
      currentNotification.externalReference.url
    ) {
      const userIdToken = "{{currentUserId}}";

      // if they have a token representing the user id then replace it.
      if (
        currentNotification.externalReference.url.includes(userIdToken) &&
        user &&
        user.loggedIn
      ) {
        const newUrl = currentNotification.externalReference.url.replace(
          userIdToken,
          user.id,
        );
    ...
    <React.Fragment>
      <Col>
        <Row className="justify-content-md-center mb-3">
          <Col>
            {currentNotification ? (
              <IsaacContent doc={currentNotification} />
            ) : (
