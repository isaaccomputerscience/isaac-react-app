...
function useEventPropertyState<T>(
...
export const ScrollShadows = <T extends HTMLElement>({
  element,
}: {
  element: T | undefined;
}) => {
  ...
  const [scrollLeft, setScrollLeft] = useEventPropertyState(
    element,
    0,
    "scrollLeft",
    ["scroll"],
  ...
  const leftOpacity =
    scrollLeft > 1 ? (scrollLeft < 60 ? scrollLeft / 60 : 1) : 0;
  const rightOpacity =
    scrollWidth - clientWidth - scrollLeft > 2
      ? scrollWidth - clientWidth - scrollLeft < 60
        ? (scrollWidth - clientWidth - scrollLeft) / 60
        : 1
      : 0;
