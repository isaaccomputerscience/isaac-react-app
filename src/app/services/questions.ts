...
const IsaacMultiChoiceQuestion = lazy(
  () => import("../components/content/IsaacMultiChoiceQuestion"),
);
const IsaacItemQuestion = lazy(
  () => import("../components/content/IsaacItemQuestion"),
);
const IsaacReorderQuestion = lazy(
  () => import("../components/content/IsaacReorderQuestion"),
);
const IsaacParsonsQuestion = lazy(
  () => import("../components/content/IsaacParsonsQuestion"),
);
const IsaacNumericQuestion = lazy(
  () => import("../components/content/IsaacNumericQuestion"),
);
const IsaacStringMatchQuestion = lazy(
  () => import("../components/content/IsaacStringMatchQuestion"),
);
const IsaacRegexMatchQuestion = lazy(
  () => import("../components/content/IsaacRegexMatchQuestion"),
);
const IsaacFreeTextQuestion = lazy(
  () => import("../components/content/IsaacFreeTextQuestion"),
);
const IsaacSymbolicLogicQuestion = lazy(
  () => import("../components/content/IsaacSymbolicLogicQuestion"),
);
const IsaacSymbolicQuestion = lazy(
  () => import("../components/content/IsaacSymbolicQuestion"),
);
const IsaacClozeQuestion = lazy(
  () => import("../components/content/IsaacClozeQuestion"),
);
...
export const QUESTION_TYPES: {
  [key: string]: React.LazyExoticComponent<
    ({ doc, questionId, readonly }: IsaacQuestionProps<any>) => JSX.Element
  >;
} = {
...
export function selectQuestionPart(
  questions?: AppQuestionDTO[],
...
export function generateQuestionTitle(doc: ContentDTO | ContentSummaryDTO) {
  ...
  if (
    ...
    title +=
      " " +
      fastTrackConceptEnumerator(doc.id) +
      (doc.tags.includes("ft_lower") ? "ii" : "i");
  }
...
export function sanitiseInequalityState(state: any) {
  ...
  if (saneState.result?.tex) {
    saneState.result.tex = saneState.result.tex
      .split("")
      .map((l: string) =>
        REVERSE_GREEK_LETTERS_MAP[l] ? "\\" + REVERSE_GREEK_LETTERS_MAP[l] : l,
      )
      .join("");
  }
...
export const parsePseudoSymbolicAvailableSymbols = (
 ...
 */
export function useCurrentQuestionAttempt<T extends ChoiceDTO>(
  questionId: string,
) {
