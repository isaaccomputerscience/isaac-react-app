...
export const user = (user: UserState = null, action: Action): UserState => {
  switch (action.type) {
    ...
    case ACTION_TYPE.USER_REGISTRATION_RESPONSE_SUCCESS:
...
export const userAuthSettings = (
  userAuthSettings: UserAuthSettingsState = null,
  action: Action,
) => {
...
export const totpChallengePending = (
  totpChallengePending: TotpChallengePendingState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const userSchoolLookup = (
  userSchoolLookup: UserSchoolLookupState = null,
  action: Action,
) => {
  switch (action.type) {
