    ...
    <svg width="100%" height={`${height}px`}>
      ...
      <g id="hexagonal-filter" transform={`translate(${focusPadding},${focusPadding})`}>
        ...
        {tiers.slice(1).map((tier, i) => {
          ...
          return (
            <g key={tier.for} transform={connectionRowTranslation(deviceSize, hexagon, i)}>
              <HexagonConnection
                ...
                targetIndices={
                  selections[i + 1]?.map((s) => choices[i + 1].map((c) => c.value).indexOf(s.value)) || [-1]
                }
        ...
        {/* Hexagons */}
        {tiers.map((tier, i) => (
          <g key={tier.for} transform={hexRowTranslation(deviceSize, hexagon, i)}>
            {choices[i].map((choice, j) => {
              ...
              return (
                <g key={choice.value} transform={hexagonTranslation(deviceSize, hexagon, i, j)}>
                  <Hexagon
                    {...hexagon}
                  ...
                  <foreignObject width={hexagon.halfWidth * 2} height={hexagon.quarterHeight * 4}>
                    ...
                    {tag.comingSoonDate && (
                      <div className={classNames(subject, "hexagon-coming-soon")}>Coming {tag.comingSoonDate}</div>
                    )}
                  </foreignObject>

                  <Hexagon
                    {...hexagon}
                    className={classNames("hex none", { clickable: !isComingSoon })}
                  ...
                  >
                    {!isComingSoon && (
                      <title>
                        {`${isSelected ? "Remove" : "Add"} the ${tier.name.toLowerCase()} "${choice.label}" ${
                          isSelected ? "from" : "to"
        ...
        <g transform={`translate(${connection.length / 2 - hexKeyPoints.x.center}, 0)`}>
          {selectionSummary.map((selection, i) => {
            ...
            return (
              <g
                key={selection}
                transform={`translate(${((hexagon.halfWidth + hexagon.padding) * 2 + connection.length) * i}, 0)`}
              >
                ...
                <Hexagon
                  className={`hex active ${selections[0]?.length ? selections[0][0].value : choices[0][0].value}`}
                  {...hexagon}
                />
              </g>
            );
            ...
            >
              <g transform={`translate(0, ${hexagon.quarterHeight * 4 + hexagon.padding})`}>
                <foreignObject width={connection.length} height={hexagon.quarterHeight * 5}>
                  <div className={`hexagon-tier-summary text-dark`}>{selection}</div>
                </foreignObject>
              </g>
            </g>
          );
