...
export const UserContextPicker = ({
  className,
  hideLabels = true,
}: {
  className?: string;
  ...
  const filteredStages = getFilteredStageOptions({
    byUser: user,
    includeNullOptions: true,
  });
  ...
  const unusual = {
    ...
    examBoard: !filteredExamBoardOptions
      .map((s) => s.value)
      .includes(userContext.examBoard),
  };
  const showUnusualContextMessage = unusual.stage || unusual.examBoard;
  const showHideOtherContentSelector = segueEnvironment === "DEV";
  const showStageSelector =
    getFilteredStageOptions({ byUser: user }).length > 1 ||
    showUnusualContextMessage;
  const showExamBoardSelector =
    getFilteredExamBoardOptions({ byUser: user }).length > 1 ||
    showUnusualContextMessage;
  ...
  return (
    <div className="d-flex">
      ...
      {showHideOtherContentSelector && (
        <FormGroup className={`mr-2 ${className}`}>
          <Label
            ...
            htmlFor="uc-show-other-content-check"
          >
            Show other content?{" "}
          </Label>
          <CustomInput
            ...
            onChange={(e) =>
              dispatch(
                transientUserContextSlice.actions.setShowOtherContent(
                  e.target.checked,
                ),
              )
            }
      ...
      {showStageSelector && (
        <FormGroup
          className={`${showExamBoardSelector ? "mr-2" : ""} ${className}`}
        >
          {!hideLabels && (
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (
                possibleExamBoards.length > 0 &&
                !possibleExamBoards.includes(EXAM_BOARD.ALL)
              ) {
                examBoard = possibleExamBoards[0];
              }
              newParams.examBoard = examBoard;
              dispatch(
                transientUserContextSlice.actions.setExamBoard(examBoard),
              );
              history.push({
                search: queryString.stringify(newParams, { encode: false }),
              });
          ...
          >
            ...
            {!filteredStages
              .map((s) => s.value)
              .includes(userContext.stage) && (
              <option key={userContext.stage} value={userContext.stage}>
                {"*"}
                {
                  getFilteredStageOptions().filter(
                    (o) => o.value === userContext.stage,
                  )[0]?.label
                }
                {"*"}
              </option>
            ...
            onChange={(e) => {
              history.push({
                search: queryString.stringify(
                  { ...qParams, examBoard: e.target.value },
                  { encode: false },
                ),
              });
              dispatch(
                transientUserContextSlice.actions.setExamBoard(
                  e.target.value as EXAM_BOARD,
                ),
              );
            }}
          >
            ...
            {!filteredExamBoardOptions
              .map((s) => s.value)
              .includes(userContext.examBoard) && (
              <option key={userContext.examBoard} value={userContext.examBoard}>
                {"*"}
                {
                  getFilteredExamBoardOptions().filter(
                    (o) => o.value === userContext.examBoard,
                  )[0]?.label
                }
                {"*"}
              </option>
      ...
      {showUnusualContextMessage && (
        <div className="mt-2 ml-1">
          <span
            id={`unusual-viewing-context-explanation`}
            className="icon-help mx-1"
          />
          <RS.UncontrolledTooltip
            placement="bottom"
            target={`unusual-viewing-context-explanation`}
          >
            You are seeing {stageLabelMap[userContext.stage]}{" "}
            {examBoardLabelMap[userContext.examBoard]} content, which is
            different to your account settings. <br />
            {unusual.stage && unusual.examBoard && (
              <>
                {userContext.explanation.stage ===
                userContext.explanation.examBoard
            ...
            {unusual.stage &&
              !unusual.examBoard &&
              `The stage was specified by your ${userContext.explanation.stage}.`}
            {unusual.examBoard &&
              !unusual.stage &&
