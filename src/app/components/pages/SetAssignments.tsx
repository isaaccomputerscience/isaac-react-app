...
const AssignGroup = ({ groups, board, allowScheduling }: AssignGroupProps) => {
  ...
  const dueDateInvalid =
    dueDate && scheduledStartDate
      ? nthHourOf(0, scheduledStartDate).valueOf() > dueDate.valueOf()
      : false;
...
const HexagonGroupsButton = ({
  toggleAssignModal,
  assignees,
  id,
}: HexagonGroupsButtonProps) => (
  <button
    onClick={toggleAssignModal}
    id={id}
    className="board-subject-hexagon-container"
  >
    <div className="board-subject-hexagon" />
    <span className="groups-assigned" title={"Groups assigned"}>
      <strong>
        {isDefined(assignees) ? assignees.length : <Spinner size="sm" />}
      </strong>{" "}
      ...
      {isDefined(assignees) && (
        <UncontrolledTooltip placement={"top"} target={"#" + id}>
          {assignees.length === 0
            ...
            : "Gameboard assigned to: " +
              assignees.map((g) => g.groupName).join(", ")}
        </UncontrolledTooltip>
      )}
    </span>
  </button>
...
const SetAssignmentsModal = (props: SetAssignmentsModalProps) => {
  ...
  const hasStarted = (a: { startDate?: Date | number }) =>
    !a.startDate || Date.now() > a.startDate.valueOf();

  const startedAssignees = useMemo(
    () => (allowScheduling ? assignees.filter(hasStarted) : assignees),
    [assignees],
  );
  const scheduledAssignees = useMemo(
    () => assignees.filter((a) => !hasStarted(a)),
    [assignees],
  );
  ...
  function confirmUnassignBoard(groupId: number, groupName?: string) {
    if (
      board?.id &&
      confirm(
        `Are you sure you want to unassign this gameboard from ${
  ...
  return (
    <Modal isOpen={isOpen} data-testid={"set-assignment-modal"} toggle={toggle}>
      <ModalHeader
        data-testid={"modal-header"}
        role={"heading"}
        className={"text-break"}
        close={
          <button className={"close text-nowrap"} onClick={toggle}>
            Close
          </button>
        }
      >
              ...
              {startedAssignees.map((assignee) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      confirmUnassignBoard(assignee.groupId, assignee.groupName)
                    }
                  >
                    ×
                  </button>
        ...
        {allowScheduling && (
          <div className="py-2">
            ...
            <UncontrolledTooltip
              placement="left"
              autohide={false}
              target={`pending-assignments-help-${board?.id}`}
            >
              Assignments that are scheduled to begin at a future date. Once the
              start date passes, students will be able to see the assignment,
              and will receive a notification email.
            </UncontrolledTooltip>
            {scheduledAssignees.length > 0 ? (
              <Container className="mb-4">
                {scheduledAssignees.map((assignee) => (
                  <Row
                    data-testid={"pending-assignment"}
                    key={assignee.groupId}
                    className="px-1"
                  >
                    <span className="flex-grow-1">{assignee.groupName}</span>
                    {assignee.startDate && (
                      <>
                        <span
                          id={`start-date-${assignee.groupId}`}
                          className="ml-auto mr-2"
                        >
                          🕑{" "}
                          {(typeof assignee.startDate === "number"
      ...
      <ModalFooter>
        <Button block color="tertiary" onClick={toggle}>
          Close
        </Button>
      </ModalFooter>
    </Modal>
...
const Board = ({
  ...
  function confirmDeleteBoard() {
    if (hasAssignedGroups) {
      if (isAdminOrEventManager(user)) {
        alert(
          "Warning: You currently have groups assigned to this gameboard. If you delete this your groups will still be assigned but you won't be able to unassign them or see the gameboard in your assigned gameboards or 'My gameboards' page.",
        );
      } else {
        dispatch(
          showErrorToast(
            "Gameboard Deletion Not Allowed",
            "You have groups assigned to this gameboard. To delete this gameboard, you must unassign all groups.",
          ),
        );
        return;
      }
    ...
    if (
      confirm(
        `Are you sure you want to remove '${board.title}' from your account?`,
      )
    ) {
      dispatch(
        unlinkUserFromGameboard({ boardId: board.id, boardTitle: board.title }),
      );
    }
  }
  ...
  const boardStagesAndDifficulties = useMemo(
    () => determineGameboardStagesAndDifficulties(board),
    [board],
  );
    ...
    <>
      {boardView == BoardViews.table ? (
        ...
        <tr
          ...
          data-testid={"assignment-gameboard-table-row"}
        >
          <td>
          ...
          <td className="align-middle">
            <a href={assignmentLink}>{board.title}</a>
          </td>
        ...
        // Card view
        <Card
          aria-label={`Gameboard ${board.title}`}
          className="board-card card-neat"
          data-testid={"assignment-gameboard-card"}
        >
                ...
                <thead>
                  <tr>
                    ...
                    <th className="w-50 font-weight-light pl-1">
                      {`Difficult${
                        boardStagesAndDifficulties.some(
                          ([, ds]) => ds.length > 1,
                        )
                          ? "ies"
                          : "y"
...
export const AddGameboardButtons = ({
  className,
  redirectBackTo,
}: {
  className: string;
    ...
    <Row className={className}>
      <Col md={6} lg={4} className="pt-1">
        <Button
          tag={Link}
          to={"/pages/gameboards"}
          onClick={() => setAssignBoardPath(redirectBackTo)}
          color="secondary"
          block
        >
          Pre-made gameboards
...
export const SetAssignments = () => {
  ...
  const groupsByGameboard = useMemo<{ [gameboardId: string]: BoardAssignee[] }>(
    () =>
      assignmentsSetByMe?.reduce(
        (acc, assignment) => {
          if (
            !isDefined(assignment?.gameboardId) ||
            !isDefined(assignment?.groupId)
          )
            return acc;
          const newAssignee = {
  ...
  const [boardCreator, setBoardCreator] = useState<BoardCreators>(
    BoardCreators.all,
  );
  ...
  const allowScheduling =
    isStaff(user) ||
    (userPreferences?.BETA_FEATURE?.SCHEDULE_ASSIGNMENTS ?? false);
  ...
  const pageHelp = (
    <span>
      Use this page to set assignments to your groups. You can assign any
      gameboard you have saved to your account.
      <br />
  ...
  return (
    <Container>
      <SetAssignmentsModal
        ...
        board={modalBoard}
        assignees={
          (isDefined(modalBoard) &&
            isDefined(modalBoard?.id) &&
            groupsByGameboard[modalBoard.id]) ||
          []
        }
      />

      <TitleAndBreadcrumb
        currentPageTitle="Set assignments"
        help={pageHelp}
        modalId="set_assignments_help"
      />
      <h4 className="mt-4 mb-3">Add a gameboard from ...</h4>
      <AddGameboardButtons
        className={"mb-4"}
        redirectBackTo={"/set_assignments"}
      />
      ...
      {boards && boards.totalResults === 0 ? (
        <h3 className="text-center mt-4 mb-5">
          You have no gameboards to assign; use one of the options above to find
          one.
        </h3>
      ) : (
        <>
          {boards && boards.totalResults > 0 && (
            <h4>
              You have <strong>{boards.totalResults}</strong> gameboard
              {boards.totalResults > 1 && "s"} ready to assign...
            </h4>
          )}
          {!boards && (
            <h4>
              You have <IsaacSpinner size="sm" inline /> gameboards ready to
              assign...
            </h4>
          )}
          <Row>
            <Col sm={6} lg={3} xl={2}>
              <Label className="w-100">
                Display in{" "}
                <Input type="select" value={boardView} onChange={switchView}>
                  {Object.values(BoardViews).map((view) => (
                ...
                >
                  <Label className="w-100">
                    ...
                    <Input
                      ...
                      value={boardLimit}
                      onChange={(e) =>
                        setBoardLimit(e.target.value as BoardLimit)
                      }
                    >
                      {Object.values(BoardLimit).map((limit) => (
                ...
                <Col xs={6} lg={4}>
                  <Label className="w-100">
                    ...
                    <Input
                      ...
                      value={boardOrder}
                      onChange={(e) =>
                        setBoardOrder(e.target.value as BoardOrder)
                      }
                    >
                      {Object.values(BoardOrder).map((order) => (
              ...
              <div>
                {boardView == BoardViews.card ? (
                  ...
                  <>
                    ...
                    >
                      <p>
                        Showing <strong>{boards.boards.length}</strong> of{" "}
                        <strong>{boards.totalResults}</strong>
                      </p>
                      {boards.boards.length < boards.totalResults && (
                        <Button onClick={viewMore} disabled={loading}>
                          {loading ? <Spinner /> : "View more"}
                        </Button>
                      )}
                    </div>
                  </>
                        ...
                        <Col lg={{ size: 2, offset: 6 }}>
                          <Label className="w-100">
                            ...
                            <Input
                              ...
                              value={boardCreator}
                              onChange={(e) =>
                                setBoardCreator(e.target.value as BoardCreators)
                              }
                            >
                              {Object.values(BoardCreators).map((creator) => (
                          ...
                          <thead>
                            <tr>
                              <th className="text-center align-middle">
                                <span className="pl-2 pr-2">Groups</span>
                              </th>
                              <th className="align-middle pointer-cursor">
                                <button
                                  className="table-button"
                                  onClick={() =>
                                    boardOrder == BoardOrder.title
                                      ? setBoardOrder(BoardOrder["-title"])
                                      : setBoardOrder(BoardOrder.title)
                                  }
                                >
                                  Board name{" "}
                              ...
                              <th className="text-center align-middle pointer-cursor">
                                <button
                                  className="table-button"
                                  onClick={() =>
                                    boardOrder == BoardOrder.created
                                      ? setBoardOrder(BoardOrder["-created"])
                                      : setBoardOrder(BoardOrder.created)
                                  }
                                >
                                  Created{" "}
                              ...
                              <th className="text-center align-middle pointer-cursor">
                                <button
                                  className="table-button"
                                  onClick={() =>
                                    boardOrder == BoardOrder.visited
                                      ? setBoardOrder(BoardOrder["-visited"])
                                      : setBoardOrder(BoardOrder.visited)
                                  }
                                >
                                  Last viewed{" "}
                            ...
                            {boards.boards
                              .filter(
                                (board) =>
                                  ...
                                  board.title
                                    .toLowerCase()
                                    .includes(boardTitleFilter.toLowerCase()) &&
                                  (formatBoardOwner(user, board) ==
                                    boardCreator ||
                                    boardCreator == "All"),
                              )
