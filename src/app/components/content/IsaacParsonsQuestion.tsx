...
const IsaacParsonsQuestion = ({
  ...
  questionId,
  readonly,
}: IsaacQuestionProps<IsaacParsonsQuestionDTO>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<ParsonsChoiceDTO>(questionId);

  const [availableItems, setAvailableItems] = useState<
    Immutable<ParsonsItemDTO>[]
  >([...(doc.items ?? [])]);
  ...
  const [currentDestinationIndex, setCurrentDestinationIndex] = useState<
    number | null
  >(null);

  const canIndent =
    (!isDefined(doc.disableIndentation) || !doc.disableIndentation) &&
    !readonly;
    ...
    (e: MouseEvent | TouchEvent) => {
      if (draggedElement) {
        ...
        if (canIndent && initialX && x) {
          ...
          const i = Math.min(
            Math.floor(d / PARSONS_INDENT_STEP),
            Math.min(currentMaxIndent, PARSONS_MAX_INDENT),
          );
          if (cursorX >= initialX) {
          ...
          const previousItem =
            currentAttempt?.items?.[(currentDestinationIndex || 0) - 1];
          setCurrentMaxIndent(
            previousItem ? (previousItem.indentation || 0) + 1 : 0,
          );
          setCurrentIndent(i);
        }
      }
  ...
  const onKeyUp = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.timeStamp > 0 && draggedElement) {
        ...
        const localCurrentIndent: number =
          currentIndent || (matches && parseInt(matches[1])) || 0;
        ...
        setDraggedElement((prevDraggedElement) =>
          Object.assign({}, prevDraggedElement, {
            className: className.replace(
              (matches && matches[0]) || `indent-${localCurrentIndent}`,
              `indent-${newIndent}`,
            ),
          }),
        );
  ...
  const onDragStart = (initial: DragStart) => {
    const draggedElement: HTMLElement | null = document.getElementById(
      initial.draggableId,
    );
    const choiceElement: HTMLElement | null = document.getElementById(
      "parsons-choice-area",
    );
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (
      ...
      result.destination.droppableId === "answerItems" &&
      currentAttempt
    ) {
      ...
      result.source.droppableId === result.destination.droppableId &&
      result.destination.droppableId === "availableItems"
    ) {
      ...
      result.source.droppableId === "availableItems" &&
      result.destination.droppableId === "answerItems"
    ) {
      ...
      result.source.droppableId === "answerItems" &&
      result.destination.droppableId === "availableItems" &&
      currentAttempt
    ) {
  ...
  const reduceIndentation = (index: number) => {
    ...
    if (isDefined(items[index].indentation)) {
      const indentedItem = {
        ...items[index],
        indentation: Math.max((items[index].indentation || 0) - 1, 0),
      };
    ...
    if (isDefined(items[index].indentation)) {
      const indentedItem = {
        ...
        indentation: Math.min(
          (items[index].indentation || 0) + 1,
          Math.min(
            (items[Math.max(index - 1, 0)].indentation || 0) + 1,
            PARSONS_MAX_INDENT,
          ),
        ),
      };
  ...
  const onCurrentAttemptUpdate = (
    newCurrentAttempt?: Immutable<ParsonsChoiceDTO>,
    newAvailableItems?: Immutable<ParsonsItemDTO>[],
  ) => {
              ...
              {(provided: DroppableProvided) => {
                return (
                  <div
                    ...
                    className={`parsons-items ${
                      availableItems && availableItems.length > 0 ? "" : "empty"
                    }`}
                  >
                    {availableItems &&
          ...
          <Col md={{ size: 6 }} className="no-print">
            ...
            <Droppable droppableId="answerItems">
              {(provided: DroppableProvided) => {
                return (
                  <div
                    id="parsons-choice-area"
                    ref={provided.innerRef}
                    className={classNames(
                      "parsons-items",
                      draggedElement ? "is-dragging" : "",
                      {
                        [`ghost-indent-${currentIndent}`]:
                          isDefined(draggedElement) && currentIndent !== null,
                        empty: !(
                          currentAttempt &&
                  ...
                  >
                    {currentAttempt &&
                      ...
                      currentAttempt.items.map((item, index) => {
                        const canDecreaseIndentation =
                          canIndent &&
                          isDefined(item?.indentation) &&
                          item.indentation > 0;
                        const canIncreaseIndentation =
                          canIndent &&
                          isDefined(item?.indentation) &&
                          index !== 0 &&
                          item.indentation <=
                            getPreviousItemIndentation(index) &&
                          item.indentation < PARSONS_MAX_INDENT;
                        return (
                          <Draggable
                            key={item.id}
                            draggableId={`${
                                    ...
                                    {canIndent && (
                                      <div className="controls">
                                        ...
                                        <span
                                          ...
                                          role="img"
                                          onMouseUp={() =>
                                            reduceIndentation(index)
                                          }
                                          aria-label={`reduce indentation ${
                                            !canDecreaseIndentation
                                              ? "(disabled)"
                                              : ""
                                          }`}
                    ...
                    {(!currentAttempt ||
                      ...
                      <div className="text-muted text-center">
                        {readonly
                          ? "No answer entered"
                          : "Drag items across to build your answer"}
                      </div>
                    )}
                    {provided.placeholder}
                  </div>
