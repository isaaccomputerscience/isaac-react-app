...
export const MyAssignments = () => {
  ...
  const [assignmentTitleFilter, setAssignmentTitleFilter] =
    useState<string>("");
  const [assignmentSetByFilter, setAssignmentSetByFilter] =
    useState<string>("All");
  const [assignmentGroupFilter, setAssignmentGroupFilter] =
    useState<string>("All");

  const showOld =
    (myAssignments.inProgressRecent.length == 0 &&
      myAssignments.inProgressOld.length > 0 &&
  ...
  const tabs: [React.ReactElement, AssignmentDTO[], string][] = [
    [
      <span key={1}>
        <span className="d-none d-md-inline">Assignments </span>To&nbsp;Do
      </span>,
    ...
    [
      <span key={2}>
        Older<span className="d-none d-md-inline"> Assignments</span>
      </span>,
    ...
    [
      <span key={3}>
        <span className="d-none d-md-inline">Completed Assignments</span>
        <span className="d-inline d-md-none">Done</span>
      </span>,
  ...
  const pageHelp = (
    <span>
      Any assignments you have been set will appear here.
      <br />
      Unfinished overdue assignments will show in Assignments To Do for 5 days
      after they are due, after which they move to Older Assignments.
    </span>
  );
          ...
          <ShowLoading until={assignments}>
            <Nav className="mt-4 mb-3" tabs>
              {tabs.map(([tabTitle, tabItems, tabAccessibleName], mapIndex) => {
                ...
                return (
                  <NavItem key={tabIndex} className="px-3">
                    <NavLink
                      className={classes}
                      tabIndex={0}
            ...
            <Row>
              ...
              <Col sm={6} lg={{ size: 2, offset: 4 }}>
                <Label className="w-100">
                  ...
                  >
                    {["All", ...getDistinctAssignmentGroups(assignments)].map(
                      (group) => (
              ...
              <Col sm={6} lg={2}>
                <Label className="w-100">
                  ...
                  <Input
                    type="select"
                    value={assignmentSetByFilter}
                    onChange={(e) => setAssignmentSetByFilter(e.target.value)}
                  >
                    {["All", ...getDistinctAssignmentSetters(assignments)].map(
                      (setter) => (
            ...
            <Row>
              <Col sm="12">
                <Assignments
                  assignments={filterAssignmentsByProperties(
                    tabs[activeTab][1],
