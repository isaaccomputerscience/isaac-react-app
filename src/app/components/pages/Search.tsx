...
const selectStyle: StylesConfig<
  Item<DOCUMENT_TYPE>,
  true,
  GroupBase<Item<DOCUMENT_TYPE>>
> = {
  multiValue: (styles: CSSObjectWithLabel) => ({
    ...styles,
    backgroundColor: "rgba(255, 181, 63, 0.9)",
  }),
...
export const Search = withRouter((props: RouteComponentProps) => {
  ...
  const searchResults = useAppSelector(
    (state: AppState) => state?.search?.searchResults || null,
  );
  ...
  if (urlFilters.length === 0) {
    initialFilters = [
      ...
      DOCUMENT_TYPE.EVENT,
      DOCUMENT_TYPE.TOPIC_SUMMARY,
      DOCUMENT_TYPE.GENERIC,
    ];
  }
  const [filtersState, setFiltersState] = useState<Item<DOCUMENT_TYPE>[]>(
    initialFilters.map(itemise),
  );

  useEffect(
    function triggerSearchAndUpdateLocalStateOnUrlChange() {
      dispatch(
        fetchSearch(
          urlQuery ?? "",
          initialFilters.length ? initialFilters.join(",") : undefined,
        ),
      );
  ...
  const filteredSearchResults =
    ...
    searchResults.results
      ...
      .filter((result) =>
        isIntendedAudience(result.audience, userContext, user),
      );
  const shortcutResponses = (
    queryState ? shortcuts(queryState) : []
  ) as ShortcutResponse[];
  const shortcutAndFilteredSearchResults = (shortcutResponses || []).concat(
              ...
              <RS.Col md={7} sm={12}>
                <RS.Form inline className="search-filters">
                  <RS.Label
                    ...
                    className="d-none d-lg-inline-block mr-1"
                  >
                    Filters
                  </RS.Label>
                  <Select
            ...
            {urlQuery != "" && (
              <RS.CardBody>
                <ShowLoading until={shortcutAndFilteredSearchResults}>
                  {shortcutAndFilteredSearchResults &&
                  shortcutAndFilteredSearchResults.length > 0 ? (
                    <LinkToContentSummaryList
                      items={shortcutAndFilteredSearchResults}
