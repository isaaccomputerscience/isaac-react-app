...
const BreadcrumbTrail = ({
  ...
  return (
    <Breadcrumb className="py-md-2 px-md-0 mb-3 mb-md-0 bread">
      {breadcrumbHistory.map((breadcrumb) =>
        formatBreadcrumbHistoryItem(breadcrumb, disallowLaTeX),
      )}
...
export const formatBreadcrumbItemTitle = (
  title: string,
  disallowLaTeX?: boolean,
) => <Markup encoding={disallowLaTeX ? "plaintext" : "latex"}>{title}</Markup>;

export const formatBreadcrumbHistoryItem = (
  breadcrumb: LinkInfo,
  disallowLaTeX?: boolean,
) => {
  const titleElement = formatBreadcrumbItemTitle(
    breadcrumb.title,
    disallowLaTeX,
  );
  ...
  return (
    <BreadcrumbItem key={breadcrumb.title}>
      {breadcrumb.to ? (
        <Link to={breadcrumb.to} replace={breadcrumb.replace}>
          {titleElement}
        </Link>
      ) : (
        titleElement
...
export const formatBreadcrumbItem = (
  currentPageTitle: string,
  disallowLaTeX?: boolean,
) => {
  return (
...
type TitleAndBreadcrumbProps = BreadcrumbTrailProps &
  PageTitleProps & {
