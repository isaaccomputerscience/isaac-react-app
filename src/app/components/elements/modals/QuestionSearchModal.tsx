...
export const QuestionSearchModal = ({
  ...
  const creationContext: AudienceContext = !isBookSearch
    ? {
        ...
        difficulty:
          searchDifficulties.length > 0 ? searchDifficulties : undefined,
  ...
  const questions = useAppSelector(
    (state: AppState) => state && state.questionSearchResult,
  );
  ...
  const searchDebounce = useCallback(
    debounce(
      (
        ...
        if (
          [searchString, topics, book, stages, difficulties, examBoards].every(
            (v) => v.length === 0,
          ) &&
          !fasttrack
        ) {
        ...
        const tags = (
          isBookSearch
            ? book
            : [...[topics].map((tags) => tags.join(" "))].filter(
                (query) => query != "",
              )
        ).join(" ");
      ...
      sortQuestions(
        ...
        questions.filter((question) => {
          ...
          const qTopicsMatch =
            ...
            (question.tags &&
              question.tags.filter((tag) => searchTopics.includes(tag)).length >
                0);
    ...
    <div className="d-lg-flex align-items-baseline">
      ...
      <div>
        <RS.Input
          ...
          disabled={isEqual(
            new Set(originalSelectedQuestions.keys()),
            new Set(selectedQuestions.keys()),
          )}
  ...
  return (
    <div className="mb-4">
      <RS.Row>
        <RS.Col
          lg={12}
          className={`text-wrap mt-2 ${isBookSearch ? "d-none" : ""}`}
        >
      ...
      <RS.Row className={isBookSearch ? "d-none" : ""}>
        ...
        <RS.Col
          lg={6}
          className={`text-wrap my-2 ${isBookSearch ? "d-none" : ""}`}
        >
        ...
        <RS.Col lg={6} className={`text-wrap my-2`}>
          ...
          <Select
            ...
            placeholder="Any"
            {...selectStyle}
            value={getFilteredExamBoardOptions({
              byStages: searchStages,
            }).filter((o) => searchExamBoards.includes(o.value))}
            options={getFilteredExamBoardOptions({ byStages: searchStages })}
            onChange={(s: MultiValue<Item<ExamBoard>>) =>
              selectOnChange(setSearchExamBoards, true)(s)
            }
          />
        </RS.Col>
      </RS.Row>
          ...
          <thead>
            <tr>
              ...
              <SortableTableHeader
                ...
                updateState={sortableTableHeaderUpdateState(
                  questionsSort,
                  setQuestionsSort,
                  "title",
                )}
      ...
      {questions && questions.length > 5 && (
        <div className="mt-2">{addSelectionsRow}</div>
      )}
    </div>
  );
};
