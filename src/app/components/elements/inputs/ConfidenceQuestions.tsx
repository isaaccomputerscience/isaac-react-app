...
const confidenceOptions: { [option in ConfidenceType]: ConfidenceVariables } = {
  question: {
    ...
    states: {
      initial: {
        question:
          "What is your level of confidence that your answer is correct?",
        options: [
      ...
      followUp: {
        question:
          "Having read the feedback, what is your level of confidence in answering this question correctly now?",
        options: [
...
interface ConfidenceQuestionsProps {
  ...
  setState: (
    cs: ConfidenceState | ((cs: ConfidenceState) => ConfidenceState),
  ) => void;
  validationPending: ValidationPendingState;
  setValidationPending: (
    vp:
      | ValidationPendingState
      | ((vp: ValidationPendingState) => ValidationPendingState),
  ) => void;
  identifier: any;
...
const confidenceInformationModal = () =>
  openActiveModal({
    closeAction: () => store.dispatch(closeActiveModal()),
    title: "Information",
    body: (
      <div className="mb-4">
        We regularly review and update the Isaac platform’s content and would
        like your input in order to prioritise content and assess the impact of
        updates. Data captured with these buttons will help us identify priority
        areas.
      </div>
    ),
  });
...
export const ConfidenceQuestions = ({
  ...
  return (
    <div
      className={classNames("quick-question-options", "p-3", {
        "quick-question-secondary": state === "followUp",
        "quick-question-muted": disabled,
      })}
    >
      {state === "initial" && (
        <Row>
          <Col md="9">
            <h4 className={classNames({ "text-muted": disabled })}>
              {confidenceVariables?.title}
            </h4>
          </Col>
          <Col md="auto" className="ml-auto text-center not-mobile">
            <Button
              outline
              color="primary"
              className={classNames("confidence-help", {
      ...
      <Row className={"justify-content-center"}>
        {confidenceStateVariables.options.map((option) => (
          <Col
            key={option.label}
            lg={4}
            md={6}
          ...
          >
            <Button
              ...
              disabled={disabled}
              block
              className={classNames({ active: state === "followUp" })}
              type="submit"
...
export const useConfidenceQuestionsValues = (
  show: boolean | undefined,
  type: ConfidenceType,
  onConfidenceStateChange?: (cs: ConfidenceState) => void,
  currentAttempt?: Immutable<ChoiceDTO>,
  canSubmit?: boolean,
  correct?: boolean,
  locked?: boolean,
) => {
  // Confidence question specific things
  const [confidenceState, setConfidenceState] =
    useState<ConfidenceState>("initial");
  const [validationPending, setValidationPending] =
    useState<ValidationPendingState>({ pending: false });
  const confidenceDisabled =
    type === "question" &&
    (!canSubmit ||
      !currentAttempt ||
      currentAttempt.value === "" ||
      (Array.isArray((currentAttempt as Immutable<ItemChoiceDTO>).items) &&
        (currentAttempt as Immutable<ItemChoiceDTO>).items?.length === 0));
  const showQuestionFeedback =
    confidenceState !== "initial" || !show || correct;

  // Reset question confidence on attempt change
  useEffect(() => {
    if (type === "question") {
      setConfidenceState("initial");
      setValidationPending((vp) =>
        vp.pending ? { ...vp, updateState: false } : vp,
      );
    }
  }, [currentAttempt]);
