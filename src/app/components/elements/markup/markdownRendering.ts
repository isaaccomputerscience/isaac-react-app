...
export const renderRemarkableMarkdown = (markdown: string) =>
...
export const renderClozeDropZones = (markdown: string) => {
  ...
  for (const match of dropZoneMatches) {
    if (match.groups) {
      ...
      if (!isNaN(index))
        reservedIndices.set(index, 1 + (reservedIndices.get(index) ?? 0));
    }
  }
  ...
  return markdown.replace(
    dropZoneRegex,
    (_match, params, indexMatch, widthMatch, heightMatch) => {
      const width = widthMatch ? widthMatch.slice("w-".length) : "100";
      const height = heightMatch ? heightMatch.slice("h-".length) : "27";
      const manualIndex: number | undefined = indexMatch
        ? parseInt(indexMatch.slice("i-".length))
        : undefined;
      let usingManualIndex =
        isDefined(manualIndex) &&
        !isNaN(manualIndex) &&
        manualIndex < dropZoneMatches.length;
      if (
        usingManualIndex &&
        (reservedIndices.get(manualIndex as number) as number) > 1
      ) {
        usingManualIndex = false;
        reservedIndices.set(
          manualIndex as number,
          (reservedIndices.get(manualIndex as number) as number) - 1,
        );
      }
...
export const renderInlineGlossaryTerms = (markdown: string) => {
  ...
  const glossaryInlineRegexp =
    /\[glossary-inline:(?<id>[a-z0-9-|]+?)\s*(?:"(?<text>[A-Za-z0-9-() ]+)")?\]/g;
...
export const regexProcessMarkdown = (markdown: string) => {
  ...
  Object.entries(regexRules).forEach(
    ([replacement, rule]) => (markdown = markdown.replace(rule, replacement)),
  );
  return markdown;
};
