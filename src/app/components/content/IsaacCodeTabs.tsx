...
export const IsaacCodeTabs = (props: any) => {
  ...
  children.forEach((child, index) => {
    const titleFromSnippet =
      ...
      child?.children[0].language &&
      programmingLanguagesMap[child.children[0].language.toUpperCase()];
    const tabTitle = titleFromSnippet || child.title || `Tab ${index + 1}`;
    if (isDefined(child.children)) {
      tabTitlesToContent[tabTitle] = (
        <IsaacCodeSnippet doc={child.children[0]} />
      );
    }
  });
  ...
  useEffect(() => {
    if (isDefined(preferredProgrammingLanguage) && isDefined(tabTitles)) {
      setDefaultTabIndex(
        tabTitles.indexOf(
          programmingLanguagesMap[preferredProgrammingLanguage],
        ),
      );
    }
  ...
  return (
    <Tabs
      ...
      activeTabOverride={defaultTabIndex !== -1 ? defaultTabIndex + 1 : 1}
    >
      {tabTitlesToContent}
    </Tabs>
  );
