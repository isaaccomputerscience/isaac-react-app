...
export function hasGameboard(
  assignment: AssignmentDTO,
): assignment is EnhancedAssignment {
...
export const filterAssignmentsByStatus = (
  assignments: AssignmentDTO[] | undefined | null,
) => {
  const now = new Date();
  ...
  if (assignments) {
    assignments.forEach((assignment) => {
      if (
        assignment?.gameboard?.percentageCompleted === undefined ||
        assignment.gameboard.percentageCompleted < 100
      ) {
        const assignmentStartDate =
          assignment.scheduledStartDate ?? assignment.creationDate;
        const noDueDateButRecent =
          !assignment.dueDate &&
          assignmentStartDate &&
          assignmentStartDate > fourWeeksAgo;
        const dueDateAndCurrent =
          assignment.dueDate && assignment.dueDate >= fiveDaysAgo;
    ...
    myAssignments.inProgressRecent = orderBy(
      myAssignments.inProgressRecent,
      ["dueDate", "creationDate"],
      ["asc", "desc"],
    );
    myAssignments.inProgressOld = orderBy(
      myAssignments.inProgressOld,
      ["creationDate"],
      ["desc"],
    );
    myAssignments.completed = orderBy(
      myAssignments.completed,
      ["creationDate"],
      ["desc"],
    );
  }
...
export const filterAssignmentsByProperties = (
    ...
    assignments.forEach((assignment) => {
      const assignmentMatchesFilter =
        assignment?.gameboard?.title
          ?.toLowerCase()
          .includes(assignmentTitleFilter.toLowerCase()) &&
        (assignmentGroupFilter === "All" ||
          assignment.groupName === assignmentGroupFilter) &&
        (assignmentSetByFilter === "All" ||
          extractTeacherName(assignment.assignerSummary) ===
...
export const getDistinctAssignmentGroups = (
  assignments: AssignmentDTO[] | undefined | null,
): Set<string> => {
...
export const getDistinctAssignmentSetters = (
  assignments: AssignmentDTO[] | undefined | null,
): Set<string> => {
  ...
  if (assignments) {
    assignments.forEach((assignment) => {
      assignment?.assignerSummary &&
        distinctFormattedAssignmentSetters.add(
          extractTeacherName(assignment.assignerSummary) as string,
        );
    });
  }
...
export const getAssignmentStartDate = (a: AssignmentDTO): number =>
  (a.scheduledStartDate ?? a.creationDate ?? 0).valueOf();

export const hasAssignmentStarted = (a: AssignmentDTO): boolean =>
  getAssignmentStartDate(a) <= Date.now();
