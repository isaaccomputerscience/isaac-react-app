...
function SingleDifficultyIconShape({
  difficultyCategory,
  difficultyCategoryLevel,
  active,
}: DifficultyIconShapeProps) {
  return (
    <g
      transform={`translate(${
        (difficultyCategoryLevel - 1) *
        (difficultyIconWidth + 2 * difficultyIconXPadding)
      }, ${yPadding + (difficultyCategory === "P" ? 0 : 2)})`}
    >
      {difficultyCategory === "P" ? (
        <Hexagon
          {...miniHexagon}
          className={"hex difficulty practice " + classnames({ active })}
        />
      ) : (
        <Rectangle
          {...miniSquare}
          className={"square difficulty challenge " + classnames({ active })}
        />
      )}
      {
        <foreignObject
          width={difficultyIconWidth}
          height={
            difficultyIconWidth +
            (difficultyCategory === "P" ? yPadding + 2 : 0)
          }
        >
          <div
            className={`difficulty-title difficulty-icon-title ${classnames({
              active,
            })} difficulty-${difficultyCategoryLevel}`}
          >
            {difficultyCategory}
          </div>
        </foreignObject>
...
export function DifficultyIcons({ difficulty }: { difficulty: Difficulty }) {
  ...
  return (
    <div>
      <svg
        width={`${
          difficultyCategoryLevels.length *
            (difficultyIconWidth + 2 * difficultyIconXPadding) -
          difficultyIconXPadding
        }px`}
