...
const IsaacSymbolicQuestion = ({
  ...
  questionId,
  readonly,
}: IsaacQuestionProps<IsaacSymbolicQuestionDTO>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt<FormulaDTO>(questionId);
    ...
    debounceTimer.current = window.setTimeout(() => {
      ...
      if (
        ...
        if (/[A-Zbd-z](sin|cos|tan|log|ln|sqrt)\(/.test(pycode)) {
          ...
          setErrors([
            "Make sure to use spaces or * signs before function names like 'sin' or 'sqrt'!",
          ]);
        } else {
          setErrors(undefined);
        }
