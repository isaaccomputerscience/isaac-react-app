...
export abstract class AbstractBaseTagService {
  ...
  abstract augmentDocWithSubject(
    doc: ContentDTO,
  ): ContentDTO & { subjectId: string };
  ...
  public allTags: Tag[] = this.getBaseTags().map((baseTag) => {
    ...
    return Object.assign(baseTag, {
      type: this.getTagHierarchy()[depth],
      level: depth,
    });
  });
  ...
  public getSubcategoryTag = this.getSpecifiedTag.bind(
    this,
    TAG_LEVEL.subcategory,
  );
  public getSubcategoryTags = this.getSpecifiedTags.bind(
    this,
    TAG_LEVEL.subcategory,
  );
  ...
  public getDescendents(tagId: TAG_ID) {
    ...
    for (let i in this.allTags) {
      if (this.allTags[i].parent == tagId) {
        ...
        descendents = descendents.concat(
          this.getDescendents(this.allTags[i].id),
        );
      }
    }
    return descendents;
