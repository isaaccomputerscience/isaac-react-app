...
export const printingSettingsSlice = createSlice({
  ...
  reducers: {
    enableHints: (state, action: PayloadAction<boolean>) => ({
      ...state,
...
export const mainContentIdSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(routerPageChange, () => null);
  },
});

export type TransientUserContextState = {
...
export const transientUserContextSlice = createSlice({
  ...
  reducers: {
    ...
    setExamBoard: (state, action: PayloadAction<EXAM_BOARD>) => ({
      ...state,
      examBoard: action.payload,
    }),
    setShowOtherContent: (state, action: PayloadAction<boolean>) => ({
      ...state,
      showOtherContent: action.payload,
    }),
  },
});

export type ErrorState =
  | { type: "generalError"; generalError: string }
...
export const errorSlice = createSlice({
  ...
  extraReducers: (builder) => {
    const generalMatcher = (
      action: any,
    ): action is { type: string; errorMessage: string } =>
      [
    ...
    builder
      .addCase(ACTION_TYPE.USER_CONSISTENCY_ERROR, () => ({
        type: "consistencyError",
      }))
      .addMatcher(generalMatcher, (_, action) => ({
        type: "generalError",
