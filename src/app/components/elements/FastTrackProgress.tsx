...
const conceptLevels: ConceptLevel[] = [
  "upperLevelQuestions",
  "lowerLevelQuestions",
];
...
function categoriseConceptQuestions(
  conceptQuestions: GameboardItem[],
): ConceptLevelQuestions {
  return {
    upperLevelQuestions: conceptQuestions.filter(
      (question) => getFastTrackLevel(question.tags) === "ft_upper",
    ),
    lowerLevelQuestions: conceptQuestions.filter(
      (question) => getFastTrackLevel(question.tags) === "ft_lower",
    ),
  };
}
...
function calculateProgressBarHeight(
  questionLevel: LevelTag,
  hexagonQuarterHeight: number,
  hexagonPadding: number,
  progressBarPadding: number,
) {
  let numberOfHexagonRows = { ft_top_ten: 1, ft_upper: 2, ft_lower: 3 }[
    questionLevel
  ];
  return (
    2 * progressBarPadding +
    4 * hexagonQuarterHeight +
    (numberOfHexagonRows - 1) * (6 * hexagonQuarterHeight + 2 * hexagonPadding)
  );
}

export function FastTrackProgress({
  doc,
  search,
}: {
  doc: IsaacFastTrackQuestionPageDTO;
  search: string;
}) {
  const { questionHistory: qhs }: { questionHistory?: string } =
    queryString.parse(search);
  const questionHistory = qhs ? qhs.split(",") : [];
  const dispatch = useAppDispatch();
  const pageQuestionParts = useAppSelector(selectors.questions.getQuestions);
  const fastTrackConcepts = useAppSelector(
    (appState: AppState) => appState && appState.fasttrackConcepts,
  );

  const deviceSize = useDeviceSize();
  const hexagonUnitLength = { xl: 28, lg: 26, md: 22, sm: 22, xs: 12.5 }[
    deviceSize
  ];
  ...
  const conceptQuestions =
    gameboard &&
    fastTrackConcepts &&
    fastTrackConcepts.gameboardId === gameboard.id &&
    fastTrackConcepts.concept === doc.title
  ...
  useEffect(() => {
    if (gameboard) {
      ...
      dispatch(
        fetchFasttrackConcepts(
          gameboard.id as string,
          doc.title as string,
          upper,
        ),
      );
  ...
  const progressBarHeight = calculateProgressBarHeight(
    currentlyWorkingOn.fastTrackLevel,
    hexagonQuarterHeight,
    hexagonPadding,
    progressBarPadding,
  );
  ...
  function augmentQuestion(
    ...
    if (questionHistory) {
      ...
      if (fastTrackLevel == "ft_top_ten") {
        ...
        newQuestionHistory = questionHistory.slice(
          0,
          questionHistory.lastIndexOf(question.id as string),
        );
      }
  ...
  function getMostRecentQuestion(
    questionHistory: string[],
    conceptLevel: LevelTag,
  ) {
    const reversedQuestionHistory = questionHistory.slice().reverse();
    const questionLevelMatchFunctions = {
      ft_top_ten: (questionId: string) =>
        !questionId.includes("upper") && !questionId.includes("lower"),
  ...
  function orderConceptQuestionsById(
    unorderedConceptQuestions: ConceptLevelQuestions,
  ) {
    let result: ConceptLevelQuestions = {
      upperLevelQuestions: [],
      lowerLevelQuestions: [],
    };
  ...
  function evaluateProgress(
    ...
    const progress: Progress = {
      title: "",
      conceptTitle: "",
      questions: { topTen: [], upper: [], lower: [] },
      connections: { topTenToUpper: [], upperToLower: [] },
    };

    // Store title information for local storage retrieval
    progress.title = gameboard?.title as string;
    progress.conceptTitle = currentlyWorkingOn.isConcept
      ? currentlyWorkingOn.title
      : "";

    const conceptQuestions = orderConceptQuestionsById(
      unorderedConceptQuestions,
    );

    // Evaluate top ten progress
    gameboard?.contents?.forEach((question: GameboardItem, index) => {
      if (question.id === currentlyWorkingOn.id) {
        const correctQuestionParts =
          pageQuestionParts?.filter((q) => q.bestAttempt?.correct) || [];
        progress.questions.topTen.push(
          augmentQuestion(
            {
              ...question,
              state:
                correctQuestionParts.length === pageQuestionParts?.length
                  ? "PERFECT"
                  : question.state,
              questionPartsCorrect: correctQuestionParts.length || 0,
              questionPartStates:
                pageQuestionParts?.map((qp) =>
                  qp.bestAttempt
                    ? qp.bestAttempt.correct
                      ? "CORRECT"
                      : "INCORRECT"
                    : "NOT_ATTEMPTED",
                ) || [],
    ...
    if (currentlyWorkingOn.isConcept) {
      const upperAndLowerConceptQuestions: Map<QuestionLevel, GameboardItem[]> =
        new Map([
          ["upper", conceptQuestions.upperLevelQuestions],
          ["lower", conceptQuestions.lowerLevelQuestions],
        ]);
      upperAndLowerConceptQuestions.forEach(
        (conceptQuestionsOfType, conceptQuestionType) => {
          conceptQuestionsOfType.forEach((question, index) => {
            if (question.id === currentlyWorkingOn.id) {
              const correctQuestionParts =
                pageQuestionParts?.filter((q) => q.bestAttempt?.correct) || [];
              progress.questions[conceptQuestionType].push(
                augmentQuestion(
                  {
                    ...question,
                    state:
    ...
    if (currentlyWorkingOn.isConcept) {
      let mostRecentTopTenQuestionId =
        getMostRecentQuestion(questionHistory, "ft_top_ten") || undefined;
      let mostRecentTopTenIndex =
        gameboard?.contents
          ?.map((question: GameboardItem) => question.id)
          .indexOf(mostRecentTopTenQuestionId) || -1;
      ...
      if (currentlyWorkingOn.fastTrackLevel === "ft_lower") {
        ...
        progress.connections.upperToLower.push({
          ...
          isMostRecent: true,
          message:
            "Practise the concept with easier questions before returning to complete the board",
        });
      }
    }
    return progress;
  ...
  function generateHexagonTitle(title: string, isCurrentQuestion: boolean) {
    ...
    let xSingleCharPosition =
      hexagon.halfWidth - { xl: 8, lg: 8, md: 6, sm: 6, xs: 5 }[deviceSize];
    let xTwoCharPosition =
      hexagon.halfWidth -
      { xl: 14, lg: 14, md: 11, sm: 11, xs: 10 }[deviceSize];
    let yPosition =
      hexagon.quarterHeight * 2 +
      { xl: 9, lg: 9, md: 7, sm: 7, xs: 6 }[deviceSize];
    return (
      <text
        fontFamily="Exo 2"
  ...
  function generateCompletionTick(isCurrentQuestion: boolean) {
    return (
      <image
        ...
        x={
          hexagon.halfWidth -
          { xl: 18, lg: 17, md: 15, sm: 15, xs: 9 }[deviceSize]
        }
        y={
          hexagon.quarterHeight -
          { xl: 2, lg: 2, md: 2, sm: 2, xs: 2 }[deviceSize]
        }
  ...
  function createQuestionHexagon(question: AugmentedQuestion) {
    const fillColour = question.isCompleted
      ? question.isCurrentQuestion
        ? hexagon.base.fill.completedColour
        : hexagon.base.fill.deselectedCompletedColour
      : question.isCurrentQuestion
      ? hexagon.base.fill.selectedColour
      : hexagon.base.fill.deselectedColour;

    return (
      <Link to={question.href}>
        <title>
          {question.title + (question.isCurrentQuestion ? " (Current)" : "")}
        </title>
        <Hexagon
          {...hexagon}
          properties={{
        ...
        <Hexagon
          {...hexagon}
          properties={hexagon.questionPartProgress}
          states={question.questionPartStates}
          selector={(state) => state === "CORRECT"}
        />
        {question.isCompleted
          ? generateCompletionTick(question.isCurrentQuestion)
          : generateHexagonTitle(
              question.hexagonTitle,
              question.isCurrentQuestion,
            )}
      </Link>
    );
  ...
  function createQuestionRow(
    ...
    fastTrackLevel: string,
    conceptRowIndex: number,
  ) {
    return (
      <g
        key={fastTrackLevel + "-question-hexagons"}
        transform={
          "translate(0," +
          conceptRowIndex * (6 * hexagon.quarterHeight + 2 * hexagon.padding) +
          ")"
        }
      >
        {questions.map((question, columnIndex) => (
          <g
            key={question.id}
            className={fastTrackLevel + "-question-hexagon"}
            transform={
              "translate(" +
              columnIndex * 2 * (hexagon.halfWidth + hexagon.padding) +
              ", 0)"
            }
          >
  ...
  function createConceptConnectionRow(
    conceptConnections: Connection[],
    connectionName: string,
    connectionRowIndex: number,
  ) {
    return (
      <g
        key={connectionName + "-concept-connections"}
        transform={
          "translate(" +
          ...
          (3 * hexagon.quarterHeight +
            hexagon.padding +
            connectionRowIndex *
              (6 * hexagon.quarterHeight + 2 * hexagon.padding)) +
          ")"
        }
      >
        {conceptConnections.map((conceptConnection) => (
          <React.Fragment key={JSON.stringify(conceptConnection)}>
  ...
  function renderProgress(progress: Progress) {
    return (
      <RS.Row className="mt-sm-3 mb-3 mb-sm-4">
        <RS.Col cols={12} lg={3}>
          <h4 className="mt-lg-1">{gameboard?.title}</h4>
          <div className="d-none d-lg-block">
            <br className="d-none d-lg-block" />
            {currentlyWorkingOn.isConcept && (
              <h4 className="mt-lg-1 mt-xl-3">
        ...
        <RS.Col cols={12} lg={9}>
          <svg id="ft-progress" width="100%" height={progressBarHeight}>
            <g
              id="progress-bar-padding"
              transform={`translate(${progressBarPadding}, ${progressBarPadding})`}
            >
              <g id="concept-connections">
                {progress.connections.topTenToUpper.length &&
                  createConceptConnectionRow(
                    progress.connections.topTenToUpper,
                    "top-ten-to-upper",
                    0,
                  )}
                {progress.connections.upperToLower.length &&
                  createConceptConnectionRow(
                    progress.connections.upperToLower,
                    "upper-to-lower",
                    1,
                  )}
              </g>
  ...
  const categorisedConceptQuestions = categoriseConceptQuestions(
    conceptQuestions || [],
  );
  const progress = evaluateProgress(
    categorisedConceptQuestions,
    questionHistory,
  );
