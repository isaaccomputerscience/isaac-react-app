...
export const useGroupAssignmentSummary = (groupId?: number) => {
  ...
  const groupBoardAssignments = assignments?.filter(
    (a) => a.groupId === groupId,
  );
  ...
  const assignmentCount =
    (groupBoardAssignments?.length ?? 0) + (groupQuizAssignments?.length ?? 0);
...
export const useGroupAssignments = (groupId?: number) => {
  const { data: groupBoardAssignments } =
    isaacApi.endpoints.getMySetAssignments.useQuery(groupId);
  const quizAssignments = useAppSelector(selectors.quizzes.assignments);
  const groupQuizAssignments = isFound(quizAssignments)
    ? quizAssignments?.filter((a) => a.groupId === groupId)
    : undefined;
  ...
  return {
    groupBoardAssignments: groupBoardAssignments as
      | EnhancedAssignment[]
      | undefined,
    groupQuizAssignments,
  };
