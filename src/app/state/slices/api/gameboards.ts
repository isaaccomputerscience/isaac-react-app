...
export const assignGameboard = createAsyncThunk(
  ...
  async (
    { boardId, groups, dueDate, scheduledStartDate, notes }: AssignmentSpec,
    ...
    if (dueDate !== undefined) {
      ...
      if (dueDate.valueOf() - today.valueOf() < 0) {
        appDispatch(
          showToast({
            color: "danger",
            title: `Gameboard assignment${
              groups.length > 1 ? "(s)" : ""
            } failed`,
            body: "Error: Due date cannot be in the past.",
            timeout: 5000,
          }),
        );
        return rejectWithValue(null);
    ...
    if (scheduledStartDate !== undefined) {
      ...
      if (
        nthHourOf(0, scheduledStartDate).valueOf() <
        nthHourOf(0, new Date()).valueOf()
      ) {
        appDispatch(
          showToast({
            color: "danger",
            ...
            body: "Error: Scheduled start date cannot be in the past.",
            timeout: 5000,
          }),
        );
        return rejectWithValue(null);
      ...
      if (nthHourOf(0, scheduledStartDate).valueOf() > dueDate.valueOf()) {
        appDispatch(
          showToast({
            ...
            body: "Error: Due date must be on or after scheduled start date.",
            timeout: 5000,
          }),
        );
        return rejectWithValue(null);
    ...
    const response = await dispatch(
      isaacApi.endpoints.assignGameboard.initiate(assignments),
    );
    if (mutationSucceeded(response)) {
      ...
      const newAssignments: AssignmentDTO[] = assigmentStatuses
        ...
        .map((a) => ({
          ...
          scheduledStartDate: scheduledStartDate?.valueOf() as unknown as
            | Date
            | undefined,
          notes,
        }));
      const successfulIds = newAssignments.map((a) => a.groupId);
      const failedIds = assigmentStatuses.filter((a) =>
        isDefined(a.errorMessage),
      );
      ...
      if (failedIds.length === 0) {
        ...
        failedIds.forEach(({ groupId, errorMessage }) => {
          appDispatch(
            showErrorToast(
              `Gameboard assignment to ${
                groupLookUp.get(groupId) ?? "unknown group"
              } failed`,
        ...
        if (failedIds.length === assigmentStatuses.length) {
          ...
          const partialSuccessMessage =
            successfulIds.length > 1
              ...
              : `Assignment to ${groupLookUp.get(
                  successfulIds[0] as number,
      ...
      successfulIds.forEach((groupId) => {
        appDispatch(
          isaacApi.util.updateQueryData(
            ...
            groupId,
            (assignmentsByMe) =>
              assignmentsByMe.concat(
                newAssignments.filter((a) => a.groupId === groupId),
              ),
          ),
        );
      });
...
export const unlinkUserFromGameboard = createAsyncThunk<
  string,
  { boardId?: string; boardTitle?: string }
>(
  "gameboards/deleteBoard",
  async (
    { boardId, boardTitle }: { boardId?: string; boardTitle?: string },
    ...
    try {
      const getAssignments = dispatch(
        isaacApi.endpoints.getMySetAssignments.initiate(undefined),
      );
      ...
      if (response.isSuccess) {
        ...
        const hasAssignedGroups =
          (assignmentsByMe?.filter((a) => a.gameboardId === boardId) ?? [])
            .length > 0;
        if (hasAssignedGroups) {
          if (
            reduxState &&
            reduxState.user &&
            reduxState.user.loggedIn &&
            isAdminOrEventManager(reduxState.user)
          ) {
            if (
              !confirm(
                `Warning: You currently have groups assigned to ${boardTitle}. If you delete this your groups will still be assigned but you won't be able to unassign them or see the gameboard in your assigned gameboards or 'My gameboards' page.`,
              )
            ) {
              return rejectWithValue(null);
            }
        ...
        const deleteResponse = await dispatch(
          isaacApi.endpoints.unlinkUserFromGameboard.initiate(boardId),
        );
        return mutationSucceeded(deleteResponse)
          ? boardId
      ...
      dispatch(
        showRTKQueryErrorToastIfNeeded("Gameboard deletion failed", e) as any,
      );
      return rejectWithValue(null);
    }
  },
...
>(
  ...
  async (
    { boardId, user, boardTitle, redirectOnSuccess },
    { dispatch, rejectWithValue },
  ) => {
    try {
      if (boardTitle) {
        ...
        const response = await dispatch(
          isaacApi.endpoints.renameAndLinkUserToGameboard.initiate({
            boardId,
            newTitle: boardTitle,
          }),
        );
        ...
        const response = await dispatch(
          isaacApi.endpoints.linkUserToGameboard.initiate(boardId),
        );
        if (!mutationSucceeded(response)) {
          return rejectWithValue(null);
        }
      ...
      if (redirectOnSuccess) {
        if (isTutorOrAbove(user)) {
          const assignBoardPath =
            persistence.load(KEY.ASSIGN_BOARD_PATH) ?? "/set_assignments";
      ...
      dispatch(
        showRTKQueryErrorToastIfNeeded("Error saving gameboard", e) as any,
      );
      return rejectWithValue(null);
    }
  },
