...
function q(questions: AppQuestionDTO[]): {
...
describe("root reducer", () => {
  it("has null as the initial state value for every property", () => {
    const actualInitialState = removeRTKProperties(
      rootReducer(undefined, ignoredTestAction),
    );
...
describe("user reducer", () => {
  ...
  const previousStates: (PotentialUser | null)[] = [
    null,
    { loggedIn: false },
    { ...dameShirley, loggedIn: true },
    { ...profWheeler, loggedIn: true },
  ];
  ...
  it("should always add a user on login response success", () => {
    const addProfWheelerAction: Action = {
...
describe("questions reducer", () => {
  ...
  it("should register a question correctly", () => {
    const registerManVsHorse: Action = {
      type: ACTION_TYPE.QUESTION_REGISTRATION,
      questions: [manVsHorse],
    };
    const testCases = [
      [null, q([manVsHorse])],
  ...
  it("should deregister questions correctly", () => {
    const deregisterManVsHorse: Action = {
      type: ACTION_TYPE.QUESTION_DEREGISTRATION,
      questionIds: [manVsHorse.id as string],
    };
    const testCases = [
      [null, null],
      [q([manVsHorse]), null],
...
describe("toasts reducer", () => {
  ...
  it("can mark existing toasts as hidden", () => {
    const toastsShowAction: Action = {
      type: ACTION_TYPE.TOASTS_HIDE,
      toastId: moreToast.id,
    };
    const previousState = [sampleToast, moreToast];
    const actualNextState = toasts(previousState, toastsShowAction);
    expect(actualNextState).toEqual([
      sampleToast,
      { ...moreToast, showing: false },
    ]);
  });

  it("can remove existing toasts", () => {
    const toastsShowAction: Action = {
      type: ACTION_TYPE.TOASTS_REMOVE,
      toastId: moreToast.id,
    };
    const previousState = [sampleToast, moreToast];
    const actualNextState = toasts(previousState, toastsShowAction);
...
describe("boards reducer", () => {
  ...
  const selector = mapValues(
    selectors.boards,
    (f) => (boardsState: BoardsState) => f({ boards: boardsState }),
  );

  const testBoards: GameboardDTO[] = [
    { id: "abc", title: "ABC Board" },
    { id: "def", title: "DEF Board" },
  ];

  const simpleState: BoardsState = { totalResults: 42, boards: testBoards };

  it("can get a new set of boards", () => {
    const action: AnyAction = createMockAPIAction(
      "getGameboards",
      "query",
      "fulfilled",
      { boards: testBoards, totalResults: 42 },
      { startIndex: 0 },
    );
    const previousStates = [null];
    previousStates.map((previousState) => {
      const actualNextState = gameboardsSlice.reducer(previousState, action);
      expect(selector.boards(actualNextState)).toEqual({
        totalResults: 42,
        boards: testBoards,
      });
    });
  ...
  it("can add to the set of boards", () => {
    const newBoards: GameboardDTO[] = [
      { id: "ghi", title: "Ghi Board" },
      { id: "jkl", title: "JKL Board" },
    ];
    const action: AnyAction = createMockAPIAction(
      "getGameboards",
      "query",
      "fulfilled",
      { boards: newBoards, totalResults: 40 },
      { startIndex: 2 },
    );
    const withDupes = {
      totalResults: 38,
      boards: [...testBoards, newBoards[1]],
    };
    ...
    previousStates.map((previousState) => {
      ...
      expect(selector.boards(actualNextState)).toEqual({
        totalResults: 40,
        boards: union(priorBoards, newBoards),
      });
    });
  });
