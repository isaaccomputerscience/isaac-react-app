...
export const IsaacQuickQuestion = ({ doc }: { doc: IsaacQuickQuestionDTO }) => {
      ...
      <Row>
        <Col sm="12" md={{ size: 10, offset: 1 }}>
          <Button
            color="secondary"
            block
            className={classNames({ active: isVisible })}
            onClick={toggle}
          >
  ...
  function FastTrackOptions() {
    return (
      <div
        className={
          "d-flex align-items-stretch flex-column-reverse flex-sm-row flex-md-column-reverse flex-lg-row mb-4"
        }
      >
        {secondaryAction && (
          <div
            className={
              "m-auto pt-3 pb-1 w-100 w-sm-50 w-md-100 w-lg-50 pr-sm-2 pr-md-0 pr-lg-3"
            }
          >
            <input
              {...secondaryAction}
        ...
        <div
          className={
            "m-auto pt-3 pb-1 w-100 w-sm-50 w-md-100 w-lg-50 pl-sm-2 pl-md-0 pl-lg-3"
          }
        >
          <input
            onClick={toggle}
            value={isVisible ? "Hide answer" : "Show answer"}
            className={classNames("h-100 btn btn-secondary btn-block", {
              active: isVisible,
            })}
          />
        </div>
      ...
      <>
        ...
        {isVisible && (
          <Row className="mt-3">
            <Col
              sm="12"
              md={!fastTrackInfo.isFastTrackPage ? { size: 10, offset: 1 } : {}}
            >
              <Button
                color="secondary"
                type={"button"}
                block
                className="active hide-answer"
                onClick={hideAnswer}
              >
                Hide answer
              </Button>
  ...
  const Options = fastTrackInfo.isFastTrackPage
    ? FastTrackOptions
    : recordConfidence
    ? ConfidenceOptions
    : DefaultOptions;
      ...
      <div className="question-component p-md-5">
        <div
          className={classNames({
            "quick-question": !fastTrackInfo.isFastTrackPage,
          })}
        >
          ...
          {isVisible && (
            <Row>
              <Col
                sm="12"
                md={
                  !fastTrackInfo.isFastTrackPage ? { size: 10, offset: 1 } : {}
                }
              >
                <Alert color="hide">
