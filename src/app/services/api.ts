...
endpoint.interceptors.response.use(
  ...
  (error) => {
    if (
      ...
      !error.response.data.bypassGenericSiteErrorPage
    ) {
...
export const api = {
  search: {
    ...
    ): AxiosPromise<ApiTypes.ResultsWrapper<ApiTypes.ContentSummaryDTO>> => {
  ...
  users: {
    ...
    getUserIdSchoolLookup: (
      userIds: number[],
    ): AxiosPromise<AppTypes.UserSchoolLookup> => {
      return endpoint.get(`/users/school_lookup?user_ids=${userIds.join(",")}`);
    },
    getProgress: (
      userIdOfInterest = "current_user",
    ): AxiosPromise<AppTypes.UserProgress> => {
  ...
  authentication: {
    ...
    login: (
      ...
      return endpoint.post(
        ...
        securePadCredentials(credentials),
      );
    },
    mfaCompleteLogin: (
  ...
  email: {
    verify: (params: {
      ...
      return endpoint.get(
        `/users/verifyemail/${params.userid}/${params.token}`,
      );
    },
    getTemplateEmail: (
      contentid: string,
    ): AxiosPromise<AppTypes.TemplateEmail> => {
      return endpoint.get(`/email/viewinbrowser/${contentid}`);
    },
    sendAdminEmail: (
      contentid: string,
      emailType: string,
      roles: EmailUserRoles,
    ): AxiosPromise => {
      return endpoint.post(`/email/sendemail/${contentid}/${emailType}`, roles);
    },
    sendAdminEmailWithIds: (
      contentid: string,
      emailType: string,
      ids: number[],
    ): AxiosPromise => {
      return endpoint.post(
        `/email/sendemailwithuserids/${contentid}/${emailType}`,
        ids,
      );
    },
    sendProvidedEmailWithUserIds: (
      emailTemplate: EmailTemplateDTO,
      emailType: string,
      ids: number[],
    ): AxiosPromise => {
      return endpoint.post(`/email/sendprovidedemailwithuserids/${emailType}`, {
        userIds: ids,
  ...
  admin: {
    userSearch: {
      get: (queryParams: {}): AxiosPromise<
        ApiTypes.UserSummaryForAdminUsersDTO[]
      > => {
  ...
  authorisations: {
    ...
    adminGet: (
      userId: number,
    ): AxiosPromise<ApiTypes.UserSummaryWithEmailAddressDTO[]> => {
    ...
    adminGetOtherUsers: (
      userId: number,
    ): AxiosPromise<ApiTypes.UserSummaryDTO[]> => {
  ...
  glossary: {
    getTerms: (): AxiosPromise<
      ApiTypes.ResultsWrapper<ApiTypes.GlossaryTermDTO>
    > => {
  ...
  questions: {
    ...
    ): AxiosPromise<QuestionSearchResponse> => {
    ...
    answeredQuestionsByDate: (
      ...
      toDate: number,
      perDay: boolean,
    ): AxiosPromise<ApiTypes.AnsweredQuestionsByDate> => {
    ...
    testFreeTextQuestion: (
      userDefinedChoices: Choice[],
      testCases: TestCaseDTO[],
    ) => {
      return endpoint.post("/questions/test?type=isaacFreeTextQuestion", {
        userDefinedChoices,
        testCases,
      });
    },
  ...
  topics: {
    get: (
      topicName: TAG_ID,
    ): AxiosPromise<ApiTypes.IsaacTopicSummaryPageDTO> => {
  ...
  events: {
    ...
    getEvents: (
      ...
      filterEventsByType: EventTypeFilter | null,
      ...
      results: ApiTypes.IsaacEventPageDTO[];
      totalResults: number;
    }> => {
      return endpoint.get(`/events`, {
        params: {
    ...
    getFirstN: (
      ...
      return endpoint.get(`/events`, {
        params: {
          start_index: 0,
          limit: numberOfActiveEvents,
          show_active_only: active,
          show_inactive_only: !active,
          show_booked_only: false,
          tags: null,
        },
      });
    },
    getEventOverviews: (
      ...
      results: AppTypes.EventOverview[];
      totalResults: number;
    }> => {
    ...
    getEventMapData: (
      ...
      results: AppTypes.EventMapData[];
      totalResults: number;
    }> => {
      return endpoint.get(`/events/map_data`, {
        params: {
          start_index: startIndex,
          limit: eventsPerPage,
          show_active_only: showActiveOnly,
  ...
  eventBookings: {
    ...
    resendUserConfirmationEmail: (eventId: string, userId: number) => {
      return endpoint.post(
        `/events/${eventId}/bookings/${userId}/resend_confirmation`,
      );
    },
    ...
    recordEventAttendance: (
      ...
      return endpoint.post(
        `/events/${eventId}/bookings/${userId}/record_attendance?attended=${attended}`,
      );
    },
  ...
  documents: {
    getWorkbooks: (pdfLocation: string) => {
      return endpoint.get(`/documents/${pdfLocation}`, {
  ...
  fasttrack: {
    concepts: (
      gameboardId: string,
      concept: string,
      upperQuestionId: string,
    ): AxiosPromise<ApiTypes.GameboardItem[]> => {
      return endpoint.get(`/fasttrack/${gameboardId}/concepts`, {
  ...
  quizzes: {
    ...
    loadQuizAssignmentAttempt: (
      quizAssignmentId: number,
    ): AxiosPromise<ApiTypes.QuizAttemptDTO> => {
    ...
    answer: (
      ...
      return endpoint.post(
        `/quiz/attempt/${quizAttemptId}/answer/${questionId}`,
        attempt,
      );
    },
    ...
    loadStudentQuizAttemptFeedback: (
      quizAssignmentId: number,
      userId: number,
    ): AxiosPromise<ApiTypes.QuizAttemptFeedbackDTO> => {
      return endpoint.get(
        `/quiz/assignment/${quizAssignmentId}/attempt/${userId}`,
      );
    },
    ...
    markQuizAttemptAsIncomplete: (
      quizAssignmentId: number,
      userId: number,
    ): AxiosPromise<ApiTypes.QuizUserFeedbackDTO> => {
      return endpoint.post(
        `/quiz/assignment/${quizAssignmentId}/${userId}/incomplete`,
      );
    },
    ...
    logQuizSectionView: (
      ...
      return endpoint.post(
        `/quiz/attempt/${quizAttemptId}/log`,
        `sectionNumber=${page}`,
        {},
      );
    },
