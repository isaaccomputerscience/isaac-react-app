...
const pageHelp = (
  <span>
    Answer the questions on each section of the test, then mark the test as
    complete to see your feedback.
  </span>
);
...
export const QuizDoFreeAttempt = ({ user }: { user: RegisteredUserDTO }) => {
  ...
  const assignedQuizError = error
    ?.toString()
    .includes("You are currently set this test");
      ...
      <ShowLoading until={attempt || error}>
        ...
        {error && (
          <>
            ...
            />
            <Alert
              color={assignedQuizError ? "warning" : "danger"}
              className="mt-4"
            >
              <h4 className="alert-heading">
                {assignedQuizError
                  ? "You have been set this test"
                  : "Error loading test!"}
              </h4>
              ...
              {assignedQuizError && (
                <>
                  <p>
                    Your teacher has set this test to you. You may not practise
                    it in advance.
                    <br />
                    If you are ready to take the test, click on it in your{" "}
                    <a href={"/tests"} target="_self" rel="noopener noreferrer">
                      assigned tests
                    </a>{" "}
                    page.
                  </p>
