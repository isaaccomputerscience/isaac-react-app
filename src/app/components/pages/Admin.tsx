...
export const Admin = ({ user }: { user: RegisteredUserDTO }) => {
  ...
  const displayVersion =
    newVersion || (contentVersion && contentVersion.liveVersion) || null;
  ...
  const startVersionUpdate = function (event?: React.FormEvent) {
    ...
    if (
      contentVersion &&
      displayVersion !== contentVersion.liveVersion &&
      newVersion != null
    ) {
  ...
  return (
    <RS.Container id="admin-page">
      <TitleAndBreadcrumb
        currentPageTitle="Isaac administration"
        breadcrumbTitleOverride="Admin tools"
      />
              ...
              <React.Fragment>
                ...
                {updateState == ContentVersionUpdatingStatus.UPDATING && (
                  <RS.Alert color="info">
                    <h4>Updating...</h4>
                    <p>
                      Replacing version {contentVersion.liveVersion} with{" "}
