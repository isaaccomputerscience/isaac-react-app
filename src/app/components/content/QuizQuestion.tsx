...
export const QuizQuestion = ({ doc }: { doc: ApiTypes.QuestionDTO }) => {
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDefined(quizAttempt) && quizAttempt.completedDate === undefined) {
        dispatch(
          submitQuizQuestionIfDirty(quizAttempt.id as number, doc.id as string),
        );
      }
    };
  ...
  const validated =
    validationResponse?.correct !== undefined ||
    (!validationResponse && isDefined(quizAttempt?.completedDate));
  const correct = validationResponse?.correct;
  const sigFigsError = (validationResponse?.explanation?.tags || []).includes(
    "sig_figs",
  );
  ...
  return (
    <React.Fragment>
      <div
        className={classnames("question-component p-md-5", {
          "parsons-layout": [
      ...
      >
        {doc.id && (
          <h3 className={"mb-3"}>Question {questionNumbers[doc.id]}</h3>
        )}
        ...
        {validated && (
          <div
            className={`validation-response-panel p-2 mt-2 ${
              correct ? "correct" : ""
            }`}
          >
            <div className="pb-1">
            ...
            {validationResponse && validationResponse.explanation && (
              <div className="mb-1">
                <IsaacContent
