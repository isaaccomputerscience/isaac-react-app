  ...
  async (action: Action) => {
    ...
    if (
      ...
      if (isDefined(user)) {
        ...
        if (
          ...
          !withinLast2Minutes(
            persistence.load(KEY.EMAIL_CONFIRMATION_MODAL_SHOWN_TIME),
          ) &&
          firstLogin === null
        ) {
          persistence.save(
            KEY.EMAIL_CONFIRMATION_MODAL_SHOWN_TIME,
            new Date().toString(),
          );
        ...
        // User context re-confirmation modal - used to request a user to update their stage and exam board
          ...
          needToUpdateUserContextDetails(
            user.registeredContextsLastConfirmed,
          ) &&
          !withinLast2Minutes(
            persistence.load(KEY.RECONFIRM_USER_CONTEXT_SHOWN_TIME),
          )
        ) {
          persistence.save(
            KEY.RECONFIRM_USER_CONTEXT_SHOWN_TIME,
            new Date().toString(),
          );
    ...
    if (action.type === ACTION_TYPE.QUESTION_ATTEMPT_REQUEST) {
      ...
      if (lastQuestionId === null) {
        ...
        !withinLast2Hours(
          persistence.load(KEY.LOGIN_OR_SIGN_UP_MODAL_SHOWN_TIME),
        )
      ) {
        dispatch(
          logAction({
        ...
        persistence.save(
          KEY.LOGIN_OR_SIGN_UP_MODAL_SHOWN_TIME,
          new Date().toString(),
        );
