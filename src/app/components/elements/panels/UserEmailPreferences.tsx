...
export const useEmailPreferenceState = (
  ...
  Dispatch<SetStateAction<Nullable<UserEmailPreferences>>>,
] => {
  const [emailPreferences, _setEmailPreferences] = useState<
    Nullable<UserEmailPreferences>
  >({ ASSIGNMENTS: true, ...initialEmailPreferences });
  const setEmailPreferences = (
    newEmailPreferences:
      | Nullable<UserEmailPreferences>
      | ((
          ep: Nullable<UserEmailPreferences>,
        ) => Nullable<UserEmailPreferences>),
  ) => {
    if (typeof newEmailPreferences === "function") {
      return _setEmailPreferences((old) => ({
        ASSIGNMENTS: true,
        ...newEmailPreferences(old),
      }));
    }
