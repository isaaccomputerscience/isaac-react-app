...
const AdditionalManagerSelfRemovalModalBody = ({ group }: { group: AppGroup }) => (
  <p>
    You are about to remove yourself as a manager from &apos;{group.groupName}&apos;. This group will no longer appear
    on your Assignment Progress page or on the Manage Groups page. You will still have student connections with the
      ...
      <Col>
        <Button
          ...
          onClick={() => {
            if (group.id && user.id) {
              store.dispatch(
                isaacApi.endpoints.deleteGroupManager.initiate({ groupId: group.id, managerUserId: user.id }),
              );
            }
            store.dispatch(closeActiveModal());
          }}
...
const CurrentGroupInviteModal = ({ firstTime, group }: CurrentGroupInviteModalProps) => {
  ...
  return (
    <>
      ...
      <p>
        Use one of the following methods to add users to your group. Students joining your group will be shown your name
        and account email and asked to confirm sharing data.
      </p>
      <ShowLoadingQuery
        query={tokenQuery}
...
const CurrentGroupManagersModal = ({
  ...
  const additionalManagers =
    (group && sortBy(group.additionalManagers, (manager) => manager.familyName && manager.familyName.toLowerCase())) ||
    [];
  ...
  return !group ? (
    ...
    <div className={"mb-4"}>
      ...
      <p>
        ...
        progress. It will not automatically let additional teachers see detailed mark data unless students give access
        to the new teacher.
      </p>
            ...
            <tbody>
              <tr key={group.ownerSummary.email} data-testid={"group-owner"}>
                <td>
                  ...
                  {group.ownerSummary.givenName} {group.ownerSummary.familyName} ({group.ownerSummary.email})
                </td>
              </tr>
            </tbody>
          </Table>
      ...
      {additionalManagers.length == 0 && <p>There are no additional group managers for this group.</p>}
            ...
            additionalManagers.map((manager) => (
              <tr key={manager.email} data-testid={"group-manager"}>
                <td>
                  ...
                  {user.id === manager.id && <span className={"text-muted"}>(you)</span>} ({manager.email})
                </td>
                {userIsOwner && (
      ...
      {userIsOwner && (
        <Alert className={"px-2 py-2 mb-2"} color={group.additionalManagerPrivileges ? "danger" : "warning"}>
          ...
          {group.additionalManagerPrivileges ? (
            <>
              <span className={"font-weight-bold"}>Caution</span>: All other group managers are allowed delete and
              modify any assignments set to this group (by any other manager including the owner), remove group members,
              and archive and rename the group. <br />
      ...
      {userIsOwner && (
        <>
          ...
          <p>
            Enter the email of another Isaac teacher account below to add them as a group manager. Note that this will
            share their email address with the students.
          </p>
...
const CurrentGroupEmailModal = ({ users }: GroupEmailModalProps) => {
  return (
    <Col>
      <Row>An admin user can use the user IDs below to email these users:</Row>
      <Row className="my-3">
        <pre>{users && users.sort((a, b) => a - b).join(",")}</pre>
      </Row>
    </Col>
  );
};
