...
const AssignmentListEntry = ({ assignment }: AssignmentListEntryProps) => {
  ...
  const [unassignGameboard] =
    isaacApi.endpoints.unassignGameboard.useMutation();
  const deleteAssignment = () => {
    if (
      confirm(
        `Are you sure you want to unassign ${
          assignment.gameboard?.title ?? "this gameboard"
        } from ${
      ...
      unassignGameboard({
        boardId: assignment.gameboardId,
  ...
  const gameboardLink = assignment.gameboardId
    ? `/gameboards#${assignment.gameboardId}`
    : undefined;
  return (
    <Card className={"my-1"}>
      ...
      <CardBody>
        ...
        {assignmentStartDate && (
          <div>
            Start date:{" "}
            <strong>{new Date(assignmentStartDate).toDateString()}</strong>
            {assignmentStartDate > TODAY().valueOf() && (
        ...
        {assignment.dueDate && (
          <div>
            Due date:{" "}
            <strong>{new Date(assignment.dueDate).toDateString()}</strong>
          </div>
        )}
        {assignment.gameboard && (
          <div>
            Assigned by:{" "}
            <strong>{assignmentOwnedByMe ? "Me" : "Someone else"}</strong>
          </div>
        )}
        {assignment.gameboard && (
          <div>
            Gameboard created by:{" "}
            <strong>{formatBoardOwner(user, assignment.gameboard)}</strong>
          </div>
        )}
...
const DateAssignmentList = ({
  date,
  assignments,
}: {
  date: number;
  assignments: ValidAssignmentWithListingDate[];
}) => {
  const [open, setOpen] = useState<boolean>(false);
  const { boardsById, collapsed, setCollapsed, viewBy } = useContext(
    AssignmentScheduleContext,
  );
  useEffect(() => {
      ...
      >
        <svg height={dateHexagon.quarterHeight * 4} width={"100%"}>
          <Hexagon className={"fill-secondary"} {...dateHexagon} />
          {
            ...
            >
              <p
                className={classNames("date-assignment-count", {
                  "text-muted": !open,
                })}
              >
                {assignments[0].listingDate.toDateString().split(" ")[0]} -{" "}
          ...
          <svg
            x={2.5 * dateHexagon.halfWidth - (open ? 7 : 3)}
            y={dateHexagon.quarterHeight * 2 - (open ? 3 : 6.5)}
          >
            <polygon
              className={classNames("date-toggle-arrow fill-secondary", {
                open: open,
              })}
          ...
          {assignments.map((a) => (
            <AssignmentListEntry
              ...
              assignment={{
                ...
                gameboard:
                  a.gameboard ??
                  (a.gameboardId ? boardsById[a.gameboardId] : undefined),
              }}
            />
          ))}
        </div>
...
const MonthAssignmentList = ({
  ...
  const { collapsed, setCollapsed, viewBy } = useContext(
    AssignmentScheduleContext,
  );
  useEffect(() => {
    if (collapsed) setOpen(false);
  ...
  return (
    <>
      <div
        ...
        role={"button"}
        aria-label={
          (open ? "Collapse" : "Expand") + ` list for ${MONTH_NAMES[month]}`
        }
      ...
      >
        ...
        >
          <svg
            height={monthHexagon.quarterHeight * 4}
            width={monthHexagon.halfWidth * 2}
          >
            <Hexagon className={"fill-secondary"} {...monthHexagon} />
            <svg
              x={monthHexagon.halfWidth - (open ? 7.4 : 3)}
              y={monthHexagon.quarterHeight * 2 - (open ? 4 : 6.5)}
            >
              <polygon
                fill={"#ffffff"}
                points="0 1.75 1.783 0 8.75 7 1.783 14 0 12.25 5.25 7"
        ...
        <div
          className={"mx-3 flex-grow-1 border-bottom"}
          style={{ height: "1.1rem" }}
        />
      ...
      {open &&
        datesAndAssignments.map(([d, as]) => (
          <DateAssignmentList key={d} date={d} assignments={as} />
        ))}
    </>
  );
};
...
const AssignmentModal = ({
  ...
  const [showGameboardPreview, setShowGameboardPreview] =
  ...
  const { boardsById, groups, gameboards, boardIdsByGroupId } = useContext(
    AssignmentScheduleContext,
  );
      ...
      setSelectedGameboard([
        {
          ...
          label:
            boardsById[assignmentToCopy.gameboardId]?.title ??
            "No gameboard title",
        },
      ]);
      setScheduledStartDate(
        assignmentToCopy.scheduledStartDate
          ? new Date(assignmentToCopy.scheduledStartDate.valueOf())
          : undefined,
      );
      setDueDate(
        assignmentToCopy.dueDate
          ? new Date(assignmentToCopy.dueDate.valueOf())
          : undefined,
      );
  ...
  const assign = useCallback(() => {
    ...
    dispatch(
      assignGameboard({
        ...
        dueDate,
        scheduledStartDate:
          scheduledStartDate && nthHourOf(7, scheduledStartDate),
  ...
  const dueDateInvalid =
    dueDate && scheduledStartDate
      ? scheduledStartDate.valueOf() > dueDate.valueOf()
      : false;
  ...
  const alreadyAssignedGroupNames = useMemo<string[]>(() => {
    ...
    return selectedGroups
      .filter(
        (g) =>
          g.value &&
          boardIdsByGroupId[g.value]?.includes(selectedGameboard[0]?.value),
      )
      .map((g) => g.label);
  }, [selectedGroups, boardIdsByGroupId, selectedGameboard]);

  const gameboardToPreview = selectedGameboard?.[0]?.value
      ...
      <ModalBody>
        <Label className="w-100 pb-2">
          ...
          <Select
            ...
            options={sortBy(
              groups,
              (group) => group.groupName && group.groupName.toLowerCase(),
            ).map((g) => itemise(g.id as number, g.groupName))}
          />
        </Label>
        <Label className="w-100 pb-2">
          Gameboard:
          <Select
            ...
            options={gameboards.map((g) =>
              itemise(g.id ?? "", g.title ?? "No gameboard title"),
            )}
          />
          {alreadyAssignedGroupNames &&
            alreadyAssignedGroupNames.length > 0 && (
              <Alert color={"warning"} className={"my-1"}>
                This gameboard is already assigned to group
                {alreadyAssignedGroupNames.length > 1 ? "s" : ""}:{" "}
                {alreadyAssignedGroupNames.join(", ")}. You must delete the
                previous assignment
                {alreadyAssignedGroupNames.length > 1 ? "s" : ""} to set it
                again.
              </Alert>
            )}
          {selectedGameboard &&
            ...
            boardsById[selectedGameboard[0].value]?.contents && (
              <Card className={"my-1"}>
                ...
                {showGameboardPreview && gameboardToPreview && (
                  <GameboardViewerInner gameboard={gameboardToPreview} />
                )}
        ...
        <Label className="w-100 pb-2">
          Schedule an assignment start date{" "}
          ...
          <DateInput
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) =>
              setScheduledStartDate(e.target.valueAsDate as Date)
            }
          />
        </Label>
        <Label className="w-100 pb-2">
...
type AssignmentsGroupedByDate = [
  number,
  [number, [number, ValidAssignmentWithListingDate[]][]][],
][];
export const AssignmentSchedule = ({ user }: { user: RegisteredUserDTO }) => {
  const assignmentsSetByMeQuery =
    isaacApi.endpoints.getMySetAssignments.useQuery(undefined);
  ...
  const groupsById = useMemo<{ [id: number]: UserGroupDTO | undefined }>(() => {
    return (
      groups?.reduce(
        (acc, g) => (g.id ? { ...acc, [g.id]: g } : acc),
        {} as { [id: number]: UserGroupDTO },
      ) ?? {}
    );
  }, [groups]);
  ...
  const groupFilter = useMemo<{ [id: number]: boolean }>(() => {
    ...
    return groupsToInclude.reduce(
      (acc, n) => ({ ...acc, [n.value]: true }),
      {},
    );
  }, [groupsToInclude, groupsById]);

  // Map from group id -> ids of boards they are assigned to
  const boardIdsByGroupId = useMemo<{
        ...
        assignmentsSetByMe
          ...
          .reduce((oldest, a) => {
            const assignmentTimestamp =
              a.scheduledStartDate?.valueOf() ??
              a.creationDate?.valueOf() ??
              Date.now();
          ...
          (a) =>
            ({
              ...
              listingDate: new Date(
                viewBy === "startDate"
                  ? getAssignmentStartDate(a)
                  : (a.dueDate ?? 0).valueOf(),
              ),
              additionalManagerPrivileges:
                (a?.groupId &&
                  groupsById[a.groupId]?.additionalManagerPrivileges) ??
                false,
        ...
        (as) =>
          Object.entries(
            mapValues(
              ...
              (_as) =>
                Object.entries(
                  groupBy(_as, (a) => a.listingDate.getDate()),
                ).map(parseNumericKey),
            ),
          ).map(parseNumericKey),
      ),
  ...
  const notAllPastAssignmentsAreListed =
    earliestShowDate.valueOf() >= oldestAssignmentDate.valueOf();

  // Modal logic
  const [assignmentToCopy, setAssignmentToCopy] = useState<
  ...
  useEffect(() => {
    if (
      ...
      !headerScrollerObserver.current &&
      !headerScrollerFlag.current
    ) {
      const options = {
        root: null,
        rootMargin: "0px",
      ...
      headerScrollerObserver.current = new IntersectionObserver(
        headerScrollerCallback,
        options,
      );
            ...
            <>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (
                    headerScrollerSentinel.current &&
                    headerScrollerSentinel.current.getBoundingClientRect().top <
                      0
                  ) {
  ...
  const pageHelp = (
    <span>
      Use this page to set and manage assignments to your groups. You can assign
      any gameboard you have saved to your account.
      <br />
  ...
  return (
    <Container>
      <TitleAndBreadcrumb
        currentPageTitle="Assignment Schedule"
        help={pageHelp}
      />
      <h4 className="mt-4 mb-3">Assign a gameboard from...</h4>
      <AddGameboardButtons
        className="mb-4"
        redirectBackTo="/assignment_schedule"
      />
      <ShowLoadingQuery
          ...
          <div className="px-md-4 pl-2 pr-2 timeline-column mb-4 pt-2">
            {!isStaff(user) && (
              <Alert className="mt-2" color="info">
                The Assignment Schedule page is an alternate way to manage your
                assignments, focusing on the start and due dates of the
                assignments, rather than the assigned gameboard.
                <br />
                It is a work in progress, and we would love to{" "}
                <a
                  target="_blank"
                  href="/contact?subject=Assignment%20Schedule%20Feedback"
                >
                  hear your feedback
                </a>
                !
            ...
            {groupsToInclude.length > 0 &&
              assignmentsGroupedByDate.length === 0 && (
                <Alert color="warning" className="mt-2">
                  There are no assignments set to group
                  {groupsToInclude.length > 1 ? "s" : ""}:{" "}
            ...
            {assignmentsGroupedByDate.length > 0 && (
              <Card className="mt-2">
                <CardBody className="pt-0">
                  ...
                  <div
                    className={classNames("timeline w-100", {
                      "pt-2": !notAllPastAssignmentsAreListed,
                    })}
                  >
                      ...
                      <Fragment key={y}>
                        ...
                        {ms.map(([m, ds]) => (
                          <MonthAssignmentList
                            key={m}
                            month={m}
                            datesAndAssignments={ds}
                          />
                        ))}
                      </Fragment>
                    ))}
                    <div
                      className={classNames("bg-timeline", {
                        "fade-in": !notAllPastAssignmentsAreListed,
                      })}
                    />
                  </div>
                </CardBody>
