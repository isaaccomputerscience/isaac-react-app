...
function retryPreviousQuestion(questionHistory: string[], board?: string) {
  ...
  return {
    ...
    type: "button",
    onClick: function () {
      goToUrl("/questions/" + previousQuestionId, {
        questionHistory: commaSeparatedQuestionHistory,
        board,
      });
    },
  };
...
function tryEasierQuestion(
  easierQuestion: ApiTypes.ContentSummaryDTO,
  currentQuestionId: string | undefined,
  pageCompleted: boolean | undefined,
  ...
  return {
    type: "button",
    title: `Try an easier question${
      easierQuestion.title ? " on " + easierQuestion.title.toLowerCase() : ""
    }`,
    value: "Easier question?",
    onClick: function () {
      goToUrl(`/questions/${easierQuestion.id}`, {
        questionHistory: commaSeparatedQuestionHistory,
        board,
      });
    },
...
function trySupportingQuestion(
  supportingQuestion: ApiTypes.ContentSummaryDTO,
  currentQuestionId: string | undefined,
  pageCompleted: boolean | undefined,
  questionHistory: string[],
  board: string | undefined,
  ...
  return {
    type: "button",
    title: `Try more questions of a similar difficulty${
      supportingQuestion.title
        ? " on " + supportingQuestion.title.toLowerCase()
        : ""
    }`,
    value: "More practice questions?",
    onClick: function () {
      goToUrl(`/questions/${supportingQuestion.id}`, {
        questionHistory: commaSeparatedQuestionHistory,
        board,
      });
    },
...
function getRelatedUnansweredEasierQuestions(
  doc: ApiTypes.QuestionDTO,
  level: number,
) {
  return doc.relatedContent
    ? doc.relatedContent.filter((relatedContent) => {
        let isQuestionPage =
          relatedContent.type &&
          [DOCUMENT_TYPE.QUESTION, DOCUMENT_TYPE.FAST_TRACK_QUESTION].indexOf(
            relatedContent.type as DOCUMENT_TYPE,
          ) >= 0;
        let isEasier =
          relatedContent.level && parseInt(relatedContent.level, 10) < level;
...
function getRelatedUnansweredSupportingQuestions(
  doc: ApiTypes.QuestionDTO,
  level: number,
) {
  return doc.relatedContent
...
export function useFastTrackInformation(
  ...
  const {
    board,
    questionHistory: questionHistoryUrl,
  }: { board?: string; questionHistory?: string } = queryString.parse(
    location.search,
  );
  const questionHistory = questionHistoryUrl?.split(",") || [];

  const page = useAppSelector((state: AppState) =>
    state?.doc && state.doc !== NOT_FOUND ? state.doc : undefined,
  );
  const isFastTrackPage = page?.type === DOCUMENT_TYPE.FAST_TRACK_QUESTION;
  const pageCompleted = useAppSelector((state: AppState) =>
    state?.questions ? state.questions.pageCompleted : false,
  );
...
export function determineFastTrackPrimaryAction(
  questionPart: FastTrackPageProperties,
) {
  if (questionPart.correct) {
    if (questionPart.pageCompleted) {
      if (questionPart.questionHistory.length) {
        return retryPreviousQuestion(
          questionPart.questionHistory,
          questionPart.board,
        );
      } else {
...
export function determineFastTrackSecondaryAction(
  questionPart: FastTrackPageProperties,
) {
  if (questionPart.page) {
    if (!questionPart.correct) {
      const relatedUnansweredEasierQuestions =
        getRelatedUnansweredEasierQuestions(
          questionPart.doc,
          questionPart.page.level || 0,
        );
      if (relatedUnansweredEasierQuestions.length > 0) {
        ...
        return tryEasierQuestion(
          easierQuestion,
          questionPart.page.id,
          questionPart.pageCompleted,
          questionPart.questionHistory,
          questionPart.board,
        );
      }
    ...
    const relatedUnansweredSupportingQuestions =
      getRelatedUnansweredSupportingQuestions(
        questionPart.doc,
        questionPart.page.level || 0,
      );
  ...
  if (
    questionPart.doc.relatedContent &&
    questionPart.doc.relatedContent.length
  ) {
    const relatedConcepts = getRelatedConcepts(
      questionPart.doc,
      questionPart.userContext,
      questionPart.user,
    );
