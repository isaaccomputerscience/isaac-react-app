...
function validate(answer: string): Validation {
  ...
  return {
    validValue,
    wordLimit: {
      exceeded: wordLimitExceeded,
      limit: wordLimit,
      current: currentNumberOfWords,
    },
    charLimit: {
      exceeded: charLimitExceeded,
      limit: charLimit,
...
function validatedChoiceDtoFromEvent(
  event: React.ChangeEvent<HTMLInputElement>,
): ValidatedChoice<StringChoiceDTO> {
  ...
  return validValue ? null : (
    <Alert color="warning" className={"no-print"}>
      ...
      <ul>
        {charLimit.exceeded && (
          <li>
            Character limit exceeded ({charLimit.current}/{charLimit.limit})
          </li>
        )}
        {wordLimit.exceeded && (
          <li>
            Word limit exceeded ({wordLimit.current}/{wordLimit.limit})
          </li>
        )}
      </ul>
    </Alert>
...
const IsaacFreeTextQuestion = ({
  doc,
  questionId,
  readonly,
}: IsaacQuestionProps<IsaacFreeTextQuestionDTO>) => {
  const { currentAttempt, dispatchSetCurrentAttempt } =
    useCurrentQuestionAttempt(questionId);
