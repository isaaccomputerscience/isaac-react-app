...
export const quizzes = (quizzes: QuizState = null, action: Action) => {
  switch (action.type) {
    case ACTION_TYPE.QUIZZES_RESPONSE_SUCCESS:
      return {
        quizzes: action.quizzes.results as QuizSummaryDTO[],
        total: action.quizzes.totalResults as number,
      };
    default:
      return quizzes;
  }
...
export const quizAssignments = (
  quizAssignments: QuizAssignmentsState = null,
  action: Action,
) => {
  switch (action.type) {
    ...
    case ACTION_TYPE.QUIZ_CANCEL_ASSIGNMENT_REQUEST:
      return quizAssignments !== null && quizAssignments !== NOT_FOUND
    ...
    case ACTION_TYPE.QUIZ_CANCEL_ASSIGNMENT_RESPONSE_SUCCESS:
      return quizAssignments !== null && quizAssignments !== NOT_FOUND
        ? quizAssignments.filter((assignment) => {
      ...
      if (isFound(quizAssignments)) {
        return quizAssignments.map((qa) => {
          if (qa.id === action.assignment.id) {
            return {
              ...qa,
              quiz: action.assignment.quiz,
              userFeedback: action.assignment.userFeedback,
            };
          } else {
            return qa;
          }
...
export const quizAssignedToMe = (
  quizAssignments: QuizAssignedToMeState = null,
  action: Action,
) => {
  switch (action.type) {
...
export const quizAttemptedFreelyByMe = (
  quizAttempts: QuizAttemptedFreelyByMeState = null,
  action: Action,
): QuizAttemptedFreelyByMeState => {
  switch (action.type) {
...
const updateQuizAttemptQuestion = (
  questionId: string,
  questionAttempt: Immutable<ChoiceDTO>,
) =>
  produce<{ attempt: QuizAttemptDTO }>((quizAttempt) => {
    const quizQuestions = extractQuestions(quizAttempt?.attempt.quiz);
    quizQuestions.forEach((question) => {
      if (
        question.id === questionId &&
        (question.bestAttempt === null ||
          question.bestAttempt?.correct === undefined)
      ) {
...
export const quizAttempt = (
  possibleAttempt: QuizAttemptState = null,
  action: Action,
): QuizAttemptState => {
  switch (action.type) {
    ...
    case ACTION_TYPE.QUIZ_LOAD_ASSIGNMENT_ATTEMPT_REQUEST:
      if (
        possibleAttempt &&
        "attempt" in possibleAttempt &&
        possibleAttempt.attempt.quizAssignmentId === action.quizAssignmentId
      ) {
    ...
    case ACTION_TYPE.QUIZ_LOAD_ATTEMPT_FEEDBACK_REQUEST:
      if (
        possibleAttempt &&
        "attempt" in possibleAttempt &&
        possibleAttempt.attempt.id === action.quizAttemptId
      ) {
        // Optimistically keep current attempt
        return possibleAttempt;
      }
    ...
    case ACTION_TYPE.QUIZ_START_FREE_ATTEMPT_REQUEST:
      if (
        possibleAttempt &&
        "attempt" in possibleAttempt &&
        possibleAttempt.attempt.quizId === action.quizId
      ) {
        // Optimistically keep current attempt
        return possibleAttempt;
      }
    ...
    case ACTION_TYPE.QUESTION_SET_CURRENT_ATTEMPT:
      if (possibleAttempt && "attempt" in possibleAttempt) {
        const questionAttempt =
          "choice" in action.attempt ? action.attempt.choice : action.attempt;
        return updateQuizAttemptQuestion(
          action.questionId,
          questionAttempt,
        )(possibleAttempt);
      }
...
type StudentQuizAttemptState =
  | { studentAttempt: QuizAttemptFeedbackDTO }
  | { error: string }
  | null;
export const studentQuizAttempt = (
  possibleAttempt: StudentQuizAttemptState = null,
  action: Action,
): StudentQuizAttemptState => {
  switch (action.type) {
...
type QuizAssignmentState =
  | { assignment: QuizAssignmentDTO }
  | { error: string }
  | null;
export const quizAssignment = (
  possibleAssignment: QuizAssignmentState = null,
  action: Action,
): QuizAssignmentState => {
  switch (action.type) {
    ...
    case ACTION_TYPE.QUIZ_ATTEMPT_MARK_INCOMPLETE_RESPONSE_SUCCESS:
      if (
        possibleAssignment &&
        "assignment" in possibleAssignment &&
        possibleAssignment.assignment.id === action.quizAssignmentId
      ) {
        return {
          assignment: {
            ...possibleAssignment.assignment,
            userFeedback: possibleAssignment.assignment.userFeedback?.map(
              (feedback) => {
...
export const quizPreview = (
  quizPreview: QuizPreviewState = null,
  action: Action,
): QuizPreviewState => {
  switch (action.type) {
    case ACTION_TYPE.QUIZ_LOAD_PREVIEW_REQUEST:
      if (
        quizPreview &&
        "quiz" in quizPreview &&
        quizPreview.quiz.id === action.quizId
      ) {
        return quizPreview;
      }
      return null;
