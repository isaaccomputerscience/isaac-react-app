...
type ExternalRedirectProps<Params extends { [K in keyof Params]: string }> =
  ExternalRedirectBaseProps &
    (
      | {
      ...
      | {
          to: (
            routeParams: Params,
            user: Immutable<RegisteredUserDTO>,
          ) => `https://${string}` | undefined | null;
          ifUser: (user: Immutable<PotentialUser>) => boolean;
        }
    );
export function ExternalRedirect<
  Params extends { [K in keyof Params]: string } = {},
>({ from, to, ifUser }: ExternalRedirectProps<Params>) {
  const ExternalRedirectInner = ({
    user,
  }: {
    user: Immutable<RegisteredUserDTO>;
  }) => {
    ...
    console.error(
      "Problem building external redirect URL, redirecting to homepage...",
    );
    return <Redirect to={"/"} />;
  };
  return (
    <TrackedRoute
      ...
      ifUser={ifUser}
      path={from}
      component={ExternalRedirectInner}
    />
  );
}
