...
const filterForQuestions = (contents: ContentSummaryDTO[]) => {
  return contents.filter(
    (content) =>
      content.type === DOCUMENT_TYPE.QUESTION ||
      content.type === DOCUMENT_TYPE.FAST_TRACK_QUESTION,
  );
};

export const filterAndSeparateRelatedContent = (
  contents: ContentSummaryDTO[],
  userContext: UseUserContextReturnType,
  user: Immutable<PotentialUser> | null,
) => {
  const examBoardFilteredContent = contents.filter(
    (c) =>
...
export const getRelatedDocs = (
  ...
  if (doc && doc != NOT_FOUND && doc.relatedContent) {
    return filterAndSeparateRelatedContent(
      doc.relatedContent,
      userContext,
      user,
    );
...
export const getRelatedConcepts = (
  doc: ContentDTO | NOT_FOUND_TYPE | null,
  userContext: UseUserContextReturnType,
  user: Immutable<PotentialUser> | null,
) => {
...
const isValidIdForTopic = (
  contentId: string,
  currentTopic: CurrentTopicState,
) => {
  if (
    currentTopic &&
    currentTopic != NOT_FOUND &&
    currentTopic.relatedContent
  ) {
    return !!currentTopic.relatedContent.filter(
      (content) => content.id === contentId,
    );
  }
};

export const determineTopicHistory = (
  currentTopic: CurrentTopicState,
  currentDocId: string,
) => {
  ...
  if (
    ...
    if (isValidIdForTopic(currentDocId, currentTopic)) {
      result.push({
        ...
        to: `/topics/${currentTopic.id.slice("topic_summary_".length)}`,
      });
    }
  }
  return result;
...
export const determineNextTopicContentLink = (
  ...
  if (
    ...
    if (isValidIdForTopic(contentId, currentTopic)) {
      const [relatedConcepts, relatedQuestions] =
        filterAndSeparateRelatedContent(
          currentTopic.relatedContent,
          userContext,
          user,
        );
      const orderedRelatedContent = relatedConcepts.concat(relatedQuestions);
      const relatedContentIds = orderedRelatedContent.map(
        (content) => content.id,
      );
      ...
      if (nextIndex < relatedContentIds.length) {
        ...
        return {
          ...
          to: `/${documentTypePathPrefix[nextContent.type as DOCUMENT_TYPE]}/${
            nextContent.id
          }`,
        };
      }
