...
export const showErrorToast = (error: string, body?: string) =>
  showToast({
    color: "danger",
    title: error,
    timeout: 5000,
    ...
    if (response.error) {
      if (response.error.status < 500) {
        return showErrorToast(
          error,
          getRTKQueryErrorMessage(response.error).message,
        );
      }
    } else {
...
// Modals
export const openActiveModal = (activeModal: ActiveModal) => ({
