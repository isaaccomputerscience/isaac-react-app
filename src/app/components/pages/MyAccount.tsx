...
import {
  ...
  UserEmailPreference,
} from "../elements/panels/UserEmailPreferences";
import {
  ACCOUNT_TAB,
...
const stateToProps = (state: AppState, props: any) => {
  ...
  return {
    ...
    firstLogin: (history?.location?.state as { firstLogin: any } | undefined)
      ?.firstLogin,
...
const AccountPageComponent = ({
  ...
  const userToEdit = useMemo(
    function wrapUserWithLoggedInStatus() {
      return adminUserToEdit
        ? { ...adminUserToEdit, loggedIn: true }
        : { loggedIn: false };
    },
    [adminUserToEdit],
  );
  ...
  const editingOtherUser =
    (!!userOfInterest &&
      user &&
      user.loggedIn &&
      user?.id?.toString() !== userOfInterest) ||
    false;
  ...
  const userChanged = useMemo(
    () =>
      !hashEqual(
        { ...(editingOtherUser ? userToEdit : user), password: "" },
        userToUpdate,
      ),
  ...
  const [myUserPreferences, setMyUserPreferences] = useState<
    UserPreferencesDTO | null | undefined
  >({});
  ...
  const otherPreferencesChanged = useMemo(
    () =>
      !hashEqual(userPreferences ?? {}, myUserPreferences ?? {}, {
        excludeKeys: (k) => k === "EMAIL_PREFERENCE",
      }),
    [myUserPreferences, userPreferences],
  );

  // - User Contexts
  const [userContextsToUpdate, setUserContextsToUpdate] = useState<
    UserContext[]
  >(
    userToUpdate.registeredContexts?.length
      ? [...userToUpdate.registeredContexts]
      : [{}],
  );
  useEffect(
    function keepUserContextsUpdated() {
      setUserContextsToUpdate(
        userToUpdate.registeredContexts?.length
          ? [...userToUpdate.registeredContexts]
          : [{}],
      );
  ...
  const passwordMeetsRequirements =
    newPasswordConfirm.length === 0 || validatePassword(newPasswordConfirm);
  const arePasswordsIdentical = newPassword === newPasswordConfirm;
  const isNewPasswordConfirmed =
    arePasswordsIdentical && passwordMeetsRequirements;
  ...
  function setBooleanNotation(newBooleanNotation: BooleanNotation) {
    setMyUserPreferences({
      ...myUserPreferences,
      BOOLEAN_NOTATION: newBooleanNotation,
    });
  }
  ...
  function setDisplaySettings(
    newDisplaySettings:
      | DisplaySettings
      | ((oldDs?: DisplaySettings) => DisplaySettings),
  ) {
    setMyUserPreferences((oldPref) => ({
      ...oldPref,
      DISPLAY_SETTING:
        typeof newDisplaySettings === "function"
  ...
  return (
    <Container id="account-page" className="mb-5">
      ...
      <ShowLoading
        until={
          editingOtherUser
            ? userToUpdate.loggedIn && userToUpdate.email
            : userToUpdate
        }
      >
        {user.loggedIn &&
              ...
              <Nav tabs className="my-4 flex-wrap">
                <NavItem>
                  <NavLink
                    className={classnames({
                      "mx-2": true,
                      active: activeTab === ACCOUNT_TAB.account,
                    })}
                    tabIndex={0}
                    onClick={() => setActiveTab(ACCOUNT_TAB.account)}
                    onKeyDown={ifKeyIsEnter(() =>
                      setActiveTab(ACCOUNT_TAB.account),
                    )}
                  >
                    Profile
                  </NavLink>
                ...
                <NavItem>
                  <NavLink
                    className={classnames({
                      "mx-2": true,
                      active: activeTab === ACCOUNT_TAB.passwordreset,
                    })}
                    tabIndex={0}
                    onClick={() => setActiveTab(ACCOUNT_TAB.passwordreset)}
                    onKeyDown={ifKeyIsEnter(() =>
                      setActiveTab(ACCOUNT_TAB.passwordreset),
                    )}
                  >
                ...
                <NavItem>
                  <NavLink
                    className={classnames({
                      "mx-2": true,
                      active: activeTab === ACCOUNT_TAB.teacherconnections,
                    })}
                    tabIndex={0}
                    onClick={() => setActiveTab(ACCOUNT_TAB.teacherconnections)}
                    onKeyDown={ifKeyIsEnter(() =>
                      setActiveTab(ACCOUNT_TAB.teacherconnections),
                    )}
                  >
                ...
                {!editingOtherUser && (
                  <NavItem>
                    <NavLink
                      className={classnames({
                        "mx-2": true,
                        active: activeTab === ACCOUNT_TAB.emailpreferences,
                      })}
                      tabIndex={0}
                      onClick={() => setActiveTab(ACCOUNT_TAB.emailpreferences)}
                      onKeyDown={ifKeyIsEnter(() =>
                        setActiveTab(ACCOUNT_TAB.emailpreferences),
                      )}
                    >
                ...
                {!editingOtherUser && (
                  <NavItem>
                    <NavLink
                      className={classnames({
                        "mx-2": true,
                        active: activeTab === ACCOUNT_TAB.betafeatures,
                      })}
                      tabIndex={0}
                      onClick={() => setActiveTab(ACCOUNT_TAB.betafeatures)}
                      onKeyDown={ifKeyIsEnter(() =>
                        setActiveTab(ACCOUNT_TAB.betafeatures),
                      )}
                    >
                ...
                <TabContent activeTab={activeTab}>
                  <TabPane tabId={ACCOUNT_TAB.account}>
                    <UserDetails
                      ...
                      programmingLanguage={
                        myUserPreferences?.PROGRAMMING_LANGUAGE
                      }
                  ...
                  <TabPane tabId={ACCOUNT_TAB.passwordreset}>
                    <UserPassword
                      currentUserEmail={
                        userToUpdate ? userToUpdate.email : user.email
                      }
                      userAuthSettings={userAuthSettings}
                      myUser={userToUpdate}
                  ...
                  {!editingOtherUser && (
                    <TabPane tabId={ACCOUNT_TAB.betafeatures}>
                      <UserBetaFeatures
                        displaySettings={
                          myUserPreferences?.DISPLAY_SETTING ?? {}
                        }
                        setDisplaySettings={setDisplaySettings}
                      />
                    </TabPane>
      ...
      <Row className="text-muted text-center mt-3">
        <Col>
          ...
          <a
            href="/contact?preset=accountDeletion"
            target="_blank"
...
export const MyAccount = withRouter(
  connect(stateToProps, dispatchToProps)(AccountPageComponent),
);
