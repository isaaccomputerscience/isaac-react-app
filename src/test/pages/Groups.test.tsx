...
const switchGroupsTab = async (
  activeOrArchived: "active" | "archived",
  expectedGroups: any[],
) => {
  // Switch to the correct tab
  const archivedTabLink = await screen.findByText(
    activeOrArchived === "archived" ? "Archived" : "Active",
  );
...
const testAddAdditionalManagerInModal = async (
  managerHandler: ResponseResolver,
  newManager: any,
) => {
  ...
  const addManagerInput = within(groupManagersModal).getByPlaceholderText(
    "Enter email address here",
  );
  await userEvent.type(addManagerInput, newManager.email);
  const addManagerButton = within(groupManagersModal).getByRole("button", {
  ...
  await waitFor(() => {
    const managerElements = within(
      groupManagersModal as HTMLElement,
    ).queryAllByTestId("group-manager");
    ...
    const removeButton = within(managerElements[0]).getByRole("button", {
      name: "Remove",
    });
...
describe("Groups", () => {
  ...
  it.each(roles)(
    ...
    async (role) => {
      ...
      const maybeArchivedGroupNames = archivedGroups.map(
        (g) => within(g).getByTestId("select-group").textContent,
      );
  ...
  it("allows you to create a new group", async () => {
    ...
    renderTestEnvironment({
      ...
      extraEndpoints: [
        ...
        rest.get(
          API_PATH + `/authorisations/token/${mockNewGroup.id}`,
          authTokenHandler,
        ),
      ],
    });
  ...
  (["active", "archived"] as const).forEach((activeOrArchived) => {
    const mockGroups =
      activeOrArchived === "active" ? mockActiveGroups : mockArchivedGroups;
    const mockOtherGroups =
      activeOrArchived === "active" ? mockArchivedGroups : mockActiveGroups;
    it(`allows you to delete ${activeOrArchived} groups`, async () => {
      ...
      const groupToDeleteElement = groups.find(
        (e) =>
          within(e).getByTestId("select-group").textContent ===
          groupToDelete.groupName,
      ) as HTMLElement;
      expect(groupToDeleteElement).toBeDefined();
      const deleteButton = within(groupToDeleteElement).getByRole("button", {
    ...
    it(`allows you to rename ${activeOrArchived} groups`, async () => {
        ...
        extraEndpoints: [
          rest.post(API_PATH + "/groups/:groupId", async (req, res, ctx) => {
            ...
            if (
              ...
              isEqual(groupToRename, {
                ...updatedGroup,
                groupName: groupToRename.groupName,
              })
            ) {
      ...
      const groupNames = groups.map(
        (e) => within(e).getByTestId("select-group").textContent,
      );
      ...
      await userEvent.click(
        within(groupToRenameElement).getByTestId("select-group"),
      );
      ...
      const groupNameInput =
        await within(groupEditor).findByPlaceholderText(/Group [Nn]ame/);
      await userEvent.clear(groupNameInput);
      await userEvent.type(groupNameInput, newGroupName);
      const updateButton = await within(groupEditor).findByRole("button", {
        name: "Update",
      });
      ...
      await waitFor(() => {
        ...
        const newGroupNames = newGroups.map(
          (e) => within(e).getByTestId("select-group").textContent,
        );
        expect(difference(groupNames, newGroupNames)).toEqual([
          groupToRename.groupName,
        ]);
    ...
    (activeOrArchived === "active" ? [false, true] : [false]).forEach(
      (shouldTryToShowArchivedTabFirst) => {
        it(`allows you to ${
          activeOrArchived === "active" ? "" : "un"
        }archive groups${
          shouldTryToShowArchivedTabFirst
            ? ", after switching to the Archived tab first"
            : ""
        }`, async () => {
          ...
          const getGroups = jest.fn((req, res, ctx) => {
            const archived =
              req.url.searchParams.get("archived_groups_only") === "true";
            const groups =
          ...
          const groupToModifyElement = groups.find(
            (e) =>
              within(e).getByTestId("select-group").textContent ===
              groupToModify.groupName,
          ) as HTMLElement;
          expect(groupToModifyElement).toBeDefined();
          const groupToModifyName =
            within(groupToModifyElement).getByTestId(
              "select-group",
            ).textContent;
          await userEvent.click(
            within(groupToModifyElement).getByTestId("select-group"),
          );
          ...
          const nextTabLink = await screen.findByText(
            activeOrArchived === "active" ? "Archived" : "Active",
          );
    ...
    it(`allows teacher owners of a group to add new group managers to an existing ${activeOrArchived} group`, async () => {
      ...
      const existingGroupManagerHandler = buildNewManagerHandler(
        mockGroup,
        mockNewManager,
      );
      renderTestEnvironment({
        ...
        extraEndpoints: [
          ...
          rest.post(
            API_PATH + `/groups/${mockGroup.id}/manager`,
            existingGroupManagerHandler,
          ),
        ],
      });
      await followHeaderNavLink("Teach", "Manage groups");
      ...
      const selectGroupButton = within(
        groups.find(
          (g) =>
            within(g).getByTestId("select-group").textContent ===
            mockGroup.groupName,
        ) as HTMLElement,
      ).getByTestId("select-group");
      await userEvent.click(selectGroupButton);
      const groupEditor = await screen.findByTestId("group-editor");
      const addManagersButton = within(groupEditor).getByRole("button", {
        name: "Add / remove group managers",
      });
      await userEvent.click(addManagersButton);
      await testAddAdditionalManagerInModal(
        existingGroupManagerHandler,
        mockNewManager,
      );
    });
    ...
    it(`does not allow tutor owners of a group to add new group managers to an existing ${activeOrArchived} group`, async () => {
      ...
      const addManagersButton = within(groupEditor).queryByRole("button", {
        name: "Add / remove group managers",
      });
      const viewManagersButton = within(groupEditor).queryByRole("button", {
        name: "View all group managers",
      });
  ...
  it(`allows teacher owners of a group to request password resets for group members that have allowed full access`, async () => {
    ...
    const memberInfos =
      await within(groupEditor).findAllByTestId("member-info");
    expect(memberInfos).toHaveLength(2);
    const resetPasswordButton1 = within(memberInfos[0]).getByRole("button", {
      name: "Reset Password",
    });
    ...
    const resetPasswordButton2 = within(memberInfos[1]).getByRole("button", {
      name: "Reset Password",
    });
  ...
  it(`doesn't allow tutor owners of a group to request a password reset for any group member`, async () => {
    ...
    const memberInfos =
      await within(groupEditor).findAllByTestId("member-info");
    expect(memberInfos).toHaveLength(2);
    const resetPasswordButton1 = within(memberInfos[0]).queryByRole("button", {
      name: "Reset Password",
    });
    const resetPasswordButton2 = within(memberInfos[1]).queryByRole("button", {
      name: "Reset Password",
    });
  ...
  it("allows teachers to add new group managers in the modal after creating a new group", async () => {
    ...
    const mockNewManager = buildMockTeacher(2);
    const newGroupManagerHandler = buildNewManagerHandler(
      mockNewGroup,
      mockNewManager,
    );
    renderTestEnvironment({
      ...
      extraEndpoints: [
        ...
        rest.get(
          API_PATH + `/authorisations/token/${mockNewGroup.id}`,
          buildAuthTokenHandler(mockNewGroup, "G3N30M"),
        ),
        rest.post(
          API_PATH + `/groups/${mockNewGroup.id}/manager`,
          newGroupManagerHandler,
        ),
      ],
    });
    await followHeaderNavLink("Teach", "Manage groups");
    ...
    const addGroupManagersButton = await within(firstModal).findByRole(
      "button",
      { name: "Add group managers" },
    );
    await userEvent.click(addGroupManagersButton);
    await waitFor(() =>
      testAddAdditionalManagerInModal(newGroupManagerHandler, mockNewManager),
    );
  });
  ...
  it("does not allow tutors to add new group managers in the modal after creating a new group", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        within(firstModal).queryByRole("button", {
          name: "Add group managers",
        }),
      ).toBeNull();
    });
  ...
  it("only allows additional group managers to remove themselves as group managers", async () => {
    ...
    const mockGroup = {
      ...
      additionalManagers: [
        ...
        buildMockUserSummary(mockOtherManager, true),
      ],
    };
    const removeSelfAsManagerHandler = jest.fn((req, res, ctx) => {
      return res(
        ...
        ctx.json({
          ...mockGroup,
          additionalManagers: mockGroup.additionalManagers.filter(
            (m) => m.id !== mockUser.id,
          ),
        }),
      );
    ...
    renderTestEnvironment({
      ...
      extraEndpoints: [
        rest.get(API_PATH + "/groups", buildGroupHandler([mockGroup])),
        rest.delete(
          API_PATH + "/groups/:groupId/manager/:userId",
          removeSelfAsManagerHandler,
        ),
      ],
    });
    await followHeaderNavLink("Teach", "Manage groups");
    ...
    const additionalManagerElements =
      within(groupManagersModal).getAllByTestId("group-manager");
    expect(additionalManagerElements).toHaveLength(2);

    // Make sure that we cannot add any more members ourselves
    const addManagerInput = within(groupManagersModal).queryByPlaceholderText(
      "Enter email address here",
    );
    expect(addManagerInput).toBeNull();
    const addManagerButton = within(groupManagersModal).queryByRole("button", {
      name: "Add group manager",
    });
    expect(addManagerButton).toBeNull();

    // "Remove" button should not be visible for the other additional manager
    const otherManagerElement = additionalManagerElements.find(
      (e) => e.textContent?.includes(mockOtherManager.email),
    );
    if (!otherManagerElement)
      fail(
        "Other additional manager not shown on modal, cannot continue test!",
      );
    const otherRemoveButton = within(
      otherManagerElement as HTMLElement,
    ).queryByRole("button", { name: "Remove", hidden: false });
    expect(otherRemoveButton).toBeNull();

    // "Remove" button should be visible and work as expected for us (mock user)
    const selfManagerElement = additionalManagerElements.find(
      (e) => e.textContent?.includes(mockUser.email),
    );
    if (!selfManagerElement)
      fail(
        "Mock users additional manager entry not shown on modal, cannot continue test!",
      );
    const selfRemoveButton = within(
      selfManagerElement as HTMLElement,
    ).getByRole("button", { name: "Remove", hidden: false });
    ...
    await waitFor(() => {
      ...
      expect(selfRemovalModal).toHaveModalTitle(
        "Remove yourself as a group manager",
      );
    });
    const removeSelfButton = within(selfRemovalModal as HTMLElement).getByRole(
      "button",
      { name: "Confirm" },
    );
    ...
    await waitFor(() => {
      expect(removeSelfAsManagerHandler).toHaveBeenRequestedWith((req) => {
        ...
        return (
          parseInt(groupId as string) === mockGroup.id &&
          parseInt(userId as string) === mockUser.id
        );
      });
    });
