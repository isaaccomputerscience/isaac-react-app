...
const schoolSearch = (
  ...
  setAsyncSelectOptionsCallback: (
    options: { value: string | School; label: string | undefined }[],
  ) => void,
) => {
  api.schools
    ...
    .then(({ data }) => {
      setAsyncSelectOptionsCallback(
        data && data.length > 0
          ? data.map((item) => ({
              value: item,
              label: schoolNameWithPostcode(item),
            }))
          : [],
      );
    })
...
const throttledSchoolSearch = throttle(schoolSearch, 450, {
  trailing: true,
  leading: true,
});
...
export const SchoolInput = ({
  ...
  const [selectedSchoolObject, setSelectedSchoolObject] =
  ...
  const schoolValue:
    | { value: string | School; label: string | undefined }
    | undefined =
  ...
  const isInvalid =
    submissionAttempted && required && !validateUserSchool(userToUpdate);
  return (
