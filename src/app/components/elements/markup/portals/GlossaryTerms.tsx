...
export const useGlossaryTermsInHtml: PortalInHtmlHook = (html) => {
  const glossaryTerms = useAppSelector(
  ...
  const termElements = htmlDom.querySelectorAll(
    "[id^='glossary-term-']",
  ...
  for (let i = 0; i < termElements.length; i++) {
    ...
    const term = getTermFromCandidateTerms(
      glossaryTerms.filter((term) => term.id?.replace(/\|/g, "-") === termId),
    );
    ...
    if (term) {
      ...
      console.error(
        'No valid term for "' + termId + '" found among the filtered terms: ',
        glossaryTerms,
      );
      termElements[i].innerHTML =
  ...
  return [
    ...
    (ref?: HTMLElement) =>
      ref
        ? [
            ...tooltips,
            ...fullTermContainers.map(({ id, term }) => (
              <GlossaryTerm key={id} id={id} term={term} rootElement={ref} />
            )),
          ]
        : tooltips,
  ];
