...
export function LevelsFilterSummary({
  levels,
  levelOptions,
}: LevelsSummaryProps) {
  ...
  return (
    <svg
      ...
      width={`${
        hexagon.padding * 2 +
        levels.length * 2 * (hexagon.halfWidth + hexagon.padding)
      }px`}
        ...
        {levelOptions
          ...
          .map((level, i) => (
            <g
              key={level.value}
              transform={`translate(${
                i * 2 * (hexagon.halfWidth + hexagon.padding)
              }, 0)`}
            >
              ...
              >
                <div
                  className={`hexagon-level-title active  hex-level-${level.value}`}
                >
                  {level.label}
                </div>
              </foreignObject>
...
export function LevelsFilterHexagonal({
  levelOptions,
  levels,
  setLevels,
}: LevelsFilterProps) {
  ...
  const levelOptionsSecondRow = levelOptions.slice(
    halfWayBreakPoint,
    levelOptions.length,
  );
                ...
                {levelOptionsRow.map((levelOption, j) => {
                  ...
                  return (
                    ...
                    >
                      <Hexagon
                        {...hexagon}
                        className={`hex level ${isSelected ? "active" : ""}`}
                      />
                      <foreignObject
                        width={hexagon.halfWidth * 2}
                        height={hexagon.quarterHeight * 4}
                      >
                        <div
                          className={`hexagon-level-title ${
                            isSelected ? "active" : ""
                          } hex-level-${levelOption.value}`}
                        >
                          {levelOption.label}
                        </div>
                      </foreignObject>
...
export function LevelsFilterSelect({
  levelOptions,
  levels,
  setLevels,
  id,
  ...
  return (
    <Select
      id={id}
      onChange={selectOnChange(setLevels, false)}
      isMulti
      value={levels}
