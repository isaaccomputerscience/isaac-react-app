  ...
  return (value: PropsValue<Item<T>>) =>
    f(
      (Array.isArray(value) ? value : !value ? [] : [value]).map((v) =>
        unwrapValue ? v.value : v,
      ),
    );
}

export function isItemEqual<T>(
  valueComparisonFunction: (x: T, y: T) => boolean,
  a: Item<T>,
  b: Item<T>,
): boolean {
