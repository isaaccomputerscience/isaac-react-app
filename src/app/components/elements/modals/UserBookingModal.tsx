...
export function userBookingModal(
  ...
  return {
    ...
    title:
      eventBookingIds.includes(selectedUser.id as number) &&
      zeroOrLess(selectedEvent.placesAvailable)
    ...
    body: function UserBookingModalBody() {
      ...
      function updateAdditionalInformation(update: AdditionalInformation) {
        setAdditionalInformation(
          Object.assign({}, additionalInformation, update),
        );
      }
      const userCanBeBookedOnEvent =
        selectedEvent.eventStatus != "WAITING_LIST_ONLY" &&
        !eventBookingIds.includes(selectedUser.id as number) &&
        atLeastOne(selectedEvent.placesAvailable);
      const userCanBeAddedToWaitingList =
        selectedEvent.eventStatus == "WAITING_LIST_ONLY" ||
        (!eventBookingIds.includes(selectedUser.id as number) &&
          zeroOrLess(selectedEvent.placesAvailable));
      ...
      function makeUserBooking(formEvent: React.FormEvent) {
        ...
        if (
          additionalInformation.authorisation === undefined ||
          (additionalInformation.authorisation === "OTHER" &&
            additionalInformation.authorisationOther === undefined)
        ) {
          dispatch(
            showToast(
              Object.assign({}, FAILURE_TOAST, {
                title: "Event booking failed",
                body: "You must provide an authorisation reason to complete this request.",
              }),
            ),
          );
          return;
      ...
      return (
        <RS.Form onSubmit={makeUserBooking} className="mb-4">
          ...
          <span>
            <p>
              <small>
                Warning, by booking a user on this event, it may eventually lead
                to their personal information being shared with a third party.
                You must therefore confirm how this booking has been authorised.
                The data owner is always the Isaac user being booked on to the
                event.
              </small>
            </p>
            ...
            <RS.Input
              ...
              value={additionalInformation.authorisation || ""}
              onChange={(event) =>
                updateAdditionalInformation({
                  authorisation: event.target.value,
                })
              }
            >
              <option value="" />
            ...
            {additionalInformation.authorisation === "OTHER" && (
              <RS.Input
                ...
                onChange={(event) =>
                  updateAdditionalInformation({
                    authorisationOther: event.target.value,
                  })
                }
              />
            )}
